#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

int32_t x1 = 28534;
uint16_t x2 = 4363U;
int32_t x4 = INT32_MAX;
static int32_t x6 = 20473113;
volatile uint64_t t2 = UINT64_MAX;
int64_t x20 = -266LL;
volatile int32_t x21 = INT32_MIN;
int32_t x27 = -1;
uint64_t t6 = 108556072828152LLU;
volatile int32_t t7 = -2364;
int8_t x34 = -1;
uint64_t x35 = 132167180078LLU;
volatile int8_t x38 = -1;
static int32_t x40 = -1;
int8_t x46 = INT8_MIN;
int64_t x52 = -1LL;
volatile int32_t t13 = -758;
int8_t x63 = -1;
static volatile int32_t t15 = 48669518;
uint32_t x67 = 3U;
uint16_t x77 = UINT16_MAX;
static int16_t x82 = -1;
volatile int64_t x89 = 327699048962LL;
int16_t x91 = INT16_MIN;
static uint16_t x100 = UINT16_MAX;
int32_t x104 = INT32_MIN;
volatile int32_t t25 = INT32_MIN;
int16_t x107 = INT16_MIN;
static int32_t t26 = -35931;
int8_t x110 = INT8_MAX;
int32_t t27 = -1135;
int16_t x120 = -1;
volatile uint16_t x121 = 27U;
volatile int16_t x122 = INT16_MIN;
volatile int8_t x127 = INT8_MAX;
static int32_t x132 = INT32_MAX;
volatile uint16_t x134 = UINT16_MAX;
static volatile int32_t x136 = INT32_MAX;
int64_t x137 = -1LL;
int32_t t35 = -13;
volatile int32_t x146 = 3674;
static uint32_t x148 = 1998886U;
int8_t x151 = INT8_MAX;
volatile int32_t t39 = -1;
int32_t x161 = -1;
static volatile uint32_t x162 = UINT32_MAX;
uint8_t x167 = 11U;
int32_t t41 = 190668440;
int32_t t42 = 3406496;
volatile uint16_t x176 = 11247U;
volatile uint32_t x178 = 937U;
static volatile uint64_t x180 = 17252974471983LLU;
volatile int32_t x182 = 72;
int32_t x183 = 1688106;
volatile uint32_t x187 = 2019U;
int64_t x189 = 28LL;
static int16_t x195 = INT16_MAX;
volatile int32_t t48 = 2704;
volatile uint32_t t53 = 382901372U;
int8_t x219 = -7;
int32_t x222 = INT32_MAX;
int8_t x229 = -23;
volatile int32_t t57 = 15;
int32_t x235 = INT32_MAX;
int8_t x238 = INT8_MAX;
int16_t x239 = 0;
static int32_t x245 = INT32_MAX;
static int16_t x246 = INT16_MIN;
uint64_t x247 = 13375805840272912LLU;
int16_t x249 = -1;
static uint32_t x250 = UINT32_MAX;
uint32_t x254 = 2437978U;
static int32_t x257 = INT32_MAX;
uint16_t x261 = 2U;
int16_t x266 = 190;
uint8_t x268 = 2U;
int8_t x269 = INT8_MAX;
static int32_t x272 = -1;
volatile int32_t t67 = -1;
uint8_t x274 = 1U;
int32_t t69 = INT32_MIN;
static volatile int32_t t73 = -288074;
int16_t x303 = -1;
int8_t x304 = INT8_MAX;
volatile int16_t x307 = -1;
int64_t t79 = INT64_MAX;
uint32_t x321 = 1962U;
int16_t x323 = INT16_MAX;
volatile int32_t t80 = -49;
int64_t t81 = INT64_MAX;
uint64_t x330 = 1394373542738422287LLU;
volatile uint32_t x342 = 1418106U;
uint32_t x354 = UINT32_MAX;
int8_t x357 = INT8_MIN;
uint16_t x360 = 4U;
uint32_t x364 = 184U;
uint16_t x367 = 41U;
static volatile int32_t t91 = 345821;
uint16_t x376 = 45U;
int64_t t94 = -63028641LL;
int32_t x386 = 4;
volatile int64_t t96 = INT64_MIN;
int16_t x389 = INT16_MAX;
int64_t x396 = -1LL;
int64_t t98 = -7065940683672962LL;
int16_t x398 = INT16_MIN;
uint32_t x399 = 156631U;
int16_t x400 = -1;


void f0(void) {
	int32_t x3 = -1;
	static volatile int32_t t0 = INT32_MAX;

	t0 = (((x1==x2)==x3)|x4);

	if (t0 != INT32_MAX) { NG(); } else { ; }
	
}

void f1(void) {
	int64_t x5 = 184018596701LL;
	static int8_t x7 = -1;
	int64_t x8 = -216749LL;
	int64_t t1 = 2366926579207965LL;

	t1 = (((x5==x6)==x7)|x8);

	if (t1 != -216749LL) { NG(); } else { ; }
	
}

void f2(void) {
	static uint32_t x9 = 1183131U;
	int16_t x10 = INT16_MIN;
	static int8_t x11 = 2;
	uint64_t x12 = UINT64_MAX;

	t2 = (((x9==x10)==x11)|x12);

	if (t2 != UINT64_MAX) { NG(); } else { ; }
	
}

void f3(void) {
	static int16_t x13 = -23;
	uint16_t x14 = UINT16_MAX;
	static int8_t x15 = -1;
	static int64_t x16 = INT64_MIN;
	volatile int64_t t3 = INT64_MIN;

	t3 = (((x13==x14)==x15)|x16);

	if (t3 != INT64_MIN) { NG(); } else { ; }
	
}

void f4(void) {
	int16_t x17 = INT16_MAX;
	static uint32_t x18 = UINT32_MAX;
	uint8_t x19 = 0U;
	volatile int64_t t4 = -11361601540547586LL;

	t4 = (((x17==x18)==x19)|x20);

	if (t4 != -265LL) { NG(); } else { ; }
	
}

void f5(void) {
	static uint32_t x22 = 223897388U;
	volatile int32_t x23 = -1;
	int16_t x24 = -1;
	int32_t t5 = -5637853;

	t5 = (((x21==x22)==x23)|x24);

	if (t5 != -1) { NG(); } else { ; }
	
}

void f6(void) {
	int8_t x25 = -2;
	static int32_t x26 = INT32_MIN;
	uint64_t x28 = 4240LLU;

	t6 = (((x25==x26)==x27)|x28);

	if (t6 != 4240LLU) { NG(); } else { ; }
	
}

void f7(void) {
	static volatile uint32_t x29 = 5U;
	volatile int32_t x30 = INT32_MAX;
	static uint8_t x31 = UINT8_MAX;
	uint8_t x32 = UINT8_MAX;

	t7 = (((x29==x30)==x31)|x32);

	if (t7 != 255) { NG(); } else { ; }
	
}

void f8(void) {
	static int8_t x33 = INT8_MIN;
	uint64_t x36 = 252871289332LLU;
	uint64_t t8 = 2621556LLU;

	t8 = (((x33==x34)==x35)|x36);

	if (t8 != 252871289332LLU) { NG(); } else { ; }
	
}

void f9(void) {
	int32_t x37 = -1;
	static uint16_t x39 = UINT16_MAX;
	int32_t t9 = 121370;

	t9 = (((x37==x38)==x39)|x40);

	if (t9 != -1) { NG(); } else { ; }
	
}

void f10(void) {
	uint16_t x41 = 63U;
	volatile uint8_t x42 = 38U;
	int16_t x43 = INT16_MIN;
	uint64_t x44 = UINT64_MAX;
	static volatile uint64_t t10 = UINT64_MAX;

	t10 = (((x41==x42)==x43)|x44);

	if (t10 != UINT64_MAX) { NG(); } else { ; }
	
}

void f11(void) {
	uint64_t x45 = 2951010LLU;
	int8_t x47 = INT8_MIN;
	int8_t x48 = INT8_MIN;
	int32_t t11 = 1;

	t11 = (((x45==x46)==x47)|x48);

	if (t11 != -128) { NG(); } else { ; }
	
}

void f12(void) {
	int64_t x49 = -1664688423743295287LL;
	static volatile int16_t x50 = -1;
	int32_t x51 = INT32_MIN;
	volatile int64_t t12 = 2LL;

	t12 = (((x49==x50)==x51)|x52);

	if (t12 != -1LL) { NG(); } else { ; }
	
}

void f13(void) {
	static int64_t x53 = 3660LL;
	int64_t x54 = INT64_MIN;
	volatile uint32_t x55 = 103034510U;
	int16_t x56 = INT16_MIN;

	t13 = (((x53==x54)==x55)|x56);

	if (t13 != -32768) { NG(); } else { ; }
	
}

void f14(void) {
	uint8_t x57 = UINT8_MAX;
	int32_t x58 = INT32_MAX;
	int8_t x59 = INT8_MIN;
	int64_t x60 = INT64_MAX;
	int64_t t14 = INT64_MAX;

	t14 = (((x57==x58)==x59)|x60);

	if (t14 != INT64_MAX) { NG(); } else { ; }
	
}

void f15(void) {
	static int8_t x61 = -1;
	volatile uint64_t x62 = UINT64_MAX;
	uint16_t x64 = 356U;

	t15 = (((x61==x62)==x63)|x64);

	if (t15 != 356) { NG(); } else { ; }
	
}

void f16(void) {
	int8_t x65 = INT8_MAX;
	int16_t x66 = -5;
	int64_t x68 = INT64_MIN;
	int64_t t16 = INT64_MIN;

	t16 = (((x65==x66)==x67)|x68);

	if (t16 != INT64_MIN) { NG(); } else { ; }
	
}

void f17(void) {
	int16_t x69 = 15377;
	int16_t x70 = -1;
	int8_t x71 = INT8_MAX;
	int64_t x72 = 66816579630342773LL;
	volatile int64_t t17 = -11LL;

	t17 = (((x69==x70)==x71)|x72);

	if (t17 != 66816579630342773LL) { NG(); } else { ; }
	
}

void f18(void) {
	int16_t x73 = -2;
	static int32_t x74 = INT32_MIN;
	int32_t x75 = INT32_MAX;
	static int8_t x76 = -1;
	volatile int32_t t18 = -2;

	t18 = (((x73==x74)==x75)|x76);

	if (t18 != -1) { NG(); } else { ; }
	
}

void f19(void) {
	volatile int8_t x78 = -3;
	int32_t x79 = INT32_MIN;
	volatile int16_t x80 = INT16_MAX;
	int32_t t19 = -97;

	t19 = (((x77==x78)==x79)|x80);

	if (t19 != 32767) { NG(); } else { ; }
	
}

void f20(void) {
	static uint32_t x81 = 237483250U;
	int32_t x83 = INT32_MAX;
	volatile int32_t x84 = -4227;
	int32_t t20 = 113720829;

	t20 = (((x81==x82)==x83)|x84);

	if (t20 != -4227) { NG(); } else { ; }
	
}

void f21(void) {
	static int64_t x85 = INT64_MIN;
	volatile uint8_t x86 = UINT8_MAX;
	uint16_t x87 = UINT16_MAX;
	int64_t x88 = -1LL;
	volatile int64_t t21 = -5LL;

	t21 = (((x85==x86)==x87)|x88);

	if (t21 != -1LL) { NG(); } else { ; }
	
}

void f22(void) {
	static int16_t x90 = INT16_MIN;
	int32_t x92 = INT32_MIN;
	volatile int32_t t22 = INT32_MIN;

	t22 = (((x89==x90)==x91)|x92);

	if (t22 != INT32_MIN) { NG(); } else { ; }
	
}

void f23(void) {
	int64_t x93 = INT64_MIN;
	uint32_t x94 = 9630U;
	uint8_t x95 = 54U;
	static volatile uint64_t x96 = UINT64_MAX;
	volatile uint64_t t23 = UINT64_MAX;

	t23 = (((x93==x94)==x95)|x96);

	if (t23 != UINT64_MAX) { NG(); } else { ; }
	
}

void f24(void) {
	static int16_t x97 = -1;
	uint32_t x98 = UINT32_MAX;
	volatile uint64_t x99 = 471071LLU;
	int32_t t24 = -6;

	t24 = (((x97==x98)==x99)|x100);

	if (t24 != 65535) { NG(); } else { ; }
	
}

void f25(void) {
	int8_t x101 = INT8_MIN;
	int32_t x102 = -1;
	int32_t x103 = INT32_MIN;

	t25 = (((x101==x102)==x103)|x104);

	if (t25 != INT32_MIN) { NG(); } else { ; }
	
}

void f26(void) {
	int64_t x105 = -1LL;
	int32_t x106 = INT32_MIN;
	volatile int16_t x108 = INT16_MIN;

	t26 = (((x105==x106)==x107)|x108);

	if (t26 != -32768) { NG(); } else { ; }
	
}

void f27(void) {
	int16_t x109 = INT16_MIN;
	int8_t x111 = -5;
	static int8_t x112 = -1;

	t27 = (((x109==x110)==x111)|x112);

	if (t27 != -1) { NG(); } else { ; }
	
}

void f28(void) {
	int64_t x113 = -1LL;
	static uint16_t x114 = UINT16_MAX;
	uint8_t x115 = 0U;
	int16_t x116 = -1;
	volatile int32_t t28 = -3083;

	t28 = (((x113==x114)==x115)|x116);

	if (t28 != -1) { NG(); } else { ; }
	
}

void f29(void) {
	int8_t x117 = -15;
	int32_t x118 = 137642377;
	uint64_t x119 = 1547716662170908292LLU;
	int32_t t29 = 1;

	t29 = (((x117==x118)==x119)|x120);

	if (t29 != -1) { NG(); } else { ; }
	
}

void f30(void) {
	int32_t x123 = INT32_MAX;
	int64_t x124 = -444841LL;
	static volatile int64_t t30 = 139631353590693619LL;

	t30 = (((x121==x122)==x123)|x124);

	if (t30 != -444841LL) { NG(); } else { ; }
	
}

void f31(void) {
	int64_t x125 = INT64_MAX;
	int64_t x126 = -1LL;
	volatile uint8_t x128 = 9U;
	int32_t t31 = 2;

	t31 = (((x125==x126)==x127)|x128);

	if (t31 != 9) { NG(); } else { ; }
	
}

void f32(void) {
	static uint32_t x129 = UINT32_MAX;
	uint16_t x130 = UINT16_MAX;
	uint32_t x131 = 25682993U;
	static int32_t t32 = INT32_MAX;

	t32 = (((x129==x130)==x131)|x132);

	if (t32 != INT32_MAX) { NG(); } else { ; }
	
}

void f33(void) {
	int32_t x133 = INT32_MIN;
	uint64_t x135 = UINT64_MAX;
	int32_t t33 = INT32_MAX;

	t33 = (((x133==x134)==x135)|x136);

	if (t33 != INT32_MAX) { NG(); } else { ; }
	
}

void f34(void) {
	volatile uint8_t x138 = UINT8_MAX;
	uint16_t x139 = 4U;
	uint8_t x140 = 11U;
	volatile int32_t t34 = 1326;

	t34 = (((x137==x138)==x139)|x140);

	if (t34 != 11) { NG(); } else { ; }
	
}

void f35(void) {
	volatile int16_t x141 = -1;
	uint64_t x142 = 1909705LLU;
	int64_t x143 = INT64_MAX;
	static volatile int16_t x144 = INT16_MAX;

	t35 = (((x141==x142)==x143)|x144);

	if (t35 != 32767) { NG(); } else { ; }
	
}

void f36(void) {
	volatile int8_t x145 = -1;
	uint16_t x147 = 200U;
	volatile uint32_t t36 = 4114159U;

	t36 = (((x145==x146)==x147)|x148);

	if (t36 != 1998886U) { NG(); } else { ; }
	
}

void f37(void) {
	int32_t x149 = INT32_MIN;
	static int32_t x150 = INT32_MAX;
	int64_t x152 = 30530792729LL;
	int64_t t37 = -576495LL;

	t37 = (((x149==x150)==x151)|x152);

	if (t37 != 30530792729LL) { NG(); } else { ; }
	
}

void f38(void) {
	static uint8_t x153 = UINT8_MAX;
	int16_t x154 = INT16_MIN;
	volatile int64_t x155 = -67422LL;
	volatile int64_t x156 = -21393253817388LL;
	volatile int64_t t38 = 2182029007789888508LL;

	t38 = (((x153==x154)==x155)|x156);

	if (t38 != -21393253817388LL) { NG(); } else { ; }
	
}

void f39(void) {
	volatile int32_t x157 = INT32_MIN;
	static volatile int16_t x158 = INT16_MIN;
	int8_t x159 = INT8_MIN;
	int32_t x160 = 912747;

	t39 = (((x157==x158)==x159)|x160);

	if (t39 != 912747) { NG(); } else { ; }
	
}

void f40(void) {
	volatile int8_t x163 = -1;
	uint64_t x164 = UINT64_MAX;
	volatile uint64_t t40 = UINT64_MAX;

	t40 = (((x161==x162)==x163)|x164);

	if (t40 != UINT64_MAX) { NG(); } else { ; }
	
}

void f41(void) {
	volatile int16_t x165 = INT16_MIN;
	int32_t x166 = INT32_MAX;
	uint8_t x168 = 56U;

	t41 = (((x165==x166)==x167)|x168);

	if (t41 != 56) { NG(); } else { ; }
	
}

void f42(void) {
	uint64_t x169 = 139069473LLU;
	uint32_t x170 = UINT32_MAX;
	int16_t x171 = -1665;
	int16_t x172 = INT16_MAX;

	t42 = (((x169==x170)==x171)|x172);

	if (t42 != 32767) { NG(); } else { ; }
	
}

void f43(void) {
	uint16_t x173 = 3533U;
	int8_t x174 = 5;
	static int32_t x175 = INT32_MAX;
	int32_t t43 = 18629425;

	t43 = (((x173==x174)==x175)|x176);

	if (t43 != 11247) { NG(); } else { ; }
	
}

void f44(void) {
	uint32_t x177 = UINT32_MAX;
	uint16_t x179 = 27U;
	volatile uint64_t t44 = 1305516206089LLU;

	t44 = (((x177==x178)==x179)|x180);

	if (t44 != 17252974471983LLU) { NG(); } else { ; }
	
}

void f45(void) {
	volatile int32_t x181 = 106;
	uint32_t x184 = 7045002U;
	uint32_t t45 = 3357155U;

	t45 = (((x181==x182)==x183)|x184);

	if (t45 != 7045002U) { NG(); } else { ; }
	
}

void f46(void) {
	int16_t x185 = 53;
	volatile int32_t x186 = INT32_MAX;
	int64_t x188 = INT64_MIN;
	volatile int64_t t46 = INT64_MIN;

	t46 = (((x185==x186)==x187)|x188);

	if (t46 != INT64_MIN) { NG(); } else { ; }
	
}

void f47(void) {
	volatile uint16_t x190 = 0U;
	static int16_t x191 = INT16_MIN;
	int32_t x192 = INT32_MAX;
	volatile int32_t t47 = INT32_MAX;

	t47 = (((x189==x190)==x191)|x192);

	if (t47 != INT32_MAX) { NG(); } else { ; }
	
}

void f48(void) {
	int16_t x193 = 1599;
	volatile uint8_t x194 = UINT8_MAX;
	static int16_t x196 = 4;

	t48 = (((x193==x194)==x195)|x196);

	if (t48 != 4) { NG(); } else { ; }
	
}

void f49(void) {
	int8_t x197 = -1;
	volatile int32_t x198 = INT32_MIN;
	int64_t x199 = -15LL;
	int8_t x200 = 0;
	volatile int32_t t49 = 2035;

	t49 = (((x197==x198)==x199)|x200);

	if (t49 != 0) { NG(); } else { ; }
	
}

void f50(void) {
	volatile uint32_t x201 = UINT32_MAX;
	uint64_t x202 = 38689023LLU;
	uint32_t x203 = UINT32_MAX;
	int32_t x204 = INT32_MIN;
	int32_t t50 = INT32_MIN;

	t50 = (((x201==x202)==x203)|x204);

	if (t50 != INT32_MIN) { NG(); } else { ; }
	
}

void f51(void) {
	int8_t x205 = INT8_MIN;
	volatile uint16_t x206 = 3043U;
	int8_t x207 = -1;
	int8_t x208 = -23;
	int32_t t51 = 2913173;

	t51 = (((x205==x206)==x207)|x208);

	if (t51 != -23) { NG(); } else { ; }
	
}

void f52(void) {
	uint64_t x209 = UINT64_MAX;
	int32_t x210 = -1;
	volatile int16_t x211 = INT16_MIN;
	int8_t x212 = INT8_MIN;
	int32_t t52 = 86;

	t52 = (((x209==x210)==x211)|x212);

	if (t52 != -128) { NG(); } else { ; }
	
}

void f53(void) {
	static int16_t x213 = -1;
	int64_t x214 = INT64_MAX;
	static int64_t x215 = -27018807LL;
	static uint32_t x216 = 826544905U;

	t53 = (((x213==x214)==x215)|x216);

	if (t53 != 826544905U) { NG(); } else { ; }
	
}

void f54(void) {
	int16_t x217 = INT16_MIN;
	uint8_t x218 = 1U;
	int8_t x220 = 1;
	static volatile int32_t t54 = -1;

	t54 = (((x217==x218)==x219)|x220);

	if (t54 != 1) { NG(); } else { ; }
	
}

void f55(void) {
	uint16_t x221 = 1U;
	static uint16_t x223 = 103U;
	uint8_t x224 = 53U;
	volatile int32_t t55 = -3;

	t55 = (((x221==x222)==x223)|x224);

	if (t55 != 53) { NG(); } else { ; }
	
}

void f56(void) {
	static int64_t x225 = INT64_MAX;
	int64_t x226 = INT64_MIN;
	int32_t x227 = -1;
	int16_t x228 = INT16_MAX;
	static int32_t t56 = -1728;

	t56 = (((x225==x226)==x227)|x228);

	if (t56 != 32767) { NG(); } else { ; }
	
}

void f57(void) {
	int16_t x230 = 1;
	uint64_t x231 = UINT64_MAX;
	static uint16_t x232 = 3376U;

	t57 = (((x229==x230)==x231)|x232);

	if (t57 != 3376) { NG(); } else { ; }
	
}

void f58(void) {
	static int64_t x233 = INT64_MIN;
	int16_t x234 = -20;
	static uint16_t x236 = 9751U;
	int32_t t58 = -1;

	t58 = (((x233==x234)==x235)|x236);

	if (t58 != 9751) { NG(); } else { ; }
	
}

void f59(void) {
	int16_t x237 = INT16_MIN;
	int64_t x240 = 5342728111LL;
	int64_t t59 = -11067431334821LL;

	t59 = (((x237==x238)==x239)|x240);

	if (t59 != 5342728111LL) { NG(); } else { ; }
	
}

void f60(void) {
	volatile int8_t x241 = INT8_MAX;
	volatile uint64_t x242 = 34539432634263700LLU;
	int32_t x243 = -1156;
	int16_t x244 = INT16_MIN;
	int32_t t60 = 2904;

	t60 = (((x241==x242)==x243)|x244);

	if (t60 != -32768) { NG(); } else { ; }
	
}

void f61(void) {
	static volatile uint8_t x248 = 11U;
	volatile int32_t t61 = 2791052;

	t61 = (((x245==x246)==x247)|x248);

	if (t61 != 11) { NG(); } else { ; }
	
}

void f62(void) {
	uint32_t x251 = 325U;
	int32_t x252 = INT32_MAX;
	static volatile int32_t t62 = INT32_MAX;

	t62 = (((x249==x250)==x251)|x252);

	if (t62 != INT32_MAX) { NG(); } else { ; }
	
}

void f63(void) {
	int16_t x253 = 203;
	uint64_t x255 = UINT64_MAX;
	uint64_t x256 = 835126LLU;
	uint64_t t63 = 1LLU;

	t63 = (((x253==x254)==x255)|x256);

	if (t63 != 835126LLU) { NG(); } else { ; }
	
}

void f64(void) {
	int8_t x258 = -5;
	int8_t x259 = -13;
	int64_t x260 = -42915376781190501LL;
	int64_t t64 = 0LL;

	t64 = (((x257==x258)==x259)|x260);

	if (t64 != -42915376781190501LL) { NG(); } else { ; }
	
}

void f65(void) {
	uint8_t x262 = 2U;
	volatile int16_t x263 = -36;
	uint32_t x264 = 1831U;
	uint32_t t65 = 4666236U;

	t65 = (((x261==x262)==x263)|x264);

	if (t65 != 1831U) { NG(); } else { ; }
	
}

void f66(void) {
	uint32_t x265 = 504123837U;
	int32_t x267 = INT32_MIN;
	int32_t t66 = 356941825;

	t66 = (((x265==x266)==x267)|x268);

	if (t66 != 2) { NG(); } else { ; }
	
}

void f67(void) {
	static uint32_t x270 = UINT32_MAX;
	volatile uint8_t x271 = 2U;

	t67 = (((x269==x270)==x271)|x272);

	if (t67 != -1) { NG(); } else { ; }
	
}

void f68(void) {
	volatile int16_t x273 = INT16_MIN;
	uint64_t x275 = UINT64_MAX;
	uint64_t x276 = UINT64_MAX;
	volatile uint64_t t68 = UINT64_MAX;

	t68 = (((x273==x274)==x275)|x276);

	if (t68 != UINT64_MAX) { NG(); } else { ; }
	
}

void f69(void) {
	uint8_t x277 = 99U;
	int32_t x278 = INT32_MIN;
	volatile int32_t x279 = INT32_MAX;
	static int32_t x280 = INT32_MIN;

	t69 = (((x277==x278)==x279)|x280);

	if (t69 != INT32_MIN) { NG(); } else { ; }
	
}

void f70(void) {
	int64_t x281 = INT64_MIN;
	static int8_t x282 = 2;
	uint64_t x283 = UINT64_MAX;
	int64_t x284 = INT64_MAX;
	int64_t t70 = INT64_MAX;

	t70 = (((x281==x282)==x283)|x284);

	if (t70 != INT64_MAX) { NG(); } else { ; }
	
}

void f71(void) {
	uint16_t x285 = UINT16_MAX;
	int8_t x286 = -1;
	int32_t x287 = -1013;
	int16_t x288 = -3;
	volatile int32_t t71 = 607;

	t71 = (((x285==x286)==x287)|x288);

	if (t71 != -3) { NG(); } else { ; }
	
}

void f72(void) {
	uint8_t x289 = 45U;
	int32_t x290 = 23684;
	uint8_t x291 = 5U;
	int8_t x292 = -1;
	int32_t t72 = 31738462;

	t72 = (((x289==x290)==x291)|x292);

	if (t72 != -1) { NG(); } else { ; }
	
}

void f73(void) {
	uint32_t x293 = 376022860U;
	volatile int32_t x294 = -2357783;
	int16_t x295 = -7483;
	volatile uint16_t x296 = 28194U;

	t73 = (((x293==x294)==x295)|x296);

	if (t73 != 28194) { NG(); } else { ; }
	
}

void f74(void) {
	uint16_t x297 = UINT16_MAX;
	volatile int32_t x298 = INT32_MIN;
	static volatile int32_t x299 = 2665938;
	volatile int32_t x300 = 1;
	volatile int32_t t74 = -822623487;

	t74 = (((x297==x298)==x299)|x300);

	if (t74 != 1) { NG(); } else { ; }
	
}

void f75(void) {
	uint8_t x301 = 23U;
	static volatile int8_t x302 = INT8_MIN;
	static int32_t t75 = -7190;

	t75 = (((x301==x302)==x303)|x304);

	if (t75 != 127) { NG(); } else { ; }
	
}

void f76(void) {
	int64_t x305 = INT64_MIN;
	uint64_t x306 = 1008LLU;
	static int32_t x308 = -19842;
	volatile int32_t t76 = 0;

	t76 = (((x305==x306)==x307)|x308);

	if (t76 != -19842) { NG(); } else { ; }
	
}

void f77(void) {
	int32_t x309 = -1;
	static uint16_t x310 = UINT16_MAX;
	static uint8_t x311 = UINT8_MAX;
	static int64_t x312 = -1LL;
	static volatile int64_t t77 = 1516LL;

	t77 = (((x309==x310)==x311)|x312);

	if (t77 != -1LL) { NG(); } else { ; }
	
}

void f78(void) {
	int64_t x313 = -1LL;
	int8_t x314 = INT8_MIN;
	int64_t x315 = INT64_MIN;
	static volatile uint8_t x316 = 61U;
	static int32_t t78 = 42;

	t78 = (((x313==x314)==x315)|x316);

	if (t78 != 61) { NG(); } else { ; }
	
}

void f79(void) {
	int8_t x317 = INT8_MAX;
	int64_t x318 = INT64_MAX;
	static int32_t x319 = INT32_MIN;
	volatile int64_t x320 = INT64_MAX;

	t79 = (((x317==x318)==x319)|x320);

	if (t79 != INT64_MAX) { NG(); } else { ; }
	
}

void f80(void) {
	int8_t x322 = INT8_MIN;
	uint8_t x324 = UINT8_MAX;

	t80 = (((x321==x322)==x323)|x324);

	if (t80 != 255) { NG(); } else { ; }
	
}

void f81(void) {
	uint8_t x325 = UINT8_MAX;
	uint32_t x326 = 112903U;
	static int8_t x327 = INT8_MIN;
	volatile int64_t x328 = INT64_MAX;

	t81 = (((x325==x326)==x327)|x328);

	if (t81 != INT64_MAX) { NG(); } else { ; }
	
}

void f82(void) {
	static uint8_t x329 = 2U;
	volatile int8_t x331 = INT8_MAX;
	int16_t x332 = -22;
	volatile int32_t t82 = 20;

	t82 = (((x329==x330)==x331)|x332);

	if (t82 != -22) { NG(); } else { ; }
	
}

void f83(void) {
	int64_t x333 = INT64_MIN;
	int8_t x334 = INT8_MIN;
	int16_t x335 = -1;
	int32_t x336 = 218624855;
	int32_t t83 = 58;

	t83 = (((x333==x334)==x335)|x336);

	if (t83 != 218624855) { NG(); } else { ; }
	
}

void f84(void) {
	int64_t x337 = -1LL;
	static volatile int16_t x338 = INT16_MAX;
	static uint8_t x339 = 4U;
	uint16_t x340 = 58U;
	volatile int32_t t84 = 13871;

	t84 = (((x337==x338)==x339)|x340);

	if (t84 != 58) { NG(); } else { ; }
	
}

void f85(void) {
	uint32_t x341 = UINT32_MAX;
	static int8_t x343 = 15;
	static volatile int32_t x344 = INT32_MIN;
	static volatile int32_t t85 = INT32_MIN;

	t85 = (((x341==x342)==x343)|x344);

	if (t85 != INT32_MIN) { NG(); } else { ; }
	
}

void f86(void) {
	uint64_t x345 = 1748931884568306LLU;
	int32_t x346 = -7404;
	int64_t x347 = INT64_MIN;
	static int64_t x348 = INT64_MAX;
	int64_t t86 = INT64_MAX;

	t86 = (((x345==x346)==x347)|x348);

	if (t86 != INT64_MAX) { NG(); } else { ; }
	
}

void f87(void) {
	uint8_t x349 = UINT8_MAX;
	int16_t x350 = INT16_MIN;
	uint64_t x351 = UINT64_MAX;
	int8_t x352 = INT8_MIN;
	int32_t t87 = -5983110;

	t87 = (((x349==x350)==x351)|x352);

	if (t87 != -128) { NG(); } else { ; }
	
}

void f88(void) {
	uint8_t x353 = 35U;
	int32_t x355 = INT32_MIN;
	volatile int32_t x356 = INT32_MIN;
	static int32_t t88 = INT32_MIN;

	t88 = (((x353==x354)==x355)|x356);

	if (t88 != INT32_MIN) { NG(); } else { ; }
	
}

void f89(void) {
	volatile int32_t x358 = INT32_MIN;
	static int64_t x359 = INT64_MIN;
	int32_t t89 = -470;

	t89 = (((x357==x358)==x359)|x360);

	if (t89 != 4) { NG(); } else { ; }
	
}

void f90(void) {
	uint64_t x361 = 31227902444178LLU;
	int32_t x362 = INT32_MAX;
	int32_t x363 = INT32_MIN;
	static uint32_t t90 = 7524999U;

	t90 = (((x361==x362)==x363)|x364);

	if (t90 != 184U) { NG(); } else { ; }
	
}

void f91(void) {
	int64_t x365 = -1LL;
	int16_t x366 = 340;
	int32_t x368 = 0;

	t91 = (((x365==x366)==x367)|x368);

	if (t91 != 0) { NG(); } else { ; }
	
}

void f92(void) {
	volatile int64_t x369 = INT64_MIN;
	volatile uint16_t x370 = 25940U;
	volatile int8_t x371 = INT8_MIN;
	uint64_t x372 = 140052156945153627LLU;
	uint64_t t92 = 69972439914LLU;

	t92 = (((x369==x370)==x371)|x372);

	if (t92 != 140052156945153627LLU) { NG(); } else { ; }
	
}

void f93(void) {
	static volatile int32_t x373 = INT32_MIN;
	int8_t x374 = INT8_MIN;
	volatile int16_t x375 = -4;
	static volatile int32_t t93 = 5783;

	t93 = (((x373==x374)==x375)|x376);

	if (t93 != 45) { NG(); } else { ; }
	
}

void f94(void) {
	volatile int32_t x377 = 0;
	uint8_t x378 = UINT8_MAX;
	static volatile int64_t x379 = INT64_MIN;
	int64_t x380 = 128178883680LL;

	t94 = (((x377==x378)==x379)|x380);

	if (t94 != 128178883680LL) { NG(); } else { ; }
	
}

void f95(void) {
	uint8_t x381 = UINT8_MAX;
	static int16_t x382 = INT16_MAX;
	uint8_t x383 = 1U;
	static volatile uint8_t x384 = 15U;
	int32_t t95 = -1;

	t95 = (((x381==x382)==x383)|x384);

	if (t95 != 15) { NG(); } else { ; }
	
}

void f96(void) {
	uint8_t x385 = 4U;
	int32_t x387 = INT32_MAX;
	static int64_t x388 = INT64_MIN;

	t96 = (((x385==x386)==x387)|x388);

	if (t96 != INT64_MIN) { NG(); } else { ; }
	
}

void f97(void) {
	static volatile uint16_t x390 = 2115U;
	static int32_t x391 = -1;
	uint32_t x392 = 562329104U;
	static volatile uint32_t t97 = 60186820U;

	t97 = (((x389==x390)==x391)|x392);

	if (t97 != 562329104U) { NG(); } else { ; }
	
}

void f98(void) {
	int8_t x393 = INT8_MIN;
	volatile uint64_t x394 = UINT64_MAX;
	uint8_t x395 = 3U;

	t98 = (((x393==x394)==x395)|x396);

	if (t98 != -1LL) { NG(); } else { ; }
	
}

void f99(void) {
	uint32_t x397 = 0U;
	int32_t t99 = -11;

	t99 = (((x397==x398)==x399)|x400);

	if (t99 != -1) { NG(); } else { ; }
	
}

int main(void) {
    f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();


    return 0;
}

