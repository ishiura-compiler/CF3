#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

volatile int32_t t0 = -1;
int16_t x9 = -1;
static uint8_t x13 = 5U;
int16_t x20 = -1;
int8_t x22 = -1;
int64_t x29 = INT64_MIN;
int16_t x42 = INT16_MAX;
static int32_t t10 = 0;
int32_t x46 = 172986;
static uint32_t x52 = 360U;
uint32_t t12 = UINT32_MAX;
int16_t x54 = INT16_MIN;
int32_t t13 = 177640949;
int8_t x62 = -2;
int16_t x64 = -1;
int32_t x68 = 1430;
int32_t t16 = 20046;
int32_t t18 = -40607;
uint32_t t19 = 1036527893U;
uint16_t x86 = 5U;
static volatile int64_t x88 = 63040LL;
volatile int8_t x89 = -2;
int16_t x90 = 0;
int32_t x97 = -1;
static int64_t x99 = INT64_MAX;
int32_t x106 = -349360333;
int8_t x108 = INT8_MIN;
static int16_t x109 = 562;
int64_t x113 = -9585311338717299LL;
int64_t t28 = -8416432403LL;
static volatile int16_t x124 = INT16_MIN;
static int8_t x129 = INT8_MIN;
int64_t x132 = INT64_MAX;
uint64_t x136 = UINT64_MAX;
static volatile uint8_t x139 = UINT8_MAX;
int32_t x145 = INT32_MAX;
int8_t x146 = 26;
volatile int32_t t36 = INT32_MAX;
volatile int32_t t40 = -1;
int16_t x168 = INT16_MIN;
uint32_t x172 = UINT32_MAX;
volatile int16_t x173 = -32;
uint64_t x176 = 28LLU;
int32_t t43 = -2311;
int8_t x178 = INT8_MAX;
static int64_t x186 = INT64_MAX;
uint16_t x191 = 3559U;
int8_t x192 = INT8_MIN;
volatile int8_t x199 = -17;
volatile uint16_t x201 = 28U;
int8_t x203 = INT8_MAX;
int8_t x204 = INT8_MIN;
volatile int32_t t50 = -1;
int32_t x209 = -61;
static uint8_t x215 = 0U;
volatile uint8_t x216 = 76U;
volatile int32_t t53 = -243;
uint32_t x217 = 9886630U;
static uint16_t x222 = UINT16_MAX;
volatile int32_t x223 = -55589;
static uint16_t x225 = 3287U;
uint16_t x231 = UINT16_MAX;
static volatile int32_t x233 = -1;
int16_t x235 = INT16_MIN;
volatile int16_t x239 = INT16_MIN;
volatile int32_t t59 = -79;
int32_t x251 = INT32_MIN;
volatile uint64_t x255 = 1493488725215320454LLU;
static volatile uint8_t x258 = UINT8_MAX;
int32_t t64 = 1711;
uint8_t x263 = 0U;
int16_t x266 = -1;
volatile uint32_t t66 = UINT32_MAX;
int32_t x274 = INT32_MAX;
volatile uint32_t x276 = UINT32_MAX;
int32_t x277 = INT32_MAX;
static uint32_t x281 = 103941U;
volatile int32_t x284 = INT32_MIN;
uint32_t t70 = 2U;
int64_t x285 = 313988381921779987LL;
uint64_t x287 = UINT64_MAX;
int32_t t74 = 7220;
static volatile int8_t x301 = 25;
uint16_t x307 = 1U;
uint8_t x311 = 1U;
volatile int64_t x314 = INT64_MIN;
volatile int8_t x316 = -20;
volatile int32_t t79 = 51;
uint16_t x323 = UINT16_MAX;
int32_t t80 = INT32_MIN;
int32_t x326 = -1;
static int64_t x328 = -1LL;
uint32_t x333 = UINT32_MAX;
uint32_t x336 = 49U;
uint8_t x338 = UINT8_MAX;
volatile uint32_t x341 = UINT32_MAX;
int32_t x343 = INT32_MAX;
uint32_t x347 = 6961U;
int64_t x352 = 68502374602LL;
static volatile uint32_t x363 = 1819U;
uint64_t t90 = UINT64_MAX;
volatile uint8_t x365 = 2U;
volatile int64_t x366 = 11815555455795198LL;
int32_t t91 = -1;
int64_t x369 = -213096477861799LL;
int16_t x370 = -1;
int32_t x383 = -30353634;
int16_t x390 = -1;
int64_t x396 = -3551568827906484232LL;


void f0(void) {
	int32_t x1 = -1;
	int64_t x2 = -4474811962LL;
	int16_t x3 = INT16_MIN;
	uint8_t x4 = 0U;

	t0 = (x1|((x2<=x3)==x4));

	if (t0 != -1) { NG(); } else { ; }
	
}

void f1(void) {
	int16_t x5 = -20;
	int32_t x6 = 133;
	int32_t x7 = 1;
	int8_t x8 = 23;
	int32_t t1 = -6820880;

	t1 = (x5|((x6<=x7)==x8));

	if (t1 != -20) { NG(); } else { ; }
	
}

void f2(void) {
	int16_t x10 = INT16_MIN;
	uint16_t x11 = UINT16_MAX;
	uint64_t x12 = UINT64_MAX;
	int32_t t2 = 62626;

	t2 = (x9|((x10<=x11)==x12));

	if (t2 != -1) { NG(); } else { ; }
	
}

void f3(void) {
	static int64_t x14 = -615045174LL;
	uint8_t x15 = 3U;
	static int16_t x16 = INT16_MIN;
	int32_t t3 = 45;

	t3 = (x13|((x14<=x15)==x16));

	if (t3 != 5) { NG(); } else { ; }
	
}

void f4(void) {
	int64_t x17 = INT64_MIN;
	int16_t x18 = INT16_MIN;
	volatile int64_t x19 = INT64_MIN;
	volatile int64_t t4 = INT64_MIN;

	t4 = (x17|((x18<=x19)==x20));

	if (t4 != INT64_MIN) { NG(); } else { ; }
	
}

void f5(void) {
	uint64_t x21 = UINT64_MAX;
	volatile int16_t x23 = INT16_MAX;
	uint64_t x24 = UINT64_MAX;
	volatile uint64_t t5 = UINT64_MAX;

	t5 = (x21|((x22<=x23)==x24));

	if (t5 != UINT64_MAX) { NG(); } else { ; }
	
}

void f6(void) {
	int16_t x25 = -319;
	static int32_t x26 = -1;
	int64_t x27 = INT64_MIN;
	uint16_t x28 = 61U;
	static volatile int32_t t6 = -19;

	t6 = (x25|((x26<=x27)==x28));

	if (t6 != -319) { NG(); } else { ; }
	
}

void f7(void) {
	volatile int8_t x30 = 1;
	int32_t x31 = INT32_MIN;
	static int16_t x32 = INT16_MIN;
	volatile int64_t t7 = INT64_MIN;

	t7 = (x29|((x30<=x31)==x32));

	if (t7 != INT64_MIN) { NG(); } else { ; }
	
}

void f8(void) {
	int8_t x33 = INT8_MAX;
	int32_t x34 = -1;
	int64_t x35 = INT64_MAX;
	int32_t x36 = 53058;
	int32_t t8 = 3720663;

	t8 = (x33|((x34<=x35)==x36));

	if (t8 != 127) { NG(); } else { ; }
	
}

void f9(void) {
	int16_t x37 = INT16_MAX;
	volatile int16_t x38 = INT16_MIN;
	uint32_t x39 = 18U;
	volatile int16_t x40 = 416;
	int32_t t9 = 6399658;

	t9 = (x37|((x38<=x39)==x40));

	if (t9 != 32767) { NG(); } else { ; }
	
}

void f10(void) {
	static int16_t x41 = INT16_MIN;
	uint8_t x43 = 3U;
	int16_t x44 = 1962;

	t10 = (x41|((x42<=x43)==x44));

	if (t10 != -32768) { NG(); } else { ; }
	
}

void f11(void) {
	volatile uint16_t x45 = 1614U;
	int16_t x47 = INT16_MIN;
	uint32_t x48 = 1U;
	static int32_t t11 = -1046489;

	t11 = (x45|((x46<=x47)==x48));

	if (t11 != 1614) { NG(); } else { ; }
	
}

void f12(void) {
	uint32_t x49 = UINT32_MAX;
	uint64_t x50 = 47593LLU;
	volatile int64_t x51 = INT64_MAX;

	t12 = (x49|((x50<=x51)==x52));

	if (t12 != UINT32_MAX) { NG(); } else { ; }
	
}

void f13(void) {
	uint16_t x53 = 2675U;
	int64_t x55 = INT64_MIN;
	volatile int32_t x56 = INT32_MAX;

	t13 = (x53|((x54<=x55)==x56));

	if (t13 != 2675) { NG(); } else { ; }
	
}

void f14(void) {
	int16_t x57 = -20;
	int16_t x58 = 0;
	volatile int64_t x59 = -13748889934355LL;
	static uint16_t x60 = 290U;
	volatile int32_t t14 = 0;

	t14 = (x57|((x58<=x59)==x60));

	if (t14 != -20) { NG(); } else { ; }
	
}

void f15(void) {
	uint32_t x61 = 592258986U;
	int64_t x63 = INT64_MIN;
	volatile uint32_t t15 = 1774268579U;

	t15 = (x61|((x62<=x63)==x64));

	if (t15 != 592258986U) { NG(); } else { ; }
	
}

void f16(void) {
	int8_t x65 = 0;
	int64_t x66 = INT64_MIN;
	int64_t x67 = INT64_MIN;

	t16 = (x65|((x66<=x67)==x68));

	if (t16 != 0) { NG(); } else { ; }
	
}

void f17(void) {
	int64_t x69 = INT64_MIN;
	int8_t x70 = INT8_MIN;
	uint8_t x71 = 3U;
	int16_t x72 = -1;
	int64_t t17 = INT64_MIN;

	t17 = (x69|((x70<=x71)==x72));

	if (t17 != INT64_MIN) { NG(); } else { ; }
	
}

void f18(void) {
	int16_t x73 = INT16_MAX;
	static volatile int16_t x74 = INT16_MIN;
	uint32_t x75 = 1U;
	int8_t x76 = -1;

	t18 = (x73|((x74<=x75)==x76));

	if (t18 != 32767) { NG(); } else { ; }
	
}

void f19(void) {
	uint32_t x77 = 566392214U;
	int16_t x78 = -1;
	uint32_t x79 = 10642054U;
	volatile int64_t x80 = INT64_MAX;

	t19 = (x77|((x78<=x79)==x80));

	if (t19 != 566392214U) { NG(); } else { ; }
	
}

void f20(void) {
	int16_t x81 = INT16_MIN;
	int8_t x82 = -7;
	volatile int64_t x83 = INT64_MIN;
	static int16_t x84 = INT16_MIN;
	volatile int32_t t20 = 116454;

	t20 = (x81|((x82<=x83)==x84));

	if (t20 != -32768) { NG(); } else { ; }
	
}

void f21(void) {
	volatile int64_t x85 = -1LL;
	static volatile uint32_t x87 = 45U;
	int64_t t21 = -1535897LL;

	t21 = (x85|((x86<=x87)==x88));

	if (t21 != -1LL) { NG(); } else { ; }
	
}

void f22(void) {
	int8_t x91 = INT8_MIN;
	int64_t x92 = -1LL;
	int32_t t22 = 1177476;

	t22 = (x89|((x90<=x91)==x92));

	if (t22 != -2) { NG(); } else { ; }
	
}

void f23(void) {
	static int8_t x93 = INT8_MAX;
	int8_t x94 = -1;
	static uint8_t x95 = 109U;
	uint64_t x96 = 453632802354405LLU;
	int32_t t23 = -211;

	t23 = (x93|((x94<=x95)==x96));

	if (t23 != 127) { NG(); } else { ; }
	
}

void f24(void) {
	uint64_t x98 = UINT64_MAX;
	static int8_t x100 = INT8_MAX;
	int32_t t24 = -13;

	t24 = (x97|((x98<=x99)==x100));

	if (t24 != -1) { NG(); } else { ; }
	
}

void f25(void) {
	uint8_t x101 = 2U;
	int64_t x102 = -4264398688LL;
	int8_t x103 = -1;
	int8_t x104 = 19;
	volatile int32_t t25 = 92341969;

	t25 = (x101|((x102<=x103)==x104));

	if (t25 != 2) { NG(); } else { ; }
	
}

void f26(void) {
	int16_t x105 = INT16_MAX;
	uint32_t x107 = 0U;
	int32_t t26 = -7509;

	t26 = (x105|((x106<=x107)==x108));

	if (t26 != 32767) { NG(); } else { ; }
	
}

void f27(void) {
	static int64_t x110 = -1LL;
	int32_t x111 = INT32_MIN;
	int32_t x112 = 24691;
	volatile int32_t t27 = -3318;

	t27 = (x109|((x110<=x111)==x112));

	if (t27 != 562) { NG(); } else { ; }
	
}

void f28(void) {
	int64_t x114 = 1721LL;
	int64_t x115 = -57305229LL;
	int16_t x116 = INT16_MIN;

	t28 = (x113|((x114<=x115)==x116));

	if (t28 != -9585311338717299LL) { NG(); } else { ; }
	
}

void f29(void) {
	volatile int8_t x117 = 0;
	volatile int32_t x118 = INT32_MAX;
	static int64_t x119 = -15987440LL;
	volatile int64_t x120 = 465028731204463593LL;
	static volatile int32_t t29 = 96;

	t29 = (x117|((x118<=x119)==x120));

	if (t29 != 0) { NG(); } else { ; }
	
}

void f30(void) {
	volatile uint32_t x121 = 7609447U;
	int32_t x122 = -488405311;
	int8_t x123 = -1;
	static volatile uint32_t t30 = 3584U;

	t30 = (x121|((x122<=x123)==x124));

	if (t30 != 7609447U) { NG(); } else { ; }
	
}

void f31(void) {
	int16_t x125 = -1;
	uint32_t x126 = UINT32_MAX;
	int16_t x127 = 4675;
	uint64_t x128 = UINT64_MAX;
	volatile int32_t t31 = -61;

	t31 = (x125|((x126<=x127)==x128));

	if (t31 != -1) { NG(); } else { ; }
	
}

void f32(void) {
	int8_t x130 = INT8_MIN;
	int16_t x131 = INT16_MIN;
	static volatile int32_t t32 = -1;

	t32 = (x129|((x130<=x131)==x132));

	if (t32 != -128) { NG(); } else { ; }
	
}

void f33(void) {
	static int64_t x133 = INT64_MIN;
	volatile int8_t x134 = INT8_MIN;
	static int32_t x135 = INT32_MAX;
	int64_t t33 = INT64_MIN;

	t33 = (x133|((x134<=x135)==x136));

	if (t33 != INT64_MIN) { NG(); } else { ; }
	
}

void f34(void) {
	uint16_t x137 = UINT16_MAX;
	int32_t x138 = -1003859;
	int32_t x140 = INT32_MIN;
	volatile int32_t t34 = -57222;

	t34 = (x137|((x138<=x139)==x140));

	if (t34 != 65535) { NG(); } else { ; }
	
}

void f35(void) {
	static volatile int64_t x141 = INT64_MAX;
	int16_t x142 = -4691;
	volatile int64_t x143 = INT64_MIN;
	int64_t x144 = INT64_MIN;
	static int64_t t35 = INT64_MAX;

	t35 = (x141|((x142<=x143)==x144));

	if (t35 != INT64_MAX) { NG(); } else { ; }
	
}

void f36(void) {
	volatile int32_t x147 = -1;
	uint8_t x148 = UINT8_MAX;

	t36 = (x145|((x146<=x147)==x148));

	if (t36 != INT32_MAX) { NG(); } else { ; }
	
}

void f37(void) {
	int32_t x149 = 369;
	static volatile uint32_t x150 = 965280730U;
	static volatile int64_t x151 = INT64_MAX;
	int8_t x152 = -5;
	int32_t t37 = 14;

	t37 = (x149|((x150<=x151)==x152));

	if (t37 != 369) { NG(); } else { ; }
	
}

void f38(void) {
	int64_t x153 = INT64_MIN;
	uint16_t x154 = UINT16_MAX;
	static int64_t x155 = INT64_MIN;
	volatile uint32_t x156 = UINT32_MAX;
	volatile int64_t t38 = INT64_MIN;

	t38 = (x153|((x154<=x155)==x156));

	if (t38 != INT64_MIN) { NG(); } else { ; }
	
}

void f39(void) {
	int16_t x157 = 10782;
	uint8_t x158 = UINT8_MAX;
	int16_t x159 = -1;
	uint64_t x160 = UINT64_MAX;
	int32_t t39 = 1;

	t39 = (x157|((x158<=x159)==x160));

	if (t39 != 10782) { NG(); } else { ; }
	
}

void f40(void) {
	static int8_t x161 = INT8_MIN;
	volatile int8_t x162 = 1;
	uint8_t x163 = UINT8_MAX;
	int32_t x164 = INT32_MIN;

	t40 = (x161|((x162<=x163)==x164));

	if (t40 != -128) { NG(); } else { ; }
	
}

void f41(void) {
	volatile uint8_t x165 = 0U;
	volatile int64_t x166 = INT64_MIN;
	static uint16_t x167 = UINT16_MAX;
	volatile int32_t t41 = -894760888;

	t41 = (x165|((x166<=x167)==x168));

	if (t41 != 0) { NG(); } else { ; }
	
}

void f42(void) {
	uint16_t x169 = UINT16_MAX;
	int32_t x170 = -1;
	static int16_t x171 = INT16_MIN;
	volatile int32_t t42 = -23;

	t42 = (x169|((x170<=x171)==x172));

	if (t42 != 65535) { NG(); } else { ; }
	
}

void f43(void) {
	static volatile int16_t x174 = INT16_MIN;
	int32_t x175 = INT32_MIN;

	t43 = (x173|((x174<=x175)==x176));

	if (t43 != -32) { NG(); } else { ; }
	
}

void f44(void) {
	int32_t x177 = -412;
	volatile int32_t x179 = INT32_MIN;
	uint16_t x180 = UINT16_MAX;
	volatile int32_t t44 = -152;

	t44 = (x177|((x178<=x179)==x180));

	if (t44 != -412) { NG(); } else { ; }
	
}

void f45(void) {
	int64_t x181 = -1LL;
	uint8_t x182 = 126U;
	int64_t x183 = -1LL;
	int16_t x184 = -1;
	int64_t t45 = 103LL;

	t45 = (x181|((x182<=x183)==x184));

	if (t45 != -1LL) { NG(); } else { ; }
	
}

void f46(void) {
	int16_t x185 = 82;
	int8_t x187 = INT8_MAX;
	static int64_t x188 = INT64_MIN;
	volatile int32_t t46 = -6;

	t46 = (x185|((x186<=x187)==x188));

	if (t46 != 82) { NG(); } else { ; }
	
}

void f47(void) {
	int32_t x189 = INT32_MIN;
	int64_t x190 = INT64_MAX;
	int32_t t47 = INT32_MIN;

	t47 = (x189|((x190<=x191)==x192));

	if (t47 != INT32_MIN) { NG(); } else { ; }
	
}

void f48(void) {
	uint32_t x193 = 31884831U;
	int64_t x194 = -1LL;
	static uint8_t x195 = 3U;
	int16_t x196 = -338;
	static volatile uint32_t t48 = 379U;

	t48 = (x193|((x194<=x195)==x196));

	if (t48 != 31884831U) { NG(); } else { ; }
	
}

void f49(void) {
	uint32_t x197 = UINT32_MAX;
	int32_t x198 = -28;
	uint8_t x200 = 1U;
	uint32_t t49 = UINT32_MAX;

	t49 = (x197|((x198<=x199)==x200));

	if (t49 != UINT32_MAX) { NG(); } else { ; }
	
}

void f50(void) {
	static volatile int32_t x202 = INT32_MAX;

	t50 = (x201|((x202<=x203)==x204));

	if (t50 != 28) { NG(); } else { ; }
	
}

void f51(void) {
	static int32_t x205 = -1;
	int32_t x206 = -135946;
	volatile int16_t x207 = INT16_MIN;
	static int8_t x208 = INT8_MAX;
	int32_t t51 = 7;

	t51 = (x205|((x206<=x207)==x208));

	if (t51 != -1) { NG(); } else { ; }
	
}

void f52(void) {
	volatile int64_t x210 = INT64_MIN;
	uint16_t x211 = 12804U;
	int32_t x212 = INT32_MIN;
	volatile int32_t t52 = -12849;

	t52 = (x209|((x210<=x211)==x212));

	if (t52 != -61) { NG(); } else { ; }
	
}

void f53(void) {
	int16_t x213 = 1;
	volatile uint32_t x214 = UINT32_MAX;

	t53 = (x213|((x214<=x215)==x216));

	if (t53 != 1) { NG(); } else { ; }
	
}

void f54(void) {
	volatile int32_t x218 = INT32_MIN;
	static int64_t x219 = -1LL;
	int8_t x220 = -1;
	static uint32_t t54 = 21U;

	t54 = (x217|((x218<=x219)==x220));

	if (t54 != 9886630U) { NG(); } else { ; }
	
}

void f55(void) {
	static int8_t x221 = 6;
	static uint16_t x224 = 23578U;
	static int32_t t55 = -824;

	t55 = (x221|((x222<=x223)==x224));

	if (t55 != 6) { NG(); } else { ; }
	
}

void f56(void) {
	volatile int64_t x226 = INT64_MIN;
	int8_t x227 = -5;
	uint16_t x228 = UINT16_MAX;
	int32_t t56 = 0;

	t56 = (x225|((x226<=x227)==x228));

	if (t56 != 3287) { NG(); } else { ; }
	
}

void f57(void) {
	int32_t x229 = -455301581;
	volatile uint32_t x230 = 0U;
	volatile int8_t x232 = -3;
	volatile int32_t t57 = 90881938;

	t57 = (x229|((x230<=x231)==x232));

	if (t57 != -455301581) { NG(); } else { ; }
	
}

void f58(void) {
	int8_t x234 = 27;
	int16_t x236 = INT16_MIN;
	volatile int32_t t58 = -8339971;

	t58 = (x233|((x234<=x235)==x236));

	if (t58 != -1) { NG(); } else { ; }
	
}

void f59(void) {
	int16_t x237 = -1;
	volatile uint32_t x238 = UINT32_MAX;
	volatile int64_t x240 = -154LL;

	t59 = (x237|((x238<=x239)==x240));

	if (t59 != -1) { NG(); } else { ; }
	
}

void f60(void) {
	int16_t x241 = -1;
	volatile uint16_t x242 = 22U;
	int32_t x243 = INT32_MIN;
	static volatile uint16_t x244 = 7U;
	int32_t t60 = -1698;

	t60 = (x241|((x242<=x243)==x244));

	if (t60 != -1) { NG(); } else { ; }
	
}

void f61(void) {
	static uint16_t x245 = 11466U;
	int8_t x246 = -5;
	int32_t x247 = INT32_MIN;
	int16_t x248 = 12216;
	int32_t t61 = 2;

	t61 = (x245|((x246<=x247)==x248));

	if (t61 != 11466) { NG(); } else { ; }
	
}

void f62(void) {
	static int32_t x249 = INT32_MIN;
	volatile uint8_t x250 = 1U;
	static uint8_t x252 = 106U;
	int32_t t62 = INT32_MIN;

	t62 = (x249|((x250<=x251)==x252));

	if (t62 != INT32_MIN) { NG(); } else { ; }
	
}

void f63(void) {
	uint32_t x253 = UINT32_MAX;
	static uint32_t x254 = 1U;
	int8_t x256 = INT8_MAX;
	volatile uint32_t t63 = UINT32_MAX;

	t63 = (x253|((x254<=x255)==x256));

	if (t63 != UINT32_MAX) { NG(); } else { ; }
	
}

void f64(void) {
	static int16_t x257 = INT16_MIN;
	int16_t x259 = INT16_MIN;
	uint16_t x260 = UINT16_MAX;

	t64 = (x257|((x258<=x259)==x260));

	if (t64 != -32768) { NG(); } else { ; }
	
}

void f65(void) {
	int8_t x261 = INT8_MIN;
	uint64_t x262 = 4147313977292LLU;
	volatile uint16_t x264 = 211U;
	volatile int32_t t65 = 40286126;

	t65 = (x261|((x262<=x263)==x264));

	if (t65 != -128) { NG(); } else { ; }
	
}

void f66(void) {
	uint32_t x265 = UINT32_MAX;
	static int64_t x267 = INT64_MIN;
	static uint8_t x268 = UINT8_MAX;

	t66 = (x265|((x266<=x267)==x268));

	if (t66 != UINT32_MAX) { NG(); } else { ; }
	
}

void f67(void) {
	int32_t x269 = INT32_MAX;
	uint32_t x270 = 149908U;
	int16_t x271 = INT16_MIN;
	int16_t x272 = -2182;
	volatile int32_t t67 = INT32_MAX;

	t67 = (x269|((x270<=x271)==x272));

	if (t67 != INT32_MAX) { NG(); } else { ; }
	
}

void f68(void) {
	volatile int64_t x273 = -1LL;
	int8_t x275 = 2;
	static int64_t t68 = -5164105883LL;

	t68 = (x273|((x274<=x275)==x276));

	if (t68 != -1LL) { NG(); } else { ; }
	
}

void f69(void) {
	int32_t x278 = INT32_MIN;
	int64_t x279 = INT64_MAX;
	volatile int16_t x280 = 1;
	volatile int32_t t69 = INT32_MAX;

	t69 = (x277|((x278<=x279)==x280));

	if (t69 != INT32_MAX) { NG(); } else { ; }
	
}

void f70(void) {
	volatile int32_t x282 = INT32_MIN;
	int8_t x283 = -1;

	t70 = (x281|((x282<=x283)==x284));

	if (t70 != 103941U) { NG(); } else { ; }
	
}

void f71(void) {
	uint8_t x286 = 5U;
	int16_t x288 = INT16_MIN;
	static volatile int64_t t71 = -80126601666230818LL;

	t71 = (x285|((x286<=x287)==x288));

	if (t71 != 313988381921779987LL) { NG(); } else { ; }
	
}

void f72(void) {
	volatile uint64_t x289 = UINT64_MAX;
	uint8_t x290 = 111U;
	uint64_t x291 = 139619LLU;
	int16_t x292 = INT16_MAX;
	volatile uint64_t t72 = UINT64_MAX;

	t72 = (x289|((x290<=x291)==x292));

	if (t72 != UINT64_MAX) { NG(); } else { ; }
	
}

void f73(void) {
	static uint16_t x293 = 2933U;
	uint64_t x294 = 1794LLU;
	uint32_t x295 = UINT32_MAX;
	int32_t x296 = INT32_MIN;
	int32_t t73 = -93611952;

	t73 = (x293|((x294<=x295)==x296));

	if (t73 != 2933) { NG(); } else { ; }
	
}

void f74(void) {
	static int16_t x297 = -1579;
	static int16_t x298 = INT16_MIN;
	int32_t x299 = -1;
	int8_t x300 = INT8_MAX;

	t74 = (x297|((x298<=x299)==x300));

	if (t74 != -1579) { NG(); } else { ; }
	
}

void f75(void) {
	uint16_t x302 = 2189U;
	int16_t x303 = -1;
	int16_t x304 = INT16_MIN;
	static int32_t t75 = 148;

	t75 = (x301|((x302<=x303)==x304));

	if (t75 != 25) { NG(); } else { ; }
	
}

void f76(void) {
	uint32_t x305 = UINT32_MAX;
	static int64_t x306 = INT64_MIN;
	static int32_t x308 = INT32_MAX;
	uint32_t t76 = UINT32_MAX;

	t76 = (x305|((x306<=x307)==x308));

	if (t76 != UINT32_MAX) { NG(); } else { ; }
	
}

void f77(void) {
	static volatile int16_t x309 = INT16_MIN;
	int16_t x310 = 0;
	int8_t x312 = INT8_MAX;
	volatile int32_t t77 = 47772;

	t77 = (x309|((x310<=x311)==x312));

	if (t77 != -32768) { NG(); } else { ; }
	
}

void f78(void) {
	uint8_t x313 = 0U;
	int64_t x315 = INT64_MIN;
	int32_t t78 = -1825706;

	t78 = (x313|((x314<=x315)==x316));

	if (t78 != 0) { NG(); } else { ; }
	
}

void f79(void) {
	volatile int32_t x317 = -46;
	static int8_t x318 = INT8_MIN;
	static int64_t x319 = INT64_MAX;
	volatile int64_t x320 = -1LL;

	t79 = (x317|((x318<=x319)==x320));

	if (t79 != -46) { NG(); } else { ; }
	
}

void f80(void) {
	static volatile int32_t x321 = INT32_MIN;
	static int32_t x322 = -1;
	int8_t x324 = 29;

	t80 = (x321|((x322<=x323)==x324));

	if (t80 != INT32_MIN) { NG(); } else { ; }
	
}

void f81(void) {
	volatile int16_t x325 = 52;
	int32_t x327 = INT32_MAX;
	volatile int32_t t81 = 1;

	t81 = (x325|((x326<=x327)==x328));

	if (t81 != 52) { NG(); } else { ; }
	
}

void f82(void) {
	int8_t x329 = -3;
	int8_t x330 = INT8_MIN;
	uint32_t x331 = 0U;
	static int32_t x332 = INT32_MIN;
	int32_t t82 = 23187;

	t82 = (x329|((x330<=x331)==x332));

	if (t82 != -3) { NG(); } else { ; }
	
}

void f83(void) {
	int8_t x334 = INT8_MIN;
	volatile uint64_t x335 = 46853987364737267LLU;
	volatile uint32_t t83 = UINT32_MAX;

	t83 = (x333|((x334<=x335)==x336));

	if (t83 != UINT32_MAX) { NG(); } else { ; }
	
}

void f84(void) {
	int8_t x337 = -1;
	int16_t x339 = -84;
	static int8_t x340 = INT8_MIN;
	volatile int32_t t84 = 2411891;

	t84 = (x337|((x338<=x339)==x340));

	if (t84 != -1) { NG(); } else { ; }
	
}

void f85(void) {
	int32_t x342 = INT32_MIN;
	uint64_t x344 = 448LLU;
	uint32_t t85 = UINT32_MAX;

	t85 = (x341|((x342<=x343)==x344));

	if (t85 != UINT32_MAX) { NG(); } else { ; }
	
}

void f86(void) {
	int32_t x345 = INT32_MIN;
	uint8_t x346 = 3U;
	volatile int64_t x348 = INT64_MAX;
	int32_t t86 = INT32_MIN;

	t86 = (x345|((x346<=x347)==x348));

	if (t86 != INT32_MIN) { NG(); } else { ; }
	
}

void f87(void) {
	volatile uint64_t x349 = 129LLU;
	static uint16_t x350 = 28U;
	uint16_t x351 = UINT16_MAX;
	static uint64_t t87 = 1111026244080870183LLU;

	t87 = (x349|((x350<=x351)==x352));

	if (t87 != 129LLU) { NG(); } else { ; }
	
}

void f88(void) {
	int32_t x353 = INT32_MAX;
	static int16_t x354 = 1815;
	volatile int64_t x355 = INT64_MAX;
	int32_t x356 = INT32_MIN;
	int32_t t88 = INT32_MAX;

	t88 = (x353|((x354<=x355)==x356));

	if (t88 != INT32_MAX) { NG(); } else { ; }
	
}

void f89(void) {
	int32_t x357 = -1;
	static int8_t x358 = 2;
	volatile int64_t x359 = 19242LL;
	uint32_t x360 = UINT32_MAX;
	int32_t t89 = -45902;

	t89 = (x357|((x358<=x359)==x360));

	if (t89 != -1) { NG(); } else { ; }
	
}

void f90(void) {
	static volatile uint64_t x361 = UINT64_MAX;
	volatile int64_t x362 = -1LL;
	int8_t x364 = INT8_MAX;

	t90 = (x361|((x362<=x363)==x364));

	if (t90 != UINT64_MAX) { NG(); } else { ; }
	
}

void f91(void) {
	uint64_t x367 = UINT64_MAX;
	uint32_t x368 = 5406828U;

	t91 = (x365|((x366<=x367)==x368));

	if (t91 != 2) { NG(); } else { ; }
	
}

void f92(void) {
	int32_t x371 = INT32_MAX;
	int64_t x372 = -388835249501351251LL;
	volatile int64_t t92 = 167545LL;

	t92 = (x369|((x370<=x371)==x372));

	if (t92 != -213096477861799LL) { NG(); } else { ; }
	
}

void f93(void) {
	int64_t x373 = INT64_MIN;
	volatile int8_t x374 = INT8_MIN;
	uint64_t x375 = UINT64_MAX;
	volatile int32_t x376 = INT32_MIN;
	int64_t t93 = INT64_MIN;

	t93 = (x373|((x374<=x375)==x376));

	if (t93 != INT64_MIN) { NG(); } else { ; }
	
}

void f94(void) {
	int16_t x377 = 26;
	volatile int64_t x378 = -1LL;
	int8_t x379 = INT8_MAX;
	uint8_t x380 = 3U;
	int32_t t94 = 9903;

	t94 = (x377|((x378<=x379)==x380));

	if (t94 != 26) { NG(); } else { ; }
	
}

void f95(void) {
	uint64_t x381 = 178096199050198LLU;
	static int8_t x382 = -1;
	volatile int16_t x384 = -12656;
	volatile uint64_t t95 = 5458609995223449312LLU;

	t95 = (x381|((x382<=x383)==x384));

	if (t95 != 178096199050198LLU) { NG(); } else { ; }
	
}

void f96(void) {
	int16_t x385 = INT16_MIN;
	static volatile uint16_t x386 = 3U;
	int64_t x387 = INT64_MIN;
	int8_t x388 = INT8_MAX;
	volatile int32_t t96 = -898;

	t96 = (x385|((x386<=x387)==x388));

	if (t96 != -32768) { NG(); } else { ; }
	
}

void f97(void) {
	uint32_t x389 = 19U;
	uint32_t x391 = 5854422U;
	static uint8_t x392 = UINT8_MAX;
	static uint32_t t97 = 201045U;

	t97 = (x389|((x390<=x391)==x392));

	if (t97 != 19U) { NG(); } else { ; }
	
}

void f98(void) {
	int8_t x393 = INT8_MAX;
	int64_t x394 = INT64_MAX;
	uint32_t x395 = 19811166U;
	volatile int32_t t98 = 22;

	t98 = (x393|((x394<=x395)==x396));

	if (t98 != 127) { NG(); } else { ; }
	
}

void f99(void) {
	uint32_t x397 = UINT32_MAX;
	int16_t x398 = INT16_MAX;
	int32_t x399 = INT32_MIN;
	int32_t x400 = -1;
	static uint32_t t99 = UINT32_MAX;

	t99 = (x397|((x398<=x399)==x400));

	if (t99 != UINT32_MAX) { NG(); } else { ; }
	
}

int main(void) {
    f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();


    return 0;
}

