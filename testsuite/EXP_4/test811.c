
/*
CF3

Copyright (c) 2015 ishiura-lab.

Released under the MIT license.  
https://github.com/ishiura-compiler/CF3/MIT-LICENSE.md
*/

#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

int64_t x6 = INT64_MIN;
static int16_t x17 = INT16_MIN;
int64_t x18 = INT64_MIN;
int64_t x21 = 2952916366LL;
int64_t t6 = 518968LL;
volatile int16_t x29 = INT16_MIN;
volatile uint64_t x31 = 287086973519LLU;
int8_t x32 = 2;
static volatile uint16_t x39 = 720U;
static int8_t x43 = 60;
int64_t x46 = -1LL;
int64_t x49 = INT64_MIN;
int32_t x52 = INT32_MIN;
uint8_t x59 = 29U;
int8_t x63 = INT8_MAX;
uint32_t t15 = UINT32_MAX;
uint64_t x67 = UINT64_MAX;
int16_t x73 = INT16_MIN;
int32_t x80 = INT32_MIN;
volatile int16_t x81 = INT16_MIN;
volatile int8_t x93 = -15;
int8_t x94 = INT8_MIN;
volatile int32_t t23 = 52;
static uint32_t x98 = 170U;
volatile int64_t x101 = INT64_MIN;
int32_t t26 = -1458121;
uint32_t x114 = 824319U;
int16_t x115 = INT16_MIN;
int8_t x116 = 13;
int64_t x119 = 19000679283LL;
volatile int16_t x123 = INT16_MIN;
int16_t x151 = INT16_MIN;
int64_t t36 = 5817LL;
volatile uint8_t x155 = 62U;
uint32_t x162 = 9619U;
int8_t x167 = -1;
int64_t t40 = -52453LL;
int8_t x169 = -1;
volatile int32_t x170 = 5933308;
int8_t x171 = 5;
uint64_t x173 = UINT64_MAX;
int64_t x175 = -1LL;
static uint8_t x198 = 3U;
int32_t x201 = INT32_MAX;
int64_t x204 = INT64_MAX;
int64_t t48 = 242739363466676LL;
volatile int64_t x214 = INT64_MAX;
uint64_t t53 = UINT64_MAX;
uint16_t x236 = 39U;
int64_t t54 = 0LL;
int32_t x240 = INT32_MAX;
volatile int8_t x242 = INT8_MIN;
static int16_t x247 = 1;
uint16_t x248 = 8775U;
int64_t x249 = 179186LL;
volatile int16_t x269 = INT16_MAX;
int8_t x277 = 26;
static uint16_t x280 = 1U;
static uint16_t x283 = 2U;
int32_t x297 = INT32_MIN;
int32_t x298 = INT32_MIN;
uint32_t x304 = UINT32_MAX;
int16_t x319 = INT16_MAX;
int64_t x324 = 229214298LL;
volatile int64_t t75 = -1533562LL;
static int16_t x333 = -6;
int64_t x336 = INT64_MIN;
static int8_t x342 = -3;
uint16_t x347 = UINT16_MAX;
uint16_t x350 = 1005U;
volatile int64_t x355 = -1LL;
int64_t x367 = INT64_MIN;
static int16_t x368 = 35;
volatile uint64_t t87 = UINT64_MAX;
uint32_t x381 = UINT32_MAX;
static volatile int64_t x386 = -1LL;
int64_t t92 = 4644809006LL;
int64_t x404 = INT64_MIN;
volatile int16_t x410 = 5;
int8_t x412 = INT8_MIN;
volatile int32_t t97 = 1427;
int64_t x416 = INT64_MAX;
static uint8_t x418 = UINT8_MAX;
volatile uint64_t x419 = UINT64_MAX;
int32_t x421 = INT32_MIN;
uint32_t x422 = UINT32_MAX;
uint32_t x424 = 1314066948U;
static int64_t t100 = -69884306883812675LL;
volatile int32_t x431 = INT32_MIN;
int32_t x439 = INT32_MAX;
int16_t x442 = 195;
volatile int16_t x444 = INT16_MIN;
uint32_t t104 = 3382U;
static int8_t x446 = 3;
int32_t x451 = -334889;
volatile int64_t x454 = 69171LL;
int16_t x458 = INT16_MIN;
int8_t x462 = -1;
int16_t x465 = -33;
static volatile uint8_t x466 = 3U;
volatile int32_t t110 = 2051419;
int64_t x481 = INT64_MIN;
uint64_t x483 = UINT64_MAX;
uint32_t x485 = 21546U;
int8_t x489 = INT8_MAX;
uint8_t x492 = UINT8_MAX;
static uint64_t x496 = 129769011920226801LLU;
uint8_t x497 = UINT8_MAX;
uint64_t x500 = UINT64_MAX;
static uint64_t x509 = 1527181505LLU;
uint64_t x510 = 40660605LLU;
volatile int16_t x512 = INT16_MIN;
volatile int8_t x515 = INT8_MIN;
int64_t t124 = 6837354LL;
static int8_t x529 = -1;
volatile uint16_t x532 = 5U;
int64_t x533 = 981475511144934430LL;
uint16_t x536 = 7U;
volatile int32_t x548 = INT32_MIN;
uint16_t x553 = 1672U;
uint64_t x556 = 225976058525858031LLU;
uint8_t x559 = 3U;
static int32_t t131 = 23487500;
volatile int32_t t132 = 219072743;
int64_t x572 = -1LL;
int32_t t136 = 2909;
volatile int64_t t137 = 2343664607290LL;
volatile int8_t x585 = INT8_MAX;
int64_t t138 = -1635068009978110LL;
static int16_t x590 = -54;
volatile uint64_t x593 = 1583LLU;
volatile uint64_t t140 = 16376651LLU;
volatile int32_t x598 = INT32_MIN;
static volatile uint16_t x604 = UINT16_MAX;
int32_t x613 = -1;
int8_t x615 = 16;
uint8_t x621 = 1U;
static int64_t x624 = -40598446503LL;
int16_t x626 = -1;
int32_t x627 = -1;
int64_t x629 = INT64_MAX;
uint32_t x634 = UINT32_MAX;
static volatile int64_t t150 = 716665311476855LL;
uint8_t x644 = UINT8_MAX;
uint32_t x645 = 28U;
int64_t x650 = INT64_MIN;
static volatile int32_t t155 = INT32_MAX;
uint64_t x662 = 33LLU;
int8_t x672 = -1;
int8_t x681 = 41;
static volatile uint64_t t160 = 6082LLU;
int16_t x688 = -1;
static uint64_t x692 = 1972343741LLU;
uint32_t x699 = 53U;
volatile uint32_t t165 = 213968770U;
int16_t x710 = -33;
int64_t x712 = -491220749666339LL;
static int16_t x718 = INT16_MIN;
uint8_t x719 = 108U;
static volatile int16_t x720 = -1;
volatile int32_t t168 = 6179683;
volatile int64_t t169 = 3626776193LL;
int8_t x725 = -1;
volatile uint32_t t172 = UINT32_MAX;
static int8_t x740 = -2;
volatile int32_t t174 = 125031819;
uint64_t x748 = 8372366261002821946LLU;
static volatile uint64_t t175 = UINT64_MAX;
int8_t x750 = -1;
static uint64_t x751 = UINT64_MAX;
int64_t x757 = INT64_MAX;
int8_t x758 = -1;
static int64_t x763 = -1LL;
uint16_t x764 = 2898U;
uint16_t x765 = UINT16_MAX;
int8_t x768 = INT8_MIN;
int32_t t180 = 1714;
int32_t x770 = INT32_MIN;
static int64_t x777 = INT64_MIN;
volatile int32_t x779 = INT32_MIN;
static int32_t x785 = -1;
int64_t x786 = -143667195LL;
volatile int8_t x794 = -1;
int16_t x809 = -8055;
static volatile uint16_t x818 = 654U;
uint16_t x820 = 1783U;
int16_t x822 = INT16_MIN;
int64_t x824 = INT64_MIN;
uint32_t x825 = 1U;
volatile uint32_t t192 = 499038611U;
int32_t x829 = -1700;
uint8_t x831 = 81U;
uint64_t x837 = 7LLU;
static int32_t x839 = INT32_MAX;
volatile uint64_t t195 = 65318713251LLU;


void f0(void) {
    	int32_t x1 = 22494;
	volatile int8_t x2 = INT8_MIN;
	int32_t x3 = INT32_MAX;
	static int32_t x4 = -1;
	volatile int32_t t0 = 174377;

    t0 = (x1|(x2-(x3%x4)));

    if (t0 != -34) { NG(); } else { ; }
	
}

void f1(void) {
    	static int64_t x5 = INT64_MIN;
	uint32_t x7 = 2062U;
	uint64_t x8 = UINT64_MAX;
	uint64_t t1 = 794LLU;

    t1 = (x5|(x6-(x7%x8)));

    if (t1 != 18446744073709549554LLU) { NG(); } else { ; }
	
}

void f2(void) {
    	uint8_t x9 = UINT8_MAX;
	int16_t x10 = INT16_MIN;
	int32_t x11 = 98829;
	static int16_t x12 = INT16_MAX;
	static int32_t t2 = -398;

    t2 = (x9|(x10-(x11%x12)));

    if (t2 != -33281) { NG(); } else { ; }
	
}

void f3(void) {
    	int8_t x13 = INT8_MAX;
	uint16_t x14 = UINT16_MAX;
	uint64_t x15 = UINT64_MAX;
	int64_t x16 = INT64_MIN;
	static volatile uint64_t t3 = 329755553LLU;

    t3 = (x13|(x14-(x15%x16)));

    if (t3 != 9223372036854841471LLU) { NG(); } else { ; }
	
}

void f4(void) {
    	int64_t x19 = -139746LL;
	volatile int8_t x20 = 4;
	int64_t t4 = 170234069366865LL;

    t4 = (x17|(x18-(x19%x20)));

    if (t4 != -32766LL) { NG(); } else { ; }
	
}

void f5(void) {
    	uint16_t x22 = 1500U;
	static uint16_t x23 = 3111U;
	int32_t x24 = 16289408;
	int64_t t5 = -25950497LL;

    t5 = (x21|(x22-(x23%x24)));

    if (t5 != -577LL) { NG(); } else { ; }
	
}

void f6(void) {
    	int16_t x25 = -13109;
	int8_t x26 = -1;
	volatile int8_t x27 = -1;
	static int64_t x28 = INT64_MIN;

    t6 = (x25|(x26-(x27%x28)));

    if (t6 != -13109LL) { NG(); } else { ; }
	
}

void f7(void) {
    	volatile uint8_t x30 = UINT8_MAX;
	uint64_t t7 = 373599123LLU;

    t7 = (x29|(x30-(x31%x32)));

    if (t7 != 18446744073709519102LLU) { NG(); } else { ; }
	
}

void f8(void) {
    	static int16_t x33 = -2586;
	uint64_t x34 = 15199823999575LLU;
	int8_t x35 = INT8_MIN;
	static volatile int32_t x36 = -66649;
	static volatile uint64_t t8 = 1895332012496628LLU;

    t8 = (x33|(x34-(x35%x36)));

    if (t8 != 18446744073709549559LLU) { NG(); } else { ; }
	
}

void f9(void) {
    	int64_t x37 = 42878467LL;
	static int16_t x38 = INT16_MAX;
	uint32_t x40 = 5U;
	int64_t t9 = -29181078597LL;

    t9 = (x37|(x38-(x39%x40)));

    if (t9 != 42893311LL) { NG(); } else { ; }
	
}

void f10(void) {
    	volatile int64_t x41 = -125531538471855LL;
	int32_t x42 = INT32_MIN;
	volatile uint64_t x44 = UINT64_MAX;
	uint64_t t10 = 659600975LLU;

    t10 = (x41|(x42-(x43%x44)));

    if (t10 != 18446744071562067925LLU) { NG(); } else { ; }
	
}

void f11(void) {
    	volatile int8_t x45 = INT8_MIN;
	static uint64_t x47 = 7LLU;
	volatile uint64_t x48 = UINT64_MAX;
	uint64_t t11 = 115366LLU;

    t11 = (x45|(x46-(x47%x48)));

    if (t11 != 18446744073709551608LLU) { NG(); } else { ; }
	
}

void f12(void) {
    	static int16_t x50 = INT16_MAX;
	static int16_t x51 = INT16_MIN;
	int64_t t12 = -97477006566381LL;

    t12 = (x49|(x50-(x51%x52)));

    if (t12 != -9223372036854710273LL) { NG(); } else { ; }
	
}

void f13(void) {
    	static uint32_t x53 = UINT32_MAX;
	uint64_t x54 = 45479612343804LLU;
	static int32_t x55 = INT32_MIN;
	static uint16_t x56 = 45U;
	volatile uint64_t t13 = 16LLU;

    t13 = (x53|(x54-(x55%x56)));

    if (t13 != 45483703664639LLU) { NG(); } else { ; }
	
}

void f14(void) {
    	int16_t x57 = INT16_MIN;
	int16_t x58 = -1;
	int64_t x60 = -237857368LL;
	static int64_t t14 = -910951459LL;

    t14 = (x57|(x58-(x59%x60)));

    if (t14 != -30LL) { NG(); } else { ; }
	
}

void f15(void) {
    	uint32_t x61 = UINT32_MAX;
	int16_t x62 = -712;
	int32_t x64 = INT32_MAX;

    t15 = (x61|(x62-(x63%x64)));

    if (t15 != UINT32_MAX) { NG(); } else { ; }
	
}

void f16(void) {
    	int8_t x65 = -1;
	uint8_t x66 = 62U;
	static int32_t x68 = -58153964;
	volatile uint64_t t16 = UINT64_MAX;

    t16 = (x65|(x66-(x67%x68)));

    if (t16 != UINT64_MAX) { NG(); } else { ; }
	
}

void f17(void) {
    	int32_t x69 = INT32_MIN;
	volatile int32_t x70 = INT32_MAX;
	static int64_t x71 = 972175479066LL;
	volatile int16_t x72 = INT16_MIN;
	int64_t t17 = -5807LL;

    t17 = (x69|(x70-(x71%x72)));

    if (t17 != -4379LL) { NG(); } else { ; }
	
}

void f18(void) {
    	static int64_t x74 = -2836450286556967654LL;
	int8_t x75 = -2;
	int32_t x76 = -37;
	volatile int64_t t18 = 3LL;

    t18 = (x73|(x74-(x75%x76)));

    if (t18 != -29412LL) { NG(); } else { ; }
	
}

void f19(void) {
    	int64_t x77 = -1LL;
	static int64_t x78 = -1LL;
	uint16_t x79 = 426U;
	static volatile int64_t t19 = -133486LL;

    t19 = (x77|(x78-(x79%x80)));

    if (t19 != -1LL) { NG(); } else { ; }
	
}

void f20(void) {
    	int8_t x82 = -8;
	uint64_t x83 = UINT64_MAX;
	uint16_t x84 = 643U;
	uint64_t t20 = 0LLU;

    t20 = (x81|(x82-(x83%x84)));

    if (t20 != 18446744073709551569LLU) { NG(); } else { ; }
	
}

void f21(void) {
    	static uint32_t x85 = UINT32_MAX;
	uint16_t x86 = 924U;
	uint64_t x87 = 312903322206731672LLU;
	volatile uint8_t x88 = 1U;
	uint64_t t21 = 14711286LLU;

    t21 = (x85|(x86-(x87%x88)));

    if (t21 != 4294967295LLU) { NG(); } else { ; }
	
}

void f22(void) {
    	uint8_t x89 = 6U;
	volatile uint64_t x90 = 126LLU;
	uint16_t x91 = 19U;
	uint64_t x92 = 18645613836482394LLU;
	uint64_t t22 = 8948622951264476546LLU;

    t22 = (x89|(x90-(x91%x92)));

    if (t22 != 111LLU) { NG(); } else { ; }
	
}

void f23(void) {
    	static uint8_t x95 = 82U;
	uint16_t x96 = 1U;

    t23 = (x93|(x94-(x95%x96)));

    if (t23 != -15) { NG(); } else { ; }
	
}

void f24(void) {
    	int64_t x97 = -26092320818LL;
	static int64_t x99 = 7574LL;
	int16_t x100 = INT16_MIN;
	static int64_t t24 = -312486LL;

    t24 = (x97|(x98-(x99%x100)));

    if (t24 != -6178LL) { NG(); } else { ; }
	
}

void f25(void) {
    	static int32_t x102 = INT32_MIN;
	volatile uint64_t x103 = UINT64_MAX;
	uint8_t x104 = UINT8_MAX;
	uint64_t t25 = 32593939043878311LLU;

    t25 = (x101|(x102-(x103%x104)));

    if (t25 != 18446744071562067968LLU) { NG(); } else { ; }
	
}

void f26(void) {
    	int8_t x105 = -1;
	volatile int8_t x106 = -17;
	static volatile int16_t x107 = INT16_MIN;
	static int8_t x108 = INT8_MIN;

    t26 = (x105|(x106-(x107%x108)));

    if (t26 != -1) { NG(); } else { ; }
	
}

void f27(void) {
    	int8_t x109 = -1;
	int8_t x110 = INT8_MIN;
	static int32_t x111 = 11;
	volatile int8_t x112 = INT8_MIN;
	static volatile int32_t t27 = 1;

    t27 = (x109|(x110-(x111%x112)));

    if (t27 != -1) { NG(); } else { ; }
	
}

void f28(void) {
    	uint8_t x113 = 19U;
	uint32_t t28 = 1U;

    t28 = (x113|(x114-(x115%x116)));

    if (t28 != 824343U) { NG(); } else { ; }
	
}

void f29(void) {
    	int16_t x117 = INT16_MIN;
	uint8_t x118 = UINT8_MAX;
	int32_t x120 = -1;
	volatile int64_t t29 = -222820568LL;

    t29 = (x117|(x118-(x119%x120)));

    if (t29 != -32513LL) { NG(); } else { ; }
	
}

void f30(void) {
    	int64_t x121 = -1LL;
	int16_t x122 = -6394;
	int16_t x124 = INT16_MAX;
	volatile int64_t t30 = 1LL;

    t30 = (x121|(x122-(x123%x124)));

    if (t30 != -1LL) { NG(); } else { ; }
	
}

void f31(void) {
    	uint8_t x125 = 1U;
	int8_t x126 = 25;
	static int16_t x127 = -9829;
	int8_t x128 = -1;
	static int32_t t31 = -369261829;

    t31 = (x125|(x126-(x127%x128)));

    if (t31 != 25) { NG(); } else { ; }
	
}

void f32(void) {
    	int16_t x129 = -1;
	volatile int8_t x130 = -3;
	int8_t x131 = -1;
	uint32_t x132 = 6U;
	uint32_t t32 = UINT32_MAX;

    t32 = (x129|(x130-(x131%x132)));

    if (t32 != UINT32_MAX) { NG(); } else { ; }
	
}

void f33(void) {
    	uint16_t x137 = 1U;
	static int32_t x138 = INT32_MAX;
	uint32_t x139 = UINT32_MAX;
	int32_t x140 = INT32_MIN;
	static uint32_t t33 = 21703923U;

    t33 = (x137|(x138-(x139%x140)));

    if (t33 != 1U) { NG(); } else { ; }
	
}

void f34(void) {
    	volatile int8_t x141 = INT8_MIN;
	uint64_t x142 = 191675LLU;
	static int32_t x143 = INT32_MIN;
	uint8_t x144 = UINT8_MAX;
	volatile uint64_t t34 = 11062LLU;

    t34 = (x141|(x142-(x143%x144)));

    if (t34 != 18446744073709551547LLU) { NG(); } else { ; }
	
}

void f35(void) {
    	static int16_t x145 = 2;
	int8_t x146 = -1;
	volatile int32_t x147 = -126198;
	static int64_t x148 = INT64_MAX;
	int64_t t35 = 1803350298929198522LL;

    t35 = (x145|(x146-(x147%x148)));

    if (t35 != 126199LL) { NG(); } else { ; }
	
}

void f36(void) {
    	int8_t x149 = INT8_MIN;
	int16_t x150 = -1;
	int64_t x152 = INT64_MIN;

    t36 = (x149|(x150-(x151%x152)));

    if (t36 != -1LL) { NG(); } else { ; }
	
}

void f37(void) {
    	int32_t x153 = INT32_MIN;
	int16_t x154 = -5;
	static int16_t x156 = INT16_MIN;
	volatile int32_t t37 = -1363;

    t37 = (x153|(x154-(x155%x156)));

    if (t37 != -67) { NG(); } else { ; }
	
}

void f38(void) {
    	uint32_t x157 = 11978976U;
	static uint16_t x158 = 1U;
	int64_t x159 = INT64_MAX;
	static uint64_t x160 = UINT64_MAX;
	uint64_t t38 = 9334LLU;

    t38 = (x157|(x158-(x159%x160)));

    if (t38 != 9223372036866754786LLU) { NG(); } else { ; }
	
}

void f39(void) {
    	int16_t x161 = INT16_MIN;
	volatile int64_t x163 = INT64_MIN;
	uint64_t x164 = UINT64_MAX;
	volatile uint64_t t39 = 12558913298LLU;

    t39 = (x161|(x162-(x163%x164)));

    if (t39 != 18446744073709528467LLU) { NG(); } else { ; }
	
}

void f40(void) {
    	static volatile int64_t x165 = INT64_MIN;
	int64_t x166 = -1521222459584LL;
	static int8_t x168 = INT8_MIN;

    t40 = (x165|(x166-(x167%x168)));

    if (t40 != -1521222459583LL) { NG(); } else { ; }
	
}

void f41(void) {
    	static int8_t x172 = 12;
	volatile int32_t t41 = -1;

    t41 = (x169|(x170-(x171%x172)));

    if (t41 != -1) { NG(); } else { ; }
	
}

void f42(void) {
    	static int8_t x174 = INT8_MIN;
	int16_t x176 = INT16_MIN;
	uint64_t t42 = UINT64_MAX;

    t42 = (x173|(x174-(x175%x176)));

    if (t42 != UINT64_MAX) { NG(); } else { ; }
	
}

void f43(void) {
    	volatile int16_t x177 = -5;
	uint32_t x178 = UINT32_MAX;
	int64_t x179 = -1LL;
	static int32_t x180 = INT32_MIN;
	volatile int64_t t43 = -1462451339LL;

    t43 = (x177|(x178-(x179%x180)));

    if (t43 != -5LL) { NG(); } else { ; }
	
}

void f44(void) {
    	volatile int16_t x185 = INT16_MIN;
	uint8_t x186 = UINT8_MAX;
	uint16_t x187 = 828U;
	uint8_t x188 = 106U;
	volatile int32_t t44 = 3876807;

    t44 = (x185|(x186-(x187%x188)));

    if (t44 != -32599) { NG(); } else { ; }
	
}

void f45(void) {
    	uint64_t x193 = UINT64_MAX;
	uint32_t x194 = 6U;
	int64_t x195 = -1LL;
	int16_t x196 = INT16_MIN;
	static uint64_t t45 = UINT64_MAX;

    t45 = (x193|(x194-(x195%x196)));

    if (t45 != UINT64_MAX) { NG(); } else { ; }
	
}

void f46(void) {
    	int32_t x197 = 31;
	uint32_t x199 = 6U;
	int32_t x200 = -1;
	volatile uint32_t t46 = UINT32_MAX;

    t46 = (x197|(x198-(x199%x200)));

    if (t46 != UINT32_MAX) { NG(); } else { ; }
	
}

void f47(void) {
    	int16_t x202 = INT16_MIN;
	int64_t x203 = INT64_MIN;
	volatile int64_t t47 = 1LL;

    t47 = (x201|(x202-(x203%x204)));

    if (t47 != -1LL) { NG(); } else { ; }
	
}

void f48(void) {
    	int8_t x205 = INT8_MAX;
	int64_t x206 = -1LL;
	int64_t x207 = 947211LL;
	static uint8_t x208 = UINT8_MAX;

    t48 = (x205|(x206-(x207%x208)));

    if (t48 != -129LL) { NG(); } else { ; }
	
}

void f49(void) {
    	int64_t x209 = INT64_MIN;
	static int16_t x210 = INT16_MAX;
	int64_t x211 = INT64_MIN;
	static int8_t x212 = -1;
	int64_t t49 = 127434532628LL;

    t49 = (x209|(x210-(x211%x212)));

    if (t49 != -9223372036854743041LL) { NG(); } else { ; }
	
}

void f50(void) {
    	int64_t x213 = -15458213LL;
	volatile uint64_t x215 = 17947LLU;
	int16_t x216 = -1;
	uint64_t t50 = 5112522419586377628LLU;

    t50 = (x213|(x214-(x215%x216)));

    if (t50 != 18446744073709533695LLU) { NG(); } else { ; }
	
}

void f51(void) {
    	volatile uint8_t x217 = 1U;
	int8_t x218 = INT8_MAX;
	static int8_t x219 = -1;
	volatile int64_t x220 = INT64_MIN;
	int64_t t51 = 31869679729598LL;

    t51 = (x217|(x218-(x219%x220)));

    if (t51 != 129LL) { NG(); } else { ; }
	
}

void f52(void) {
    	int32_t x221 = -1;
	static uint32_t x222 = 628U;
	uint64_t x223 = UINT64_MAX;
	static int16_t x224 = -1;
	volatile uint64_t t52 = UINT64_MAX;

    t52 = (x221|(x222-(x223%x224)));

    if (t52 != UINT64_MAX) { NG(); } else { ; }
	
}

void f53(void) {
    	static int32_t x225 = -125;
	static int16_t x226 = -1;
	uint64_t x227 = UINT64_MAX;
	static int8_t x228 = INT8_MAX;

    t53 = (x225|(x226-(x227%x228)));

    if (t53 != UINT64_MAX) { NG(); } else { ; }
	
}

void f54(void) {
    	int8_t x233 = INT8_MIN;
	volatile int32_t x234 = INT32_MIN;
	int64_t x235 = INT64_MIN;

    t54 = (x233|(x234-(x235%x236)));

    if (t54 != -120LL) { NG(); } else { ; }
	
}

void f55(void) {
    	int8_t x237 = INT8_MIN;
	int16_t x238 = 5334;
	int64_t x239 = 27616263394LL;
	static volatile int64_t t55 = 13225861350LL;

    t55 = (x237|(x238-(x239%x240)));

    if (t55 != -24LL) { NG(); } else { ; }
	
}

void f56(void) {
    	uint32_t x241 = UINT32_MAX;
	static volatile int8_t x243 = INT8_MIN;
	int8_t x244 = -2;
	static volatile uint32_t t56 = UINT32_MAX;

    t56 = (x241|(x242-(x243%x244)));

    if (t56 != UINT32_MAX) { NG(); } else { ; }
	
}

void f57(void) {
    	static uint64_t x245 = 26549030LLU;
	uint16_t x246 = 4U;
	uint64_t t57 = 17152LLU;

    t57 = (x245|(x246-(x247%x248)));

    if (t57 != 26549031LLU) { NG(); } else { ; }
	
}

void f58(void) {
    	int64_t x250 = 398594371205072LL;
	volatile int16_t x251 = INT16_MIN;
	int64_t x252 = -44368996449017620LL;
	static int64_t t58 = 154060381682LL;

    t58 = (x249|(x250-(x251%x252)));

    if (t58 != 398594371403762LL) { NG(); } else { ; }
	
}

void f59(void) {
    	int32_t x253 = -837187256;
	volatile uint8_t x254 = 1U;
	int64_t x255 = -1LL;
	static int32_t x256 = INT32_MIN;
	volatile int64_t t59 = -57LL;

    t59 = (x253|(x254-(x255%x256)));

    if (t59 != -837187254LL) { NG(); } else { ; }
	
}

void f60(void) {
    	int64_t x257 = 1994943234262253889LL;
	volatile int16_t x258 = INT16_MAX;
	int8_t x259 = INT8_MIN;
	static int32_t x260 = -1;
	volatile int64_t t60 = 10487102046680272LL;

    t60 = (x257|(x258-(x259%x260)));

    if (t60 != 1994943234262269951LL) { NG(); } else { ; }
	
}

void f61(void) {
    	int16_t x261 = INT16_MIN;
	static int16_t x262 = INT16_MAX;
	volatile int64_t x263 = INT64_MIN;
	uint32_t x264 = 63269U;
	volatile int64_t t61 = -6LL;

    t61 = (x261|(x262-(x263%x264)));

    if (t61 != -30346LL) { NG(); } else { ; }
	
}

void f62(void) {
    	volatile int8_t x265 = 6;
	int32_t x266 = INT32_MIN;
	static volatile int32_t x267 = -160453;
	static uint16_t x268 = 64U;
	static volatile int32_t t62 = 4883;

    t62 = (x265|(x266-(x267%x268)));

    if (t62 != -2147483641) { NG(); } else { ; }
	
}

void f63(void) {
    	volatile int32_t x270 = INT32_MIN;
	int32_t x271 = INT32_MIN;
	int64_t x272 = INT64_MIN;
	static int64_t t63 = 684818162873620LL;

    t63 = (x269|(x270-(x271%x272)));

    if (t63 != 32767LL) { NG(); } else { ; }
	
}

void f64(void) {
    	static volatile int64_t x273 = -1LL;
	static int64_t x274 = -232LL;
	static uint8_t x275 = 19U;
	uint8_t x276 = 7U;
	int64_t t64 = 791733LL;

    t64 = (x273|(x274-(x275%x276)));

    if (t64 != -1LL) { NG(); } else { ; }
	
}

void f65(void) {
    	int32_t x278 = -1;
	int32_t x279 = -325;
	int32_t t65 = -2263;

    t65 = (x277|(x278-(x279%x280)));

    if (t65 != -1) { NG(); } else { ; }
	
}

void f66(void) {
    	int8_t x281 = -3;
	int64_t x282 = 10945LL;
	uint8_t x284 = UINT8_MAX;
	int64_t t66 = 131121730655988LL;

    t66 = (x281|(x282-(x283%x284)));

    if (t66 != -1LL) { NG(); } else { ; }
	
}

void f67(void) {
    	int64_t x289 = -1748867076933LL;
	int64_t x290 = INT64_MIN;
	uint32_t x291 = 494152U;
	int16_t x292 = 2;
	volatile int64_t t67 = 1139120011LL;

    t67 = (x289|(x290-(x291%x292)));

    if (t67 != -1748867076933LL) { NG(); } else { ; }
	
}

void f68(void) {
    	uint64_t x293 = UINT64_MAX;
	volatile uint16_t x294 = UINT16_MAX;
	volatile int32_t x295 = INT32_MIN;
	uint64_t x296 = 1076810727319LLU;
	volatile uint64_t t68 = UINT64_MAX;

    t68 = (x293|(x294-(x295%x296)));

    if (t68 != UINT64_MAX) { NG(); } else { ; }
	
}

void f69(void) {
    	uint64_t x299 = 603437693911144LLU;
	static int16_t x300 = -1;
	static volatile uint64_t t69 = 1299LLU;

    t69 = (x297|(x298-(x299%x300)));

    if (t69 != 18446744072478277528LLU) { NG(); } else { ; }
	
}

void f70(void) {
    	static volatile int16_t x301 = INT16_MIN;
	int8_t x302 = INT8_MIN;
	volatile uint32_t x303 = 20U;
	static uint32_t t70 = 1U;

    t70 = (x301|(x302-(x303%x304)));

    if (t70 != 4294967148U) { NG(); } else { ; }
	
}

void f71(void) {
    	int64_t x305 = 1059491504862LL;
	volatile uint32_t x306 = UINT32_MAX;
	static uint8_t x307 = 3U;
	int32_t x308 = -1;
	volatile int64_t t71 = 14LL;

    t71 = (x305|(x306-(x307%x308)));

    if (t71 != 1060856922111LL) { NG(); } else { ; }
	
}

void f72(void) {
    	int64_t x309 = INT64_MAX;
	uint64_t x310 = UINT64_MAX;
	uint8_t x311 = UINT8_MAX;
	uint32_t x312 = 2206U;
	static uint64_t t72 = UINT64_MAX;

    t72 = (x309|(x310-(x311%x312)));

    if (t72 != UINT64_MAX) { NG(); } else { ; }
	
}

void f73(void) {
    	int32_t x313 = INT32_MIN;
	int64_t x314 = -908001959893LL;
	int8_t x315 = -1;
	uint8_t x316 = 10U;
	volatile int64_t t73 = -42LL;

    t73 = (x313|(x314-(x315%x316)));

    if (t73 != -1763860436LL) { NG(); } else { ; }
	
}

void f74(void) {
    	int8_t x317 = -1;
	static int32_t x318 = INT32_MAX;
	uint32_t x320 = 10443U;
	volatile uint32_t t74 = UINT32_MAX;

    t74 = (x317|(x318-(x319%x320)));

    if (t74 != UINT32_MAX) { NG(); } else { ; }
	
}

void f75(void) {
    	int32_t x321 = -14363158;
	static int32_t x322 = INT32_MIN;
	volatile int8_t x323 = -1;

    t75 = (x321|(x322-(x323%x324)));

    if (t75 != -14363157LL) { NG(); } else { ; }
	
}

void f76(void) {
    	int32_t x325 = INT32_MIN;
	static int32_t x326 = INT32_MAX;
	int8_t x327 = INT8_MIN;
	int64_t x328 = INT64_MIN;
	volatile int64_t t76 = 278593114710LL;

    t76 = (x325|(x326-(x327%x328)));

    if (t76 != -2147483521LL) { NG(); } else { ; }
	
}

void f77(void) {
    	static int64_t x329 = 3512LL;
	int16_t x330 = INT16_MIN;
	static uint8_t x331 = UINT8_MAX;
	int16_t x332 = INT16_MIN;
	volatile int64_t t77 = -1707410262237033LL;

    t77 = (x329|(x330-(x331%x332)));

    if (t77 != -32839LL) { NG(); } else { ; }
	
}

void f78(void) {
    	static uint32_t x334 = 1209474059U;
	int64_t x335 = INT64_MIN;
	volatile int64_t t78 = 776929598966608734LL;

    t78 = (x333|(x334-(x335%x336)));

    if (t78 != -5LL) { NG(); } else { ; }
	
}

void f79(void) {
    	int64_t x337 = -1LL;
	int16_t x338 = 3241;
	volatile int64_t x339 = INT64_MIN;
	uint16_t x340 = 489U;
	volatile int64_t t79 = 3018053904620LL;

    t79 = (x337|(x338-(x339%x340)));

    if (t79 != -1LL) { NG(); } else { ; }
	
}

void f80(void) {
    	volatile int32_t x341 = INT32_MAX;
	volatile int16_t x343 = -1;
	static volatile int8_t x344 = INT8_MIN;
	volatile int32_t t80 = -91;

    t80 = (x341|(x342-(x343%x344)));

    if (t80 != -1) { NG(); } else { ; }
	
}

void f81(void) {
    	volatile uint64_t x345 = 61420LLU;
	int64_t x346 = INT64_MAX;
	static int8_t x348 = INT8_MAX;
	uint64_t t81 = 39614530556457LLU;

    t81 = (x345|(x346-(x347%x348)));

    if (t81 != 9223372036854775804LLU) { NG(); } else { ; }
	
}

void f82(void) {
    	uint32_t x349 = 97922U;
	uint8_t x351 = 16U;
	uint8_t x352 = 1U;
	uint32_t t82 = 3590156U;

    t82 = (x349|(x350-(x351%x352)));

    if (t82 != 98287U) { NG(); } else { ; }
	
}

void f83(void) {
    	int32_t x353 = 1;
	uint16_t x354 = UINT16_MAX;
	volatile int16_t x356 = -1;
	int64_t t83 = 1983812LL;

    t83 = (x353|(x354-(x355%x356)));

    if (t83 != 65535LL) { NG(); } else { ; }
	
}

void f84(void) {
    	int8_t x357 = INT8_MIN;
	int16_t x358 = INT16_MIN;
	uint16_t x359 = UINT16_MAX;
	volatile uint32_t x360 = UINT32_MAX;
	uint32_t t84 = 1967U;

    t84 = (x357|(x358-(x359%x360)));

    if (t84 != 4294967169U) { NG(); } else { ; }
	
}

void f85(void) {
    	int64_t x361 = INT64_MIN;
	volatile uint64_t x362 = 5734349893383LLU;
	int16_t x363 = INT16_MIN;
	int16_t x364 = INT16_MAX;
	volatile uint64_t t85 = 217533661990134653LLU;

    t85 = (x361|(x362-(x363%x364)));

    if (t85 != 9223377771204669192LLU) { NG(); } else { ; }
	
}

void f86(void) {
    	int16_t x365 = 832;
	uint64_t x366 = UINT64_MAX;
	volatile uint64_t t86 = 11LLU;

    t86 = (x365|(x366-(x367%x368)));

    if (t86 != 839LLU) { NG(); } else { ; }
	
}

void f87(void) {
    	uint64_t x369 = 100740879298LLU;
	volatile int32_t x370 = -1;
	static volatile int64_t x371 = INT64_MIN;
	int32_t x372 = INT32_MIN;

    t87 = (x369|(x370-(x371%x372)));

    if (t87 != UINT64_MAX) { NG(); } else { ; }
	
}

void f88(void) {
    	static volatile int64_t x373 = -1LL;
	int64_t x374 = 3425LL;
	int64_t x375 = -1LL;
	int64_t x376 = INT64_MAX;
	int64_t t88 = -726873LL;

    t88 = (x373|(x374-(x375%x376)));

    if (t88 != -1LL) { NG(); } else { ; }
	
}

void f89(void) {
    	uint32_t x377 = 169284U;
	static uint32_t x378 = 212U;
	uint32_t x379 = 36017958U;
	int32_t x380 = -4;
	uint32_t t89 = 16759U;

    t89 = (x377|(x378-(x379%x380)));

    if (t89 != 4258987502U) { NG(); } else { ; }
	
}

void f90(void) {
    	int64_t x382 = -1LL;
	int8_t x383 = INT8_MIN;
	uint32_t x384 = 484U;
	volatile int64_t t90 = 249LL;

    t90 = (x381|(x382-(x383%x384)));

    if (t90 != -1LL) { NG(); } else { ; }
	
}

void f91(void) {
    	int16_t x385 = INT16_MIN;
	int8_t x387 = INT8_MAX;
	volatile uint64_t x388 = UINT64_MAX;
	static uint64_t t91 = 2480791611956343480LLU;

    t91 = (x385|(x386-(x387%x388)));

    if (t91 != 18446744073709551488LLU) { NG(); } else { ; }
	
}

void f92(void) {
    	int8_t x389 = INT8_MAX;
	static int32_t x390 = INT32_MIN;
	volatile int16_t x391 = -1;
	int64_t x392 = INT64_MIN;

    t92 = (x389|(x390-(x391%x392)));

    if (t92 != -2147483521LL) { NG(); } else { ; }
	
}

void f93(void) {
    	int8_t x393 = -1;
	static int32_t x394 = -1;
	volatile int8_t x395 = INT8_MIN;
	int64_t x396 = -1LL;
	volatile int64_t t93 = -225303594310257271LL;

    t93 = (x393|(x394-(x395%x396)));

    if (t93 != -1LL) { NG(); } else { ; }
	
}

void f94(void) {
    	static int16_t x397 = INT16_MIN;
	static int32_t x398 = INT32_MIN;
	static int32_t x399 = INT32_MIN;
	static int8_t x400 = INT8_MIN;
	int32_t t94 = -3737270;

    t94 = (x397|(x398-(x399%x400)));

    if (t94 != -32768) { NG(); } else { ; }
	
}

void f95(void) {
    	volatile uint8_t x401 = UINT8_MAX;
	static int16_t x402 = -691;
	int8_t x403 = INT8_MIN;
	volatile int64_t t95 = 13877LL;

    t95 = (x401|(x402-(x403%x404)));

    if (t95 != -513LL) { NG(); } else { ; }
	
}

void f96(void) {
    	volatile int32_t x405 = INT32_MIN;
	static uint16_t x406 = 0U;
	int64_t x407 = INT64_MAX;
	static volatile uint64_t x408 = 12LLU;
	uint64_t t96 = 127950883850LLU;

    t96 = (x405|(x406-(x407%x408)));

    if (t96 != 18446744073709551609LLU) { NG(); } else { ; }
	
}

void f97(void) {
    	uint8_t x409 = 2U;
	volatile uint8_t x411 = 30U;

    t97 = (x409|(x410-(x411%x412)));

    if (t97 != -25) { NG(); } else { ; }
	
}

void f98(void) {
    	static int16_t x413 = 0;
	int64_t x414 = -261881584364646LL;
	volatile int32_t x415 = INT32_MIN;
	volatile int64_t t98 = -7573609991172LL;

    t98 = (x413|(x414-(x415%x416)));

    if (t98 != -261879436880998LL) { NG(); } else { ; }
	
}

void f99(void) {
    	int8_t x417 = INT8_MIN;
	int64_t x420 = -16269225691062LL;
	volatile uint64_t t99 = 60593116582LLU;

    t99 = (x417|(x418-(x419%x420)));

    if (t99 != 18446744073709551562LLU) { NG(); } else { ; }
	
}

void f100(void) {
    	static int64_t x423 = -1LL;

    t100 = (x421|(x422-(x423%x424)));

    if (t100 != -2147483648LL) { NG(); } else { ; }
	
}

void f101(void) {
    	volatile int16_t x429 = -7;
	uint32_t x430 = UINT32_MAX;
	static int64_t x432 = -1040822239725325920LL;
	int64_t t101 = 2717043972941769LL;

    t101 = (x429|(x430-(x431%x432)));

    if (t101 != -1LL) { NG(); } else { ; }
	
}

void f102(void) {
    	uint64_t x433 = 1LLU;
	uint8_t x434 = UINT8_MAX;
	int32_t x435 = INT32_MIN;
	int32_t x436 = INT32_MIN;
	static uint64_t t102 = 502419691372307LLU;

    t102 = (x433|(x434-(x435%x436)));

    if (t102 != 255LLU) { NG(); } else { ; }
	
}

void f103(void) {
    	volatile uint8_t x437 = UINT8_MAX;
	static int16_t x438 = -1;
	volatile uint32_t x440 = UINT32_MAX;
	volatile uint32_t t103 = 417U;

    t103 = (x437|(x438-(x439%x440)));

    if (t103 != 2147483903U) { NG(); } else { ; }
	
}

void f104(void) {
    	int32_t x441 = INT32_MIN;
	uint32_t x443 = 2451129U;

    t104 = (x441|(x442-(x443%x444)));

    if (t104 != 4292516362U) { NG(); } else { ; }
	
}

void f105(void) {
    	int8_t x445 = INT8_MIN;
	uint8_t x447 = 1U;
	static uint16_t x448 = 6U;
	int32_t t105 = -620;

    t105 = (x445|(x446-(x447%x448)));

    if (t105 != -126) { NG(); } else { ; }
	
}

void f106(void) {
    	uint64_t x449 = UINT64_MAX;
	int16_t x450 = INT16_MAX;
	uint32_t x452 = 2003U;
	volatile uint64_t t106 = UINT64_MAX;

    t106 = (x449|(x450-(x451%x452)));

    if (t106 != UINT64_MAX) { NG(); } else { ; }
	
}

void f107(void) {
    	int16_t x453 = INT16_MIN;
	uint64_t x455 = 110016745LLU;
	static volatile int8_t x456 = INT8_MAX;
	volatile uint64_t t107 = 563663243027555LLU;

    t107 = (x453|(x454-(x455%x456)));

    if (t107 != 18446744073709522409LLU) { NG(); } else { ; }
	
}

void f108(void) {
    	static int16_t x457 = INT16_MIN;
	uint8_t x459 = 48U;
	uint8_t x460 = UINT8_MAX;
	static volatile int32_t t108 = 1;

    t108 = (x457|(x458-(x459%x460)));

    if (t108 != -48) { NG(); } else { ; }
	
}

void f109(void) {
    	volatile uint8_t x461 = 0U;
	volatile uint32_t x463 = 113520U;
	int64_t x464 = -73587482866LL;
	volatile int64_t t109 = 8398450147LL;

    t109 = (x461|(x462-(x463%x464)));

    if (t109 != -113521LL) { NG(); } else { ; }
	
}

void f110(void) {
    	int32_t x467 = -3867;
	int32_t x468 = -536295735;

    t110 = (x465|(x466-(x467%x468)));

    if (t110 != -33) { NG(); } else { ; }
	
}

void f111(void) {
    	static volatile int8_t x469 = -1;
	int64_t x470 = -1297728271LL;
	int64_t x471 = 0LL;
	volatile uint64_t x472 = 801LLU;
	volatile uint64_t t111 = UINT64_MAX;

    t111 = (x469|(x470-(x471%x472)));

    if (t111 != UINT64_MAX) { NG(); } else { ; }
	
}

void f112(void) {
    	static int32_t x473 = -1;
	static int32_t x474 = -1;
	uint32_t x475 = 14855U;
	volatile uint32_t x476 = 365689828U;
	volatile uint32_t t112 = UINT32_MAX;

    t112 = (x473|(x474-(x475%x476)));

    if (t112 != UINT32_MAX) { NG(); } else { ; }
	
}

void f113(void) {
    	volatile uint32_t x482 = 235771U;
	volatile uint64_t x484 = 684878222439593LLU;
	volatile uint64_t t113 = 97968644142245568LLU;

    t113 = (x481|(x482-(x483%x484)));

    if (t113 != 18446510043188233634LLU) { NG(); } else { ; }
	
}

void f114(void) {
    	uint32_t x486 = 16042359U;
	uint64_t x487 = 252816448LLU;
	uint8_t x488 = UINT8_MAX;
	volatile uint64_t t114 = 116846487LLU;

    t114 = (x485|(x486-(x487%x488)));

    if (t114 != 16047466LLU) { NG(); } else { ; }
	
}

void f115(void) {
    	uint16_t x490 = UINT16_MAX;
	int8_t x491 = -1;
	int32_t t115 = -259069450;

    t115 = (x489|(x490-(x491%x492)));

    if (t115 != 65663) { NG(); } else { ; }
	
}

void f116(void) {
    	uint16_t x493 = 20986U;
	uint64_t x494 = UINT64_MAX;
	static int32_t x495 = 252;
	static uint64_t t116 = 3894LLU;

    t116 = (x493|(x494-(x495%x496)));

    if (t116 != 18446744073709551611LLU) { NG(); } else { ; }
	
}

void f117(void) {
    	static uint16_t x498 = 4820U;
	uint32_t x499 = 0U;
	volatile uint64_t t117 = 1073054718LLU;

    t117 = (x497|(x498-(x499%x500)));

    if (t117 != 4863LLU) { NG(); } else { ; }
	
}

void f118(void) {
    	uint8_t x501 = 1U;
	volatile uint32_t x502 = UINT32_MAX;
	int8_t x503 = 2;
	int8_t x504 = INT8_MIN;
	uint32_t t118 = 75U;

    t118 = (x501|(x502-(x503%x504)));

    if (t118 != 4294967293U) { NG(); } else { ; }
	
}

void f119(void) {
    	static volatile uint8_t x505 = 1U;
	int64_t x506 = -1LL;
	int16_t x507 = INT16_MAX;
	uint32_t x508 = UINT32_MAX;
	int64_t t119 = 14008068721556LL;

    t119 = (x505|(x506-(x507%x508)));

    if (t119 != -32767LL) { NG(); } else { ; }
	
}

void f120(void) {
    	uint32_t x511 = UINT32_MAX;
	volatile uint64_t t120 = 199253005LLU;

    t120 = (x509|(x510-(x511%x512)));

    if (t120 != 1534066431LLU) { NG(); } else { ; }
	
}

void f121(void) {
    	int32_t x513 = -1;
	uint64_t x514 = UINT64_MAX;
	uint16_t x516 = 68U;
	uint64_t t121 = UINT64_MAX;

    t121 = (x513|(x514-(x515%x516)));

    if (t121 != UINT64_MAX) { NG(); } else { ; }
	
}

void f122(void) {
    	uint16_t x517 = UINT16_MAX;
	volatile int16_t x518 = INT16_MIN;
	static int64_t x519 = -1LL;
	uint64_t x520 = 3915316238166LLU;
	uint64_t t122 = 4721539LLU;

    t122 = (x517|(x518-(x519%x520)));

    if (t122 != 18446742299298693119LLU) { NG(); } else { ; }
	
}

void f123(void) {
    	volatile int64_t x521 = -1LL;
	static int8_t x522 = INT8_MIN;
	uint64_t x523 = UINT64_MAX;
	uint8_t x524 = UINT8_MAX;
	uint64_t t123 = UINT64_MAX;

    t123 = (x521|(x522-(x523%x524)));

    if (t123 != UINT64_MAX) { NG(); } else { ; }
	
}

void f124(void) {
    	int8_t x525 = 52;
	uint16_t x526 = 5U;
	static int8_t x527 = INT8_MIN;
	int64_t x528 = -1LL;

    t124 = (x525|(x526-(x527%x528)));

    if (t124 != 53LL) { NG(); } else { ; }
	
}

void f125(void) {
    	uint8_t x530 = 7U;
	uint16_t x531 = 859U;
	volatile int32_t t125 = 567964;

    t125 = (x529|(x530-(x531%x532)));

    if (t125 != -1) { NG(); } else { ; }
	
}

void f126(void) {
    	uint8_t x534 = UINT8_MAX;
	int64_t x535 = INT64_MIN;
	volatile int64_t t126 = -21LL;

    t126 = (x533|(x534-(x535%x536)));

    if (t126 != 981475511144934686LL) { NG(); } else { ; }
	
}

void f127(void) {
    	int16_t x541 = INT16_MAX;
	static uint32_t x542 = 14207411U;
	int8_t x543 = 3;
	uint16_t x544 = UINT16_MAX;
	volatile uint32_t t127 = 4U;

    t127 = (x541|(x542-(x543%x544)));

    if (t127 != 14221311U) { NG(); } else { ; }
	
}

void f128(void) {
    	int16_t x545 = INT16_MAX;
	uint8_t x546 = UINT8_MAX;
	uint64_t x547 = UINT64_MAX;
	volatile uint64_t t128 = 59757310LLU;

    t128 = (x545|(x546-(x547%x548)));

    if (t128 != 18446744071562100735LLU) { NG(); } else { ; }
	
}

void f129(void) {
    	volatile int8_t x549 = INT8_MAX;
	uint8_t x550 = 93U;
	int32_t x551 = -1;
	static volatile int32_t x552 = INT32_MIN;
	int32_t t129 = 30;

    t129 = (x549|(x550-(x551%x552)));

    if (t129 != 127) { NG(); } else { ; }
	
}

void f130(void) {
    	volatile uint32_t x554 = UINT32_MAX;
	volatile int64_t x555 = -1LL;
	volatile uint64_t t130 = 46679426407458LLU;

    t130 = (x553|(x554-(x555%x556)));

    if (t130 != 18304060744889468831LLU) { NG(); } else { ; }
	
}

void f131(void) {
    	int16_t x557 = INT16_MIN;
	int16_t x558 = INT16_MAX;
	int8_t x560 = 44;

    t131 = (x557|(x558-(x559%x560)));

    if (t131 != -4) { NG(); } else { ; }
	
}

void f132(void) {
    	uint8_t x561 = UINT8_MAX;
	int32_t x562 = -1;
	volatile int8_t x563 = 7;
	static volatile int8_t x564 = -1;

    t132 = (x561|(x562-(x563%x564)));

    if (t132 != -1) { NG(); } else { ; }
	
}

void f133(void) {
    	static uint16_t x565 = UINT16_MAX;
	static int16_t x566 = -1;
	int64_t x567 = INT64_MAX;
	int32_t x568 = INT32_MIN;
	volatile int64_t t133 = 3847433178790LL;

    t133 = (x565|(x566-(x567%x568)));

    if (t133 != -2147418113LL) { NG(); } else { ; }
	
}

void f134(void) {
    	static volatile uint16_t x569 = UINT16_MAX;
	int8_t x570 = INT8_MIN;
	int32_t x571 = -4;
	int64_t t134 = 21LL;

    t134 = (x569|(x570-(x571%x572)));

    if (t134 != -1LL) { NG(); } else { ; }
	
}

void f135(void) {
    	int32_t x573 = -1;
	uint64_t x574 = UINT64_MAX;
	uint32_t x575 = 15614U;
	uint64_t x576 = 12116148LLU;
	uint64_t t135 = UINT64_MAX;

    t135 = (x573|(x574-(x575%x576)));

    if (t135 != UINT64_MAX) { NG(); } else { ; }
	
}

void f136(void) {
    	int8_t x577 = INT8_MIN;
	int8_t x578 = -1;
	uint8_t x579 = 7U;
	int8_t x580 = INT8_MIN;

    t136 = (x577|(x578-(x579%x580)));

    if (t136 != -8) { NG(); } else { ; }
	
}

void f137(void) {
    	int32_t x581 = 722576;
	volatile uint16_t x582 = 485U;
	int32_t x583 = -703;
	int64_t x584 = -5009064193LL;

    t137 = (x581|(x582-(x583%x584)));

    if (t137 != 722612LL) { NG(); } else { ; }
	
}

void f138(void) {
    	volatile int32_t x586 = -1;
	int64_t x587 = -1LL;
	int64_t x588 = -52625LL;

    t138 = (x585|(x586-(x587%x588)));

    if (t138 != 127LL) { NG(); } else { ; }
	
}

void f139(void) {
    	int64_t x589 = 902560680070LL;
	uint8_t x591 = UINT8_MAX;
	volatile uint16_t x592 = UINT16_MAX;
	int64_t t139 = -1416546529LL;

    t139 = (x589|(x590-(x591%x592)));

    if (t139 != -305LL) { NG(); } else { ; }
	
}

void f140(void) {
    	uint64_t x594 = 3333625LLU;
	int64_t x595 = INT64_MIN;
	volatile int32_t x596 = INT32_MIN;

    t140 = (x593|(x594-(x595%x596)));

    if (t140 != 3334143LLU) { NG(); } else { ; }
	
}

void f141(void) {
    	int16_t x597 = -1;
	uint8_t x599 = 0U;
	uint32_t x600 = 7U;
	uint32_t t141 = UINT32_MAX;

    t141 = (x597|(x598-(x599%x600)));

    if (t141 != UINT32_MAX) { NG(); } else { ; }
	
}

void f142(void) {
    	static int8_t x601 = INT8_MAX;
	int64_t x602 = INT64_MAX;
	volatile uint64_t x603 = 1082LLU;
	uint64_t t142 = 504LLU;

    t142 = (x601|(x602-(x603%x604)));

    if (t142 != 9223372036854774783LLU) { NG(); } else { ; }
	
}

void f143(void) {
    	volatile int64_t x605 = INT64_MIN;
	uint16_t x606 = UINT16_MAX;
	static int16_t x607 = INT16_MIN;
	int64_t x608 = INT64_MIN;
	int64_t t143 = 2LL;

    t143 = (x605|(x606-(x607%x608)));

    if (t143 != -9223372036854677505LL) { NG(); } else { ; }
	
}

void f144(void) {
    	uint64_t x609 = 247103990LLU;
	int8_t x610 = INT8_MIN;
	int16_t x611 = INT16_MIN;
	int8_t x612 = -7;
	uint64_t t144 = 2077967LLU;

    t144 = (x609|(x610-(x611%x612)));

    if (t144 != 18446744073709551607LLU) { NG(); } else { ; }
	
}

void f145(void) {
    	uint64_t x614 = UINT64_MAX;
	uint16_t x616 = 71U;
	volatile uint64_t t145 = UINT64_MAX;

    t145 = (x613|(x614-(x615%x616)));

    if (t145 != UINT64_MAX) { NG(); } else { ; }
	
}

void f146(void) {
    	int64_t x617 = 15664215489LL;
	static int8_t x618 = INT8_MIN;
	int64_t x619 = -1LL;
	int16_t x620 = -5431;
	int64_t t146 = -946LL;

    t146 = (x617|(x618-(x619%x620)));

    if (t146 != -63LL) { NG(); } else { ; }
	
}

void f147(void) {
    	int8_t x622 = -15;
	int32_t x623 = 47886399;
	volatile int64_t t147 = -62370914LL;

    t147 = (x621|(x622-(x623%x624)));

    if (t147 != -47886413LL) { NG(); } else { ; }
	
}

void f148(void) {
    	uint8_t x625 = 12U;
	volatile uint32_t x628 = 217U;
	uint32_t t148 = 2452879U;

    t148 = (x625|(x626-(x627%x628)));

    if (t148 != 4294967292U) { NG(); } else { ; }
	
}

void f149(void) {
    	int8_t x630 = 1;
	int64_t x631 = 9195799504LL;
	volatile int32_t x632 = INT32_MAX;
	int64_t t149 = 2381702938977706LL;

    t149 = (x629|(x630-(x631%x632)));

    if (t149 != -1LL) { NG(); } else { ; }
	
}

void f150(void) {
    	int64_t x633 = INT64_MIN;
	static int32_t x635 = INT32_MAX;
	int32_t x636 = -121;

    t150 = (x633|(x634-(x635%x636)));

    if (t150 != -9223372032559808602LL) { NG(); } else { ; }
	
}

void f151(void) {
    	uint16_t x637 = UINT16_MAX;
	int32_t x638 = INT32_MAX;
	int64_t x639 = INT64_MAX;
	int8_t x640 = INT8_MAX;
	volatile int64_t t151 = 304LL;

    t151 = (x637|(x638-(x639%x640)));

    if (t151 != 2147483647LL) { NG(); } else { ; }
	
}

void f152(void) {
    	int8_t x641 = -1;
	volatile int32_t x642 = INT32_MIN;
	volatile uint16_t x643 = UINT16_MAX;
	volatile int32_t t152 = 180;

    t152 = (x641|(x642-(x643%x644)));

    if (t152 != -1) { NG(); } else { ; }
	
}

void f153(void) {
    	int8_t x646 = -1;
	static int32_t x647 = INT32_MIN;
	static int32_t x648 = INT32_MIN;
	volatile uint32_t t153 = UINT32_MAX;

    t153 = (x645|(x646-(x647%x648)));

    if (t153 != UINT32_MAX) { NG(); } else { ; }
	
}

void f154(void) {
    	uint16_t x649 = UINT16_MAX;
	uint64_t x651 = 17178474984LLU;
	int16_t x652 = INT16_MIN;
	static volatile uint64_t t154 = 51262771387920LLU;

    t154 = (x649|(x650-(x651%x652)));

    if (t154 != 9223372019676348415LLU) { NG(); } else { ; }
	
}

void f155(void) {
    	int32_t x653 = INT32_MAX;
	static int8_t x654 = -1;
	int8_t x655 = -8;
	static volatile int16_t x656 = INT16_MIN;

    t155 = (x653|(x654-(x655%x656)));

    if (t155 != INT32_MAX) { NG(); } else { ; }
	
}

void f156(void) {
    	int16_t x657 = 1;
	uint64_t x658 = 34LLU;
	uint64_t x659 = 417598557193988LLU;
	int16_t x660 = INT16_MAX;
	volatile uint64_t t156 = 260433317LLU;

    t156 = (x657|(x658-(x659%x660)));

    if (t156 != 18446744073709522571LLU) { NG(); } else { ; }
	
}

void f157(void) {
    	static int16_t x661 = 22;
	int64_t x663 = -17392LL;
	static int32_t x664 = -1;
	volatile uint64_t t157 = 26LLU;

    t157 = (x661|(x662-(x663%x664)));

    if (t157 != 55LLU) { NG(); } else { ; }
	
}

void f158(void) {
    	int8_t x669 = -1;
	int8_t x670 = -1;
	static volatile int8_t x671 = INT8_MIN;
	volatile int32_t t158 = 54;

    t158 = (x669|(x670-(x671%x672)));

    if (t158 != -1) { NG(); } else { ; }
	
}

void f159(void) {
    	int32_t x673 = INT32_MIN;
	int8_t x674 = -14;
	static int32_t x675 = -612777055;
	uint8_t x676 = 79U;
	int32_t t159 = -187807;

    t159 = (x673|(x674-(x675%x676)));

    if (t159 != -2147483616) { NG(); } else { ; }
	
}

void f160(void) {
    	uint64_t x682 = 190LLU;
	uint16_t x683 = UINT16_MAX;
	int16_t x684 = 5633;

    t160 = (x681|(x682-(x683%x684)));

    if (t160 != 18446744073709548267LLU) { NG(); } else { ; }
	
}

void f161(void) {
    	int64_t x685 = INT64_MIN;
	int32_t x686 = INT32_MIN;
	static int32_t x687 = INT32_MIN;
	int64_t t161 = -638177186LL;

    t161 = (x685|(x686-(x687%x688)));

    if (t161 != -2147483648LL) { NG(); } else { ; }
	
}

void f162(void) {
    	static uint16_t x689 = 51U;
	int16_t x690 = 993;
	int64_t x691 = INT64_MIN;
	uint64_t t162 = 12923LLU;

    t162 = (x689|(x690-(x691%x692)));

    if (t162 != 18446744072671365947LLU) { NG(); } else { ; }
	
}

void f163(void) {
    	int64_t x693 = 17196840407622042LL;
	volatile uint16_t x694 = UINT16_MAX;
	int16_t x695 = -485;
	int8_t x696 = INT8_MAX;
	int64_t t163 = 91LL;

    t163 = (x693|(x694-(x695%x696)));

    if (t163 != 17196840407622143LL) { NG(); } else { ; }
	
}

void f164(void) {
    	int64_t x697 = INT64_MIN;
	int16_t x698 = 700;
	int8_t x700 = -1;
	volatile int64_t t164 = 2672230619391LL;

    t164 = (x697|(x698-(x699%x700)));

    if (t164 != -9223372036854775161LL) { NG(); } else { ; }
	
}

void f165(void) {
    	int16_t x701 = INT16_MIN;
	static uint32_t x702 = UINT32_MAX;
	static int16_t x703 = 8;
	volatile int8_t x704 = INT8_MIN;

    t165 = (x701|(x702-(x703%x704)));

    if (t165 != 4294967287U) { NG(); } else { ; }
	
}

void f166(void) {
    	uint64_t x709 = 115104217005LLU;
	volatile uint16_t x711 = 7U;
	volatile uint64_t t166 = 1023103073141898542LLU;

    t166 = (x709|(x710-(x711%x712)));

    if (t166 != 18446744073709551613LLU) { NG(); } else { ; }
	
}

void f167(void) {
    	int64_t x713 = 208LL;
	static uint64_t x714 = 116710814LLU;
	volatile int32_t x715 = INT32_MAX;
	uint64_t x716 = 7081216407496LLU;
	volatile uint64_t t167 = 1357LLU;

    t167 = (x713|(x714-(x715%x716)));

    if (t167 != 18446744071678778847LLU) { NG(); } else { ; }
	
}

void f168(void) {
    	int8_t x717 = INT8_MIN;

    t168 = (x717|(x718-(x719%x720)));

    if (t168 != -128) { NG(); } else { ; }
	
}

void f169(void) {
    	volatile uint16_t x721 = 30145U;
	volatile int64_t x722 = -1LL;
	int64_t x723 = -68096523401LL;
	uint16_t x724 = UINT16_MAX;

    t169 = (x721|(x722-(x723%x724)));

    if (t169 != 30711LL) { NG(); } else { ; }
	
}

void f170(void) {
    	uint32_t x726 = 32190U;
	volatile int32_t x727 = INT32_MAX;
	uint8_t x728 = UINT8_MAX;
	uint32_t t170 = UINT32_MAX;

    t170 = (x725|(x726-(x727%x728)));

    if (t170 != UINT32_MAX) { NG(); } else { ; }
	
}

void f171(void) {
    	static int64_t x729 = INT64_MAX;
	int32_t x730 = INT32_MAX;
	uint16_t x731 = 56U;
	uint16_t x732 = 20634U;
	static int64_t t171 = INT64_MAX;

    t171 = (x729|(x730-(x731%x732)));

    if (t171 != INT64_MAX) { NG(); } else { ; }
	
}

void f172(void) {
    	int16_t x733 = INT16_MAX;
	static int8_t x734 = INT8_MIN;
	static int16_t x735 = 3;
	uint32_t x736 = UINT32_MAX;

    t172 = (x733|(x734-(x735%x736)));

    if (t172 != UINT32_MAX) { NG(); } else { ; }
	
}

void f173(void) {
    	int8_t x737 = -1;
	volatile int32_t x738 = INT32_MIN;
	uint8_t x739 = 0U;
	volatile int32_t t173 = -1081004;

    t173 = (x737|(x738-(x739%x740)));

    if (t173 != -1) { NG(); } else { ; }
	
}

void f174(void) {
    	static volatile uint16_t x741 = UINT16_MAX;
	volatile int32_t x742 = INT32_MIN;
	int32_t x743 = -1;
	uint8_t x744 = UINT8_MAX;

    t174 = (x741|(x742-(x743%x744)));

    if (t174 != -2147418113) { NG(); } else { ; }
	
}

void f175(void) {
    	static int8_t x745 = -1;
	int32_t x746 = INT32_MAX;
	static volatile int64_t x747 = -1LL;

    t175 = (x745|(x746-(x747%x748)));

    if (t175 != UINT64_MAX) { NG(); } else { ; }
	
}

void f176(void) {
    	static volatile uint64_t x749 = 8172949778698836220LLU;
	int8_t x752 = INT8_MAX;
	static uint64_t t176 = 1112206116266535LLU;

    t176 = (x749|(x750-(x751%x752)));

    if (t176 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f177(void) {
    	int16_t x753 = 102;
	int8_t x754 = 12;
	uint32_t x755 = 255306020U;
	volatile int8_t x756 = 14;
	volatile uint32_t t177 = 196397798U;

    t177 = (x753|(x754-(x755%x756)));

    if (t177 != 110U) { NG(); } else { ; }
	
}

void f178(void) {
    	int32_t x759 = INT32_MAX;
	static int8_t x760 = INT8_MIN;
	volatile int64_t t178 = -249484031LL;

    t178 = (x757|(x758-(x759%x760)));

    if (t178 != -1LL) { NG(); } else { ; }
	
}

void f179(void) {
    	int32_t x761 = -1;
	uint16_t x762 = 3890U;
	static volatile int64_t t179 = 693262448187580LL;

    t179 = (x761|(x762-(x763%x764)));

    if (t179 != -1LL) { NG(); } else { ; }
	
}

void f180(void) {
    	int8_t x766 = INT8_MAX;
	static int8_t x767 = -2;

    t180 = (x765|(x766-(x767%x768)));

    if (t180 != 65535) { NG(); } else { ; }
	
}

void f181(void) {
    	int8_t x769 = -54;
	int16_t x771 = -2;
	static int64_t x772 = -159690169397340LL;
	volatile int64_t t181 = -1516495878189806LL;

    t181 = (x769|(x770-(x771%x772)));

    if (t181 != -54LL) { NG(); } else { ; }
	
}

void f182(void) {
    	uint8_t x778 = UINT8_MAX;
	int16_t x780 = INT16_MIN;
	int64_t t182 = 475839LL;

    t182 = (x777|(x778-(x779%x780)));

    if (t182 != -9223372036854775553LL) { NG(); } else { ; }
	
}

void f183(void) {
    	static volatile int8_t x781 = INT8_MIN;
	int16_t x782 = INT16_MAX;
	volatile int64_t x783 = INT64_MAX;
	int8_t x784 = 1;
	volatile int64_t t183 = 0LL;

    t183 = (x781|(x782-(x783%x784)));

    if (t183 != -1LL) { NG(); } else { ; }
	
}

void f184(void) {
    	int8_t x787 = -1;
	volatile uint64_t x788 = UINT64_MAX;
	volatile uint64_t t184 = UINT64_MAX;

    t184 = (x785|(x786-(x787%x788)));

    if (t184 != UINT64_MAX) { NG(); } else { ; }
	
}

void f185(void) {
    	uint64_t x789 = 3414777461909412LLU;
	static int32_t x790 = INT32_MIN;
	int16_t x791 = 0;
	uint16_t x792 = 58U;
	static uint64_t t185 = 918037LLU;

    t185 = (x789|(x790-(x791%x792)));

    if (t185 != 18446744072998266788LLU) { NG(); } else { ; }
	
}

void f186(void) {
    	static int16_t x793 = INT16_MAX;
	static int64_t x795 = INT64_MIN;
	int16_t x796 = INT16_MIN;
	int64_t t186 = 1248LL;

    t186 = (x793|(x794-(x795%x796)));

    if (t186 != -1LL) { NG(); } else { ; }
	
}

void f187(void) {
    	static volatile int64_t x801 = INT64_MAX;
	volatile uint64_t x802 = UINT64_MAX;
	static int32_t x803 = INT32_MIN;
	int64_t x804 = -777436872LL;
	static volatile uint64_t t187 = 141LLU;

    t187 = (x801|(x802-(x803%x804)));

    if (t187 != 9223372036854775807LLU) { NG(); } else { ; }
	
}

void f188(void) {
    	static int64_t x810 = -1LL;
	volatile uint8_t x811 = UINT8_MAX;
	volatile uint8_t x812 = 4U;
	static int64_t t188 = 734770223412313LL;

    t188 = (x809|(x810-(x811%x812)));

    if (t188 != -3LL) { NG(); } else { ; }
	
}

void f189(void) {
    	int64_t x813 = INT64_MIN;
	uint32_t x814 = UINT32_MAX;
	int64_t x815 = INT64_MIN;
	uint64_t x816 = 56526557316003LLU;
	static uint64_t t189 = 536426096301109933LLU;

    t189 = (x813|(x814-(x815%x816)));

    if (t189 != 18446697345287320607LLU) { NG(); } else { ; }
	
}

void f190(void) {
    	uint8_t x817 = 52U;
	static int32_t x819 = 1;
	int32_t t190 = 148935;

    t190 = (x817|(x818-(x819%x820)));

    if (t190 != 701) { NG(); } else { ; }
	
}

void f191(void) {
    	static int8_t x821 = -1;
	uint32_t x823 = 2166487U;
	static int64_t t191 = -4070386116076LL;

    t191 = (x821|(x822-(x823%x824)));

    if (t191 != -1LL) { NG(); } else { ; }
	
}

void f192(void) {
    	uint32_t x826 = 11033U;
	uint16_t x827 = UINT16_MAX;
	uint16_t x828 = UINT16_MAX;

    t192 = (x825|(x826-(x827%x828)));

    if (t192 != 11033U) { NG(); } else { ; }
	
}

void f193(void) {
    	int8_t x830 = -1;
	volatile int32_t x832 = -56;
	volatile int32_t t193 = -156517220;

    t193 = (x829|(x830-(x831%x832)));

    if (t193 != -2) { NG(); } else { ; }
	
}

void f194(void) {
    	uint16_t x833 = 859U;
	uint64_t x834 = 310039536LLU;
	uint8_t x835 = UINT8_MAX;
	static uint32_t x836 = UINT32_MAX;
	uint64_t t194 = 73568004187LLU;

    t194 = (x833|(x834-(x835%x836)));

    if (t194 != 310039547LLU) { NG(); } else { ; }
	
}

void f195(void) {
    	static int32_t x838 = 30;
	static uint64_t x840 = 46515748711119LLU;

    t195 = (x837|(x838-(x839%x840)));

    if (t195 != 18446744071562067999LLU) { NG(); } else { ; }
	
}

void f196(void) {
    	static int16_t x841 = INT16_MIN;
	static int64_t x842 = -1LL;
	int32_t x843 = -1;
	static int64_t x844 = INT64_MAX;
	static volatile int64_t t196 = -101514148400LL;

    t196 = (x841|(x842-(x843%x844)));

    if (t196 != -32768LL) { NG(); } else { ; }
	
}

void f197(void) {
    	static int16_t x845 = INT16_MAX;
	int16_t x846 = INT16_MIN;
	int64_t x847 = INT64_MIN;
	uint32_t x848 = 193U;
	volatile int64_t t197 = -402566700LL;

    t197 = (x845|(x846-(x847%x848)));

    if (t197 != -1LL) { NG(); } else { ; }
	
}

void f198(void) {
    	uint16_t x849 = 1U;
	static int32_t x850 = -1;
	uint64_t x851 = UINT64_MAX;
	static int32_t x852 = -1;
	volatile uint64_t t198 = UINT64_MAX;

    t198 = (x849|(x850-(x851%x852)));

    if (t198 != UINT64_MAX) { NG(); } else { ; }
	
}

void f199(void) {
    	uint32_t x853 = UINT32_MAX;
	uint8_t x854 = 113U;
	int64_t x855 = -12695408782367660LL;
	int8_t x856 = INT8_MIN;
	int64_t t199 = -2LL;

    t199 = (x853|(x854-(x855%x856)));

    if (t199 != 4294967295LL) { NG(); } else { ; }
	
}

int main(void) {
        f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();
    f100();
    f101();
    f102();
    f103();
    f104();
    f105();
    f106();
    f107();
    f108();
    f109();
    f110();
    f111();
    f112();
    f113();
    f114();
    f115();
    f116();
    f117();
    f118();
    f119();
    f120();
    f121();
    f122();
    f123();
    f124();
    f125();
    f126();
    f127();
    f128();
    f129();
    f130();
    f131();
    f132();
    f133();
    f134();
    f135();
    f136();
    f137();
    f138();
    f139();
    f140();
    f141();
    f142();
    f143();
    f144();
    f145();
    f146();
    f147();
    f148();
    f149();
    f150();
    f151();
    f152();
    f153();
    f154();
    f155();
    f156();
    f157();
    f158();
    f159();
    f160();
    f161();
    f162();
    f163();
    f164();
    f165();
    f166();
    f167();
    f168();
    f169();
    f170();
    f171();
    f172();
    f173();
    f174();
    f175();
    f176();
    f177();
    f178();
    f179();
    f180();
    f181();
    f182();
    f183();
    f184();
    f185();
    f186();
    f187();
    f188();
    f189();
    f190();
    f191();
    f192();
    f193();
    f194();
    f195();
    f196();
    f197();
    f198();
    f199();


    return 0;
}

