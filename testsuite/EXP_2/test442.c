#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

int16_t x7 = INT16_MIN;
int8_t x13 = INT8_MAX;
static volatile uint8_t x20 = UINT8_MAX;
static int32_t t3 = 9881;
int64_t x31 = -12429994LL;
static int32_t t5 = 1;
static int32_t x45 = 344;
int8_t x49 = -1;
uint16_t x50 = UINT16_MAX;
int64_t x52 = INT64_MAX;
static int32_t t10 = 9011;
int16_t x75 = -14393;
uint8_t x104 = UINT8_MAX;
uint32_t x105 = 478U;
volatile uint64_t x115 = 264153LLU;
static uint32_t x116 = UINT32_MAX;
volatile int32_t t21 = INT32_MIN;
uint64_t x133 = 5LLU;
static int16_t x148 = -1583;
int32_t t25 = 481453866;
static uint64_t x155 = UINT64_MAX;
int64_t x160 = -1LL;
volatile uint32_t x168 = UINT32_MAX;
int32_t x173 = INT32_MIN;
int16_t x174 = 3;
uint32_t x181 = 6606951U;
volatile int32_t x183 = -1;
int64_t t34 = -2181789832LL;
static int32_t x222 = -1;
int8_t x225 = -1;
volatile uint64_t x226 = 5LLU;
uint16_t x244 = 1U;
static int64_t x249 = INT64_MAX;
int8_t x250 = 7;
int64_t x259 = -2217LL;
volatile uint32_t x265 = 11U;
uint32_t x272 = 174799U;
static volatile uint32_t t48 = 100687U;
int64_t x281 = INT64_MIN;
int64_t x289 = 57042324222501057LL;
static int16_t x294 = -213;
static uint8_t x296 = 24U;
uint32_t x314 = UINT32_MAX;
int32_t t57 = -18520;
static int16_t x334 = INT16_MIN;
volatile uint64_t x335 = 1550346949436468494LLU;
int64_t x337 = -1LL;
volatile uint32_t x340 = 21930U;
uint32_t x342 = UINT32_MAX;
volatile int32_t t64 = 0;
uint16_t x349 = UINT16_MAX;
int64_t x352 = -1LL;
int16_t x362 = -1490;
int32_t t67 = 5211301;
int8_t x369 = -55;
int64_t x387 = 8077415583LL;
int16_t x402 = INT16_MIN;
uint32_t x406 = 806U;
int64_t x413 = INT64_MAX;
uint64_t x414 = 638620880173065LLU;
uint8_t x415 = UINT8_MAX;
volatile int64_t t77 = INT64_MIN;
static int64_t x432 = INT64_MIN;
static int64_t x436 = -1LL;
int32_t x442 = 0;
int32_t t82 = -1;
static int64_t x446 = 18831LL;
static uint32_t x448 = UINT32_MAX;
int16_t x461 = -1;
static int64_t x462 = -152697177107775LL;
uint8_t x466 = 0U;
int8_t x471 = INT8_MIN;
uint32_t x485 = 8059441U;
uint32_t x500 = 22443238U;
static uint32_t x505 = UINT32_MAX;
int32_t t94 = 1;
static int32_t x516 = -1;
volatile int32_t x523 = -1;
static int32_t t97 = 902280326;
int16_t x525 = -49;
int8_t x526 = INT8_MAX;
int16_t x527 = -1;
int32_t x531 = 1;


void f0(void) {
	int8_t x1 = -7;
	int32_t x2 = -1;
	int16_t x3 = 5;
	int64_t x4 = -1LL;
	static int64_t t0 = -24637LL;

	t0 = ((x1==(x2*x3))^x4);

	if (t0 != -1LL) { NG(); } else { ; }
	
}

void f1(void) {
	uint16_t x5 = 3685U;
	int64_t x6 = -1LL;
	int8_t x8 = -1;
	int32_t t1 = -27210;

	t1 = ((x5==(x6*x7))^x8);

	if (t1 != -1) { NG(); } else { ; }
	
}

void f2(void) {
	uint16_t x14 = 32586U;
	static int64_t x15 = -227LL;
	static int8_t x16 = INT8_MAX;
	static int32_t t2 = 139089;

	t2 = ((x13==(x14*x15))^x16);

	if (t2 != 127) { NG(); } else { ; }
	
}

void f3(void) {
	uint32_t x17 = UINT32_MAX;
	int16_t x18 = -1;
	uint8_t x19 = 1U;

	t3 = ((x17==(x18*x19))^x20);

	if (t3 != 254) { NG(); } else { ; }
	
}

void f4(void) {
	int64_t x29 = -7540750LL;
	static volatile uint8_t x30 = UINT8_MAX;
	uint8_t x32 = UINT8_MAX;
	static int32_t t4 = 821639256;

	t4 = ((x29==(x30*x31))^x32);

	if (t4 != 255) { NG(); } else { ; }
	
}

void f5(void) {
	static volatile uint8_t x37 = 11U;
	uint32_t x38 = 3U;
	uint8_t x39 = 35U;
	int16_t x40 = INT16_MAX;

	t5 = ((x37==(x38*x39))^x40);

	if (t5 != 32767) { NG(); } else { ; }
	
}

void f6(void) {
	uint16_t x41 = 463U;
	volatile uint8_t x42 = 5U;
	uint8_t x43 = 1U;
	int32_t x44 = -1;
	int32_t t6 = 614163;

	t6 = ((x41==(x42*x43))^x44);

	if (t6 != -1) { NG(); } else { ; }
	
}

void f7(void) {
	uint16_t x46 = UINT16_MAX;
	int16_t x47 = -1;
	volatile uint16_t x48 = 28067U;
	volatile int32_t t7 = -1466729;

	t7 = ((x45==(x46*x47))^x48);

	if (t7 != 28067) { NG(); } else { ; }
	
}

void f8(void) {
	volatile uint32_t x51 = UINT32_MAX;
	int64_t t8 = INT64_MAX;

	t8 = ((x49==(x50*x51))^x52);

	if (t8 != INT64_MAX) { NG(); } else { ; }
	
}

void f9(void) {
	int8_t x53 = -1;
	int8_t x54 = -9;
	uint16_t x55 = 396U;
	int32_t x56 = -730290;
	volatile int32_t t9 = -199;

	t9 = ((x53==(x54*x55))^x56);

	if (t9 != -730290) { NG(); } else { ; }
	
}

void f10(void) {
	static int64_t x57 = INT64_MIN;
	int8_t x58 = INT8_MIN;
	volatile uint8_t x59 = 112U;
	int16_t x60 = 14624;

	t10 = ((x57==(x58*x59))^x60);

	if (t10 != 14624) { NG(); } else { ; }
	
}

void f11(void) {
	int8_t x65 = 0;
	int8_t x66 = 1;
	uint64_t x67 = 2736534811209LLU;
	volatile int8_t x68 = -1;
	volatile int32_t t11 = 12;

	t11 = ((x65==(x66*x67))^x68);

	if (t11 != -1) { NG(); } else { ; }
	
}

void f12(void) {
	int32_t x69 = INT32_MAX;
	int16_t x70 = -1;
	int16_t x71 = INT16_MIN;
	int64_t x72 = INT64_MAX;
	static int64_t t12 = INT64_MAX;

	t12 = ((x69==(x70*x71))^x72);

	if (t12 != INT64_MAX) { NG(); } else { ; }
	
}

void f13(void) {
	int32_t x73 = -463335;
	uint16_t x74 = 12U;
	int64_t x76 = INT64_MAX;
	int64_t t13 = INT64_MAX;

	t13 = ((x73==(x74*x75))^x76);

	if (t13 != INT64_MAX) { NG(); } else { ; }
	
}

void f14(void) {
	static int16_t x81 = -1;
	uint64_t x82 = 25061957153LLU;
	volatile int16_t x83 = -1290;
	uint16_t x84 = 7U;
	volatile int32_t t14 = -43460623;

	t14 = ((x81==(x82*x83))^x84);

	if (t14 != 7) { NG(); } else { ; }
	
}

void f15(void) {
	uint32_t x89 = UINT32_MAX;
	int16_t x90 = 99;
	volatile int16_t x91 = 1;
	int16_t x92 = INT16_MAX;
	int32_t t15 = 2430;

	t15 = ((x89==(x90*x91))^x92);

	if (t15 != 32767) { NG(); } else { ; }
	
}

void f16(void) {
	static int32_t x97 = 24554785;
	int64_t x98 = -1LL;
	int16_t x99 = INT16_MIN;
	volatile int64_t x100 = INT64_MIN;
	volatile int64_t t16 = INT64_MIN;

	t16 = ((x97==(x98*x99))^x100);

	if (t16 != INT64_MIN) { NG(); } else { ; }
	
}

void f17(void) {
	uint16_t x101 = 41U;
	static uint64_t x102 = 609003011LLU;
	int16_t x103 = -1;
	volatile int32_t t17 = 313;

	t17 = ((x101==(x102*x103))^x104);

	if (t17 != 255) { NG(); } else { ; }
	
}

void f18(void) {
	uint16_t x106 = UINT16_MAX;
	volatile int64_t x107 = -1LL;
	int16_t x108 = INT16_MIN;
	volatile int32_t t18 = -8401557;

	t18 = ((x105==(x106*x107))^x108);

	if (t18 != -32768) { NG(); } else { ; }
	
}

void f19(void) {
	int32_t x109 = -309622331;
	int8_t x110 = INT8_MIN;
	int8_t x111 = -1;
	int32_t x112 = INT32_MIN;
	volatile int32_t t19 = INT32_MIN;

	t19 = ((x109==(x110*x111))^x112);

	if (t19 != INT32_MIN) { NG(); } else { ; }
	
}

void f20(void) {
	static int64_t x113 = 540431LL;
	int64_t x114 = 14LL;
	static volatile uint32_t t20 = UINT32_MAX;

	t20 = ((x113==(x114*x115))^x116);

	if (t20 != UINT32_MAX) { NG(); } else { ; }
	
}

void f21(void) {
	int32_t x117 = INT32_MAX;
	uint16_t x118 = 3232U;
	int16_t x119 = INT16_MIN;
	int32_t x120 = INT32_MIN;

	t21 = ((x117==(x118*x119))^x120);

	if (t21 != INT32_MIN) { NG(); } else { ; }
	
}

void f22(void) {
	static int64_t x129 = INT64_MIN;
	volatile uint64_t x130 = 4500039803653LLU;
	uint32_t x131 = 2035281283U;
	volatile int8_t x132 = INT8_MIN;
	int32_t t22 = 6899107;

	t22 = ((x129==(x130*x131))^x132);

	if (t22 != -128) { NG(); } else { ; }
	
}

void f23(void) {
	uint8_t x134 = 11U;
	int64_t x135 = -1LL;
	uint32_t x136 = 1132843843U;
	uint32_t t23 = 125U;

	t23 = ((x133==(x134*x135))^x136);

	if (t23 != 1132843843U) { NG(); } else { ; }
	
}

void f24(void) {
	static uint8_t x137 = UINT8_MAX;
	int16_t x138 = -14;
	int16_t x139 = -1;
	int64_t x140 = INT64_MIN;
	volatile int64_t t24 = INT64_MIN;

	t24 = ((x137==(x138*x139))^x140);

	if (t24 != INT64_MIN) { NG(); } else { ; }
	
}

void f25(void) {
	int16_t x145 = INT16_MIN;
	int32_t x146 = -1;
	volatile int16_t x147 = INT16_MAX;

	t25 = ((x145==(x146*x147))^x148);

	if (t25 != -1583) { NG(); } else { ; }
	
}

void f26(void) {
	uint32_t x149 = 1782U;
	static int32_t x150 = INT32_MIN;
	volatile uint64_t x151 = UINT64_MAX;
	static int64_t x152 = INT64_MIN;
	volatile int64_t t26 = INT64_MIN;

	t26 = ((x149==(x150*x151))^x152);

	if (t26 != INT64_MIN) { NG(); } else { ; }
	
}

void f27(void) {
	int32_t x153 = -1;
	int32_t x154 = INT32_MAX;
	static int8_t x156 = INT8_MAX;
	volatile int32_t t27 = -1526632;

	t27 = ((x153==(x154*x155))^x156);

	if (t27 != 127) { NG(); } else { ; }
	
}

void f28(void) {
	static uint64_t x157 = 2077669080461318837LLU;
	uint64_t x158 = UINT64_MAX;
	static volatile int32_t x159 = INT32_MAX;
	static int64_t t28 = -33476519328172655LL;

	t28 = ((x157==(x158*x159))^x160);

	if (t28 != -1LL) { NG(); } else { ; }
	
}

void f29(void) {
	static uint64_t x165 = UINT64_MAX;
	int16_t x166 = 9020;
	static uint32_t x167 = UINT32_MAX;
	volatile uint32_t t29 = UINT32_MAX;

	t29 = ((x165==(x166*x167))^x168);

	if (t29 != UINT32_MAX) { NG(); } else { ; }
	
}

void f30(void) {
	volatile int32_t x175 = -80455;
	int32_t x176 = INT32_MIN;
	static int32_t t30 = INT32_MIN;

	t30 = ((x173==(x174*x175))^x176);

	if (t30 != INT32_MIN) { NG(); } else { ; }
	
}

void f31(void) {
	uint16_t x182 = 13U;
	int8_t x184 = 1;
	volatile int32_t t31 = 2633;

	t31 = ((x181==(x182*x183))^x184);

	if (t31 != 1) { NG(); } else { ; }
	
}

void f32(void) {
	int8_t x185 = 9;
	uint32_t x186 = 109826U;
	volatile int8_t x187 = -26;
	uint16_t x188 = UINT16_MAX;
	volatile int32_t t32 = -50896585;

	t32 = ((x185==(x186*x187))^x188);

	if (t32 != 65535) { NG(); } else { ; }
	
}

void f33(void) {
	static volatile int8_t x193 = -1;
	static volatile int16_t x194 = -1;
	int32_t x195 = INT32_MAX;
	uint8_t x196 = 52U;
	volatile int32_t t33 = -7827216;

	t33 = ((x193==(x194*x195))^x196);

	if (t33 != 52) { NG(); } else { ; }
	
}

void f34(void) {
	int32_t x197 = -1;
	static uint64_t x198 = 24LLU;
	uint32_t x199 = UINT32_MAX;
	int64_t x200 = -1LL;

	t34 = ((x197==(x198*x199))^x200);

	if (t34 != -1LL) { NG(); } else { ; }
	
}

void f35(void) {
	int16_t x201 = 788;
	int16_t x202 = INT16_MIN;
	volatile int32_t x203 = -53524;
	int64_t x204 = -1LL;
	volatile int64_t t35 = 30978508467138LL;

	t35 = ((x201==(x202*x203))^x204);

	if (t35 != -1LL) { NG(); } else { ; }
	
}

void f36(void) {
	int64_t x209 = -223466703498LL;
	static uint16_t x210 = 2U;
	uint64_t x211 = UINT64_MAX;
	uint64_t x212 = UINT64_MAX;
	static uint64_t t36 = UINT64_MAX;

	t36 = ((x209==(x210*x211))^x212);

	if (t36 != UINT64_MAX) { NG(); } else { ; }
	
}

void f37(void) {
	int8_t x217 = -1;
	int16_t x218 = -1;
	volatile uint32_t x219 = UINT32_MAX;
	int64_t x220 = INT64_MAX;
	int64_t t37 = INT64_MAX;

	t37 = ((x217==(x218*x219))^x220);

	if (t37 != INT64_MAX) { NG(); } else { ; }
	
}

void f38(void) {
	int32_t x221 = INT32_MIN;
	int8_t x223 = INT8_MIN;
	static int64_t x224 = -1LL;
	static volatile int64_t t38 = 7LL;

	t38 = ((x221==(x222*x223))^x224);

	if (t38 != -1LL) { NG(); } else { ; }
	
}

void f39(void) {
	volatile int8_t x227 = 3;
	int64_t x228 = 4462947129479LL;
	int64_t t39 = 1795280008LL;

	t39 = ((x225==(x226*x227))^x228);

	if (t39 != 4462947129479LL) { NG(); } else { ; }
	
}

void f40(void) {
	uint16_t x229 = 9030U;
	static int16_t x230 = INT16_MIN;
	static int8_t x231 = -2;
	volatile int64_t x232 = -349350799LL;
	volatile int64_t t40 = 203450910421LL;

	t40 = ((x229==(x230*x231))^x232);

	if (t40 != -349350799LL) { NG(); } else { ; }
	
}

void f41(void) {
	int64_t x237 = 17919LL;
	volatile uint16_t x238 = 1U;
	static int64_t x239 = INT64_MAX;
	uint16_t x240 = UINT16_MAX;
	volatile int32_t t41 = -466724474;

	t41 = ((x237==(x238*x239))^x240);

	if (t41 != 65535) { NG(); } else { ; }
	
}

void f42(void) {
	int64_t x241 = INT64_MAX;
	int16_t x242 = INT16_MIN;
	int16_t x243 = -1;
	volatile int32_t t42 = -98;

	t42 = ((x241==(x242*x243))^x244);

	if (t42 != 1) { NG(); } else { ; }
	
}

void f43(void) {
	volatile uint8_t x245 = UINT8_MAX;
	static int16_t x246 = INT16_MIN;
	uint16_t x247 = UINT16_MAX;
	static uint16_t x248 = 1U;
	int32_t t43 = 1799;

	t43 = ((x245==(x246*x247))^x248);

	if (t43 != 1) { NG(); } else { ; }
	
}

void f44(void) {
	volatile uint16_t x251 = 44U;
	int16_t x252 = -1;
	int32_t t44 = 8345;

	t44 = ((x249==(x250*x251))^x252);

	if (t44 != -1) { NG(); } else { ; }
	
}

void f45(void) {
	int64_t x253 = -1LL;
	static uint32_t x254 = UINT32_MAX;
	volatile int32_t x255 = 957;
	volatile int32_t x256 = -49749779;
	volatile int32_t t45 = -275118834;

	t45 = ((x253==(x254*x255))^x256);

	if (t45 != -49749779) { NG(); } else { ; }
	
}

void f46(void) {
	static int8_t x257 = -1;
	static int8_t x258 = 0;
	volatile int16_t x260 = INT16_MIN;
	volatile int32_t t46 = -5169;

	t46 = ((x257==(x258*x259))^x260);

	if (t46 != -32768) { NG(); } else { ; }
	
}

void f47(void) {
	int64_t x266 = -24996264211LL;
	int8_t x267 = INT8_MAX;
	volatile int16_t x268 = INT16_MIN;
	volatile int32_t t47 = -114378;

	t47 = ((x265==(x266*x267))^x268);

	if (t47 != -32768) { NG(); } else { ; }
	
}

void f48(void) {
	static int8_t x269 = -36;
	int16_t x270 = INT16_MIN;
	int16_t x271 = INT16_MIN;

	t48 = ((x269==(x270*x271))^x272);

	if (t48 != 174799U) { NG(); } else { ; }
	
}

void f49(void) {
	int16_t x273 = INT16_MIN;
	static int32_t x274 = 12849;
	uint64_t x275 = 6115141683673205594LLU;
	int8_t x276 = INT8_MIN;
	int32_t t49 = 36;

	t49 = ((x273==(x274*x275))^x276);

	if (t49 != -128) { NG(); } else { ; }
	
}

void f50(void) {
	uint16_t x277 = 12613U;
	static int16_t x278 = INT16_MIN;
	int32_t x279 = -1;
	int64_t x280 = -1LL;
	int64_t t50 = 1901266038674586992LL;

	t50 = ((x277==(x278*x279))^x280);

	if (t50 != -1LL) { NG(); } else { ; }
	
}

void f51(void) {
	uint32_t x282 = UINT32_MAX;
	uint32_t x283 = 1U;
	int32_t x284 = -2;
	int32_t t51 = 823;

	t51 = ((x281==(x282*x283))^x284);

	if (t51 != -2) { NG(); } else { ; }
	
}

void f52(void) {
	uint64_t x285 = 377LLU;
	uint32_t x286 = 631512U;
	uint8_t x287 = 27U;
	int32_t x288 = -1;
	int32_t t52 = -119;

	t52 = ((x285==(x286*x287))^x288);

	if (t52 != -1) { NG(); } else { ; }
	
}

void f53(void) {
	uint8_t x290 = UINT8_MAX;
	volatile int32_t x291 = 10;
	static int32_t x292 = INT32_MIN;
	volatile int32_t t53 = INT32_MIN;

	t53 = ((x289==(x290*x291))^x292);

	if (t53 != INT32_MIN) { NG(); } else { ; }
	
}

void f54(void) {
	volatile int8_t x293 = INT8_MIN;
	int32_t x295 = -1;
	volatile int32_t t54 = 13645128;

	t54 = ((x293==(x294*x295))^x296);

	if (t54 != 24) { NG(); } else { ; }
	
}

void f55(void) {
	static int64_t x305 = -32LL;
	int8_t x306 = INT8_MIN;
	static int16_t x307 = 23;
	int8_t x308 = INT8_MIN;
	int32_t t55 = 876;

	t55 = ((x305==(x306*x307))^x308);

	if (t55 != -128) { NG(); } else { ; }
	
}

void f56(void) {
	static uint32_t x309 = 123U;
	static volatile int16_t x310 = INT16_MAX;
	int16_t x311 = 12;
	int64_t x312 = INT64_MAX;
	volatile int64_t t56 = INT64_MAX;

	t56 = ((x309==(x310*x311))^x312);

	if (t56 != INT64_MAX) { NG(); } else { ; }
	
}

void f57(void) {
	uint64_t x313 = 75LLU;
	static int16_t x315 = INT16_MIN;
	uint8_t x316 = 1U;

	t57 = ((x313==(x314*x315))^x316);

	if (t57 != 1) { NG(); } else { ; }
	
}

void f58(void) {
	int16_t x317 = INT16_MIN;
	int16_t x318 = -1;
	static int32_t x319 = -41584;
	uint32_t x320 = UINT32_MAX;
	uint32_t t58 = UINT32_MAX;

	t58 = ((x317==(x318*x319))^x320);

	if (t58 != UINT32_MAX) { NG(); } else { ; }
	
}

void f59(void) {
	static int32_t x321 = -1096;
	uint64_t x322 = 467001501094152LLU;
	volatile uint32_t x323 = 525876341U;
	int32_t x324 = INT32_MIN;
	static volatile int32_t t59 = INT32_MIN;

	t59 = ((x321==(x322*x323))^x324);

	if (t59 != INT32_MIN) { NG(); } else { ; }
	
}

void f60(void) {
	int8_t x329 = INT8_MIN;
	int64_t x330 = INT64_MAX;
	volatile int64_t x331 = -1LL;
	int32_t x332 = INT32_MAX;
	int32_t t60 = INT32_MAX;

	t60 = ((x329==(x330*x331))^x332);

	if (t60 != INT32_MAX) { NG(); } else { ; }
	
}

void f61(void) {
	static int16_t x333 = -1;
	int16_t x336 = 0;
	static int32_t t61 = -9131605;

	t61 = ((x333==(x334*x335))^x336);

	if (t61 != 0) { NG(); } else { ; }
	
}

void f62(void) {
	uint64_t x338 = UINT64_MAX;
	uint16_t x339 = UINT16_MAX;
	static volatile uint32_t t62 = 427U;

	t62 = ((x337==(x338*x339))^x340);

	if (t62 != 21930U) { NG(); } else { ; }
	
}

void f63(void) {
	static int32_t x341 = -1639186;
	static int16_t x343 = INT16_MAX;
	uint64_t x344 = 42505LLU;
	volatile uint64_t t63 = 46348137LLU;

	t63 = ((x341==(x342*x343))^x344);

	if (t63 != 42505LLU) { NG(); } else { ; }
	
}

void f64(void) {
	static int8_t x345 = -1;
	volatile uint16_t x346 = 1U;
	int64_t x347 = 957LL;
	volatile int8_t x348 = INT8_MIN;

	t64 = ((x345==(x346*x347))^x348);

	if (t64 != -128) { NG(); } else { ; }
	
}

void f65(void) {
	static volatile uint8_t x350 = 5U;
	volatile int16_t x351 = INT16_MAX;
	volatile int64_t t65 = -10366981996LL;

	t65 = ((x349==(x350*x351))^x352);

	if (t65 != -1LL) { NG(); } else { ; }
	
}

void f66(void) {
	int64_t x361 = INT64_MAX;
	static int64_t x363 = -1LL;
	int8_t x364 = INT8_MIN;
	int32_t t66 = -603;

	t66 = ((x361==(x362*x363))^x364);

	if (t66 != -128) { NG(); } else { ; }
	
}

void f67(void) {
	int16_t x365 = INT16_MIN;
	int8_t x366 = 15;
	volatile int32_t x367 = 192;
	int16_t x368 = INT16_MIN;

	t67 = ((x365==(x366*x367))^x368);

	if (t67 != -32768) { NG(); } else { ; }
	
}

void f68(void) {
	volatile uint16_t x370 = UINT16_MAX;
	int64_t x371 = -1LL;
	static volatile int64_t x372 = INT64_MAX;
	volatile int64_t t68 = INT64_MAX;

	t68 = ((x369==(x370*x371))^x372);

	if (t68 != INT64_MAX) { NG(); } else { ; }
	
}

void f69(void) {
	static volatile uint32_t x373 = 35286144U;
	uint16_t x374 = UINT16_MAX;
	volatile uint32_t x375 = 1698697U;
	volatile uint8_t x376 = 0U;
	volatile int32_t t69 = 10627;

	t69 = ((x373==(x374*x375))^x376);

	if (t69 != 0) { NG(); } else { ; }
	
}

void f70(void) {
	volatile int8_t x377 = INT8_MAX;
	static int32_t x378 = 1046;
	static int32_t x379 = -488427;
	int8_t x380 = 4;
	int32_t t70 = 2890;

	t70 = ((x377==(x378*x379))^x380);

	if (t70 != 4) { NG(); } else { ; }
	
}

void f71(void) {
	uint32_t x385 = 7U;
	uint16_t x386 = 1U;
	int8_t x388 = INT8_MIN;
	volatile int32_t t71 = 15205;

	t71 = ((x385==(x386*x387))^x388);

	if (t71 != -128) { NG(); } else { ; }
	
}

void f72(void) {
	int16_t x397 = INT16_MAX;
	int8_t x398 = -1;
	uint16_t x399 = 7474U;
	int32_t x400 = INT32_MIN;
	int32_t t72 = INT32_MIN;

	t72 = ((x397==(x398*x399))^x400);

	if (t72 != INT32_MIN) { NG(); } else { ; }
	
}

void f73(void) {
	uint32_t x401 = 30591U;
	int16_t x403 = -1;
	uint8_t x404 = UINT8_MAX;
	int32_t t73 = 1608559;

	t73 = ((x401==(x402*x403))^x404);

	if (t73 != 255) { NG(); } else { ; }
	
}

void f74(void) {
	int64_t x405 = INT64_MIN;
	volatile int8_t x407 = INT8_MAX;
	int8_t x408 = -1;
	volatile int32_t t74 = -53763;

	t74 = ((x405==(x406*x407))^x408);

	if (t74 != -1) { NG(); } else { ; }
	
}

void f75(void) {
	volatile uint8_t x409 = 0U;
	volatile int8_t x410 = INT8_MIN;
	int8_t x411 = INT8_MIN;
	static int8_t x412 = 51;
	int32_t t75 = -19;

	t75 = ((x409==(x410*x411))^x412);

	if (t75 != 51) { NG(); } else { ; }
	
}

void f76(void) {
	volatile int64_t x416 = INT64_MAX;
	static volatile int64_t t76 = INT64_MAX;

	t76 = ((x413==(x414*x415))^x416);

	if (t76 != INT64_MAX) { NG(); } else { ; }
	
}

void f77(void) {
	uint64_t x417 = UINT64_MAX;
	uint8_t x418 = 11U;
	volatile int8_t x419 = -1;
	int64_t x420 = INT64_MIN;

	t77 = ((x417==(x418*x419))^x420);

	if (t77 != INT64_MIN) { NG(); } else { ; }
	
}

void f78(void) {
	int64_t x425 = -1LL;
	uint8_t x426 = 1U;
	uint64_t x427 = 9006LLU;
	static int64_t x428 = INT64_MIN;
	volatile int64_t t78 = INT64_MIN;

	t78 = ((x425==(x426*x427))^x428);

	if (t78 != INT64_MIN) { NG(); } else { ; }
	
}

void f79(void) {
	static int16_t x429 = INT16_MIN;
	int32_t x430 = 14;
	static volatile uint32_t x431 = 59U;
	volatile int64_t t79 = INT64_MIN;

	t79 = ((x429==(x430*x431))^x432);

	if (t79 != INT64_MIN) { NG(); } else { ; }
	
}

void f80(void) {
	int16_t x433 = INT16_MIN;
	int16_t x434 = 6;
	int32_t x435 = -1;
	volatile int64_t t80 = -3121LL;

	t80 = ((x433==(x434*x435))^x436);

	if (t80 != -1LL) { NG(); } else { ; }
	
}

void f81(void) {
	static volatile int8_t x437 = -1;
	static uint16_t x438 = UINT16_MAX;
	int64_t x439 = 3392546577441LL;
	static uint8_t x440 = 27U;
	static volatile int32_t t81 = -717;

	t81 = ((x437==(x438*x439))^x440);

	if (t81 != 27) { NG(); } else { ; }
	
}

void f82(void) {
	uint64_t x441 = 761139LLU;
	int64_t x443 = -18705LL;
	int8_t x444 = INT8_MIN;

	t82 = ((x441==(x442*x443))^x444);

	if (t82 != -128) { NG(); } else { ; }
	
}

void f83(void) {
	int8_t x445 = -1;
	int8_t x447 = 63;
	volatile uint32_t t83 = UINT32_MAX;

	t83 = ((x445==(x446*x447))^x448);

	if (t83 != UINT32_MAX) { NG(); } else { ; }
	
}

void f84(void) {
	int64_t x449 = -7221414289479283LL;
	static volatile int8_t x450 = INT8_MIN;
	volatile int64_t x451 = -11482132216374949LL;
	int8_t x452 = 1;
	static int32_t t84 = 3;

	t84 = ((x449==(x450*x451))^x452);

	if (t84 != 1) { NG(); } else { ; }
	
}

void f85(void) {
	volatile int32_t x453 = 116983;
	static int16_t x454 = INT16_MIN;
	int16_t x455 = -1;
	int32_t x456 = INT32_MAX;
	volatile int32_t t85 = INT32_MAX;

	t85 = ((x453==(x454*x455))^x456);

	if (t85 != INT32_MAX) { NG(); } else { ; }
	
}

void f86(void) {
	int16_t x457 = -1;
	int16_t x458 = -1;
	uint8_t x459 = 0U;
	volatile uint32_t x460 = UINT32_MAX;
	volatile uint32_t t86 = UINT32_MAX;

	t86 = ((x457==(x458*x459))^x460);

	if (t86 != UINT32_MAX) { NG(); } else { ; }
	
}

void f87(void) {
	int32_t x463 = 1;
	int64_t x464 = INT64_MAX;
	volatile int64_t t87 = INT64_MAX;

	t87 = ((x461==(x462*x463))^x464);

	if (t87 != INT64_MAX) { NG(); } else { ; }
	
}

void f88(void) {
	static int8_t x465 = INT8_MIN;
	int16_t x467 = -1;
	volatile uint32_t x468 = 479990U;
	uint32_t t88 = 989U;

	t88 = ((x465==(x466*x467))^x468);

	if (t88 != 479990U) { NG(); } else { ; }
	
}

void f89(void) {
	uint16_t x469 = 11U;
	static int64_t x470 = -14LL;
	int64_t x472 = INT64_MAX;
	volatile int64_t t89 = INT64_MAX;

	t89 = ((x469==(x470*x471))^x472);

	if (t89 != INT64_MAX) { NG(); } else { ; }
	
}

void f90(void) {
	int8_t x477 = -1;
	static uint8_t x478 = 2U;
	int8_t x479 = -1;
	uint16_t x480 = 563U;
	int32_t t90 = -4724;

	t90 = ((x477==(x478*x479))^x480);

	if (t90 != 563) { NG(); } else { ; }
	
}

void f91(void) {
	static int32_t x486 = -1;
	uint8_t x487 = UINT8_MAX;
	static int32_t x488 = INT32_MIN;
	volatile int32_t t91 = INT32_MIN;

	t91 = ((x485==(x486*x487))^x488);

	if (t91 != INT32_MIN) { NG(); } else { ; }
	
}

void f92(void) {
	int32_t x489 = INT32_MIN;
	uint16_t x490 = 14U;
	int32_t x491 = 39;
	volatile int64_t x492 = INT64_MIN;
	int64_t t92 = INT64_MIN;

	t92 = ((x489==(x490*x491))^x492);

	if (t92 != INT64_MIN) { NG(); } else { ; }
	
}

void f93(void) {
	uint64_t x497 = UINT64_MAX;
	uint8_t x498 = 0U;
	volatile int64_t x499 = -1LL;
	volatile uint32_t t93 = 59978U;

	t93 = ((x497==(x498*x499))^x500);

	if (t93 != 22443238U) { NG(); } else { ; }
	
}

void f94(void) {
	uint64_t x506 = 322LLU;
	volatile int8_t x507 = 54;
	int8_t x508 = -1;

	t94 = ((x505==(x506*x507))^x508);

	if (t94 != -1) { NG(); } else { ; }
	
}

void f95(void) {
	volatile int16_t x509 = INT16_MIN;
	volatile int32_t x510 = INT32_MAX;
	uint8_t x511 = 0U;
	uint16_t x512 = 10149U;
	volatile int32_t t95 = 57;

	t95 = ((x509==(x510*x511))^x512);

	if (t95 != 10149) { NG(); } else { ; }
	
}

void f96(void) {
	static volatile int64_t x513 = -1LL;
	uint64_t x514 = 2LLU;
	static int64_t x515 = 21277529LL;
	int32_t t96 = -75;

	t96 = ((x513==(x514*x515))^x516);

	if (t96 != -1) { NG(); } else { ; }
	
}

void f97(void) {
	int64_t x521 = 46299630LL;
	int16_t x522 = 4361;
	uint8_t x524 = UINT8_MAX;

	t97 = ((x521==(x522*x523))^x524);

	if (t97 != 255) { NG(); } else { ; }
	
}

void f98(void) {
	int64_t x528 = INT64_MAX;
	volatile int64_t t98 = INT64_MAX;

	t98 = ((x525==(x526*x527))^x528);

	if (t98 != INT64_MAX) { NG(); } else { ; }
	
}

void f99(void) {
	int8_t x529 = -1;
	static uint16_t x530 = 3U;
	uint64_t x532 = UINT64_MAX;
	volatile uint64_t t99 = UINT64_MAX;

	t99 = ((x529==(x530*x531))^x532);

	if (t99 != UINT64_MAX) { NG(); } else { ; }
	
}

int main(void) {
    f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();


    return 0;
}

