
/*
CF3

Copyright (c) 2015 ishiura-lab.

Released under the MIT license.  
https://github.com/ishiura-compiler/CF3/MIT-LICENSE.md
*/

#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

uint16_t x3 = 9205U;
int16_t x7 = 51;
uint64_t x10 = 272568LLU;
static volatile uint32_t x17 = UINT32_MAX;
volatile uint64_t t4 = 404915099LLU;
static int8_t x21 = -1;
uint32_t x22 = 20U;
uint32_t x24 = 15748U;
int8_t x30 = INT8_MIN;
int32_t x33 = INT32_MIN;
static volatile int64_t x35 = 785LL;
int32_t t8 = 1556;
uint32_t x37 = 64811547U;
uint32_t x38 = UINT32_MAX;
int8_t x39 = 0;
int32_t t12 = -642201842;
uint64_t x54 = 4561351512725218055LLU;
static int32_t x62 = 7827;
volatile int32_t t16 = 1;
uint8_t x71 = 0U;
volatile int64_t x74 = -1LL;
int32_t x81 = INT32_MIN;
volatile int64_t t20 = INT64_MIN;
int64_t t24 = -58080656371509LL;
int32_t x102 = 761185;
static volatile int32_t x103 = INT32_MIN;
uint16_t x116 = 11U;
int32_t x119 = -1;
uint16_t x120 = UINT16_MAX;
uint64_t x122 = UINT64_MAX;
int16_t x123 = INT16_MIN;
uint64_t x130 = UINT64_MAX;
uint64_t x136 = 55609764LLU;
uint16_t x138 = 31U;
int64_t x140 = 15011774359728LL;
static uint16_t x141 = 25075U;
volatile int32_t t40 = INT32_MAX;
int64_t t42 = 142955191119277LL;
int32_t t43 = -2;
static int16_t x177 = 20;
int32_t t44 = -120589;
volatile int32_t x184 = INT32_MIN;
volatile int16_t x185 = -39;
volatile int32_t t46 = INT32_MAX;
uint64_t x195 = 337601376360LLU;
uint16_t x201 = 0U;
uint8_t x203 = 31U;
uint16_t x205 = UINT16_MAX;
int32_t t51 = -1;
volatile uint16_t x211 = 3400U;
volatile int32_t t52 = -62960240;
static int16_t x222 = -1;
uint32_t x226 = 2213388U;
volatile int64_t t57 = INT64_MAX;
int16_t x237 = -1;
uint16_t x238 = 93U;
volatile int8_t x243 = INT8_MIN;
uint64_t x244 = 3535764177LLU;
static uint16_t x246 = 17565U;
uint8_t x248 = 5U;
volatile int32_t t61 = -75;
static int16_t x251 = -695;
int16_t x253 = INT16_MAX;
volatile int8_t x265 = -1;
int16_t x271 = -1;
uint32_t x274 = 1U;
int8_t x277 = 4;
static uint64_t x279 = UINT64_MAX;
volatile int16_t x280 = INT16_MAX;
static volatile uint8_t x281 = 1U;
int32_t x286 = INT32_MAX;
int16_t x287 = INT16_MAX;
int32_t t71 = 23;
uint16_t x296 = 219U;
volatile uint8_t x297 = 36U;
int16_t x302 = -110;
int32_t t75 = 26;
volatile int64_t x305 = -1LL;
uint32_t x307 = UINT32_MAX;
static int64_t x309 = -421051852632LL;
int16_t x322 = -1;
volatile int16_t x323 = 3568;
volatile uint64_t x338 = UINT64_MAX;
int8_t x346 = INT8_MIN;
int16_t x347 = 513;
uint64_t x349 = UINT64_MAX;
int32_t x355 = 232781;
int32_t x359 = -1;
uint32_t t89 = 861959806U;
volatile int16_t x362 = INT16_MIN;
static volatile uint64_t t91 = 0LLU;
static volatile int32_t x373 = INT32_MAX;
int64_t x382 = INT64_MIN;
int32_t x384 = -1;
static volatile int32_t x390 = -1;
int8_t x393 = INT8_MAX;
static int64_t x399 = -1979038012661370LL;
int32_t t104 = 1014844;
uint8_t x422 = 10U;
static int32_t x424 = INT32_MIN;
volatile int32_t t106 = 480;
static uint32_t x432 = UINT32_MAX;
static uint32_t x433 = 19U;
volatile int16_t x440 = -1;
int64_t x460 = INT64_MIN;
int32_t x463 = -1;
volatile uint32_t t115 = 226174U;
int8_t x465 = INT8_MIN;
static int16_t x466 = -1;
int32_t t116 = 463;
int64_t x470 = INT64_MIN;
int64_t x472 = 101800265LL;
volatile int32_t t121 = 425;
int8_t x489 = 0;
volatile uint8_t x492 = 1U;
volatile int64_t t123 = -3571545654898754554LL;
int8_t x500 = INT8_MIN;
int32_t x501 = 64;
uint64_t x506 = UINT64_MAX;
int64_t x507 = INT64_MIN;
static int16_t x509 = INT16_MAX;
volatile uint16_t x511 = 1U;
uint8_t x517 = UINT8_MAX;
volatile uint64_t t129 = UINT64_MAX;
volatile int32_t t131 = 0;
static int32_t x531 = 236;
int16_t x532 = INT16_MIN;
int32_t t134 = -1;
int64_t x544 = INT64_MIN;
volatile int64_t t135 = INT64_MIN;
int8_t x546 = INT8_MIN;
int32_t x548 = 67;
uint32_t x549 = 323553094U;
static volatile int16_t x558 = -1;
uint32_t x563 = 136605052U;
uint8_t x564 = UINT8_MAX;
int32_t t140 = 1134894;
uint64_t x568 = UINT64_MAX;
static volatile uint64_t t141 = UINT64_MAX;
volatile int32_t t142 = INT32_MIN;
int32_t t143 = -722361117;
int32_t t144 = -18839;
static int8_t x588 = INT8_MAX;
static uint16_t x589 = 1U;
static volatile int16_t x590 = INT16_MIN;
uint64_t x591 = UINT64_MAX;
uint16_t x592 = 2U;
int8_t x596 = INT8_MAX;
static uint64_t x597 = 690LLU;
uint32_t x607 = 11811110U;
volatile int32_t x617 = INT32_MAX;
volatile int32_t t154 = 526;
int64_t x624 = INT64_MIN;
int16_t x626 = -34;
uint8_t x634 = 0U;
static volatile uint64_t t158 = 221139LLU;
uint32_t x641 = UINT32_MAX;
static int32_t x644 = INT32_MIN;
int32_t t160 = -8989340;
static volatile int32_t x654 = INT32_MIN;
int64_t x656 = -1LL;
int64_t t163 = -12326LL;
int8_t x659 = 6;
int8_t x663 = INT8_MAX;
static int32_t t165 = 246;
uint8_t x669 = 5U;
uint16_t x670 = 704U;
int64_t x672 = INT64_MAX;
int64_t x673 = -15LL;
static uint8_t x674 = 2U;
int64_t x690 = INT64_MIN;
volatile int32_t t172 = -1539;
int32_t x698 = -1;
static volatile int8_t x708 = INT8_MIN;
volatile int64_t x718 = INT64_MAX;
static int64_t x720 = 2LL;
volatile int8_t x723 = INT8_MAX;
int16_t x724 = INT16_MIN;
uint16_t x738 = UINT16_MAX;
int16_t x744 = INT16_MIN;
uint32_t x746 = UINT32_MAX;
static int64_t x747 = INT64_MAX;
static volatile int32_t t186 = INT32_MIN;
uint64_t x751 = UINT64_MAX;
volatile int16_t x752 = INT16_MIN;
volatile int16_t x758 = INT16_MIN;
int32_t x759 = INT32_MIN;
uint8_t x761 = 0U;
int32_t x763 = -1;
static uint16_t x765 = 849U;
int32_t x768 = INT32_MIN;
volatile int64_t t192 = INT64_MIN;
static int16_t x777 = INT16_MIN;
int16_t x781 = -1;
int8_t x784 = INT8_MIN;
volatile int32_t t195 = 0;
int16_t x792 = 357;
volatile uint64_t x794 = 171991240LLU;
static int32_t x796 = 644707697;
int32_t t198 = -1598998;
volatile uint8_t x797 = UINT8_MAX;


void f0(void) {
    	volatile int64_t x1 = -1LL;
	int64_t x2 = -1LL;
	static int32_t x4 = -1;
	int32_t t0 = 233281;

    t0 = (((x1^x2)>x3)|x4);

    if (t0 != -1) { NG(); } else { ; }
	
}

void f1(void) {
    	static uint16_t x5 = 3169U;
	volatile int8_t x6 = INT8_MIN;
	uint32_t x8 = 1842142U;
	volatile uint32_t t1 = 91732231U;

    t1 = (((x5^x6)>x7)|x8);

    if (t1 != 1842142U) { NG(); } else { ; }
	
}

void f2(void) {
    	int8_t x9 = -13;
	volatile int64_t x11 = -176828254805911LL;
	static int64_t x12 = INT64_MIN;
	int64_t t2 = -12LL;

    t2 = (((x9^x10)>x11)|x12);

    if (t2 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f3(void) {
    	int64_t x13 = INT64_MIN;
	uint8_t x14 = 1U;
	static int16_t x15 = INT16_MAX;
	int8_t x16 = INT8_MIN;
	int32_t t3 = -17321857;

    t3 = (((x13^x14)>x15)|x16);

    if (t3 != -128) { NG(); } else { ; }
	
}

void f4(void) {
    	uint64_t x18 = UINT64_MAX;
	uint8_t x19 = UINT8_MAX;
	uint64_t x20 = 11LLU;

    t4 = (((x17^x18)>x19)|x20);

    if (t4 != 11LLU) { NG(); } else { ; }
	
}

void f5(void) {
    	uint32_t x23 = 2869712U;
	static volatile uint32_t t5 = 95910U;

    t5 = (((x21^x22)>x23)|x24);

    if (t5 != 15749U) { NG(); } else { ; }
	
}

void f6(void) {
    	static int32_t x25 = INT32_MIN;
	static int8_t x26 = INT8_MAX;
	uint64_t x27 = 6LLU;
	static volatile int16_t x28 = INT16_MIN;
	int32_t t6 = 25625;

    t6 = (((x25^x26)>x27)|x28);

    if (t6 != -32767) { NG(); } else { ; }
	
}

void f7(void) {
    	static uint8_t x29 = 0U;
	volatile int16_t x31 = INT16_MAX;
	volatile int8_t x32 = INT8_MIN;
	volatile int32_t t7 = 14088220;

    t7 = (((x29^x30)>x31)|x32);

    if (t7 != -128) { NG(); } else { ; }
	
}

void f8(void) {
    	int64_t x34 = INT64_MIN;
	uint16_t x36 = 1477U;

    t8 = (((x33^x34)>x35)|x36);

    if (t8 != 1477) { NG(); } else { ; }
	
}

void f9(void) {
    	int64_t x40 = 28867LL;
	volatile int64_t t9 = -48612843LL;

    t9 = (((x37^x38)>x39)|x40);

    if (t9 != 28867LL) { NG(); } else { ; }
	
}

void f10(void) {
    	uint8_t x41 = 7U;
	int64_t x42 = 17160166890095LL;
	int64_t x43 = INT64_MIN;
	int64_t x44 = -1LL;
	volatile int64_t t10 = 24880125780148LL;

    t10 = (((x41^x42)>x43)|x44);

    if (t10 != -1LL) { NG(); } else { ; }
	
}

void f11(void) {
    	static uint64_t x45 = 21120LLU;
	uint32_t x46 = UINT32_MAX;
	int64_t x47 = 237227291LL;
	int32_t x48 = -1;
	int32_t t11 = 513978847;

    t11 = (((x45^x46)>x47)|x48);

    if (t11 != -1) { NG(); } else { ; }
	
}

void f12(void) {
    	uint32_t x49 = UINT32_MAX;
	volatile int16_t x50 = -1;
	int32_t x51 = -1;
	int16_t x52 = INT16_MIN;

    t12 = (((x49^x50)>x51)|x52);

    if (t12 != -32768) { NG(); } else { ; }
	
}

void f13(void) {
    	int64_t x53 = -1LL;
	static int8_t x55 = 0;
	int64_t x56 = 6439LL;
	static volatile int64_t t13 = -326599141LL;

    t13 = (((x53^x54)>x55)|x56);

    if (t13 != 6439LL) { NG(); } else { ; }
	
}

void f14(void) {
    	int8_t x57 = -1;
	volatile int8_t x58 = -1;
	static int32_t x59 = INT32_MIN;
	volatile int32_t x60 = INT32_MIN;
	volatile int32_t t14 = -917093;

    t14 = (((x57^x58)>x59)|x60);

    if (t14 != -2147483647) { NG(); } else { ; }
	
}

void f15(void) {
    	int16_t x61 = INT16_MIN;
	int64_t x63 = INT64_MIN;
	static uint64_t x64 = 14LLU;
	volatile uint64_t t15 = 1582LLU;

    t15 = (((x61^x62)>x63)|x64);

    if (t15 != 15LLU) { NG(); } else { ; }
	
}

void f16(void) {
    	volatile uint8_t x65 = 43U;
	int64_t x66 = INT64_MIN;
	int16_t x67 = 927;
	int16_t x68 = 1;

    t16 = (((x65^x66)>x67)|x68);

    if (t16 != 1) { NG(); } else { ; }
	
}

void f17(void) {
    	volatile uint64_t x69 = UINT64_MAX;
	volatile uint8_t x70 = 2U;
	int32_t x72 = INT32_MIN;
	int32_t t17 = -1;

    t17 = (((x69^x70)>x71)|x72);

    if (t17 != -2147483647) { NG(); } else { ; }
	
}

void f18(void) {
    	volatile int32_t x73 = INT32_MIN;
	uint16_t x75 = 2913U;
	int16_t x76 = INT16_MIN;
	volatile int32_t t18 = -7336637;

    t18 = (((x73^x74)>x75)|x76);

    if (t18 != -32767) { NG(); } else { ; }
	
}

void f19(void) {
    	static volatile int16_t x77 = -1;
	int32_t x78 = 127;
	static volatile int64_t x79 = -1LL;
	int16_t x80 = INT16_MIN;
	volatile int32_t t19 = -82;

    t19 = (((x77^x78)>x79)|x80);

    if (t19 != -32768) { NG(); } else { ; }
	
}

void f20(void) {
    	uint16_t x82 = 4U;
	volatile int32_t x83 = -1;
	static int64_t x84 = INT64_MIN;

    t20 = (((x81^x82)>x83)|x84);

    if (t20 != INT64_MIN) { NG(); } else { ; }
	
}

void f21(void) {
    	volatile int32_t x85 = -12438;
	volatile int16_t x86 = -1;
	int8_t x87 = 0;
	uint64_t x88 = 50586399308695LLU;
	static volatile uint64_t t21 = 6966648201015242334LLU;

    t21 = (((x85^x86)>x87)|x88);

    if (t21 != 50586399308695LLU) { NG(); } else { ; }
	
}

void f22(void) {
    	volatile uint16_t x89 = 892U;
	uint16_t x90 = 21803U;
	uint16_t x91 = UINT16_MAX;
	int16_t x92 = INT16_MIN;
	static int32_t t22 = 481;

    t22 = (((x89^x90)>x91)|x92);

    if (t22 != -32768) { NG(); } else { ; }
	
}

void f23(void) {
    	int8_t x93 = INT8_MIN;
	uint16_t x94 = 1U;
	volatile uint16_t x95 = UINT16_MAX;
	int16_t x96 = INT16_MAX;
	int32_t t23 = -1568485;

    t23 = (((x93^x94)>x95)|x96);

    if (t23 != 32767) { NG(); } else { ; }
	
}

void f24(void) {
    	uint32_t x97 = 9791218U;
	static int64_t x98 = -1LL;
	volatile int8_t x99 = INT8_MIN;
	int64_t x100 = -598826932321619811LL;

    t24 = (((x97^x98)>x99)|x100);

    if (t24 != -598826932321619811LL) { NG(); } else { ; }
	
}

void f25(void) {
    	uint64_t x101 = 676681254674838300LLU;
	static int8_t x104 = INT8_MAX;
	volatile int32_t t25 = 5766247;

    t25 = (((x101^x102)>x103)|x104);

    if (t25 != 127) { NG(); } else { ; }
	
}

void f26(void) {
    	uint16_t x105 = UINT16_MAX;
	volatile uint8_t x106 = 54U;
	int8_t x107 = -1;
	int16_t x108 = INT16_MAX;
	int32_t t26 = 108751;

    t26 = (((x105^x106)>x107)|x108);

    if (t26 != 32767) { NG(); } else { ; }
	
}

void f27(void) {
    	int16_t x109 = 58;
	uint32_t x110 = 92U;
	static uint16_t x111 = UINT16_MAX;
	uint32_t x112 = 870434U;
	volatile uint32_t t27 = 6268784U;

    t27 = (((x109^x110)>x111)|x112);

    if (t27 != 870434U) { NG(); } else { ; }
	
}

void f28(void) {
    	int64_t x113 = INT64_MAX;
	volatile int32_t x114 = INT32_MIN;
	int64_t x115 = -2080884092562045733LL;
	int32_t t28 = 84657824;

    t28 = (((x113^x114)>x115)|x116);

    if (t28 != 11) { NG(); } else { ; }
	
}

void f29(void) {
    	int32_t x117 = INT32_MAX;
	volatile uint16_t x118 = UINT16_MAX;
	volatile int32_t t29 = 1873;

    t29 = (((x117^x118)>x119)|x120);

    if (t29 != 65535) { NG(); } else { ; }
	
}

void f30(void) {
    	int32_t x121 = -1;
	uint32_t x124 = 1U;
	volatile uint32_t t30 = 31857U;

    t30 = (((x121^x122)>x123)|x124);

    if (t30 != 1U) { NG(); } else { ; }
	
}

void f31(void) {
    	volatile uint8_t x125 = 116U;
	volatile int8_t x126 = -1;
	int16_t x127 = INT16_MIN;
	int16_t x128 = -1;
	volatile int32_t t31 = -13349;

    t31 = (((x125^x126)>x127)|x128);

    if (t31 != -1) { NG(); } else { ; }
	
}

void f32(void) {
    	int8_t x129 = 29;
	static volatile uint8_t x131 = 3U;
	int16_t x132 = INT16_MAX;
	int32_t t32 = -1;

    t32 = (((x129^x130)>x131)|x132);

    if (t32 != 32767) { NG(); } else { ; }
	
}

void f33(void) {
    	volatile int64_t x133 = 317023743459528755LL;
	static int32_t x134 = INT32_MIN;
	static int64_t x135 = INT64_MAX;
	volatile uint64_t t33 = 152LLU;

    t33 = (((x133^x134)>x135)|x136);

    if (t33 != 55609764LLU) { NG(); } else { ; }
	
}

void f34(void) {
    	volatile int8_t x137 = 51;
	static volatile int16_t x139 = INT16_MIN;
	int64_t t34 = 510482867152250664LL;

    t34 = (((x137^x138)>x139)|x140);

    if (t34 != 15011774359729LL) { NG(); } else { ; }
	
}

void f35(void) {
    	int8_t x142 = 30;
	volatile int32_t x143 = INT32_MAX;
	static int8_t x144 = -1;
	volatile int32_t t35 = 5;

    t35 = (((x141^x142)>x143)|x144);

    if (t35 != -1) { NG(); } else { ; }
	
}

void f36(void) {
    	uint8_t x145 = 52U;
	int8_t x146 = -24;
	int64_t x147 = INT64_MAX;
	uint64_t x148 = 0LLU;
	volatile uint64_t t36 = 52305897868LLU;

    t36 = (((x145^x146)>x147)|x148);

    if (t36 != 0LLU) { NG(); } else { ; }
	
}

void f37(void) {
    	static int8_t x149 = INT8_MIN;
	static int32_t x150 = INT32_MIN;
	int8_t x151 = INT8_MIN;
	static int64_t x152 = INT64_MAX;
	volatile int64_t t37 = INT64_MAX;

    t37 = (((x149^x150)>x151)|x152);

    if (t37 != INT64_MAX) { NG(); } else { ; }
	
}

void f38(void) {
    	int16_t x153 = 4714;
	int8_t x154 = 4;
	volatile int16_t x155 = INT16_MIN;
	uint8_t x156 = 108U;
	volatile int32_t t38 = 1863870;

    t38 = (((x153^x154)>x155)|x156);

    if (t38 != 109) { NG(); } else { ; }
	
}

void f39(void) {
    	int64_t x157 = 45217287LL;
	static volatile uint8_t x158 = 29U;
	volatile uint8_t x159 = 3U;
	uint8_t x160 = UINT8_MAX;
	static int32_t t39 = 3824;

    t39 = (((x157^x158)>x159)|x160);

    if (t39 != 255) { NG(); } else { ; }
	
}

void f40(void) {
    	static int8_t x161 = INT8_MIN;
	int16_t x162 = 0;
	volatile int32_t x163 = INT32_MAX;
	int32_t x164 = INT32_MAX;

    t40 = (((x161^x162)>x163)|x164);

    if (t40 != INT32_MAX) { NG(); } else { ; }
	
}

void f41(void) {
    	volatile int32_t x165 = INT32_MIN;
	int32_t x166 = -1851505;
	int64_t x167 = INT64_MAX;
	uint16_t x168 = 1933U;
	int32_t t41 = 384591;

    t41 = (((x165^x166)>x167)|x168);

    if (t41 != 1933) { NG(); } else { ; }
	
}

void f42(void) {
    	int8_t x169 = INT8_MAX;
	volatile uint8_t x170 = UINT8_MAX;
	int64_t x171 = -1456728194170LL;
	int64_t x172 = -1LL;

    t42 = (((x169^x170)>x171)|x172);

    if (t42 != -1LL) { NG(); } else { ; }
	
}

void f43(void) {
    	static int16_t x173 = 0;
	int64_t x174 = 135935818LL;
	volatile int64_t x175 = INT64_MAX;
	static int16_t x176 = -5170;

    t43 = (((x173^x174)>x175)|x176);

    if (t43 != -5170) { NG(); } else { ; }
	
}

void f44(void) {
    	int8_t x178 = -1;
	volatile int16_t x179 = -1;
	int8_t x180 = INT8_MIN;

    t44 = (((x177^x178)>x179)|x180);

    if (t44 != -128) { NG(); } else { ; }
	
}

void f45(void) {
    	int64_t x181 = INT64_MIN;
	int64_t x182 = -18940559400499994LL;
	static int16_t x183 = INT16_MAX;
	static int32_t t45 = 1;

    t45 = (((x181^x182)>x183)|x184);

    if (t45 != -2147483647) { NG(); } else { ; }
	
}

void f46(void) {
    	volatile int32_t x186 = 15;
	int8_t x187 = 2;
	int32_t x188 = INT32_MAX;

    t46 = (((x185^x186)>x187)|x188);

    if (t46 != INT32_MAX) { NG(); } else { ; }
	
}

void f47(void) {
    	uint32_t x189 = UINT32_MAX;
	int64_t x190 = INT64_MIN;
	int16_t x191 = -822;
	int8_t x192 = -1;
	int32_t t47 = 1;

    t47 = (((x189^x190)>x191)|x192);

    if (t47 != -1) { NG(); } else { ; }
	
}

void f48(void) {
    	static int8_t x193 = -1;
	uint32_t x194 = 91039887U;
	static int64_t x196 = -104348713502LL;
	volatile int64_t t48 = 4830714553LL;

    t48 = (((x193^x194)>x195)|x196);

    if (t48 != -104348713502LL) { NG(); } else { ; }
	
}

void f49(void) {
    	int8_t x197 = -1;
	uint16_t x198 = UINT16_MAX;
	int8_t x199 = INT8_MIN;
	int16_t x200 = INT16_MAX;
	int32_t t49 = 6396327;

    t49 = (((x197^x198)>x199)|x200);

    if (t49 != 32767) { NG(); } else { ; }
	
}

void f50(void) {
    	static uint8_t x202 = 0U;
	int8_t x204 = INT8_MAX;
	int32_t t50 = 10;

    t50 = (((x201^x202)>x203)|x204);

    if (t50 != 127) { NG(); } else { ; }
	
}

void f51(void) {
    	static int8_t x206 = -1;
	int8_t x207 = 31;
	int16_t x208 = -1;

    t51 = (((x205^x206)>x207)|x208);

    if (t51 != -1) { NG(); } else { ; }
	
}

void f52(void) {
    	volatile int32_t x209 = -16;
	volatile int16_t x210 = INT16_MIN;
	volatile int32_t x212 = -1;

    t52 = (((x209^x210)>x211)|x212);

    if (t52 != -1) { NG(); } else { ; }
	
}

void f53(void) {
    	uint16_t x213 = 172U;
	int32_t x214 = 252555711;
	volatile int16_t x215 = INT16_MIN;
	int8_t x216 = INT8_MAX;
	static int32_t t53 = -3554712;

    t53 = (((x213^x214)>x215)|x216);

    if (t53 != 127) { NG(); } else { ; }
	
}

void f54(void) {
    	int32_t x217 = INT32_MIN;
	int32_t x218 = 1;
	int64_t x219 = 4108445361296458LL;
	volatile int64_t x220 = 705005LL;
	volatile int64_t t54 = 460936LL;

    t54 = (((x217^x218)>x219)|x220);

    if (t54 != 705005LL) { NG(); } else { ; }
	
}

void f55(void) {
    	volatile int16_t x221 = -1;
	int64_t x223 = INT64_MIN;
	int64_t x224 = INT64_MIN;
	volatile int64_t t55 = -13006264727141LL;

    t55 = (((x221^x222)>x223)|x224);

    if (t55 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f56(void) {
    	uint32_t x225 = UINT32_MAX;
	uint64_t x227 = UINT64_MAX;
	int8_t x228 = INT8_MIN;
	static int32_t t56 = -107;

    t56 = (((x225^x226)>x227)|x228);

    if (t56 != -128) { NG(); } else { ; }
	
}

void f57(void) {
    	int32_t x229 = INT32_MIN;
	volatile uint8_t x230 = 13U;
	static int8_t x231 = 23;
	int64_t x232 = INT64_MAX;

    t57 = (((x229^x230)>x231)|x232);

    if (t57 != INT64_MAX) { NG(); } else { ; }
	
}

void f58(void) {
    	int64_t x233 = -1289863440406969419LL;
	volatile int32_t x234 = INT32_MAX;
	uint16_t x235 = 62U;
	static int64_t x236 = INT64_MIN;
	volatile int64_t t58 = INT64_MIN;

    t58 = (((x233^x234)>x235)|x236);

    if (t58 != INT64_MIN) { NG(); } else { ; }
	
}

void f59(void) {
    	int64_t x239 = -157222149901772LL;
	int16_t x240 = INT16_MAX;
	volatile int32_t t59 = 3440700;

    t59 = (((x237^x238)>x239)|x240);

    if (t59 != 32767) { NG(); } else { ; }
	
}

void f60(void) {
    	int64_t x241 = 1LL;
	static uint16_t x242 = 23U;
	uint64_t t60 = 5870663LLU;

    t60 = (((x241^x242)>x243)|x244);

    if (t60 != 3535764177LLU) { NG(); } else { ; }
	
}

void f61(void) {
    	uint8_t x245 = 14U;
	int8_t x247 = INT8_MAX;

    t61 = (((x245^x246)>x247)|x248);

    if (t61 != 5) { NG(); } else { ; }
	
}

void f62(void) {
    	uint16_t x249 = UINT16_MAX;
	static int16_t x250 = INT16_MIN;
	int64_t x252 = -1LL;
	volatile int64_t t62 = 16462482LL;

    t62 = (((x249^x250)>x251)|x252);

    if (t62 != -1LL) { NG(); } else { ; }
	
}

void f63(void) {
    	static int32_t x254 = INT32_MIN;
	int64_t x255 = INT64_MIN;
	volatile int16_t x256 = INT16_MIN;
	int32_t t63 = -40;

    t63 = (((x253^x254)>x255)|x256);

    if (t63 != -32767) { NG(); } else { ; }
	
}

void f64(void) {
    	int8_t x257 = 6;
	int16_t x258 = INT16_MIN;
	static volatile uint8_t x259 = 61U;
	int64_t x260 = INT64_MIN;
	volatile int64_t t64 = INT64_MIN;

    t64 = (((x257^x258)>x259)|x260);

    if (t64 != INT64_MIN) { NG(); } else { ; }
	
}

void f65(void) {
    	int32_t x261 = 307184515;
	int32_t x262 = INT32_MIN;
	static int64_t x263 = 1LL;
	int64_t x264 = -1LL;
	static int64_t t65 = 31117001759LL;

    t65 = (((x261^x262)>x263)|x264);

    if (t65 != -1LL) { NG(); } else { ; }
	
}

void f66(void) {
    	uint8_t x266 = UINT8_MAX;
	int16_t x267 = 16;
	static uint8_t x268 = UINT8_MAX;
	volatile int32_t t66 = -5673776;

    t66 = (((x265^x266)>x267)|x268);

    if (t66 != 255) { NG(); } else { ; }
	
}

void f67(void) {
    	uint32_t x269 = 16U;
	int8_t x270 = -1;
	int16_t x272 = -1;
	volatile int32_t t67 = -400;

    t67 = (((x269^x270)>x271)|x272);

    if (t67 != -1) { NG(); } else { ; }
	
}

void f68(void) {
    	int16_t x273 = INT16_MIN;
	volatile uint64_t x275 = UINT64_MAX;
	int32_t x276 = -1;
	volatile int32_t t68 = 65297489;

    t68 = (((x273^x274)>x275)|x276);

    if (t68 != -1) { NG(); } else { ; }
	
}

void f69(void) {
    	static int32_t x278 = -1;
	static int32_t t69 = 31721055;

    t69 = (((x277^x278)>x279)|x280);

    if (t69 != 32767) { NG(); } else { ; }
	
}

void f70(void) {
    	uint32_t x282 = UINT32_MAX;
	uint8_t x283 = 0U;
	int8_t x284 = 1;
	int32_t t70 = 5271750;

    t70 = (((x281^x282)>x283)|x284);

    if (t70 != 1) { NG(); } else { ; }
	
}

void f71(void) {
    	volatile uint16_t x285 = 25741U;
	int8_t x288 = 3;

    t71 = (((x285^x286)>x287)|x288);

    if (t71 != 3) { NG(); } else { ; }
	
}

void f72(void) {
    	static uint8_t x289 = UINT8_MAX;
	static volatile uint16_t x290 = 147U;
	uint8_t x291 = 0U;
	int64_t x292 = INT64_MAX;
	volatile int64_t t72 = INT64_MAX;

    t72 = (((x289^x290)>x291)|x292);

    if (t72 != INT64_MAX) { NG(); } else { ; }
	
}

void f73(void) {
    	uint64_t x293 = UINT64_MAX;
	uint64_t x294 = 6671LLU;
	static volatile int64_t x295 = -1LL;
	int32_t t73 = -817407;

    t73 = (((x293^x294)>x295)|x296);

    if (t73 != 219) { NG(); } else { ; }
	
}

void f74(void) {
    	int8_t x298 = -5;
	uint8_t x299 = 49U;
	int16_t x300 = INT16_MAX;
	static int32_t t74 = 2346271;

    t74 = (((x297^x298)>x299)|x300);

    if (t74 != 32767) { NG(); } else { ; }
	
}

void f75(void) {
    	int64_t x301 = -1LL;
	int8_t x303 = 0;
	static int8_t x304 = 0;

    t75 = (((x301^x302)>x303)|x304);

    if (t75 != 1) { NG(); } else { ; }
	
}

void f76(void) {
    	volatile int64_t x306 = -80019807357972LL;
	uint32_t x308 = UINT32_MAX;
	volatile uint32_t t76 = UINT32_MAX;

    t76 = (((x305^x306)>x307)|x308);

    if (t76 != UINT32_MAX) { NG(); } else { ; }
	
}

void f77(void) {
    	int64_t x310 = -3435509LL;
	int32_t x311 = -1;
	int8_t x312 = INT8_MIN;
	volatile int32_t t77 = 4009286;

    t77 = (((x309^x310)>x311)|x312);

    if (t77 != -127) { NG(); } else { ; }
	
}

void f78(void) {
    	volatile int32_t x313 = -1;
	int16_t x314 = -4;
	static int16_t x315 = INT16_MAX;
	int64_t x316 = -1LL;
	static int64_t t78 = -950643263445457LL;

    t78 = (((x313^x314)>x315)|x316);

    if (t78 != -1LL) { NG(); } else { ; }
	
}

void f79(void) {
    	int32_t x317 = INT32_MAX;
	int16_t x318 = INT16_MIN;
	int16_t x319 = 920;
	int32_t x320 = INT32_MIN;
	volatile int32_t t79 = INT32_MIN;

    t79 = (((x317^x318)>x319)|x320);

    if (t79 != INT32_MIN) { NG(); } else { ; }
	
}

void f80(void) {
    	int64_t x321 = 10725406739LL;
	volatile uint32_t x324 = UINT32_MAX;
	volatile uint32_t t80 = UINT32_MAX;

    t80 = (((x321^x322)>x323)|x324);

    if (t80 != UINT32_MAX) { NG(); } else { ; }
	
}

void f81(void) {
    	static uint32_t x325 = 286946U;
	volatile int32_t x326 = INT32_MAX;
	static int64_t x327 = 1LL;
	static int32_t x328 = INT32_MIN;
	volatile int32_t t81 = 192820652;

    t81 = (((x325^x326)>x327)|x328);

    if (t81 != -2147483647) { NG(); } else { ; }
	
}

void f82(void) {
    	uint16_t x329 = UINT16_MAX;
	volatile int16_t x330 = INT16_MIN;
	int32_t x331 = INT32_MIN;
	int64_t x332 = 14049328LL;
	int64_t t82 = 59689LL;

    t82 = (((x329^x330)>x331)|x332);

    if (t82 != 14049329LL) { NG(); } else { ; }
	
}

void f83(void) {
    	volatile uint16_t x333 = UINT16_MAX;
	static uint64_t x334 = 2143969443546LLU;
	int16_t x335 = -1;
	int64_t x336 = 23032634819LL;
	int64_t t83 = -71LL;

    t83 = (((x333^x334)>x335)|x336);

    if (t83 != 23032634819LL) { NG(); } else { ; }
	
}

void f84(void) {
    	static int16_t x337 = INT16_MIN;
	int16_t x339 = -35;
	uint64_t x340 = UINT64_MAX;
	static uint64_t t84 = UINT64_MAX;

    t84 = (((x337^x338)>x339)|x340);

    if (t84 != UINT64_MAX) { NG(); } else { ; }
	
}

void f85(void) {
    	int64_t x341 = -1LL;
	int8_t x342 = 2;
	uint32_t x343 = 44013U;
	static volatile int16_t x344 = 0;
	static volatile int32_t t85 = 16059;

    t85 = (((x341^x342)>x343)|x344);

    if (t85 != 0) { NG(); } else { ; }
	
}

void f86(void) {
    	uint16_t x345 = 1281U;
	int64_t x348 = 4752221256065LL;
	volatile int64_t t86 = 24846LL;

    t86 = (((x345^x346)>x347)|x348);

    if (t86 != 4752221256065LL) { NG(); } else { ; }
	
}

void f87(void) {
    	static int16_t x350 = INT16_MIN;
	int64_t x351 = INT64_MIN;
	uint16_t x352 = UINT16_MAX;
	static int32_t t87 = 6;

    t87 = (((x349^x350)>x351)|x352);

    if (t87 != 65535) { NG(); } else { ; }
	
}

void f88(void) {
    	int64_t x353 = -1LL;
	int16_t x354 = 278;
	uint32_t x356 = 5592U;
	static uint32_t t88 = 4866U;

    t88 = (((x353^x354)>x355)|x356);

    if (t88 != 5592U) { NG(); } else { ; }
	
}

void f89(void) {
    	volatile int16_t x357 = -28;
	int8_t x358 = INT8_MIN;
	uint32_t x360 = 884252U;

    t89 = (((x357^x358)>x359)|x360);

    if (t89 != 884253U) { NG(); } else { ; }
	
}

void f90(void) {
    	int16_t x361 = -1710;
	static volatile int16_t x363 = -6799;
	volatile int16_t x364 = INT16_MIN;
	int32_t t90 = 4;

    t90 = (((x361^x362)>x363)|x364);

    if (t90 != -32767) { NG(); } else { ; }
	
}

void f91(void) {
    	int64_t x365 = -1LL;
	uint32_t x366 = 5353700U;
	static uint32_t x367 = 665991111U;
	uint64_t x368 = 641201656229028394LLU;

    t91 = (((x365^x366)>x367)|x368);

    if (t91 != 641201656229028394LLU) { NG(); } else { ; }
	
}

void f92(void) {
    	int32_t x369 = 66394804;
	int16_t x370 = INT16_MIN;
	int32_t x371 = INT32_MAX;
	int16_t x372 = INT16_MAX;
	int32_t t92 = -25061;

    t92 = (((x369^x370)>x371)|x372);

    if (t92 != 32767) { NG(); } else { ; }
	
}

void f93(void) {
    	uint8_t x374 = UINT8_MAX;
	int32_t x375 = INT32_MIN;
	static int8_t x376 = INT8_MAX;
	int32_t t93 = -1956;

    t93 = (((x373^x374)>x375)|x376);

    if (t93 != 127) { NG(); } else { ; }
	
}

void f94(void) {
    	int16_t x377 = -1;
	int16_t x378 = INT16_MIN;
	int16_t x379 = -402;
	volatile uint64_t x380 = UINT64_MAX;
	uint64_t t94 = UINT64_MAX;

    t94 = (((x377^x378)>x379)|x380);

    if (t94 != UINT64_MAX) { NG(); } else { ; }
	
}

void f95(void) {
    	int32_t x381 = INT32_MIN;
	static uint32_t x383 = 130366U;
	int32_t t95 = 216772;

    t95 = (((x381^x382)>x383)|x384);

    if (t95 != -1) { NG(); } else { ; }
	
}

void f96(void) {
    	volatile int64_t x385 = -6108439428439549LL;
	volatile int16_t x386 = -1;
	int64_t x387 = INT64_MAX;
	volatile int64_t x388 = INT64_MIN;
	static volatile int64_t t96 = INT64_MIN;

    t96 = (((x385^x386)>x387)|x388);

    if (t96 != INT64_MIN) { NG(); } else { ; }
	
}

void f97(void) {
    	int32_t x389 = INT32_MIN;
	volatile uint32_t x391 = 8069U;
	int32_t x392 = 4824;
	volatile int32_t t97 = 0;

    t97 = (((x389^x390)>x391)|x392);

    if (t97 != 4825) { NG(); } else { ; }
	
}

void f98(void) {
    	int8_t x394 = INT8_MIN;
	uint64_t x395 = UINT64_MAX;
	static uint8_t x396 = 15U;
	volatile int32_t t98 = -749107;

    t98 = (((x393^x394)>x395)|x396);

    if (t98 != 15) { NG(); } else { ; }
	
}

void f99(void) {
    	static uint64_t x397 = 240LLU;
	volatile int8_t x398 = INT8_MAX;
	int64_t x400 = -58959722527LL;
	int64_t t99 = 1357222942623119LL;

    t99 = (((x397^x398)>x399)|x400);

    if (t99 != -58959722527LL) { NG(); } else { ; }
	
}

void f100(void) {
    	uint32_t x401 = 65512U;
	volatile int8_t x402 = INT8_MIN;
	int32_t x403 = -1;
	uint32_t x404 = 895405U;
	volatile uint32_t t100 = 5U;

    t100 = (((x401^x402)>x403)|x404);

    if (t100 != 895405U) { NG(); } else { ; }
	
}

void f101(void) {
    	int16_t x405 = INT16_MIN;
	uint16_t x406 = UINT16_MAX;
	int16_t x407 = INT16_MAX;
	int8_t x408 = 29;
	int32_t t101 = 1036;

    t101 = (((x405^x406)>x407)|x408);

    if (t101 != 29) { NG(); } else { ; }
	
}

void f102(void) {
    	uint32_t x409 = 33007545U;
	uint64_t x410 = UINT64_MAX;
	int8_t x411 = -3;
	int32_t x412 = INT32_MIN;
	volatile int32_t t102 = INT32_MIN;

    t102 = (((x409^x410)>x411)|x412);

    if (t102 != INT32_MIN) { NG(); } else { ; }
	
}

void f103(void) {
    	uint32_t x413 = UINT32_MAX;
	uint32_t x414 = UINT32_MAX;
	int32_t x415 = INT32_MIN;
	int8_t x416 = -15;
	static volatile int32_t t103 = -53809081;

    t103 = (((x413^x414)>x415)|x416);

    if (t103 != -15) { NG(); } else { ; }
	
}

void f104(void) {
    	uint64_t x417 = UINT64_MAX;
	uint32_t x418 = 360U;
	volatile int16_t x419 = 0;
	int16_t x420 = -1;

    t104 = (((x417^x418)>x419)|x420);

    if (t104 != -1) { NG(); } else { ; }
	
}

void f105(void) {
    	uint16_t x421 = 366U;
	static uint16_t x423 = 1749U;
	static int32_t t105 = INT32_MIN;

    t105 = (((x421^x422)>x423)|x424);

    if (t105 != INT32_MIN) { NG(); } else { ; }
	
}

void f106(void) {
    	static volatile int32_t x425 = INT32_MAX;
	int16_t x426 = -792;
	static int16_t x427 = -8665;
	int8_t x428 = -25;

    t106 = (((x425^x426)>x427)|x428);

    if (t106 != -25) { NG(); } else { ; }
	
}

void f107(void) {
    	int64_t x429 = -66779572LL;
	static int16_t x430 = -21;
	uint32_t x431 = 23885219U;
	static volatile uint32_t t107 = UINT32_MAX;

    t107 = (((x429^x430)>x431)|x432);

    if (t107 != UINT32_MAX) { NG(); } else { ; }
	
}

void f108(void) {
    	uint16_t x434 = 4U;
	int8_t x435 = INT8_MIN;
	int64_t x436 = INT64_MIN;
	int64_t t108 = INT64_MIN;

    t108 = (((x433^x434)>x435)|x436);

    if (t108 != INT64_MIN) { NG(); } else { ; }
	
}

void f109(void) {
    	static int32_t x437 = 468265;
	static int8_t x438 = INT8_MIN;
	volatile int64_t x439 = -4474587LL;
	volatile int32_t t109 = -5;

    t109 = (((x437^x438)>x439)|x440);

    if (t109 != -1) { NG(); } else { ; }
	
}

void f110(void) {
    	static int64_t x441 = -1LL;
	int32_t x442 = 35;
	int8_t x443 = INT8_MAX;
	int64_t x444 = -1LL;
	volatile int64_t t110 = -1310862083756132878LL;

    t110 = (((x441^x442)>x443)|x444);

    if (t110 != -1LL) { NG(); } else { ; }
	
}

void f111(void) {
    	static int8_t x445 = -1;
	volatile int8_t x446 = INT8_MIN;
	int64_t x447 = 40725LL;
	int16_t x448 = -207;
	static int32_t t111 = 85;

    t111 = (((x445^x446)>x447)|x448);

    if (t111 != -207) { NG(); } else { ; }
	
}

void f112(void) {
    	uint64_t x449 = 540340068412LLU;
	int64_t x450 = INT64_MAX;
	int16_t x451 = -1;
	int64_t x452 = INT64_MIN;
	volatile int64_t t112 = INT64_MIN;

    t112 = (((x449^x450)>x451)|x452);

    if (t112 != INT64_MIN) { NG(); } else { ; }
	
}

void f113(void) {
    	int8_t x453 = -18;
	int8_t x454 = INT8_MIN;
	static int16_t x455 = INT16_MAX;
	int32_t x456 = INT32_MIN;
	static int32_t t113 = INT32_MIN;

    t113 = (((x453^x454)>x455)|x456);

    if (t113 != INT32_MIN) { NG(); } else { ; }
	
}

void f114(void) {
    	static int64_t x457 = INT64_MIN;
	volatile uint16_t x458 = 1U;
	int8_t x459 = INT8_MIN;
	volatile int64_t t114 = INT64_MIN;

    t114 = (((x457^x458)>x459)|x460);

    if (t114 != INT64_MIN) { NG(); } else { ; }
	
}

void f115(void) {
    	int16_t x461 = -1;
	volatile int16_t x462 = INT16_MAX;
	uint32_t x464 = 3591165U;

    t115 = (((x461^x462)>x463)|x464);

    if (t115 != 3591165U) { NG(); } else { ; }
	
}

void f116(void) {
    	int8_t x467 = -61;
	int8_t x468 = INT8_MIN;

    t116 = (((x465^x466)>x467)|x468);

    if (t116 != -127) { NG(); } else { ; }
	
}

void f117(void) {
    	volatile int8_t x469 = 5;
	int16_t x471 = INT16_MAX;
	static volatile int64_t t117 = 1503780LL;

    t117 = (((x469^x470)>x471)|x472);

    if (t117 != 101800265LL) { NG(); } else { ; }
	
}

void f118(void) {
    	int32_t x473 = INT32_MAX;
	uint16_t x474 = 446U;
	int64_t x475 = INT64_MAX;
	static int64_t x476 = INT64_MIN;
	int64_t t118 = INT64_MIN;

    t118 = (((x473^x474)>x475)|x476);

    if (t118 != INT64_MIN) { NG(); } else { ; }
	
}

void f119(void) {
    	int32_t x477 = -125;
	int32_t x478 = 1926;
	int64_t x479 = -1LL;
	int64_t x480 = INT64_MIN;
	volatile int64_t t119 = INT64_MIN;

    t119 = (((x477^x478)>x479)|x480);

    if (t119 != INT64_MIN) { NG(); } else { ; }
	
}

void f120(void) {
    	volatile uint64_t x481 = UINT64_MAX;
	static int64_t x482 = INT64_MIN;
	int8_t x483 = -1;
	static uint8_t x484 = 52U;
	int32_t t120 = -82;

    t120 = (((x481^x482)>x483)|x484);

    if (t120 != 52) { NG(); } else { ; }
	
}

void f121(void) {
    	int32_t x485 = INT32_MAX;
	volatile int64_t x486 = INT64_MAX;
	int64_t x487 = INT64_MIN;
	uint8_t x488 = 67U;

    t121 = (((x485^x486)>x487)|x488);

    if (t121 != 67) { NG(); } else { ; }
	
}

void f122(void) {
    	uint64_t x490 = UINT64_MAX;
	int8_t x491 = -1;
	volatile int32_t t122 = 20;

    t122 = (((x489^x490)>x491)|x492);

    if (t122 != 1) { NG(); } else { ; }
	
}

void f123(void) {
    	int64_t x493 = -1LL;
	volatile int16_t x494 = -1;
	int8_t x495 = -1;
	int64_t x496 = INT64_MIN;

    t123 = (((x493^x494)>x495)|x496);

    if (t123 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f124(void) {
    	int64_t x497 = -3753300436060LL;
	static volatile int64_t x498 = INT64_MIN;
	static int64_t x499 = 143950LL;
	volatile int32_t t124 = 5982;

    t124 = (((x497^x498)>x499)|x500);

    if (t124 != -127) { NG(); } else { ; }
	
}

void f125(void) {
    	int32_t x502 = INT32_MIN;
	volatile uint8_t x503 = 12U;
	int32_t x504 = INT32_MIN;
	volatile int32_t t125 = INT32_MIN;

    t125 = (((x501^x502)>x503)|x504);

    if (t125 != INT32_MIN) { NG(); } else { ; }
	
}

void f126(void) {
    	uint64_t x505 = 909501732615097553LLU;
	volatile uint64_t x508 = 320060463897600LLU;
	static uint64_t t126 = 28958823026LLU;

    t126 = (((x505^x506)>x507)|x508);

    if (t126 != 320060463897601LLU) { NG(); } else { ; }
	
}

void f127(void) {
    	int32_t x510 = 11;
	int32_t x512 = INT32_MIN;
	int32_t t127 = -5135378;

    t127 = (((x509^x510)>x511)|x512);

    if (t127 != -2147483647) { NG(); } else { ; }
	
}

void f128(void) {
    	uint16_t x513 = UINT16_MAX;
	int32_t x514 = -59482;
	int16_t x515 = -1;
	volatile uint8_t x516 = UINT8_MAX;
	volatile int32_t t128 = 1662304;

    t128 = (((x513^x514)>x515)|x516);

    if (t128 != 255) { NG(); } else { ; }
	
}

void f129(void) {
    	int64_t x518 = -1LL;
	static uint32_t x519 = UINT32_MAX;
	uint64_t x520 = UINT64_MAX;

    t129 = (((x517^x518)>x519)|x520);

    if (t129 != UINT64_MAX) { NG(); } else { ; }
	
}

void f130(void) {
    	uint64_t x521 = 874668547LLU;
	volatile uint8_t x522 = UINT8_MAX;
	uint16_t x523 = 59U;
	int8_t x524 = 8;
	int32_t t130 = 0;

    t130 = (((x521^x522)>x523)|x524);

    if (t130 != 9) { NG(); } else { ; }
	
}

void f131(void) {
    	uint16_t x525 = 12780U;
	static volatile uint32_t x526 = 10U;
	static int8_t x527 = INT8_MIN;
	int16_t x528 = -779;

    t131 = (((x525^x526)>x527)|x528);

    if (t131 != -779) { NG(); } else { ; }
	
}

void f132(void) {
    	volatile int32_t x529 = INT32_MIN;
	int32_t x530 = 2730;
	static volatile int32_t t132 = -1;

    t132 = (((x529^x530)>x531)|x532);

    if (t132 != -32768) { NG(); } else { ; }
	
}

void f133(void) {
    	volatile uint8_t x533 = 1U;
	uint64_t x534 = UINT64_MAX;
	uint8_t x535 = UINT8_MAX;
	uint32_t x536 = UINT32_MAX;
	uint32_t t133 = UINT32_MAX;

    t133 = (((x533^x534)>x535)|x536);

    if (t133 != UINT32_MAX) { NG(); } else { ; }
	
}

void f134(void) {
    	volatile int16_t x537 = INT16_MIN;
	static uint8_t x538 = 2U;
	int8_t x539 = INT8_MIN;
	int8_t x540 = INT8_MIN;

    t134 = (((x537^x538)>x539)|x540);

    if (t134 != -128) { NG(); } else { ; }
	
}

void f135(void) {
    	volatile int8_t x541 = -4;
	uint16_t x542 = 2U;
	int16_t x543 = INT16_MAX;

    t135 = (((x541^x542)>x543)|x544);

    if (t135 != INT64_MIN) { NG(); } else { ; }
	
}

void f136(void) {
    	static int64_t x545 = -1LL;
	int16_t x547 = INT16_MAX;
	int32_t t136 = 130356205;

    t136 = (((x545^x546)>x547)|x548);

    if (t136 != 67) { NG(); } else { ; }
	
}

void f137(void) {
    	uint16_t x550 = UINT16_MAX;
	int8_t x551 = INT8_MIN;
	int32_t x552 = -1;
	volatile int32_t t137 = 1773;

    t137 = (((x549^x550)>x551)|x552);

    if (t137 != -1) { NG(); } else { ; }
	
}

void f138(void) {
    	volatile int16_t x553 = INT16_MIN;
	int16_t x554 = -1;
	int16_t x555 = -1;
	static uint8_t x556 = 2U;
	volatile int32_t t138 = -3624288;

    t138 = (((x553^x554)>x555)|x556);

    if (t138 != 3) { NG(); } else { ; }
	
}

void f139(void) {
    	int32_t x557 = INT32_MIN;
	int32_t x559 = INT32_MIN;
	static int16_t x560 = -1;
	volatile int32_t t139 = 3160;

    t139 = (((x557^x558)>x559)|x560);

    if (t139 != -1) { NG(); } else { ; }
	
}

void f140(void) {
    	uint16_t x561 = UINT16_MAX;
	uint16_t x562 = UINT16_MAX;

    t140 = (((x561^x562)>x563)|x564);

    if (t140 != 255) { NG(); } else { ; }
	
}

void f141(void) {
    	int64_t x565 = INT64_MIN;
	static int8_t x566 = 0;
	static int32_t x567 = -24;

    t141 = (((x565^x566)>x567)|x568);

    if (t141 != UINT64_MAX) { NG(); } else { ; }
	
}

void f142(void) {
    	static int64_t x569 = 119744325260642LL;
	static int16_t x570 = INT16_MIN;
	int32_t x571 = -507116828;
	static int32_t x572 = INT32_MIN;

    t142 = (((x569^x570)>x571)|x572);

    if (t142 != INT32_MIN) { NG(); } else { ; }
	
}

void f143(void) {
    	volatile int32_t x573 = INT32_MIN;
	int32_t x574 = -1;
	static int64_t x575 = INT64_MAX;
	volatile int8_t x576 = INT8_MIN;

    t143 = (((x573^x574)>x575)|x576);

    if (t143 != -128) { NG(); } else { ; }
	
}

void f144(void) {
    	static uint32_t x577 = 14565U;
	static int8_t x578 = -5;
	int16_t x579 = INT16_MAX;
	int32_t x580 = -1;

    t144 = (((x577^x578)>x579)|x580);

    if (t144 != -1) { NG(); } else { ; }
	
}

void f145(void) {
    	int32_t x581 = -8767477;
	uint8_t x582 = 7U;
	int16_t x583 = INT16_MIN;
	int64_t x584 = INT64_MAX;
	int64_t t145 = INT64_MAX;

    t145 = (((x581^x582)>x583)|x584);

    if (t145 != INT64_MAX) { NG(); } else { ; }
	
}

void f146(void) {
    	int64_t x585 = -1LL;
	int8_t x586 = -2;
	int64_t x587 = INT64_MIN;
	volatile int32_t t146 = -37;

    t146 = (((x585^x586)>x587)|x588);

    if (t146 != 127) { NG(); } else { ; }
	
}

void f147(void) {
    	volatile int32_t t147 = 1818;

    t147 = (((x589^x590)>x591)|x592);

    if (t147 != 2) { NG(); } else { ; }
	
}

void f148(void) {
    	int16_t x593 = -1;
	int32_t x594 = -188879066;
	static volatile uint32_t x595 = UINT32_MAX;
	int32_t t148 = -108421;

    t148 = (((x593^x594)>x595)|x596);

    if (t148 != 127) { NG(); } else { ; }
	
}

void f149(void) {
    	int16_t x598 = INT16_MIN;
	int16_t x599 = 5;
	int32_t x600 = -1;
	int32_t t149 = -1;

    t149 = (((x597^x598)>x599)|x600);

    if (t149 != -1) { NG(); } else { ; }
	
}

void f150(void) {
    	uint32_t x601 = 3542U;
	uint64_t x602 = UINT64_MAX;
	int32_t x603 = 4909531;
	static uint8_t x604 = 51U;
	static volatile int32_t t150 = -241747;

    t150 = (((x601^x602)>x603)|x604);

    if (t150 != 51) { NG(); } else { ; }
	
}

void f151(void) {
    	int16_t x605 = INT16_MAX;
	uint32_t x606 = UINT32_MAX;
	int64_t x608 = -1LL;
	static volatile int64_t t151 = -21LL;

    t151 = (((x605^x606)>x607)|x608);

    if (t151 != -1LL) { NG(); } else { ; }
	
}

void f152(void) {
    	int32_t x609 = -1;
	int32_t x610 = INT32_MAX;
	static uint8_t x611 = 6U;
	volatile int32_t x612 = 93;
	int32_t t152 = -1;

    t152 = (((x609^x610)>x611)|x612);

    if (t152 != 93) { NG(); } else { ; }
	
}

void f153(void) {
    	int16_t x613 = INT16_MIN;
	int64_t x614 = INT64_MIN;
	uint32_t x615 = 13582048U;
	volatile uint16_t x616 = 5U;
	static int32_t t153 = 0;

    t153 = (((x613^x614)>x615)|x616);

    if (t153 != 5) { NG(); } else { ; }
	
}

void f154(void) {
    	volatile int8_t x618 = INT8_MAX;
	int64_t x619 = INT64_MAX;
	uint16_t x620 = 5408U;

    t154 = (((x617^x618)>x619)|x620);

    if (t154 != 5408) { NG(); } else { ; }
	
}

void f155(void) {
    	int8_t x621 = INT8_MIN;
	static uint32_t x622 = 245U;
	static int64_t x623 = INT64_MIN;
	static volatile int64_t t155 = 5636924LL;

    t155 = (((x621^x622)>x623)|x624);

    if (t155 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f156(void) {
    	volatile int64_t x625 = -1LL;
	volatile int32_t x627 = INT32_MIN;
	static int64_t x628 = INT64_MIN;
	static int64_t t156 = -368670671794889240LL;

    t156 = (((x625^x626)>x627)|x628);

    if (t156 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f157(void) {
    	int8_t x629 = -1;
	uint32_t x630 = 2654497U;
	volatile uint16_t x631 = UINT16_MAX;
	uint32_t x632 = 288850910U;
	volatile uint32_t t157 = 3453U;

    t157 = (((x629^x630)>x631)|x632);

    if (t157 != 288850911U) { NG(); } else { ; }
	
}

void f158(void) {
    	uint16_t x633 = UINT16_MAX;
	static volatile int16_t x635 = INT16_MIN;
	uint64_t x636 = 11LLU;

    t158 = (((x633^x634)>x635)|x636);

    if (t158 != 11LLU) { NG(); } else { ; }
	
}

void f159(void) {
    	int8_t x637 = -11;
	volatile uint8_t x638 = 92U;
	int32_t x639 = 984;
	uint16_t x640 = 24U;
	int32_t t159 = -449949;

    t159 = (((x637^x638)>x639)|x640);

    if (t159 != 24) { NG(); } else { ; }
	
}

void f160(void) {
    	int16_t x642 = 1;
	int64_t x643 = 19471LL;

    t160 = (((x641^x642)>x643)|x644);

    if (t160 != -2147483647) { NG(); } else { ; }
	
}

void f161(void) {
    	int64_t x645 = INT64_MIN;
	int32_t x646 = INT32_MIN;
	uint32_t x647 = 3378U;
	volatile int32_t x648 = -1;
	static int32_t t161 = 7;

    t161 = (((x645^x646)>x647)|x648);

    if (t161 != -1) { NG(); } else { ; }
	
}

void f162(void) {
    	volatile int8_t x649 = 7;
	int32_t x650 = -1;
	uint32_t x651 = 126U;
	uint16_t x652 = 1U;
	int32_t t162 = 45;

    t162 = (((x649^x650)>x651)|x652);

    if (t162 != 1) { NG(); } else { ; }
	
}

void f163(void) {
    	int8_t x653 = -1;
	uint64_t x655 = 490086565850LLU;

    t163 = (((x653^x654)>x655)|x656);

    if (t163 != -1LL) { NG(); } else { ; }
	
}

void f164(void) {
    	volatile int32_t x657 = -38950937;
	static uint8_t x658 = 25U;
	int32_t x660 = INT32_MIN;
	volatile int32_t t164 = INT32_MIN;

    t164 = (((x657^x658)>x659)|x660);

    if (t164 != INT32_MIN) { NG(); } else { ; }
	
}

void f165(void) {
    	uint16_t x661 = UINT16_MAX;
	volatile int64_t x662 = 2104LL;
	volatile int16_t x664 = -1;

    t165 = (((x661^x662)>x663)|x664);

    if (t165 != -1) { NG(); } else { ; }
	
}

void f166(void) {
    	int32_t x665 = 51;
	int32_t x666 = INT32_MAX;
	static int64_t x667 = 9LL;
	int32_t x668 = INT32_MAX;
	int32_t t166 = INT32_MAX;

    t166 = (((x665^x666)>x667)|x668);

    if (t166 != INT32_MAX) { NG(); } else { ; }
	
}

void f167(void) {
    	int64_t x671 = INT64_MAX;
	int64_t t167 = INT64_MAX;

    t167 = (((x669^x670)>x671)|x672);

    if (t167 != INT64_MAX) { NG(); } else { ; }
	
}

void f168(void) {
    	uint16_t x675 = 22U;
	uint8_t x676 = 1U;
	volatile int32_t t168 = 29;

    t168 = (((x673^x674)>x675)|x676);

    if (t168 != 1) { NG(); } else { ; }
	
}

void f169(void) {
    	static int16_t x677 = 3404;
	static int8_t x678 = -1;
	int8_t x679 = INT8_MIN;
	uint16_t x680 = 478U;
	volatile int32_t t169 = 35;

    t169 = (((x677^x678)>x679)|x680);

    if (t169 != 478) { NG(); } else { ; }
	
}

void f170(void) {
    	static int32_t x681 = 0;
	int32_t x682 = INT32_MIN;
	uint64_t x683 = UINT64_MAX;
	static uint8_t x684 = UINT8_MAX;
	static volatile int32_t t170 = 25742;

    t170 = (((x681^x682)>x683)|x684);

    if (t170 != 255) { NG(); } else { ; }
	
}

void f171(void) {
    	uint8_t x685 = UINT8_MAX;
	uint16_t x686 = 162U;
	int8_t x687 = -17;
	uint64_t x688 = 141232111506458LLU;
	uint64_t t171 = 55901383750LLU;

    t171 = (((x685^x686)>x687)|x688);

    if (t171 != 141232111506459LLU) { NG(); } else { ; }
	
}

void f172(void) {
    	int64_t x689 = -213LL;
	int32_t x691 = 10442747;
	int8_t x692 = INT8_MIN;

    t172 = (((x689^x690)>x691)|x692);

    if (t172 != -127) { NG(); } else { ; }
	
}

void f173(void) {
    	static uint32_t x693 = 3U;
	int16_t x694 = INT16_MIN;
	uint64_t x695 = UINT64_MAX;
	uint8_t x696 = UINT8_MAX;
	static volatile int32_t t173 = -1476;

    t173 = (((x693^x694)>x695)|x696);

    if (t173 != 255) { NG(); } else { ; }
	
}

void f174(void) {
    	uint32_t x697 = UINT32_MAX;
	int16_t x699 = -1;
	int16_t x700 = -1;
	static volatile int32_t t174 = 89975778;

    t174 = (((x697^x698)>x699)|x700);

    if (t174 != -1) { NG(); } else { ; }
	
}

void f175(void) {
    	uint8_t x701 = 5U;
	uint16_t x702 = 99U;
	volatile uint32_t x703 = 918U;
	int8_t x704 = -1;
	int32_t t175 = 29371;

    t175 = (((x701^x702)>x703)|x704);

    if (t175 != -1) { NG(); } else { ; }
	
}

void f176(void) {
    	uint64_t x705 = 140518982843722201LLU;
	int32_t x706 = 15777436;
	int16_t x707 = INT16_MIN;
	volatile int32_t t176 = -12956;

    t176 = (((x705^x706)>x707)|x708);

    if (t176 != -128) { NG(); } else { ; }
	
}

void f177(void) {
    	static int32_t x709 = INT32_MIN;
	static uint8_t x710 = UINT8_MAX;
	static uint32_t x711 = UINT32_MAX;
	volatile int32_t x712 = INT32_MIN;
	volatile int32_t t177 = INT32_MIN;

    t177 = (((x709^x710)>x711)|x712);

    if (t177 != INT32_MIN) { NG(); } else { ; }
	
}

void f178(void) {
    	int32_t x713 = -1;
	static int32_t x714 = INT32_MIN;
	uint64_t x715 = 208982784089401078LLU;
	int64_t x716 = INT64_MAX;
	static volatile int64_t t178 = INT64_MAX;

    t178 = (((x713^x714)>x715)|x716);

    if (t178 != INT64_MAX) { NG(); } else { ; }
	
}

void f179(void) {
    	uint32_t x717 = UINT32_MAX;
	static volatile uint32_t x719 = 241330U;
	volatile int64_t t179 = -2361LL;

    t179 = (((x717^x718)>x719)|x720);

    if (t179 != 3LL) { NG(); } else { ; }
	
}

void f180(void) {
    	static uint8_t x721 = 1U;
	int16_t x722 = -1;
	static int32_t t180 = -15482986;

    t180 = (((x721^x722)>x723)|x724);

    if (t180 != -32768) { NG(); } else { ; }
	
}

void f181(void) {
    	static int64_t x725 = 398LL;
	uint8_t x726 = 123U;
	int32_t x727 = -631096982;
	int16_t x728 = INT16_MIN;
	static volatile int32_t t181 = -64587;

    t181 = (((x725^x726)>x727)|x728);

    if (t181 != -32767) { NG(); } else { ; }
	
}

void f182(void) {
    	int8_t x729 = INT8_MIN;
	uint32_t x730 = 1U;
	volatile int64_t x731 = -279519480511982419LL;
	volatile uint64_t x732 = 10632586LLU;
	uint64_t t182 = 121691LLU;

    t182 = (((x729^x730)>x731)|x732);

    if (t182 != 10632587LLU) { NG(); } else { ; }
	
}

void f183(void) {
    	int16_t x733 = -1;
	static int64_t x734 = 0LL;
	static volatile int64_t x735 = -1LL;
	uint8_t x736 = UINT8_MAX;
	volatile int32_t t183 = 46;

    t183 = (((x733^x734)>x735)|x736);

    if (t183 != 255) { NG(); } else { ; }
	
}

void f184(void) {
    	uint16_t x737 = 10517U;
	uint16_t x739 = 31982U;
	volatile uint32_t x740 = UINT32_MAX;
	static volatile uint32_t t184 = UINT32_MAX;

    t184 = (((x737^x738)>x739)|x740);

    if (t184 != UINT32_MAX) { NG(); } else { ; }
	
}

void f185(void) {
    	int16_t x741 = 13640;
	int64_t x742 = INT64_MIN;
	uint64_t x743 = 524034926940LLU;
	volatile int32_t t185 = 9074;

    t185 = (((x741^x742)>x743)|x744);

    if (t185 != -32767) { NG(); } else { ; }
	
}

void f186(void) {
    	uint16_t x745 = 137U;
	static volatile int32_t x748 = INT32_MIN;

    t186 = (((x745^x746)>x747)|x748);

    if (t186 != INT32_MIN) { NG(); } else { ; }
	
}

void f187(void) {
    	static uint32_t x749 = 24360U;
	int16_t x750 = INT16_MIN;
	int32_t t187 = -1550333;

    t187 = (((x749^x750)>x751)|x752);

    if (t187 != -32768) { NG(); } else { ; }
	
}

void f188(void) {
    	static uint16_t x753 = 6802U;
	int64_t x754 = -1LL;
	uint16_t x755 = UINT16_MAX;
	int64_t x756 = 13LL;
	static int64_t t188 = 5545845799137262LL;

    t188 = (((x753^x754)>x755)|x756);

    if (t188 != 13LL) { NG(); } else { ; }
	
}

void f189(void) {
    	int64_t x757 = INT64_MAX;
	static uint32_t x760 = UINT32_MAX;
	volatile uint32_t t189 = UINT32_MAX;

    t189 = (((x757^x758)>x759)|x760);

    if (t189 != UINT32_MAX) { NG(); } else { ; }
	
}

void f190(void) {
    	volatile int16_t x762 = -1;
	int16_t x764 = -1;
	volatile int32_t t190 = 28144;

    t190 = (((x761^x762)>x763)|x764);

    if (t190 != -1) { NG(); } else { ; }
	
}

void f191(void) {
    	int16_t x766 = INT16_MIN;
	uint64_t x767 = UINT64_MAX;
	volatile int32_t t191 = INT32_MIN;

    t191 = (((x765^x766)>x767)|x768);

    if (t191 != INT32_MIN) { NG(); } else { ; }
	
}

void f192(void) {
    	int64_t x769 = 380694461986827383LL;
	static int32_t x770 = INT32_MIN;
	int8_t x771 = -1;
	int64_t x772 = INT64_MIN;

    t192 = (((x769^x770)>x771)|x772);

    if (t192 != INT64_MIN) { NG(); } else { ; }
	
}

void f193(void) {
    	static int8_t x773 = INT8_MAX;
	volatile uint8_t x774 = 9U;
	volatile int16_t x775 = INT16_MIN;
	static int16_t x776 = INT16_MIN;
	static int32_t t193 = -95803;

    t193 = (((x773^x774)>x775)|x776);

    if (t193 != -32767) { NG(); } else { ; }
	
}

void f194(void) {
    	int16_t x778 = INT16_MIN;
	uint32_t x779 = 25U;
	int64_t x780 = -29LL;
	volatile int64_t t194 = 13LL;

    t194 = (((x777^x778)>x779)|x780);

    if (t194 != -29LL) { NG(); } else { ; }
	
}

void f195(void) {
    	uint16_t x782 = UINT16_MAX;
	uint32_t x783 = 31939U;

    t195 = (((x781^x782)>x783)|x784);

    if (t195 != -127) { NG(); } else { ; }
	
}

void f196(void) {
    	int32_t x785 = -3;
	uint32_t x786 = 198843603U;
	int8_t x787 = INT8_MIN;
	int8_t x788 = INT8_MIN;
	int32_t t196 = 0;

    t196 = (((x785^x786)>x787)|x788);

    if (t196 != -128) { NG(); } else { ; }
	
}

void f197(void) {
    	volatile int16_t x789 = -6;
	uint16_t x790 = 61U;
	int32_t x791 = INT32_MAX;
	int32_t t197 = 5690;

    t197 = (((x789^x790)>x791)|x792);

    if (t197 != 357) { NG(); } else { ; }
	
}

void f198(void) {
    	int64_t x793 = -1LL;
	uint32_t x795 = UINT32_MAX;

    t198 = (((x793^x794)>x795)|x796);

    if (t198 != 644707697) { NG(); } else { ; }
	
}

void f199(void) {
    	static uint16_t x798 = 2546U;
	volatile int8_t x799 = -1;
	int16_t x800 = INT16_MIN;
	int32_t t199 = -20;

    t199 = (((x797^x798)>x799)|x800);

    if (t199 != -32767) { NG(); } else { ; }
	
}

int main(void) {
        f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();
    f100();
    f101();
    f102();
    f103();
    f104();
    f105();
    f106();
    f107();
    f108();
    f109();
    f110();
    f111();
    f112();
    f113();
    f114();
    f115();
    f116();
    f117();
    f118();
    f119();
    f120();
    f121();
    f122();
    f123();
    f124();
    f125();
    f126();
    f127();
    f128();
    f129();
    f130();
    f131();
    f132();
    f133();
    f134();
    f135();
    f136();
    f137();
    f138();
    f139();
    f140();
    f141();
    f142();
    f143();
    f144();
    f145();
    f146();
    f147();
    f148();
    f149();
    f150();
    f151();
    f152();
    f153();
    f154();
    f155();
    f156();
    f157();
    f158();
    f159();
    f160();
    f161();
    f162();
    f163();
    f164();
    f165();
    f166();
    f167();
    f168();
    f169();
    f170();
    f171();
    f172();
    f173();
    f174();
    f175();
    f176();
    f177();
    f178();
    f179();
    f180();
    f181();
    f182();
    f183();
    f184();
    f185();
    f186();
    f187();
    f188();
    f189();
    f190();
    f191();
    f192();
    f193();
    f194();
    f195();
    f196();
    f197();
    f198();
    f199();


    return 0;
}

