#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

uint16_t x1 = UINT16_MAX;
volatile int32_t x5 = INT32_MIN;
int64_t x7 = -1LL;
volatile uint64_t t1 = 270256488318LLU;
int32_t x27 = -1;
static int64_t x31 = -1LL;
uint64_t x33 = UINT64_MAX;
uint32_t x42 = 1900U;
int16_t x44 = INT16_MIN;
volatile int32_t t10 = -33802500;
volatile int32_t x45 = -7662016;
uint32_t x49 = UINT32_MAX;
uint32_t x50 = UINT32_MAX;
static int32_t t12 = 2544;
uint8_t x54 = 6U;
int32_t x60 = INT32_MAX;
uint8_t x66 = 3U;
int64_t x68 = INT64_MAX;
int64_t t16 = INT64_MAX;
int8_t x69 = INT8_MIN;
int32_t t17 = 0;
uint16_t x78 = 414U;
int8_t x79 = -1;
uint64_t t19 = 20062LLU;
volatile int32_t t20 = 138096;
uint32_t x88 = 695455U;
int16_t x89 = INT16_MIN;
volatile int8_t x90 = INT8_MAX;
static uint64_t x96 = 1045421682LLU;
static volatile int32_t t24 = 353;
int16_t x103 = INT16_MIN;
volatile int64_t x107 = INT64_MIN;
volatile int32_t t26 = 602120176;
int32_t x118 = INT32_MAX;
int8_t x126 = INT8_MIN;
uint32_t x128 = UINT32_MAX;
volatile int32_t t32 = INT32_MIN;
int8_t x139 = -1;
volatile int8_t x141 = 11;
uint8_t x143 = 27U;
int32_t t35 = INT32_MIN;
int64_t x148 = -488633996760711448LL;
int64_t x151 = 63058424791763LL;
int64_t x152 = INT64_MAX;
int64_t t37 = INT64_MAX;
static volatile int16_t x154 = 1131;
int8_t x155 = 0;
int32_t t38 = -63;
int16_t x157 = -1;
volatile uint16_t x160 = 515U;
volatile int16_t x161 = -1;
int8_t x166 = INT8_MIN;
static volatile int64_t x167 = INT64_MIN;
int64_t x178 = INT64_MIN;
volatile uint32_t x179 = UINT32_MAX;
int32_t t45 = 22695976;
uint8_t x186 = 5U;
int32_t x188 = INT32_MIN;
uint8_t x197 = 52U;
int32_t x200 = -1;
int32_t x201 = 782309451;
int32_t x210 = -1;
int32_t t52 = -242470;
int64_t x215 = INT64_MIN;
int64_t x222 = INT64_MAX;
int32_t x227 = INT32_MAX;
volatile uint64_t x236 = 65341LLU;
static int64_t x247 = -715578478209210605LL;
static int32_t x255 = INT32_MIN;
uint64_t t63 = UINT64_MAX;
int32_t t64 = 1;
volatile int16_t x262 = INT16_MAX;
int32_t x264 = INT32_MIN;
int32_t x272 = INT32_MIN;
static int64_t x274 = INT64_MIN;
static uint8_t x280 = UINT8_MAX;
static int32_t t69 = 46396802;
static uint8_t x282 = 4U;
volatile uint8_t x286 = UINT8_MAX;
static int64_t t72 = -4LL;
volatile int64_t x293 = -430780913805LL;
int8_t x299 = INT8_MAX;
uint32_t x300 = 2874U;
volatile uint32_t t74 = 48298381U;
int32_t t76 = -59;
int64_t x310 = INT64_MIN;
int32_t x311 = -1;
static int64_t x313 = -1LL;
int16_t x319 = INT16_MIN;
volatile uint32_t x323 = 86U;
uint64_t t80 = 9541280LLU;
uint64_t x326 = 13483761LLU;
volatile int32_t x328 = INT32_MIN;
uint64_t x330 = 258237276923162LLU;
static volatile uint8_t x333 = 7U;
uint64_t x342 = 525582632139118LLU;
uint64_t x349 = 239526750073670LLU;
uint64_t x354 = 5410176223044776LLU;
static int16_t x357 = INT16_MIN;
uint64_t x359 = 194035303151342516LLU;
int16_t x362 = INT16_MIN;
int16_t x363 = INT16_MAX;
volatile int8_t x366 = -1;
int16_t x371 = 47;
int8_t x374 = INT8_MIN;
uint16_t x376 = 383U;
int32_t x381 = INT32_MIN;
int32_t x383 = INT32_MIN;
volatile uint64_t x394 = 65743485LLU;
uint64_t x397 = 335493LLU;
int32_t x398 = 1131113;
int32_t t99 = 356;


void f0(void) {
	static int16_t x2 = -1;
	static volatile int8_t x3 = 20;
	uint32_t x4 = 4703U;
	uint32_t t0 = 501U;

	t0 = ((x1==(x2&x3))^x4);

	if (t0 != 4703U) { NG(); } else { ; }
	
}

void f1(void) {
	volatile int32_t x6 = -1;
	uint64_t x8 = 383578170906LLU;

	t1 = ((x5==(x6&x7))^x8);

	if (t1 != 383578170906LLU) { NG(); } else { ; }
	
}

void f2(void) {
	static volatile uint64_t x9 = UINT64_MAX;
	int8_t x10 = -1;
	int16_t x11 = -4;
	volatile int16_t x12 = INT16_MIN;
	int32_t t2 = 8;

	t2 = ((x9==(x10&x11))^x12);

	if (t2 != -32768) { NG(); } else { ; }
	
}

void f3(void) {
	int16_t x13 = -6288;
	int32_t x14 = -497297785;
	static volatile uint8_t x15 = 98U;
	int16_t x16 = -6280;
	volatile int32_t t3 = -176618;

	t3 = ((x13==(x14&x15))^x16);

	if (t3 != -6280) { NG(); } else { ; }
	
}

void f4(void) {
	int32_t x17 = INT32_MAX;
	static int64_t x18 = -1LL;
	uint64_t x19 = 417203880199LLU;
	volatile uint8_t x20 = 126U;
	volatile int32_t t4 = -455;

	t4 = ((x17==(x18&x19))^x20);

	if (t4 != 126) { NG(); } else { ; }
	
}

void f5(void) {
	int64_t x21 = 44112555927LL;
	static uint8_t x22 = UINT8_MAX;
	int32_t x23 = 59;
	int16_t x24 = INT16_MIN;
	int32_t t5 = -3640;

	t5 = ((x21==(x22&x23))^x24);

	if (t5 != -32768) { NG(); } else { ; }
	
}

void f6(void) {
	static int16_t x25 = 11;
	static uint32_t x26 = UINT32_MAX;
	uint64_t x28 = 13189071346302522LLU;
	volatile uint64_t t6 = 13732361LLU;

	t6 = ((x25==(x26&x27))^x28);

	if (t6 != 13189071346302522LLU) { NG(); } else { ; }
	
}

void f7(void) {
	static uint32_t x29 = UINT32_MAX;
	static int8_t x30 = 20;
	static uint64_t x32 = 0LLU;
	uint64_t t7 = 6742671871LLU;

	t7 = ((x29==(x30&x31))^x32);

	if (t7 != 0LLU) { NG(); } else { ; }
	
}

void f8(void) {
	static uint8_t x34 = UINT8_MAX;
	int32_t x35 = -1;
	int8_t x36 = INT8_MAX;
	int32_t t8 = -1052482239;

	t8 = ((x33==(x34&x35))^x36);

	if (t8 != 127) { NG(); } else { ; }
	
}

void f9(void) {
	uint32_t x37 = 1029U;
	int64_t x38 = INT64_MIN;
	static int8_t x39 = INT8_MIN;
	int8_t x40 = INT8_MAX;
	volatile int32_t t9 = 9607;

	t9 = ((x37==(x38&x39))^x40);

	if (t9 != 127) { NG(); } else { ; }
	
}

void f10(void) {
	volatile int64_t x41 = INT64_MAX;
	uint64_t x43 = UINT64_MAX;

	t10 = ((x41==(x42&x43))^x44);

	if (t10 != -32768) { NG(); } else { ; }
	
}

void f11(void) {
	int16_t x46 = -9;
	int32_t x47 = 3024502;
	uint16_t x48 = 1884U;
	static volatile int32_t t11 = -52571;

	t11 = ((x45==(x46&x47))^x48);

	if (t11 != 1884) { NG(); } else { ; }
	
}

void f12(void) {
	int64_t x51 = -1LL;
	int8_t x52 = INT8_MIN;

	t12 = ((x49==(x50&x51))^x52);

	if (t12 != -127) { NG(); } else { ; }
	
}

void f13(void) {
	uint16_t x53 = 20303U;
	volatile int32_t x55 = -1;
	static uint16_t x56 = 16U;
	volatile int32_t t13 = 471558;

	t13 = ((x53==(x54&x55))^x56);

	if (t13 != 16) { NG(); } else { ; }
	
}

void f14(void) {
	volatile int8_t x57 = INT8_MIN;
	int32_t x58 = INT32_MIN;
	int32_t x59 = -1;
	volatile int32_t t14 = INT32_MAX;

	t14 = ((x57==(x58&x59))^x60);

	if (t14 != INT32_MAX) { NG(); } else { ; }
	
}

void f15(void) {
	int8_t x61 = 1;
	uint32_t x62 = 0U;
	int8_t x63 = INT8_MIN;
	int64_t x64 = -1LL;
	static volatile int64_t t15 = 29815371LL;

	t15 = ((x61==(x62&x63))^x64);

	if (t15 != -1LL) { NG(); } else { ; }
	
}

void f16(void) {
	int64_t x65 = INT64_MAX;
	volatile int8_t x67 = 31;

	t16 = ((x65==(x66&x67))^x68);

	if (t16 != INT64_MAX) { NG(); } else { ; }
	
}

void f17(void) {
	uint64_t x70 = 100208818LLU;
	volatile uint64_t x71 = 96167740938065825LLU;
	static int8_t x72 = INT8_MAX;

	t17 = ((x69==(x70&x71))^x72);

	if (t17 != 127) { NG(); } else { ; }
	
}

void f18(void) {
	static volatile int32_t x73 = INT32_MIN;
	int16_t x74 = 101;
	static volatile uint16_t x75 = UINT16_MAX;
	volatile int8_t x76 = -1;
	int32_t t18 = 2;

	t18 = ((x73==(x74&x75))^x76);

	if (t18 != -1) { NG(); } else { ; }
	
}

void f19(void) {
	int64_t x77 = -1LL;
	static uint64_t x80 = 2931880LLU;

	t19 = ((x77==(x78&x79))^x80);

	if (t19 != 2931880LLU) { NG(); } else { ; }
	
}

void f20(void) {
	uint8_t x81 = 106U;
	volatile int32_t x82 = INT32_MAX;
	volatile uint32_t x83 = UINT32_MAX;
	int16_t x84 = -1;

	t20 = ((x81==(x82&x83))^x84);

	if (t20 != -1) { NG(); } else { ; }
	
}

void f21(void) {
	int8_t x85 = -1;
	volatile int32_t x86 = INT32_MIN;
	int8_t x87 = INT8_MAX;
	uint32_t t21 = 1U;

	t21 = ((x85==(x86&x87))^x88);

	if (t21 != 695455U) { NG(); } else { ; }
	
}

void f22(void) {
	int64_t x91 = INT64_MAX;
	uint8_t x92 = UINT8_MAX;
	volatile int32_t t22 = 0;

	t22 = ((x89==(x90&x91))^x92);

	if (t22 != 255) { NG(); } else { ; }
	
}

void f23(void) {
	int8_t x93 = INT8_MAX;
	static int32_t x94 = -16;
	uint8_t x95 = UINT8_MAX;
	uint64_t t23 = 235LLU;

	t23 = ((x93==(x94&x95))^x96);

	if (t23 != 1045421682LLU) { NG(); } else { ; }
	
}

void f24(void) {
	uint16_t x97 = 26U;
	int64_t x98 = -1LL;
	static uint32_t x99 = 1181689U;
	int8_t x100 = INT8_MIN;

	t24 = ((x97==(x98&x99))^x100);

	if (t24 != -128) { NG(); } else { ; }
	
}

void f25(void) {
	uint16_t x101 = 4U;
	int16_t x102 = -1;
	volatile int16_t x104 = INT16_MIN;
	volatile int32_t t25 = -14554253;

	t25 = ((x101==(x102&x103))^x104);

	if (t25 != -32768) { NG(); } else { ; }
	
}

void f26(void) {
	static int8_t x105 = -1;
	uint64_t x106 = UINT64_MAX;
	int16_t x108 = -1;

	t26 = ((x105==(x106&x107))^x108);

	if (t26 != -1) { NG(); } else { ; }
	
}

void f27(void) {
	volatile int64_t x109 = -79788452864038133LL;
	int64_t x110 = -1LL;
	uint64_t x111 = 1938187231LLU;
	static volatile uint32_t x112 = 0U;
	static volatile uint32_t t27 = 87U;

	t27 = ((x109==(x110&x111))^x112);

	if (t27 != 0U) { NG(); } else { ; }
	
}

void f28(void) {
	uint16_t x113 = 7U;
	int16_t x114 = INT16_MAX;
	int16_t x115 = -3;
	int32_t x116 = INT32_MIN;
	int32_t t28 = INT32_MIN;

	t28 = ((x113==(x114&x115))^x116);

	if (t28 != INT32_MIN) { NG(); } else { ; }
	
}

void f29(void) {
	volatile uint32_t x117 = 3798941U;
	static uint64_t x119 = 111021036456460LLU;
	static volatile int8_t x120 = 12;
	int32_t t29 = 893;

	t29 = ((x117==(x118&x119))^x120);

	if (t29 != 12) { NG(); } else { ; }
	
}

void f30(void) {
	static int8_t x121 = INT8_MIN;
	static int8_t x122 = INT8_MAX;
	uint64_t x123 = UINT64_MAX;
	int8_t x124 = -1;
	int32_t t30 = 0;

	t30 = ((x121==(x122&x123))^x124);

	if (t30 != -1) { NG(); } else { ; }
	
}

void f31(void) {
	int16_t x125 = -6;
	volatile uint32_t x127 = 1294643U;
	volatile uint32_t t31 = UINT32_MAX;

	t31 = ((x125==(x126&x127))^x128);

	if (t31 != UINT32_MAX) { NG(); } else { ; }
	
}

void f32(void) {
	int8_t x129 = 1;
	volatile uint16_t x130 = UINT16_MAX;
	volatile uint64_t x131 = 271626764072LLU;
	static int32_t x132 = INT32_MIN;

	t32 = ((x129==(x130&x131))^x132);

	if (t32 != INT32_MIN) { NG(); } else { ; }
	
}

void f33(void) {
	int8_t x133 = INT8_MAX;
	static int16_t x134 = -1;
	volatile uint32_t x135 = 2796U;
	uint32_t x136 = UINT32_MAX;
	uint32_t t33 = UINT32_MAX;

	t33 = ((x133==(x134&x135))^x136);

	if (t33 != UINT32_MAX) { NG(); } else { ; }
	
}

void f34(void) {
	int64_t x137 = 55LL;
	uint16_t x138 = 1U;
	static int32_t x140 = INT32_MIN;
	static int32_t t34 = INT32_MIN;

	t34 = ((x137==(x138&x139))^x140);

	if (t34 != INT32_MIN) { NG(); } else { ; }
	
}

void f35(void) {
	static int64_t x142 = INT64_MIN;
	int32_t x144 = INT32_MIN;

	t35 = ((x141==(x142&x143))^x144);

	if (t35 != INT32_MIN) { NG(); } else { ; }
	
}

void f36(void) {
	volatile int64_t x145 = INT64_MIN;
	uint64_t x146 = 93069585487700574LLU;
	int8_t x147 = -5;
	int64_t t36 = 2440144339009LL;

	t36 = ((x145==(x146&x147))^x148);

	if (t36 != -488633996760711448LL) { NG(); } else { ; }
	
}

void f37(void) {
	int16_t x149 = 3011;
	int16_t x150 = 0;

	t37 = ((x149==(x150&x151))^x152);

	if (t37 != INT64_MAX) { NG(); } else { ; }
	
}

void f38(void) {
	int64_t x153 = -1LL;
	int16_t x156 = INT16_MAX;

	t38 = ((x153==(x154&x155))^x156);

	if (t38 != 32767) { NG(); } else { ; }
	
}

void f39(void) {
	static int32_t x158 = INT32_MIN;
	int16_t x159 = INT16_MIN;
	static int32_t t39 = 6;

	t39 = ((x157==(x158&x159))^x160);

	if (t39 != 515) { NG(); } else { ; }
	
}

void f40(void) {
	int32_t x162 = INT32_MAX;
	volatile int16_t x163 = INT16_MAX;
	static uint64_t x164 = 971494361502LLU;
	static volatile uint64_t t40 = 358896LLU;

	t40 = ((x161==(x162&x163))^x164);

	if (t40 != 971494361502LLU) { NG(); } else { ; }
	
}

void f41(void) {
	static int16_t x165 = INT16_MIN;
	int16_t x168 = INT16_MAX;
	volatile int32_t t41 = -83605;

	t41 = ((x165==(x166&x167))^x168);

	if (t41 != 32767) { NG(); } else { ; }
	
}

void f42(void) {
	int8_t x169 = -1;
	static uint64_t x170 = UINT64_MAX;
	uint64_t x171 = 8698968LLU;
	volatile int32_t x172 = -1;
	volatile int32_t t42 = -431;

	t42 = ((x169==(x170&x171))^x172);

	if (t42 != -1) { NG(); } else { ; }
	
}

void f43(void) {
	static volatile uint16_t x173 = 925U;
	int32_t x174 = -1;
	uint64_t x175 = UINT64_MAX;
	int64_t x176 = INT64_MAX;
	static int64_t t43 = INT64_MAX;

	t43 = ((x173==(x174&x175))^x176);

	if (t43 != INT64_MAX) { NG(); } else { ; }
	
}

void f44(void) {
	volatile int16_t x177 = -1;
	static uint16_t x180 = 224U;
	int32_t t44 = 62628400;

	t44 = ((x177==(x178&x179))^x180);

	if (t44 != 224) { NG(); } else { ; }
	
}

void f45(void) {
	static int16_t x181 = -1;
	volatile uint64_t x182 = 31658LLU;
	static uint16_t x183 = UINT16_MAX;
	int8_t x184 = 0;

	t45 = ((x181==(x182&x183))^x184);

	if (t45 != 0) { NG(); } else { ; }
	
}

void f46(void) {
	static uint64_t x185 = 5542115684LLU;
	int64_t x187 = -70009773LL;
	volatile int32_t t46 = INT32_MIN;

	t46 = ((x185==(x186&x187))^x188);

	if (t46 != INT32_MIN) { NG(); } else { ; }
	
}

void f47(void) {
	int64_t x189 = INT64_MAX;
	uint64_t x190 = 114023LLU;
	uint8_t x191 = 0U;
	int8_t x192 = 0;
	static volatile int32_t t47 = -26942;

	t47 = ((x189==(x190&x191))^x192);

	if (t47 != 0) { NG(); } else { ; }
	
}

void f48(void) {
	static int16_t x193 = INT16_MAX;
	uint64_t x194 = 41432740LLU;
	static uint16_t x195 = 1U;
	static int8_t x196 = INT8_MIN;
	int32_t t48 = -34274966;

	t48 = ((x193==(x194&x195))^x196);

	if (t48 != -128) { NG(); } else { ; }
	
}

void f49(void) {
	volatile uint8_t x198 = 15U;
	int8_t x199 = -1;
	volatile int32_t t49 = 27;

	t49 = ((x197==(x198&x199))^x200);

	if (t49 != -1) { NG(); } else { ; }
	
}

void f50(void) {
	uint64_t x202 = 2564613LLU;
	volatile uint64_t x203 = 13985593870LLU;
	uint64_t x204 = 65493LLU;
	volatile uint64_t t50 = 16LLU;

	t50 = ((x201==(x202&x203))^x204);

	if (t50 != 65493LLU) { NG(); } else { ; }
	
}

void f51(void) {
	int32_t x205 = INT32_MIN;
	int16_t x206 = -1;
	static int16_t x207 = 123;
	int64_t x208 = -1LL;
	static int64_t t51 = -14981877907190LL;

	t51 = ((x205==(x206&x207))^x208);

	if (t51 != -1LL) { NG(); } else { ; }
	
}

void f52(void) {
	int8_t x209 = INT8_MIN;
	int64_t x211 = INT64_MIN;
	volatile int32_t x212 = -69121;

	t52 = ((x209==(x210&x211))^x212);

	if (t52 != -69121) { NG(); } else { ; }
	
}

void f53(void) {
	int32_t x213 = INT32_MIN;
	volatile int16_t x214 = -1;
	uint16_t x216 = UINT16_MAX;
	static int32_t t53 = 4;

	t53 = ((x213==(x214&x215))^x216);

	if (t53 != 65535) { NG(); } else { ; }
	
}

void f54(void) {
	uint16_t x217 = 188U;
	uint64_t x218 = 32600574LLU;
	int8_t x219 = -1;
	static int64_t x220 = INT64_MIN;
	volatile int64_t t54 = INT64_MIN;

	t54 = ((x217==(x218&x219))^x220);

	if (t54 != INT64_MIN) { NG(); } else { ; }
	
}

void f55(void) {
	static int32_t x221 = -1;
	static volatile int32_t x223 = INT32_MAX;
	volatile int32_t x224 = INT32_MIN;
	int32_t t55 = INT32_MIN;

	t55 = ((x221==(x222&x223))^x224);

	if (t55 != INT32_MIN) { NG(); } else { ; }
	
}

void f56(void) {
	int32_t x225 = INT32_MAX;
	uint64_t x226 = 12419630142869276LLU;
	static uint16_t x228 = UINT16_MAX;
	int32_t t56 = -409;

	t56 = ((x225==(x226&x227))^x228);

	if (t56 != 65535) { NG(); } else { ; }
	
}

void f57(void) {
	static volatile uint8_t x229 = UINT8_MAX;
	volatile uint16_t x230 = 0U;
	int64_t x231 = INT64_MIN;
	uint8_t x232 = 2U;
	volatile int32_t t57 = -88774;

	t57 = ((x229==(x230&x231))^x232);

	if (t57 != 2) { NG(); } else { ; }
	
}

void f58(void) {
	static int8_t x233 = -1;
	int16_t x234 = -1;
	volatile int8_t x235 = INT8_MIN;
	uint64_t t58 = 92467342LLU;

	t58 = ((x233==(x234&x235))^x236);

	if (t58 != 65341LLU) { NG(); } else { ; }
	
}

void f59(void) {
	int64_t x237 = INT64_MIN;
	volatile uint8_t x238 = 73U;
	int8_t x239 = -8;
	static int64_t x240 = -1LL;
	int64_t t59 = -499152485LL;

	t59 = ((x237==(x238&x239))^x240);

	if (t59 != -1LL) { NG(); } else { ; }
	
}

void f60(void) {
	int16_t x241 = INT16_MIN;
	int64_t x242 = -1LL;
	int64_t x243 = -1LL;
	int16_t x244 = INT16_MAX;
	volatile int32_t t60 = 96;

	t60 = ((x241==(x242&x243))^x244);

	if (t60 != 32767) { NG(); } else { ; }
	
}

void f61(void) {
	uint16_t x245 = 22673U;
	uint64_t x246 = 8093318839160154LLU;
	volatile int64_t x248 = INT64_MAX;
	volatile int64_t t61 = INT64_MAX;

	t61 = ((x245==(x246&x247))^x248);

	if (t61 != INT64_MAX) { NG(); } else { ; }
	
}

void f62(void) {
	int16_t x249 = 1;
	int32_t x250 = INT32_MAX;
	int8_t x251 = INT8_MAX;
	static volatile int32_t x252 = INT32_MAX;
	volatile int32_t t62 = INT32_MAX;

	t62 = ((x249==(x250&x251))^x252);

	if (t62 != INT32_MAX) { NG(); } else { ; }
	
}

void f63(void) {
	int8_t x253 = INT8_MIN;
	int32_t x254 = INT32_MIN;
	uint64_t x256 = UINT64_MAX;

	t63 = ((x253==(x254&x255))^x256);

	if (t63 != UINT64_MAX) { NG(); } else { ; }
	
}

void f64(void) {
	int64_t x257 = 119121LL;
	int16_t x258 = INT16_MIN;
	static uint8_t x259 = 8U;
	int16_t x260 = INT16_MIN;

	t64 = ((x257==(x258&x259))^x260);

	if (t64 != -32768) { NG(); } else { ; }
	
}

void f65(void) {
	volatile int16_t x261 = 11869;
	volatile uint64_t x263 = UINT64_MAX;
	int32_t t65 = INT32_MIN;

	t65 = ((x261==(x262&x263))^x264);

	if (t65 != INT32_MIN) { NG(); } else { ; }
	
}

void f66(void) {
	int8_t x265 = -1;
	uint16_t x266 = UINT16_MAX;
	int64_t x267 = -1207LL;
	uint8_t x268 = 19U;
	volatile int32_t t66 = -2;

	t66 = ((x265==(x266&x267))^x268);

	if (t66 != 19) { NG(); } else { ; }
	
}

void f67(void) {
	static int16_t x269 = INT16_MIN;
	volatile uint32_t x270 = 1U;
	static int8_t x271 = INT8_MAX;
	int32_t t67 = INT32_MIN;

	t67 = ((x269==(x270&x271))^x272);

	if (t67 != INT32_MIN) { NG(); } else { ; }
	
}

void f68(void) {
	int8_t x273 = 13;
	int32_t x275 = 421663597;
	int8_t x276 = INT8_MIN;
	int32_t t68 = 14;

	t68 = ((x273==(x274&x275))^x276);

	if (t68 != -128) { NG(); } else { ; }
	
}

void f69(void) {
	int16_t x277 = -1;
	volatile int8_t x278 = INT8_MAX;
	int16_t x279 = -1;

	t69 = ((x277==(x278&x279))^x280);

	if (t69 != 255) { NG(); } else { ; }
	
}

void f70(void) {
	int64_t x281 = INT64_MAX;
	volatile int16_t x283 = INT16_MAX;
	int8_t x284 = INT8_MAX;
	int32_t t70 = -189;

	t70 = ((x281==(x282&x283))^x284);

	if (t70 != 127) { NG(); } else { ; }
	
}

void f71(void) {
	volatile uint8_t x285 = 41U;
	int32_t x287 = 87;
	static int64_t x288 = INT64_MIN;
	int64_t t71 = INT64_MIN;

	t71 = ((x285==(x286&x287))^x288);

	if (t71 != INT64_MIN) { NG(); } else { ; }
	
}

void f72(void) {
	int16_t x289 = INT16_MIN;
	int16_t x290 = -1;
	int16_t x291 = 5;
	int64_t x292 = -1LL;

	t72 = ((x289==(x290&x291))^x292);

	if (t72 != -1LL) { NG(); } else { ; }
	
}

void f73(void) {
	int8_t x294 = -1;
	int64_t x295 = -899LL;
	static volatile uint32_t x296 = UINT32_MAX;
	uint32_t t73 = UINT32_MAX;

	t73 = ((x293==(x294&x295))^x296);

	if (t73 != UINT32_MAX) { NG(); } else { ; }
	
}

void f74(void) {
	int32_t x297 = INT32_MAX;
	int8_t x298 = 29;

	t74 = ((x297==(x298&x299))^x300);

	if (t74 != 2874U) { NG(); } else { ; }
	
}

void f75(void) {
	int32_t x301 = INT32_MAX;
	volatile uint32_t x302 = 47766448U;
	uint64_t x303 = 1422LLU;
	int64_t x304 = 6LL;
	static int64_t t75 = 5282216533725202LL;

	t75 = ((x301==(x302&x303))^x304);

	if (t75 != 6LL) { NG(); } else { ; }
	
}

void f76(void) {
	volatile int8_t x305 = INT8_MIN;
	volatile int8_t x306 = INT8_MIN;
	uint32_t x307 = 61U;
	uint8_t x308 = 57U;

	t76 = ((x305==(x306&x307))^x308);

	if (t76 != 57) { NG(); } else { ; }
	
}

void f77(void) {
	volatile int32_t x309 = 5150;
	uint32_t x312 = UINT32_MAX;
	uint32_t t77 = UINT32_MAX;

	t77 = ((x309==(x310&x311))^x312);

	if (t77 != UINT32_MAX) { NG(); } else { ; }
	
}

void f78(void) {
	int64_t x314 = INT64_MAX;
	uint16_t x315 = 1U;
	int8_t x316 = -3;
	volatile int32_t t78 = -16070;

	t78 = ((x313==(x314&x315))^x316);

	if (t78 != -3) { NG(); } else { ; }
	
}

void f79(void) {
	volatile int16_t x317 = 1;
	int32_t x318 = -1;
	uint64_t x320 = UINT64_MAX;
	volatile uint64_t t79 = UINT64_MAX;

	t79 = ((x317==(x318&x319))^x320);

	if (t79 != UINT64_MAX) { NG(); } else { ; }
	
}

void f80(void) {
	int8_t x321 = INT8_MIN;
	uint32_t x322 = 16703U;
	uint64_t x324 = 1360255660809221849LLU;

	t80 = ((x321==(x322&x323))^x324);

	if (t80 != 1360255660809221849LLU) { NG(); } else { ; }
	
}

void f81(void) {
	uint64_t x325 = UINT64_MAX;
	uint32_t x327 = 80968814U;
	volatile int32_t t81 = INT32_MIN;

	t81 = ((x325==(x326&x327))^x328);

	if (t81 != INT32_MIN) { NG(); } else { ; }
	
}

void f82(void) {
	static int8_t x329 = INT8_MAX;
	int8_t x331 = -1;
	int32_t x332 = -1;
	int32_t t82 = -1816;

	t82 = ((x329==(x330&x331))^x332);

	if (t82 != -1) { NG(); } else { ; }
	
}

void f83(void) {
	int8_t x334 = INT8_MIN;
	static int8_t x335 = -17;
	uint32_t x336 = 2092615079U;
	uint32_t t83 = 1757U;

	t83 = ((x333==(x334&x335))^x336);

	if (t83 != 2092615079U) { NG(); } else { ; }
	
}

void f84(void) {
	int32_t x337 = 24881;
	uint16_t x338 = 13159U;
	volatile int16_t x339 = -1;
	int16_t x340 = INT16_MIN;
	static int32_t t84 = 394875;

	t84 = ((x337==(x338&x339))^x340);

	if (t84 != -32768) { NG(); } else { ; }
	
}

void f85(void) {
	int16_t x341 = INT16_MIN;
	volatile uint64_t x343 = UINT64_MAX;
	int64_t x344 = INT64_MIN;
	int64_t t85 = INT64_MIN;

	t85 = ((x341==(x342&x343))^x344);

	if (t85 != INT64_MIN) { NG(); } else { ; }
	
}

void f86(void) {
	static volatile int32_t x345 = 2;
	static int32_t x346 = INT32_MIN;
	volatile int32_t x347 = -14940;
	int32_t x348 = INT32_MIN;
	int32_t t86 = INT32_MIN;

	t86 = ((x345==(x346&x347))^x348);

	if (t86 != INT32_MIN) { NG(); } else { ; }
	
}

void f87(void) {
	uint8_t x350 = 21U;
	static volatile int8_t x351 = -1;
	uint64_t x352 = UINT64_MAX;
	volatile uint64_t t87 = UINT64_MAX;

	t87 = ((x349==(x350&x351))^x352);

	if (t87 != UINT64_MAX) { NG(); } else { ; }
	
}

void f88(void) {
	int32_t x353 = 102557;
	volatile uint8_t x355 = 2U;
	int16_t x356 = -28;
	static int32_t t88 = -4;

	t88 = ((x353==(x354&x355))^x356);

	if (t88 != -28) { NG(); } else { ; }
	
}

void f89(void) {
	int16_t x358 = -596;
	static int16_t x360 = -1;
	volatile int32_t t89 = 1;

	t89 = ((x357==(x358&x359))^x360);

	if (t89 != -1) { NG(); } else { ; }
	
}

void f90(void) {
	uint16_t x361 = 5186U;
	uint16_t x364 = UINT16_MAX;
	static int32_t t90 = -3559;

	t90 = ((x361==(x362&x363))^x364);

	if (t90 != 65535) { NG(); } else { ; }
	
}

void f91(void) {
	int8_t x365 = -1;
	int32_t x367 = INT32_MIN;
	int16_t x368 = INT16_MAX;
	static volatile int32_t t91 = -2;

	t91 = ((x365==(x366&x367))^x368);

	if (t91 != 32767) { NG(); } else { ; }
	
}

void f92(void) {
	uint32_t x369 = 172749U;
	int64_t x370 = -1LL;
	uint8_t x372 = 11U;
	int32_t t92 = -49995;

	t92 = ((x369==(x370&x371))^x372);

	if (t92 != 11) { NG(); } else { ; }
	
}

void f93(void) {
	int64_t x373 = INT64_MAX;
	static volatile uint16_t x375 = 741U;
	int32_t t93 = 2403;

	t93 = ((x373==(x374&x375))^x376);

	if (t93 != 383) { NG(); } else { ; }
	
}

void f94(void) {
	int64_t x377 = -1649352012635LL;
	uint16_t x378 = 13U;
	int64_t x379 = INT64_MIN;
	int64_t x380 = INT64_MIN;
	int64_t t94 = INT64_MIN;

	t94 = ((x377==(x378&x379))^x380);

	if (t94 != INT64_MIN) { NG(); } else { ; }
	
}

void f95(void) {
	uint16_t x382 = 629U;
	int16_t x384 = INT16_MIN;
	int32_t t95 = -1;

	t95 = ((x381==(x382&x383))^x384);

	if (t95 != -32768) { NG(); } else { ; }
	
}

void f96(void) {
	int8_t x385 = INT8_MAX;
	static int8_t x386 = INT8_MIN;
	int32_t x387 = INT32_MIN;
	uint8_t x388 = 44U;
	int32_t t96 = 101675;

	t96 = ((x385==(x386&x387))^x388);

	if (t96 != 44) { NG(); } else { ; }
	
}

void f97(void) {
	int32_t x389 = INT32_MAX;
	uint8_t x390 = 59U;
	int8_t x391 = INT8_MIN;
	int32_t x392 = INT32_MIN;
	int32_t t97 = INT32_MIN;

	t97 = ((x389==(x390&x391))^x392);

	if (t97 != INT32_MIN) { NG(); } else { ; }
	
}

void f98(void) {
	int64_t x393 = -1LL;
	uint8_t x395 = 25U;
	volatile int32_t x396 = -1;
	int32_t t98 = 66058374;

	t98 = ((x393==(x394&x395))^x396);

	if (t98 != -1) { NG(); } else { ; }
	
}

void f99(void) {
	uint64_t x399 = UINT64_MAX;
	volatile int16_t x400 = INT16_MIN;

	t99 = ((x397==(x398&x399))^x400);

	if (t99 != -32768) { NG(); } else { ; }
	
}

int main(void) {
    f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();


    return 0;
}

