#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

int32_t t0 = 1;
int16_t x7 = INT16_MIN;
volatile int32_t t1 = -253539104;
int8_t x36 = -1;
int32_t x37 = 4;
int16_t x38 = -118;
int32_t t7 = -2933;
int64_t x51 = 1469260LL;
uint64_t x61 = UINT64_MAX;
int64_t x64 = -1LL;
int16_t x79 = INT16_MIN;
int32_t t14 = 66645;
uint64_t x89 = 7733167LLU;
uint8_t x90 = 24U;
static uint8_t x97 = 21U;
volatile int32_t t18 = -28;
int32_t x104 = -245657086;
uint32_t x110 = UINT32_MAX;
int8_t x124 = INT8_MAX;
uint16_t x127 = 285U;
uint8_t x128 = 11U;
uint8_t x140 = 2U;
int64_t x145 = -1LL;
int32_t t29 = 0;
static uint32_t x156 = 86611210U;
uint32_t x159 = 14806754U;
uint32_t x163 = UINT32_MAX;
int32_t x164 = -7598289;
static int64_t x172 = 201LL;
int32_t x178 = -1;
volatile int8_t x182 = -1;
uint16_t x184 = 178U;
volatile uint32_t t36 = UINT32_MAX;
int32_t t37 = 2683879;
uint64_t x191 = 1LLU;
volatile int8_t x192 = -6;
volatile uint8_t x196 = 7U;
int32_t x202 = INT32_MAX;
volatile int32_t t41 = -1;
static int32_t t42 = -2290;
static uint32_t x220 = 100282284U;
static volatile uint16_t x221 = 1U;
volatile uint32_t x224 = UINT32_MAX;
int32_t t44 = -719319;
volatile int32_t x229 = -4;
static int8_t x233 = 0;
volatile uint8_t x234 = 25U;
int8_t x248 = INT8_MIN;
uint16_t x255 = UINT16_MAX;
static uint32_t x270 = 0U;
int32_t t53 = -7;
volatile int16_t x291 = INT16_MAX;
volatile uint32_t x294 = UINT32_MAX;
int64_t x298 = INT64_MIN;
int64_t x300 = -1LL;
int32_t t58 = -1038520272;
static uint32_t x310 = 0U;
uint32_t x312 = 2083U;
static int8_t x313 = INT8_MIN;
int8_t x315 = INT8_MIN;
int8_t x319 = -47;
static uint64_t x322 = 1653436210596285406LLU;
int32_t t64 = 1;
volatile int32_t t66 = 1;
int16_t x346 = INT16_MAX;
uint16_t x347 = UINT16_MAX;
int64_t x349 = -1LL;
int32_t x362 = INT32_MAX;
int32_t t70 = 187225;
uint32_t x365 = 22920U;
static volatile int64_t x367 = -1LL;
int32_t x373 = -1;
uint32_t x377 = 7U;
volatile int16_t x381 = INT16_MIN;
int8_t x382 = -1;
int16_t x391 = INT16_MAX;
uint64_t x403 = 846LLU;
int32_t x404 = INT32_MAX;
volatile int32_t t79 = 0;
static int8_t x406 = -1;
int32_t t80 = 17762;
int64_t x421 = INT64_MIN;
int64_t x424 = -24029325004705LL;
int64_t t82 = INT64_MIN;
volatile int32_t x429 = INT32_MIN;
uint64_t x430 = UINT64_MAX;
static int8_t x431 = INT8_MIN;
int32_t t86 = -1;
int8_t x443 = INT8_MIN;
volatile int32_t x451 = INT32_MAX;
int8_t x452 = 0;
volatile int16_t x459 = INT16_MIN;
static int8_t x474 = INT8_MAX;
int16_t x475 = 0;
static int8_t x480 = INT8_MIN;
static uint64_t x483 = UINT64_MAX;
int64_t x484 = INT64_MIN;
uint64_t x488 = UINT64_MAX;
static volatile int8_t x492 = INT8_MAX;
volatile int32_t t97 = 44166;
static volatile int64_t x497 = INT64_MIN;
uint16_t x498 = 6958U;
int16_t x499 = INT16_MIN;


void f0(void) {
	volatile int8_t x1 = -1;
	uint16_t x2 = 21407U;
	static uint8_t x3 = 1U;
	uint8_t x4 = UINT8_MAX;

	t0 = (x1^((x2+x3)<=x4));

	if (t0 != -1) { NG(); } else { ; }
	
}

void f1(void) {
	int8_t x5 = INT8_MIN;
	uint64_t x6 = UINT64_MAX;
	int8_t x8 = 37;

	t1 = (x5^((x6+x7)<=x8));

	if (t1 != -128) { NG(); } else { ; }
	
}

void f2(void) {
	int8_t x9 = -6;
	volatile uint64_t x10 = 1543223237133811LLU;
	int64_t x11 = INT64_MAX;
	int64_t x12 = INT64_MIN;
	int32_t t2 = -7938;

	t2 = (x9^((x10+x11)<=x12));

	if (t2 != -6) { NG(); } else { ; }
	
}

void f3(void) {
	volatile uint16_t x13 = 187U;
	static uint64_t x14 = 4136468080201321LLU;
	static volatile int64_t x15 = INT64_MIN;
	volatile int32_t x16 = INT32_MIN;
	int32_t t3 = -1279;

	t3 = (x13^((x14+x15)<=x16));

	if (t3 != 186) { NG(); } else { ; }
	
}

void f4(void) {
	int32_t x21 = INT32_MIN;
	uint32_t x22 = 1U;
	static uint32_t x23 = UINT32_MAX;
	uint16_t x24 = UINT16_MAX;
	int32_t t4 = -208;

	t4 = (x21^((x22+x23)<=x24));

	if (t4 != -2147483647) { NG(); } else { ; }
	
}

void f5(void) {
	static int8_t x29 = INT8_MIN;
	static int8_t x30 = INT8_MIN;
	volatile uint32_t x31 = 7077U;
	int32_t x32 = INT32_MAX;
	volatile int32_t t5 = -30;

	t5 = (x29^((x30+x31)<=x32));

	if (t5 != -127) { NG(); } else { ; }
	
}

void f6(void) {
	uint16_t x33 = 4U;
	int32_t x34 = 138;
	uint64_t x35 = UINT64_MAX;
	int32_t t6 = 320;

	t6 = (x33^((x34+x35)<=x36));

	if (t6 != 5) { NG(); } else { ; }
	
}

void f7(void) {
	int32_t x39 = -1;
	int32_t x40 = -1;

	t7 = (x37^((x38+x39)<=x40));

	if (t7 != 5) { NG(); } else { ; }
	
}

void f8(void) {
	int16_t x45 = INT16_MAX;
	int16_t x46 = 4023;
	int16_t x47 = INT16_MIN;
	volatile int8_t x48 = INT8_MIN;
	int32_t t8 = 0;

	t8 = (x45^((x46+x47)<=x48));

	if (t8 != 32766) { NG(); } else { ; }
	
}

void f9(void) {
	static int8_t x49 = 4;
	static volatile uint8_t x50 = 1U;
	volatile int16_t x52 = INT16_MIN;
	volatile int32_t t9 = 7170;

	t9 = (x49^((x50+x51)<=x52));

	if (t9 != 4) { NG(); } else { ; }
	
}

void f10(void) {
	uint64_t x57 = UINT64_MAX;
	uint64_t x58 = 1980160245LLU;
	volatile int8_t x59 = 3;
	static volatile int16_t x60 = INT16_MAX;
	volatile uint64_t t10 = UINT64_MAX;

	t10 = (x57^((x58+x59)<=x60));

	if (t10 != UINT64_MAX) { NG(); } else { ; }
	
}

void f11(void) {
	uint64_t x62 = UINT64_MAX;
	int64_t x63 = -1LL;
	uint64_t t11 = 1LLU;

	t11 = (x61^((x62+x63)<=x64));

	if (t11 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f12(void) {
	volatile int32_t x65 = 601417199;
	uint32_t x66 = 1105726200U;
	int64_t x67 = INT64_MIN;
	int32_t x68 = INT32_MIN;
	static volatile int32_t t12 = -114;

	t12 = (x65^((x66+x67)<=x68));

	if (t12 != 601417198) { NG(); } else { ; }
	
}

void f13(void) {
	uint8_t x73 = UINT8_MAX;
	uint16_t x74 = 43U;
	static int16_t x75 = 1;
	uint32_t x76 = 0U;
	volatile int32_t t13 = -7;

	t13 = (x73^((x74+x75)<=x76));

	if (t13 != 255) { NG(); } else { ; }
	
}

void f14(void) {
	int16_t x77 = 7;
	int8_t x78 = 15;
	int32_t x80 = INT32_MIN;

	t14 = (x77^((x78+x79)<=x80));

	if (t14 != 7) { NG(); } else { ; }
	
}

void f15(void) {
	static int64_t x81 = 5279438LL;
	volatile int8_t x82 = INT8_MIN;
	int64_t x83 = INT64_MAX;
	static volatile int64_t x84 = -327080LL;
	static int64_t t15 = 477270LL;

	t15 = (x81^((x82+x83)<=x84));

	if (t15 != 5279438LL) { NG(); } else { ; }
	
}

void f16(void) {
	static uint32_t x85 = UINT32_MAX;
	uint64_t x86 = UINT64_MAX;
	static volatile int64_t x87 = -1LL;
	static int8_t x88 = INT8_MAX;
	uint32_t t16 = UINT32_MAX;

	t16 = (x85^((x86+x87)<=x88));

	if (t16 != UINT32_MAX) { NG(); } else { ; }
	
}

void f17(void) {
	static int32_t x91 = -1;
	static uint16_t x92 = UINT16_MAX;
	static volatile uint64_t t17 = 21213878301LLU;

	t17 = (x89^((x90+x91)<=x92));

	if (t17 != 7733166LLU) { NG(); } else { ; }
	
}

void f18(void) {
	volatile int16_t x98 = 10196;
	int32_t x99 = INT32_MIN;
	uint32_t x100 = 2867U;

	t18 = (x97^((x98+x99)<=x100));

	if (t18 != 21) { NG(); } else { ; }
	
}

void f19(void) {
	static volatile uint32_t x101 = 751614U;
	uint32_t x102 = UINT32_MAX;
	uint8_t x103 = 2U;
	uint32_t t19 = 260874728U;

	t19 = (x101^((x102+x103)<=x104));

	if (t19 != 751615U) { NG(); } else { ; }
	
}

void f20(void) {
	static uint32_t x109 = 28264848U;
	uint32_t x111 = UINT32_MAX;
	uint32_t x112 = 1214720U;
	static uint32_t t20 = 27U;

	t20 = (x109^((x110+x111)<=x112));

	if (t20 != 28264848U) { NG(); } else { ; }
	
}

void f21(void) {
	int16_t x113 = INT16_MIN;
	int16_t x114 = INT16_MIN;
	int64_t x115 = 357805038698LL;
	int8_t x116 = -1;
	int32_t t21 = -9264192;

	t21 = (x113^((x114+x115)<=x116));

	if (t21 != -32768) { NG(); } else { ; }
	
}

void f22(void) {
	volatile int8_t x117 = 31;
	uint32_t x118 = 1U;
	static int32_t x119 = -812390302;
	int16_t x120 = INT16_MIN;
	int32_t t22 = -801;

	t22 = (x117^((x118+x119)<=x120));

	if (t22 != 30) { NG(); } else { ; }
	
}

void f23(void) {
	static volatile int32_t x121 = INT32_MAX;
	uint16_t x122 = UINT16_MAX;
	int16_t x123 = INT16_MIN;
	static volatile int32_t t23 = INT32_MAX;

	t23 = (x121^((x122+x123)<=x124));

	if (t23 != INT32_MAX) { NG(); } else { ; }
	
}

void f24(void) {
	int32_t x125 = INT32_MIN;
	int16_t x126 = 1;
	int32_t t24 = INT32_MIN;

	t24 = (x125^((x126+x127)<=x128));

	if (t24 != INT32_MIN) { NG(); } else { ; }
	
}

void f25(void) {
	volatile int64_t x133 = INT64_MAX;
	volatile int32_t x134 = INT32_MIN;
	uint64_t x135 = UINT64_MAX;
	uint32_t x136 = 11102U;
	int64_t t25 = INT64_MAX;

	t25 = (x133^((x134+x135)<=x136));

	if (t25 != INT64_MAX) { NG(); } else { ; }
	
}

void f26(void) {
	int32_t x137 = INT32_MAX;
	uint16_t x138 = 1845U;
	uint32_t x139 = 35576516U;
	volatile int32_t t26 = INT32_MAX;

	t26 = (x137^((x138+x139)<=x140));

	if (t26 != INT32_MAX) { NG(); } else { ; }
	
}

void f27(void) {
	int16_t x141 = INT16_MIN;
	int32_t x142 = -1;
	static uint8_t x143 = 13U;
	int32_t x144 = -1;
	static int32_t t27 = -9255995;

	t27 = (x141^((x142+x143)<=x144));

	if (t27 != -32768) { NG(); } else { ; }
	
}

void f28(void) {
	int32_t x146 = -1;
	static int64_t x147 = -1LL;
	int16_t x148 = 862;
	int64_t t28 = 89312LL;

	t28 = (x145^((x146+x147)<=x148));

	if (t28 != -2LL) { NG(); } else { ; }
	
}

void f29(void) {
	int16_t x149 = -1;
	volatile int16_t x150 = INT16_MIN;
	volatile int32_t x151 = -1;
	uint8_t x152 = 2U;

	t29 = (x149^((x150+x151)<=x152));

	if (t29 != -2) { NG(); } else { ; }
	
}

void f30(void) {
	uint8_t x153 = 22U;
	int8_t x154 = INT8_MIN;
	volatile int8_t x155 = INT8_MAX;
	volatile int32_t t30 = 1;

	t30 = (x153^((x154+x155)<=x156));

	if (t30 != 22) { NG(); } else { ; }
	
}

void f31(void) {
	static uint16_t x157 = 29U;
	static int8_t x158 = -3;
	static int16_t x160 = INT16_MAX;
	int32_t t31 = -427464058;

	t31 = (x157^((x158+x159)<=x160));

	if (t31 != 29) { NG(); } else { ; }
	
}

void f32(void) {
	uint32_t x161 = UINT32_MAX;
	uint64_t x162 = 126790632708LLU;
	volatile uint32_t t32 = 657415U;

	t32 = (x161^((x162+x163)<=x164));

	if (t32 != 4294967294U) { NG(); } else { ; }
	
}

void f33(void) {
	volatile uint64_t x165 = 28020233605LLU;
	int16_t x166 = INT16_MAX;
	static uint8_t x167 = 0U;
	int64_t x168 = INT64_MIN;
	uint64_t t33 = 15LLU;

	t33 = (x165^((x166+x167)<=x168));

	if (t33 != 28020233605LLU) { NG(); } else { ; }
	
}

void f34(void) {
	static uint8_t x169 = 7U;
	int16_t x170 = INT16_MIN;
	int16_t x171 = -1;
	volatile int32_t t34 = 3785;

	t34 = (x169^((x170+x171)<=x172));

	if (t34 != 6) { NG(); } else { ; }
	
}

void f35(void) {
	volatile uint32_t x177 = 41U;
	volatile int8_t x179 = INT8_MIN;
	volatile int8_t x180 = INT8_MIN;
	volatile uint32_t t35 = 865U;

	t35 = (x177^((x178+x179)<=x180));

	if (t35 != 40U) { NG(); } else { ; }
	
}

void f36(void) {
	uint32_t x181 = UINT32_MAX;
	static int32_t x183 = 96737;

	t36 = (x181^((x182+x183)<=x184));

	if (t36 != UINT32_MAX) { NG(); } else { ; }
	
}

void f37(void) {
	int16_t x185 = INT16_MIN;
	int8_t x186 = 8;
	int8_t x187 = -5;
	int16_t x188 = INT16_MIN;

	t37 = (x185^((x186+x187)<=x188));

	if (t37 != -32768) { NG(); } else { ; }
	
}

void f38(void) {
	volatile int16_t x189 = INT16_MIN;
	int64_t x190 = 22747865LL;
	static volatile int32_t t38 = 3;

	t38 = (x189^((x190+x191)<=x192));

	if (t38 != -32767) { NG(); } else { ; }
	
}

void f39(void) {
	static uint32_t x193 = 529U;
	volatile int64_t x194 = INT64_MIN;
	static volatile uint16_t x195 = UINT16_MAX;
	uint32_t t39 = 7414962U;

	t39 = (x193^((x194+x195)<=x196));

	if (t39 != 528U) { NG(); } else { ; }
	
}

void f40(void) {
	volatile uint32_t x201 = 7U;
	uint64_t x203 = UINT64_MAX;
	uint16_t x204 = UINT16_MAX;
	uint32_t t40 = 14803U;

	t40 = (x201^((x202+x203)<=x204));

	if (t40 != 7U) { NG(); } else { ; }
	
}

void f41(void) {
	int32_t x205 = INT32_MIN;
	volatile uint32_t x206 = 3093U;
	uint32_t x207 = 0U;
	int8_t x208 = INT8_MIN;

	t41 = (x205^((x206+x207)<=x208));

	if (t41 != -2147483647) { NG(); } else { ; }
	
}

void f42(void) {
	int16_t x213 = -381;
	int16_t x214 = -3;
	static volatile uint16_t x215 = UINT16_MAX;
	volatile int16_t x216 = INT16_MIN;

	t42 = (x213^((x214+x215)<=x216));

	if (t42 != -381) { NG(); } else { ; }
	
}

void f43(void) {
	int64_t x217 = INT64_MIN;
	uint16_t x218 = UINT16_MAX;
	static uint16_t x219 = 3U;
	int64_t t43 = 13969434LL;

	t43 = (x217^((x218+x219)<=x220));

	if (t43 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f44(void) {
	int16_t x222 = 502;
	int8_t x223 = INT8_MIN;

	t44 = (x221^((x222+x223)<=x224));

	if (t44 != 0) { NG(); } else { ; }
	
}

void f45(void) {
	int16_t x225 = 1341;
	uint64_t x226 = 145160125197LLU;
	int64_t x227 = INT64_MIN;
	volatile int8_t x228 = INT8_MAX;
	volatile int32_t t45 = 1403;

	t45 = (x225^((x226+x227)<=x228));

	if (t45 != 1341) { NG(); } else { ; }
	
}

void f46(void) {
	int8_t x230 = 0;
	int32_t x231 = -1;
	uint64_t x232 = 42802387354363LLU;
	int32_t t46 = -2247;

	t46 = (x229^((x230+x231)<=x232));

	if (t46 != -4) { NG(); } else { ; }
	
}

void f47(void) {
	volatile int16_t x235 = INT16_MAX;
	int32_t x236 = INT32_MIN;
	static int32_t t47 = -56333;

	t47 = (x233^((x234+x235)<=x236));

	if (t47 != 0) { NG(); } else { ; }
	
}

void f48(void) {
	static volatile int64_t x245 = 1300868229126LL;
	uint8_t x246 = UINT8_MAX;
	uint16_t x247 = 996U;
	int64_t t48 = -1505503641843396040LL;

	t48 = (x245^((x246+x247)<=x248));

	if (t48 != 1300868229126LL) { NG(); } else { ; }
	
}

void f49(void) {
	int8_t x253 = 0;
	int8_t x254 = 0;
	int8_t x256 = INT8_MIN;
	int32_t t49 = -14;

	t49 = (x253^((x254+x255)<=x256));

	if (t49 != 0) { NG(); } else { ; }
	
}

void f50(void) {
	uint16_t x257 = 24U;
	uint64_t x258 = UINT64_MAX;
	volatile int64_t x259 = -416489LL;
	int32_t x260 = -6066220;
	static int32_t t50 = 5241737;

	t50 = (x257^((x258+x259)<=x260));

	if (t50 != 24) { NG(); } else { ; }
	
}

void f51(void) {
	int8_t x261 = -1;
	int32_t x262 = -1;
	int64_t x263 = 11141LL;
	int8_t x264 = 3;
	int32_t t51 = 36414784;

	t51 = (x261^((x262+x263)<=x264));

	if (t51 != -1) { NG(); } else { ; }
	
}

void f52(void) {
	volatile uint16_t x265 = 1U;
	static int32_t x266 = -4323;
	int16_t x267 = -1;
	volatile uint8_t x268 = 8U;
	int32_t t52 = -114539;

	t52 = (x265^((x266+x267)<=x268));

	if (t52 != 0) { NG(); } else { ; }
	
}

void f53(void) {
	int16_t x269 = INT16_MIN;
	static uint8_t x271 = 16U;
	int16_t x272 = 71;

	t53 = (x269^((x270+x271)<=x272));

	if (t53 != -32767) { NG(); } else { ; }
	
}

void f54(void) {
	static volatile int32_t x277 = -46577926;
	int64_t x278 = 603985522006771294LL;
	static uint32_t x279 = UINT32_MAX;
	volatile int8_t x280 = INT8_MAX;
	static volatile int32_t t54 = 12;

	t54 = (x277^((x278+x279)<=x280));

	if (t54 != -46577926) { NG(); } else { ; }
	
}

void f55(void) {
	int64_t x281 = INT64_MIN;
	int32_t x282 = INT32_MIN;
	volatile uint32_t x283 = 1U;
	volatile int32_t x284 = INT32_MIN;
	int64_t t55 = INT64_MIN;

	t55 = (x281^((x282+x283)<=x284));

	if (t55 != INT64_MIN) { NG(); } else { ; }
	
}

void f56(void) {
	static int16_t x289 = 442;
	int16_t x290 = 14;
	int64_t x292 = -1LL;
	volatile int32_t t56 = -14783;

	t56 = (x289^((x290+x291)<=x292));

	if (t56 != 442) { NG(); } else { ; }
	
}

void f57(void) {
	int16_t x293 = INT16_MIN;
	int8_t x295 = INT8_MIN;
	int16_t x296 = INT16_MIN;
	volatile int32_t t57 = 192;

	t57 = (x293^((x294+x295)<=x296));

	if (t57 != -32768) { NG(); } else { ; }
	
}

void f58(void) {
	static int16_t x297 = INT16_MAX;
	uint16_t x299 = UINT16_MAX;

	t58 = (x297^((x298+x299)<=x300));

	if (t58 != 32766) { NG(); } else { ; }
	
}

void f59(void) {
	int16_t x301 = INT16_MIN;
	int16_t x302 = 0;
	uint16_t x303 = 15U;
	static uint16_t x304 = UINT16_MAX;
	volatile int32_t t59 = 304;

	t59 = (x301^((x302+x303)<=x304));

	if (t59 != -32767) { NG(); } else { ; }
	
}

void f60(void) {
	int64_t x309 = INT64_MAX;
	int16_t x311 = INT16_MIN;
	volatile int64_t t60 = INT64_MAX;

	t60 = (x309^((x310+x311)<=x312));

	if (t60 != INT64_MAX) { NG(); } else { ; }
	
}

void f61(void) {
	static uint8_t x314 = 1U;
	int32_t x316 = -24605;
	int32_t t61 = 2;

	t61 = (x313^((x314+x315)<=x316));

	if (t61 != -128) { NG(); } else { ; }
	
}

void f62(void) {
	static int8_t x317 = INT8_MAX;
	volatile int16_t x318 = 0;
	int64_t x320 = INT64_MAX;
	int32_t t62 = -15797;

	t62 = (x317^((x318+x319)<=x320));

	if (t62 != 126) { NG(); } else { ; }
	
}

void f63(void) {
	volatile uint16_t x321 = UINT16_MAX;
	uint8_t x323 = 1U;
	int32_t x324 = INT32_MAX;
	volatile int32_t t63 = -17840296;

	t63 = (x321^((x322+x323)<=x324));

	if (t63 != 65535) { NG(); } else { ; }
	
}

void f64(void) {
	volatile int16_t x329 = 25;
	int16_t x330 = INT16_MAX;
	uint32_t x331 = 889U;
	int64_t x332 = -1LL;

	t64 = (x329^((x330+x331)<=x332));

	if (t64 != 25) { NG(); } else { ; }
	
}

void f65(void) {
	static volatile int32_t x337 = INT32_MIN;
	int32_t x338 = 95471;
	uint8_t x339 = UINT8_MAX;
	uint64_t x340 = 1621874LLU;
	volatile int32_t t65 = 6;

	t65 = (x337^((x338+x339)<=x340));

	if (t65 != -2147483647) { NG(); } else { ; }
	
}

void f66(void) {
	int8_t x341 = INT8_MIN;
	int8_t x342 = -3;
	static uint16_t x343 = UINT16_MAX;
	uint32_t x344 = UINT32_MAX;

	t66 = (x341^((x342+x343)<=x344));

	if (t66 != -127) { NG(); } else { ; }
	
}

void f67(void) {
	static int8_t x345 = INT8_MIN;
	uint64_t x348 = UINT64_MAX;
	int32_t t67 = -6129;

	t67 = (x345^((x346+x347)<=x348));

	if (t67 != -127) { NG(); } else { ; }
	
}

void f68(void) {
	uint64_t x350 = UINT64_MAX;
	int64_t x351 = 185095LL;
	int8_t x352 = INT8_MIN;
	int64_t t68 = 103LL;

	t68 = (x349^((x350+x351)<=x352));

	if (t68 != -2LL) { NG(); } else { ; }
	
}

void f69(void) {
	int64_t x353 = INT64_MIN;
	uint16_t x354 = 0U;
	static int8_t x355 = -1;
	int32_t x356 = -30;
	volatile int64_t t69 = INT64_MIN;

	t69 = (x353^((x354+x355)<=x356));

	if (t69 != INT64_MIN) { NG(); } else { ; }
	
}

void f70(void) {
	volatile int8_t x361 = INT8_MIN;
	int8_t x363 = -1;
	volatile int64_t x364 = INT64_MIN;

	t70 = (x361^((x362+x363)<=x364));

	if (t70 != -128) { NG(); } else { ; }
	
}

void f71(void) {
	uint32_t x366 = 502U;
	int8_t x368 = 0;
	volatile uint32_t t71 = 79U;

	t71 = (x365^((x366+x367)<=x368));

	if (t71 != 22920U) { NG(); } else { ; }
	
}

void f72(void) {
	int8_t x369 = INT8_MAX;
	int32_t x370 = 58168525;
	uint16_t x371 = UINT16_MAX;
	uint32_t x372 = UINT32_MAX;
	volatile int32_t t72 = 31;

	t72 = (x369^((x370+x371)<=x372));

	if (t72 != 126) { NG(); } else { ; }
	
}

void f73(void) {
	uint16_t x374 = 8424U;
	static int8_t x375 = -1;
	static int32_t x376 = 1388623;
	volatile int32_t t73 = 656185;

	t73 = (x373^((x374+x375)<=x376));

	if (t73 != -2) { NG(); } else { ; }
	
}

void f74(void) {
	int64_t x378 = -1LL;
	volatile int16_t x379 = 519;
	volatile uint32_t x380 = 57323U;
	uint32_t t74 = 353560094U;

	t74 = (x377^((x378+x379)<=x380));

	if (t74 != 6U) { NG(); } else { ; }
	
}

void f75(void) {
	volatile uint16_t x383 = 118U;
	static uint16_t x384 = 14162U;
	int32_t t75 = -338373217;

	t75 = (x381^((x382+x383)<=x384));

	if (t75 != -32767) { NG(); } else { ; }
	
}

void f76(void) {
	uint64_t x385 = 32088885457916136LLU;
	int32_t x386 = INT32_MIN;
	int64_t x387 = INT64_MAX;
	uint32_t x388 = 450538U;
	uint64_t t76 = 12174743532728LLU;

	t76 = (x385^((x386+x387)<=x388));

	if (t76 != 32088885457916136LLU) { NG(); } else { ; }
	
}

void f77(void) {
	static volatile int32_t x389 = -197475579;
	int64_t x390 = -6300903LL;
	static uint16_t x392 = 3U;
	int32_t t77 = -12519366;

	t77 = (x389^((x390+x391)<=x392));

	if (t77 != -197475580) { NG(); } else { ; }
	
}

void f78(void) {
	int64_t x397 = INT64_MIN;
	volatile uint16_t x398 = 6589U;
	volatile int64_t x399 = INT64_MIN;
	volatile uint8_t x400 = 1U;
	static volatile int64_t t78 = -211716935177LL;

	t78 = (x397^((x398+x399)<=x400));

	if (t78 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f79(void) {
	int16_t x401 = -2690;
	volatile int16_t x402 = 12248;

	t79 = (x401^((x402+x403)<=x404));

	if (t79 != -2689) { NG(); } else { ; }
	
}

void f80(void) {
	int16_t x405 = -419;
	int32_t x407 = 32385;
	uint64_t x408 = UINT64_MAX;

	t80 = (x405^((x406+x407)<=x408));

	if (t80 != -420) { NG(); } else { ; }
	
}

void f81(void) {
	int32_t x409 = -1;
	volatile int64_t x410 = 211LL;
	int32_t x411 = INT32_MIN;
	uint64_t x412 = 12028778LLU;
	volatile int32_t t81 = -3400;

	t81 = (x409^((x410+x411)<=x412));

	if (t81 != -1) { NG(); } else { ; }
	
}

void f82(void) {
	static volatile int64_t x422 = -5LL;
	volatile int8_t x423 = INT8_MIN;

	t82 = (x421^((x422+x423)<=x424));

	if (t82 != INT64_MIN) { NG(); } else { ; }
	
}

void f83(void) {
	int32_t x425 = INT32_MAX;
	volatile int64_t x426 = INT64_MAX;
	int16_t x427 = -1;
	uint32_t x428 = 133843U;
	volatile int32_t t83 = INT32_MAX;

	t83 = (x425^((x426+x427)<=x428));

	if (t83 != INT32_MAX) { NG(); } else { ; }
	
}

void f84(void) {
	int64_t x432 = INT64_MAX;
	static volatile int32_t t84 = INT32_MIN;

	t84 = (x429^((x430+x431)<=x432));

	if (t84 != INT32_MIN) { NG(); } else { ; }
	
}

void f85(void) {
	int8_t x433 = INT8_MAX;
	int16_t x434 = INT16_MIN;
	uint8_t x435 = 11U;
	uint16_t x436 = UINT16_MAX;
	volatile int32_t t85 = -174142;

	t85 = (x433^((x434+x435)<=x436));

	if (t85 != 126) { NG(); } else { ; }
	
}

void f86(void) {
	int16_t x437 = -1;
	uint8_t x438 = 53U;
	int16_t x439 = 3116;
	int32_t x440 = INT32_MAX;

	t86 = (x437^((x438+x439)<=x440));

	if (t86 != -2) { NG(); } else { ; }
	
}

void f87(void) {
	static int64_t x441 = -1LL;
	int32_t x442 = INT32_MAX;
	volatile int64_t x444 = INT64_MAX;
	int64_t t87 = -530254902510LL;

	t87 = (x441^((x442+x443)<=x444));

	if (t87 != -2LL) { NG(); } else { ; }
	
}

void f88(void) {
	static int8_t x449 = INT8_MAX;
	int16_t x450 = INT16_MIN;
	static volatile int32_t t88 = -48854261;

	t88 = (x449^((x450+x451)<=x452));

	if (t88 != 127) { NG(); } else { ; }
	
}

void f89(void) {
	volatile int8_t x453 = 0;
	int64_t x454 = -1LL;
	static uint16_t x455 = 85U;
	int32_t x456 = INT32_MAX;
	volatile int32_t t89 = 59843;

	t89 = (x453^((x454+x455)<=x456));

	if (t89 != 1) { NG(); } else { ; }
	
}

void f90(void) {
	int32_t x457 = -1;
	volatile int8_t x458 = INT8_MIN;
	int8_t x460 = INT8_MIN;
	int32_t t90 = -389;

	t90 = (x457^((x458+x459)<=x460));

	if (t90 != -2) { NG(); } else { ; }
	
}

void f91(void) {
	int8_t x461 = INT8_MAX;
	int8_t x462 = -1;
	uint8_t x463 = 5U;
	int32_t x464 = INT32_MIN;
	volatile int32_t t91 = -276372;

	t91 = (x461^((x462+x463)<=x464));

	if (t91 != 127) { NG(); } else { ; }
	
}

void f92(void) {
	int16_t x465 = INT16_MAX;
	volatile int8_t x466 = -1;
	int32_t x467 = -656429;
	int8_t x468 = INT8_MAX;
	int32_t t92 = 281;

	t92 = (x465^((x466+x467)<=x468));

	if (t92 != 32766) { NG(); } else { ; }
	
}

void f93(void) {
	int16_t x473 = INT16_MAX;
	int64_t x476 = -1LL;
	int32_t t93 = 139;

	t93 = (x473^((x474+x475)<=x476));

	if (t93 != 32767) { NG(); } else { ; }
	
}

void f94(void) {
	static uint32_t x477 = 6U;
	int16_t x478 = -1;
	int16_t x479 = -1;
	volatile uint32_t t94 = 465120U;

	t94 = (x477^((x478+x479)<=x480));

	if (t94 != 6U) { NG(); } else { ; }
	
}

void f95(void) {
	int64_t x481 = 184520466LL;
	int8_t x482 = INT8_MIN;
	static volatile int64_t t95 = 1407259873LL;

	t95 = (x481^((x482+x483)<=x484));

	if (t95 != 184520466LL) { NG(); } else { ; }
	
}

void f96(void) {
	static uint32_t x485 = 1554411545U;
	static volatile int64_t x486 = -16728402492LL;
	int8_t x487 = 0;
	static uint32_t t96 = 7534U;

	t96 = (x485^((x486+x487)<=x488));

	if (t96 != 1554411544U) { NG(); } else { ; }
	
}

void f97(void) {
	uint8_t x489 = 6U;
	volatile int8_t x490 = 0;
	volatile int8_t x491 = 0;

	t97 = (x489^((x490+x491)<=x492));

	if (t97 != 7) { NG(); } else { ; }
	
}

void f98(void) {
	static int8_t x493 = -1;
	uint8_t x494 = 6U;
	volatile uint64_t x495 = 8840172411346LLU;
	static uint32_t x496 = UINT32_MAX;
	volatile int32_t t98 = -111782857;

	t98 = (x493^((x494+x495)<=x496));

	if (t98 != -1) { NG(); } else { ; }
	
}

void f99(void) {
	uint64_t x500 = 389LLU;
	volatile int64_t t99 = INT64_MIN;

	t99 = (x497^((x498+x499)<=x500));

	if (t99 != INT64_MIN) { NG(); } else { ; }
	
}

int main(void) {
    f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();


    return 0;
}

