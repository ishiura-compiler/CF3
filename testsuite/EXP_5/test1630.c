
/*
CF3

Copyright (c) 2015 ishiura-lab.

Released under the MIT license.  
https://github.com/ishiura-compiler/CF3/MIT-LICENSE.md
*/

#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

uint16_t x5 = 10082U;
static volatile uint64_t x7 = 12691477002LLU;
static int8_t x9 = INT8_MAX;
int32_t t1 = 5;
uint64_t x21 = UINT64_MAX;
int8_t x24 = INT8_MIN;
uint16_t x27 = 10U;
uint32_t x31 = 159U;
volatile uint32_t x35 = 8084U;
uint64_t x39 = 26432895271953LLU;
int32_t x48 = -1;
uint64_t x53 = 628207706554172425LLU;
static uint32_t x67 = UINT32_MAX;
volatile uint8_t x101 = 39U;
static volatile int32_t t17 = 15743166;
uint16_t x105 = 9596U;
int32_t x107 = INT32_MIN;
int8_t x108 = -1;
uint8_t x125 = 72U;
static int8_t x128 = INT8_MIN;
static int64_t x136 = INT64_MIN;
int64_t x167 = INT64_MIN;
int64_t t27 = 908867619LL;
volatile uint64_t x189 = UINT64_MAX;
int8_t x192 = 1;
uint64_t t30 = UINT64_MAX;
volatile uint64_t x202 = 16292073LLU;
int64_t x205 = INT64_MAX;
uint16_t x209 = UINT16_MAX;
uint64_t x212 = 2046711461LLU;
int32_t x214 = INT32_MIN;
uint32_t x225 = 273877945U;
uint32_t t36 = 25605U;
uint8_t x236 = 22U;
volatile int8_t x248 = -1;
volatile int64_t x273 = 2358105431LL;
int64_t x275 = 0LL;
volatile int16_t x276 = INT16_MIN;
static uint32_t x281 = 261438U;
int64_t x282 = INT64_MIN;
uint32_t t44 = 1194938U;
volatile int32_t x286 = -1;
uint64_t x304 = UINT64_MAX;
int64_t x307 = -329451926482LL;
uint8_t x309 = 6U;
static volatile int32_t x310 = INT32_MIN;
uint32_t x311 = 81U;
volatile int32_t t48 = 11;
volatile int32_t t49 = -7109270;
uint32_t x329 = 3937427U;
volatile int16_t x347 = 16349;
int32_t t52 = 9260049;
int8_t x352 = INT8_MIN;
static volatile int32_t t53 = 1;
int8_t x370 = INT8_MAX;
int64_t x394 = INT64_MAX;
static int16_t x424 = INT16_MAX;
static volatile int8_t x455 = INT8_MIN;
volatile int32_t x471 = -1;
int8_t x475 = INT8_MIN;
uint32_t x486 = UINT32_MAX;
volatile int32_t t71 = -1;
int8_t x506 = INT8_MIN;
volatile int32_t t73 = -16906632;
volatile uint32_t x510 = UINT32_MAX;
volatile int8_t x520 = 1;
int8_t x522 = -1;
static int16_t x540 = INT16_MIN;
int32_t t79 = -5;
int8_t x556 = 2;
static int32_t x557 = 3;
int8_t x563 = INT8_MIN;
volatile int8_t x564 = INT8_MAX;
static int32_t t85 = -10940;
int32_t t87 = -75111;
static int16_t x609 = 676;
static uint8_t x612 = UINT8_MAX;
static int8_t x627 = INT8_MIN;
uint8_t x628 = 24U;
uint64_t x629 = 779370318667185LLU;
static int64_t x657 = 3132800691590545335LL;
uint32_t x660 = 57954U;
uint64_t x673 = 189532279030997LLU;
int16_t x679 = -1;
int64_t x686 = -1LL;
uint16_t x689 = UINT16_MAX;
static volatile int32_t t101 = 262410;
volatile int8_t x693 = 4;
static int64_t x694 = 1993329590968935LL;
int16_t x704 = INT16_MIN;
static volatile int32_t t103 = INT32_MAX;
uint32_t x713 = 533533620U;
int8_t x718 = -1;
int8_t x731 = -1;
int32_t t108 = -990458598;
uint16_t x749 = 11U;
uint64_t x750 = 552557824213LLU;
static int64_t x782 = -1LL;
int64_t x784 = INT64_MIN;
int8_t x798 = 0;
int16_t x812 = -1;
uint16_t x827 = UINT16_MAX;
static int8_t x831 = INT8_MAX;
uint64_t x837 = 1102897734933097367LLU;
uint64_t x844 = 1605258LLU;
int32_t x863 = INT32_MIN;
int8_t x866 = INT8_MIN;
volatile uint16_t x873 = UINT16_MAX;
int16_t x877 = 3;
uint64_t x885 = UINT64_MAX;
int16_t x886 = -1;
volatile int8_t x887 = INT8_MAX;
uint64_t t128 = UINT64_MAX;
uint8_t x892 = 101U;
uint16_t x901 = 335U;
static int8_t x902 = -7;
int16_t x918 = 70;
int16_t x932 = INT16_MIN;
uint64_t t135 = 915197558LLU;
volatile uint16_t x937 = 0U;
volatile int16_t x951 = INT16_MIN;
volatile uint16_t x962 = 2U;
volatile int16_t x969 = 24;
volatile uint32_t x970 = 10U;
static uint8_t x981 = UINT8_MAX;
int32_t t144 = -14;
static uint64_t x1001 = 63920666034610414LLU;
uint8_t x1014 = UINT8_MAX;
uint32_t t148 = 15465245U;
int64_t x1021 = 0LL;
volatile int64_t t149 = -20909325328LL;
static int16_t x1028 = INT16_MIN;
uint8_t x1033 = UINT8_MAX;
uint32_t x1036 = 2787808U;
int32_t x1038 = -2644641;
uint32_t x1040 = UINT32_MAX;
volatile uint8_t x1052 = 55U;
uint32_t x1069 = 1219U;
int8_t x1071 = INT8_MIN;
volatile uint32_t t156 = 781U;
uint16_t x1097 = 30U;
static volatile uint64_t x1107 = 69681704908515712LLU;
volatile int16_t x1111 = -1;
int64_t x1113 = 4804640545LL;
int32_t x1114 = -1;
uint8_t x1117 = UINT8_MAX;
volatile int32_t t163 = -1536699;
static uint8_t x1146 = UINT8_MAX;
int8_t x1150 = 60;
static int32_t t166 = 0;
static int64_t x1177 = 0LL;
uint32_t x1179 = UINT32_MAX;
volatile int64_t t168 = -200LL;
int32_t x1200 = INT32_MIN;
uint64_t x1208 = 290832800521942726LLU;
int16_t x1212 = -1;
uint64_t t173 = 61LLU;
uint8_t x1238 = 31U;
volatile uint32_t t174 = 290186U;
int8_t x1241 = 31;
int32_t x1242 = -403463;
uint8_t x1244 = UINT8_MAX;
volatile int32_t t175 = -49335685;
int64_t x1267 = -1LL;
uint32_t x1277 = 5554U;
uint32_t x1282 = 714766U;
volatile int32_t x1284 = INT32_MAX;
int32_t t178 = INT32_MAX;
volatile uint64_t x1287 = UINT64_MAX;
volatile int8_t x1290 = INT8_MIN;
int32_t x1292 = -1;
int64_t x1294 = -1LL;
int64_t x1300 = -11LL;
uint16_t x1313 = 19381U;
int64_t x1314 = -1LL;
int64_t x1350 = INT64_MAX;
uint32_t t188 = 556U;
uint32_t x1369 = 3U;
int8_t x1384 = -1;
int16_t x1390 = -1;
int32_t x1398 = -799834904;
uint64_t x1401 = UINT64_MAX;
volatile uint64_t t194 = UINT64_MAX;
static uint64_t x1406 = 442572LLU;
volatile int64_t x1409 = INT64_MAX;
uint64_t x1416 = 382676565547660LLU;
uint32_t t197 = 43138079U;


void f0(void) {
    	uint16_t x6 = UINT16_MAX;
	uint16_t x8 = 48U;
	volatile int32_t t0 = -178474;

    t0 = (x5<<((x6>x7)%x8));

    if (t0 != 10082) { NG(); } else { ; }
	
}

void f1(void) {
    	volatile uint8_t x10 = 39U;
	int64_t x11 = -1LL;
	int64_t x12 = INT64_MAX;

    t1 = (x9<<((x10>x11)%x12));

    if (t1 != 254) { NG(); } else { ; }
	
}

void f2(void) {
    	uint32_t x13 = UINT32_MAX;
	int8_t x14 = -1;
	int16_t x15 = INT16_MIN;
	int32_t x16 = -4721126;
	volatile uint32_t t2 = 3120U;

    t2 = (x13<<((x14>x15)%x16));

    if (t2 != 4294967294U) { NG(); } else { ; }
	
}

void f3(void) {
    	int16_t x22 = INT16_MIN;
	uint64_t x23 = 131372224454160LLU;
	uint64_t t3 = 4289207297003213LLU;

    t3 = (x21<<((x22>x23)%x24));

    if (t3 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f4(void) {
    	int64_t x25 = INT64_MAX;
	static uint8_t x26 = UINT8_MAX;
	static uint64_t x28 = 1LLU;
	int64_t t4 = INT64_MAX;

    t4 = (x25<<((x26>x27)%x28));

    if (t4 != INT64_MAX) { NG(); } else { ; }
	
}

void f5(void) {
    	int64_t x29 = 8281590049182LL;
	int32_t x30 = -3;
	static int32_t x32 = -1;
	int64_t t5 = -4329489178LL;

    t5 = (x29<<((x30>x31)%x32));

    if (t5 != 8281590049182LL) { NG(); } else { ; }
	
}

void f6(void) {
    	uint16_t x33 = 26U;
	int32_t x34 = 0;
	int32_t x36 = -6245;
	static volatile int32_t t6 = -71349;

    t6 = (x33<<((x34>x35)%x36));

    if (t6 != 26) { NG(); } else { ; }
	
}

void f7(void) {
    	uint8_t x37 = 12U;
	int64_t x38 = -9LL;
	static int32_t x40 = INT32_MIN;
	int32_t t7 = 7383066;

    t7 = (x37<<((x38>x39)%x40));

    if (t7 != 24) { NG(); } else { ; }
	
}

void f8(void) {
    	uint16_t x41 = 75U;
	int8_t x42 = INT8_MIN;
	volatile int64_t x43 = INT64_MIN;
	volatile int16_t x44 = INT16_MIN;
	volatile int32_t t8 = 604985;

    t8 = (x41<<((x42>x43)%x44));

    if (t8 != 150) { NG(); } else { ; }
	
}

void f9(void) {
    	uint8_t x45 = UINT8_MAX;
	uint32_t x46 = 71134049U;
	volatile int64_t x47 = INT64_MIN;
	volatile int32_t t9 = -1463;

    t9 = (x45<<((x46>x47)%x48));

    if (t9 != 255) { NG(); } else { ; }
	
}

void f10(void) {
    	uint8_t x54 = 3U;
	int64_t x55 = 18566940544784069LL;
	uint32_t x56 = 601U;
	uint64_t t10 = 1417739698LLU;

    t10 = (x53<<((x54>x55)%x56));

    if (t10 != 628207706554172425LLU) { NG(); } else { ; }
	
}

void f11(void) {
    	uint16_t x57 = 5U;
	uint64_t x58 = UINT64_MAX;
	volatile uint8_t x59 = 2U;
	volatile int32_t x60 = INT32_MIN;
	volatile int32_t t11 = -2692594;

    t11 = (x57<<((x58>x59)%x60));

    if (t11 != 10) { NG(); } else { ; }
	
}

void f12(void) {
    	uint16_t x65 = 23U;
	volatile int8_t x66 = -1;
	uint16_t x68 = UINT16_MAX;
	int32_t t12 = 86345;

    t12 = (x65<<((x66>x67)%x68));

    if (t12 != 23) { NG(); } else { ; }
	
}

void f13(void) {
    	uint64_t x69 = UINT64_MAX;
	int16_t x70 = -293;
	uint8_t x71 = 25U;
	volatile uint8_t x72 = 8U;
	volatile uint64_t t13 = UINT64_MAX;

    t13 = (x69<<((x70>x71)%x72));

    if (t13 != UINT64_MAX) { NG(); } else { ; }
	
}

void f14(void) {
    	static uint8_t x73 = 58U;
	int32_t x74 = 4;
	uint8_t x75 = UINT8_MAX;
	int16_t x76 = 3;
	volatile int32_t t14 = -1;

    t14 = (x73<<((x74>x75)%x76));

    if (t14 != 58) { NG(); } else { ; }
	
}

void f15(void) {
    	int16_t x89 = 2;
	int32_t x90 = INT32_MIN;
	int32_t x91 = INT32_MAX;
	volatile int32_t x92 = INT32_MIN;
	volatile int32_t t15 = -198320816;

    t15 = (x89<<((x90>x91)%x92));

    if (t15 != 2) { NG(); } else { ; }
	
}

void f16(void) {
    	int64_t x97 = 2LL;
	int64_t x98 = INT64_MIN;
	uint16_t x99 = UINT16_MAX;
	uint16_t x100 = UINT16_MAX;
	int64_t t16 = 33948645516156LL;

    t16 = (x97<<((x98>x99)%x100));

    if (t16 != 2LL) { NG(); } else { ; }
	
}

void f17(void) {
    	static uint64_t x102 = 0LLU;
	volatile uint64_t x103 = 48481LLU;
	uint8_t x104 = 3U;

    t17 = (x101<<((x102>x103)%x104));

    if (t17 != 39) { NG(); } else { ; }
	
}

void f18(void) {
    	volatile uint64_t x106 = 1131943912247602074LLU;
	volatile int32_t t18 = -773302;

    t18 = (x105<<((x106>x107)%x108));

    if (t18 != 9596) { NG(); } else { ; }
	
}

void f19(void) {
    	int16_t x117 = 2547;
	int8_t x118 = INT8_MIN;
	int16_t x119 = 6;
	uint8_t x120 = UINT8_MAX;
	int32_t t19 = 14573;

    t19 = (x117<<((x118>x119)%x120));

    if (t19 != 2547) { NG(); } else { ; }
	
}

void f20(void) {
    	static int64_t x126 = INT64_MIN;
	int8_t x127 = INT8_MIN;
	volatile int32_t t20 = -63828614;

    t20 = (x125<<((x126>x127)%x128));

    if (t20 != 72) { NG(); } else { ; }
	
}

void f21(void) {
    	uint16_t x129 = 885U;
	volatile int64_t x130 = INT64_MAX;
	int8_t x131 = INT8_MIN;
	uint64_t x132 = 80736817LLU;
	volatile int32_t t21 = 3826;

    t21 = (x129<<((x130>x131)%x132));

    if (t21 != 1770) { NG(); } else { ; }
	
}

void f22(void) {
    	uint64_t x133 = 1LLU;
	int32_t x134 = -1;
	uint8_t x135 = 123U;
	static volatile uint64_t t22 = 37589341LLU;

    t22 = (x133<<((x134>x135)%x136));

    if (t22 != 1LLU) { NG(); } else { ; }
	
}

void f23(void) {
    	volatile uint8_t x141 = UINT8_MAX;
	int64_t x142 = -4456LL;
	static volatile int8_t x143 = 2;
	static int8_t x144 = INT8_MIN;
	volatile int32_t t23 = -1;

    t23 = (x141<<((x142>x143)%x144));

    if (t23 != 255) { NG(); } else { ; }
	
}

void f24(void) {
    	int32_t x149 = 289050716;
	int16_t x150 = -1;
	int16_t x151 = 4867;
	uint64_t x152 = UINT64_MAX;
	static int32_t t24 = -338439;

    t24 = (x149<<((x150>x151)%x152));

    if (t24 != 289050716) { NG(); } else { ; }
	
}

void f25(void) {
    	int16_t x165 = INT16_MAX;
	int32_t x166 = 47118;
	uint8_t x168 = UINT8_MAX;
	volatile int32_t t25 = -103450281;

    t25 = (x165<<((x166>x167)%x168));

    if (t25 != 65534) { NG(); } else { ; }
	
}

void f26(void) {
    	static uint8_t x169 = 72U;
	volatile int64_t x170 = INT64_MAX;
	int64_t x171 = -2111608644859LL;
	int64_t x172 = INT64_MIN;
	int32_t t26 = -1;

    t26 = (x169<<((x170>x171)%x172));

    if (t26 != 144) { NG(); } else { ; }
	
}

void f27(void) {
    	int64_t x177 = 1133245906863LL;
	uint32_t x178 = 28232U;
	uint32_t x179 = 572921U;
	int64_t x180 = INT64_MAX;

    t27 = (x177<<((x178>x179)%x180));

    if (t27 != 1133245906863LL) { NG(); } else { ; }
	
}

void f28(void) {
    	static volatile int8_t x181 = INT8_MAX;
	static int8_t x182 = INT8_MIN;
	static int8_t x183 = INT8_MAX;
	uint32_t x184 = 5574528U;
	static int32_t t28 = 1279;

    t28 = (x181<<((x182>x183)%x184));

    if (t28 != 127) { NG(); } else { ; }
	
}

void f29(void) {
    	static uint32_t x185 = 8320786U;
	int32_t x186 = -175120669;
	uint32_t x187 = UINT32_MAX;
	uint8_t x188 = 1U;
	uint32_t t29 = 7745U;

    t29 = (x185<<((x186>x187)%x188));

    if (t29 != 8320786U) { NG(); } else { ; }
	
}

void f30(void) {
    	static int32_t x190 = -5;
	int64_t x191 = INT64_MAX;

    t30 = (x189<<((x190>x191)%x192));

    if (t30 != UINT64_MAX) { NG(); } else { ; }
	
}

void f31(void) {
    	uint64_t x197 = 0LLU;
	static int64_t x198 = -1LL;
	static int16_t x199 = INT16_MAX;
	int32_t x200 = INT32_MIN;
	volatile uint64_t t31 = 171418629840148LLU;

    t31 = (x197<<((x198>x199)%x200));

    if (t31 != 0LLU) { NG(); } else { ; }
	
}

void f32(void) {
    	uint8_t x201 = UINT8_MAX;
	int8_t x203 = 0;
	volatile uint64_t x204 = UINT64_MAX;
	int32_t t32 = 0;

    t32 = (x201<<((x202>x203)%x204));

    if (t32 != 510) { NG(); } else { ; }
	
}

void f33(void) {
    	static uint64_t x206 = 1945993538LLU;
	static int32_t x207 = INT32_MIN;
	static uint32_t x208 = UINT32_MAX;
	static int64_t t33 = INT64_MAX;

    t33 = (x205<<((x206>x207)%x208));

    if (t33 != INT64_MAX) { NG(); } else { ; }
	
}

void f34(void) {
    	volatile int8_t x210 = INT8_MAX;
	volatile int64_t x211 = -1LL;
	volatile int32_t t34 = -19267046;

    t34 = (x209<<((x210>x211)%x212));

    if (t34 != 131070) { NG(); } else { ; }
	
}

void f35(void) {
    	static uint8_t x213 = UINT8_MAX;
	volatile int64_t x215 = -1LL;
	uint16_t x216 = 2U;
	int32_t t35 = 49;

    t35 = (x213<<((x214>x215)%x216));

    if (t35 != 255) { NG(); } else { ; }
	
}

void f36(void) {
    	int64_t x226 = INT64_MIN;
	volatile int64_t x227 = -92556LL;
	static int16_t x228 = INT16_MIN;

    t36 = (x225<<((x226>x227)%x228));

    if (t36 != 273877945U) { NG(); } else { ; }
	
}

void f37(void) {
    	uint64_t x233 = UINT64_MAX;
	int32_t x234 = -1;
	uint64_t x235 = 1085002941718028391LLU;
	uint64_t t37 = 258755423200991LLU;

    t37 = (x233<<((x234>x235)%x236));

    if (t37 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f38(void) {
    	int32_t x237 = INT32_MAX;
	uint8_t x238 = 2U;
	uint64_t x239 = 1744771233LLU;
	volatile int32_t x240 = 8197;
	int32_t t38 = INT32_MAX;

    t38 = (x237<<((x238>x239)%x240));

    if (t38 != INT32_MAX) { NG(); } else { ; }
	
}

void f39(void) {
    	int32_t x245 = INT32_MAX;
	volatile uint8_t x246 = UINT8_MAX;
	int32_t x247 = INT32_MIN;
	int32_t t39 = INT32_MAX;

    t39 = (x245<<((x246>x247)%x248));

    if (t39 != INT32_MAX) { NG(); } else { ; }
	
}

void f40(void) {
    	uint8_t x257 = 1U;
	static int8_t x258 = -1;
	uint32_t x259 = 53664716U;
	int64_t x260 = -75326457LL;
	volatile int32_t t40 = -31;

    t40 = (x257<<((x258>x259)%x260));

    if (t40 != 2) { NG(); } else { ; }
	
}

void f41(void) {
    	uint8_t x265 = UINT8_MAX;
	static int32_t x266 = INT32_MAX;
	uint32_t x267 = 2U;
	int64_t x268 = -850516579LL;
	int32_t t41 = -14;

    t41 = (x265<<((x266>x267)%x268));

    if (t41 != 510) { NG(); } else { ; }
	
}

void f42(void) {
    	int16_t x274 = 0;
	volatile int64_t t42 = -4277510LL;

    t42 = (x273<<((x274>x275)%x276));

    if (t42 != 2358105431LL) { NG(); } else { ; }
	
}

void f43(void) {
    	static int16_t x277 = 95;
	int32_t x278 = 62395;
	static int64_t x279 = INT64_MIN;
	int32_t x280 = INT32_MIN;
	volatile int32_t t43 = 80116;

    t43 = (x277<<((x278>x279)%x280));

    if (t43 != 190) { NG(); } else { ; }
	
}

void f44(void) {
    	volatile int8_t x283 = 1;
	int8_t x284 = INT8_MAX;

    t44 = (x281<<((x282>x283)%x284));

    if (t44 != 261438U) { NG(); } else { ; }
	
}

void f45(void) {
    	int16_t x285 = 1;
	int8_t x287 = INT8_MIN;
	static uint16_t x288 = 2617U;
	static volatile int32_t t45 = -370883387;

    t45 = (x285<<((x286>x287)%x288));

    if (t45 != 2) { NG(); } else { ; }
	
}

void f46(void) {
    	int64_t x301 = INT64_MAX;
	int8_t x302 = -1;
	int16_t x303 = 1;
	volatile int64_t t46 = INT64_MAX;

    t46 = (x301<<((x302>x303)%x304));

    if (t46 != INT64_MAX) { NG(); } else { ; }
	
}

void f47(void) {
    	uint16_t x305 = 7328U;
	int16_t x306 = -1;
	static int32_t x308 = 1851;
	int32_t t47 = -6;

    t47 = (x305<<((x306>x307)%x308));

    if (t47 != 14656) { NG(); } else { ; }
	
}

void f48(void) {
    	int64_t x312 = -3684898977193380529LL;

    t48 = (x309<<((x310>x311)%x312));

    if (t48 != 12) { NG(); } else { ; }
	
}

void f49(void) {
    	volatile uint8_t x313 = 37U;
	int8_t x314 = INT8_MIN;
	int64_t x315 = -1LL;
	volatile uint32_t x316 = 27131298U;

    t49 = (x313<<((x314>x315)%x316));

    if (t49 != 37) { NG(); } else { ; }
	
}

void f50(void) {
    	uint32_t x321 = UINT32_MAX;
	volatile uint64_t x322 = 0LLU;
	int8_t x323 = INT8_MIN;
	int8_t x324 = -22;
	uint32_t t50 = UINT32_MAX;

    t50 = (x321<<((x322>x323)%x324));

    if (t50 != UINT32_MAX) { NG(); } else { ; }
	
}

void f51(void) {
    	int64_t x330 = INT64_MIN;
	uint8_t x331 = UINT8_MAX;
	int16_t x332 = INT16_MAX;
	static uint32_t t51 = 499809058U;

    t51 = (x329<<((x330>x331)%x332));

    if (t51 != 3937427U) { NG(); } else { ; }
	
}

void f52(void) {
    	uint16_t x345 = 0U;
	int8_t x346 = INT8_MIN;
	int8_t x348 = INT8_MIN;

    t52 = (x345<<((x346>x347)%x348));

    if (t52 != 0) { NG(); } else { ; }
	
}

void f53(void) {
    	int8_t x349 = 1;
	static int32_t x350 = -14;
	uint8_t x351 = 3U;

    t53 = (x349<<((x350>x351)%x352));

    if (t53 != 1) { NG(); } else { ; }
	
}

void f54(void) {
    	volatile int32_t x361 = 612801609;
	static uint64_t x362 = UINT64_MAX;
	static uint16_t x363 = 7U;
	static volatile int32_t x364 = INT32_MIN;
	static int32_t t54 = -1;

    t54 = (x361<<((x362>x363)%x364));

    if (t54 != 1225603218) { NG(); } else { ; }
	
}

void f55(void) {
    	int16_t x365 = 77;
	int8_t x366 = INT8_MAX;
	int32_t x367 = INT32_MAX;
	int32_t x368 = 1;
	static int32_t t55 = 55223;

    t55 = (x365<<((x366>x367)%x368));

    if (t55 != 77) { NG(); } else { ; }
	
}

void f56(void) {
    	volatile int8_t x369 = 13;
	static int64_t x371 = -1LL;
	volatile uint16_t x372 = UINT16_MAX;
	volatile int32_t t56 = 24293;

    t56 = (x369<<((x370>x371)%x372));

    if (t56 != 26) { NG(); } else { ; }
	
}

void f57(void) {
    	uint32_t x381 = UINT32_MAX;
	static int16_t x382 = INT16_MIN;
	volatile int64_t x383 = INT64_MIN;
	uint32_t x384 = 15122U;
	volatile uint32_t t57 = 295187161U;

    t57 = (x381<<((x382>x383)%x384));

    if (t57 != 4294967294U) { NG(); } else { ; }
	
}

void f58(void) {
    	volatile uint64_t x385 = 4824352170333776LLU;
	int64_t x386 = 1LL;
	int32_t x387 = INT32_MIN;
	int32_t x388 = INT32_MIN;
	volatile uint64_t t58 = 26313LLU;

    t58 = (x385<<((x386>x387)%x388));

    if (t58 != 9648704340667552LLU) { NG(); } else { ; }
	
}

void f59(void) {
    	uint16_t x389 = 1U;
	uint64_t x390 = UINT64_MAX;
	int16_t x391 = -526;
	static int8_t x392 = INT8_MIN;
	int32_t t59 = 214;

    t59 = (x389<<((x390>x391)%x392));

    if (t59 != 2) { NG(); } else { ; }
	
}

void f60(void) {
    	int32_t x393 = INT32_MAX;
	int64_t x395 = 2018086980699LL;
	int32_t x396 = -1;
	volatile int32_t t60 = INT32_MAX;

    t60 = (x393<<((x394>x395)%x396));

    if (t60 != INT32_MAX) { NG(); } else { ; }
	
}

void f61(void) {
    	int8_t x405 = INT8_MAX;
	static int32_t x406 = INT32_MIN;
	static int64_t x407 = -1LL;
	volatile int8_t x408 = 15;
	int32_t t61 = -62804241;

    t61 = (x405<<((x406>x407)%x408));

    if (t61 != 127) { NG(); } else { ; }
	
}

void f62(void) {
    	static int16_t x417 = 2330;
	int16_t x418 = -1;
	uint8_t x419 = UINT8_MAX;
	static uint16_t x420 = 115U;
	int32_t t62 = -262357;

    t62 = (x417<<((x418>x419)%x420));

    if (t62 != 2330) { NG(); } else { ; }
	
}

void f63(void) {
    	uint16_t x421 = UINT16_MAX;
	int32_t x422 = 412215;
	int32_t x423 = 6;
	volatile int32_t t63 = 15;

    t63 = (x421<<((x422>x423)%x424));

    if (t63 != 131070) { NG(); } else { ; }
	
}

void f64(void) {
    	static uint64_t x441 = 4225007491LLU;
	int64_t x442 = INT64_MAX;
	int16_t x443 = 18;
	int8_t x444 = INT8_MIN;
	volatile uint64_t t64 = 1518532749LLU;

    t64 = (x441<<((x442>x443)%x444));

    if (t64 != 8450014982LLU) { NG(); } else { ; }
	
}

void f65(void) {
    	int32_t x445 = 648748;
	static int32_t x446 = INT32_MIN;
	int16_t x447 = INT16_MAX;
	volatile int32_t x448 = INT32_MIN;
	int32_t t65 = -2;

    t65 = (x445<<((x446>x447)%x448));

    if (t65 != 648748) { NG(); } else { ; }
	
}

void f66(void) {
    	uint32_t x453 = 4507U;
	int16_t x454 = 1745;
	int64_t x456 = INT64_MIN;
	uint32_t t66 = 2U;

    t66 = (x453<<((x454>x455)%x456));

    if (t66 != 9014U) { NG(); } else { ; }
	
}

void f67(void) {
    	static uint32_t x465 = 2903943U;
	int32_t x466 = -2042544;
	int64_t x467 = -1769473726151LL;
	static int32_t x468 = INT32_MAX;
	uint32_t t67 = 47863U;

    t67 = (x465<<((x466>x467)%x468));

    if (t67 != 5807886U) { NG(); } else { ; }
	
}

void f68(void) {
    	int32_t x469 = 5817351;
	uint8_t x470 = 2U;
	int16_t x472 = INT16_MIN;
	static volatile int32_t t68 = 6;

    t68 = (x469<<((x470>x471)%x472));

    if (t68 != 11634702) { NG(); } else { ; }
	
}

void f69(void) {
    	uint8_t x473 = UINT8_MAX;
	int64_t x474 = INT64_MIN;
	volatile uint16_t x476 = 9U;
	static volatile int32_t t69 = -1216;

    t69 = (x473<<((x474>x475)%x476));

    if (t69 != 255) { NG(); } else { ; }
	
}

void f70(void) {
    	static int8_t x481 = INT8_MAX;
	uint16_t x482 = 24U;
	int16_t x483 = INT16_MAX;
	volatile int16_t x484 = -1;
	volatile int32_t t70 = 23827;

    t70 = (x481<<((x482>x483)%x484));

    if (t70 != 127) { NG(); } else { ; }
	
}

void f71(void) {
    	uint8_t x485 = 3U;
	int32_t x487 = INT32_MAX;
	int16_t x488 = INT16_MAX;

    t71 = (x485<<((x486>x487)%x488));

    if (t71 != 6) { NG(); } else { ; }
	
}

void f72(void) {
    	static int16_t x489 = INT16_MAX;
	static int16_t x490 = INT16_MAX;
	volatile int64_t x491 = INT64_MAX;
	uint64_t x492 = 468113076LLU;
	static volatile int32_t t72 = 3238760;

    t72 = (x489<<((x490>x491)%x492));

    if (t72 != 32767) { NG(); } else { ; }
	
}

void f73(void) {
    	static volatile uint16_t x505 = UINT16_MAX;
	volatile int64_t x507 = -1LL;
	int32_t x508 = INT32_MIN;

    t73 = (x505<<((x506>x507)%x508));

    if (t73 != 65535) { NG(); } else { ; }
	
}

void f74(void) {
    	uint32_t x509 = 52767U;
	static int32_t x511 = 2998;
	static int16_t x512 = INT16_MAX;
	uint32_t t74 = 64U;

    t74 = (x509<<((x510>x511)%x512));

    if (t74 != 105534U) { NG(); } else { ; }
	
}

void f75(void) {
    	uint64_t x513 = 70426008263LLU;
	int16_t x514 = 8;
	uint64_t x515 = UINT64_MAX;
	volatile int64_t x516 = INT64_MIN;
	uint64_t t75 = 1LLU;

    t75 = (x513<<((x514>x515)%x516));

    if (t75 != 70426008263LLU) { NG(); } else { ; }
	
}

void f76(void) {
    	uint16_t x517 = UINT16_MAX;
	static volatile int64_t x518 = INT64_MIN;
	int64_t x519 = 480903181572211LL;
	int32_t t76 = 12393;

    t76 = (x517<<((x518>x519)%x520));

    if (t76 != 65535) { NG(); } else { ; }
	
}

void f77(void) {
    	uint64_t x521 = UINT64_MAX;
	int32_t x523 = -15207893;
	int64_t x524 = -1LL;
	volatile uint64_t t77 = UINT64_MAX;

    t77 = (x521<<((x522>x523)%x524));

    if (t77 != UINT64_MAX) { NG(); } else { ; }
	
}

void f78(void) {
    	uint64_t x525 = 110686000962LLU;
	int16_t x526 = 144;
	int16_t x527 = INT16_MAX;
	volatile uint64_t x528 = 254445702594202955LLU;
	volatile uint64_t t78 = 2654857304141LLU;

    t78 = (x525<<((x526>x527)%x528));

    if (t78 != 110686000962LLU) { NG(); } else { ; }
	
}

void f79(void) {
    	uint16_t x537 = 11U;
	static int16_t x538 = -1;
	int8_t x539 = INT8_MIN;

    t79 = (x537<<((x538>x539)%x540));

    if (t79 != 22) { NG(); } else { ; }
	
}

void f80(void) {
    	static int16_t x545 = 0;
	int8_t x546 = INT8_MAX;
	volatile int8_t x547 = INT8_MIN;
	static int16_t x548 = -53;
	int32_t t80 = -53;

    t80 = (x545<<((x546>x547)%x548));

    if (t80 != 0) { NG(); } else { ; }
	
}

void f81(void) {
    	uint16_t x553 = UINT16_MAX;
	static int16_t x554 = INT16_MIN;
	static uint64_t x555 = 1100357736375582551LLU;
	int32_t t81 = 920126;

    t81 = (x553<<((x554>x555)%x556));

    if (t81 != 131070) { NG(); } else { ; }
	
}

void f82(void) {
    	int16_t x558 = INT16_MIN;
	int8_t x559 = -1;
	static uint8_t x560 = UINT8_MAX;
	static volatile int32_t t82 = 23238;

    t82 = (x557<<((x558>x559)%x560));

    if (t82 != 3) { NG(); } else { ; }
	
}

void f83(void) {
    	static uint16_t x561 = UINT16_MAX;
	int16_t x562 = -1;
	volatile int32_t t83 = -64451378;

    t83 = (x561<<((x562>x563)%x564));

    if (t83 != 131070) { NG(); } else { ; }
	
}

void f84(void) {
    	int8_t x573 = 45;
	volatile int64_t x574 = 988169676410LL;
	uint32_t x575 = 350U;
	int64_t x576 = 195614LL;
	volatile int32_t t84 = -176;

    t84 = (x573<<((x574>x575)%x576));

    if (t84 != 90) { NG(); } else { ; }
	
}

void f85(void) {
    	volatile int32_t x581 = 7;
	int8_t x582 = INT8_MIN;
	int8_t x583 = 30;
	int8_t x584 = INT8_MIN;

    t85 = (x581<<((x582>x583)%x584));

    if (t85 != 7) { NG(); } else { ; }
	
}

void f86(void) {
    	static uint16_t x585 = 2U;
	static int16_t x586 = INT16_MAX;
	volatile int32_t x587 = INT32_MAX;
	int8_t x588 = -1;
	volatile int32_t t86 = 137;

    t86 = (x585<<((x586>x587)%x588));

    if (t86 != 2) { NG(); } else { ; }
	
}

void f87(void) {
    	volatile int16_t x593 = 0;
	int64_t x594 = INT64_MIN;
	volatile uint64_t x595 = 83895757918LLU;
	volatile int32_t x596 = INT32_MIN;

    t87 = (x593<<((x594>x595)%x596));

    if (t87 != 0) { NG(); } else { ; }
	
}

void f88(void) {
    	uint16_t x605 = UINT16_MAX;
	int32_t x606 = INT32_MAX;
	static int32_t x607 = 189948;
	volatile int16_t x608 = INT16_MAX;
	int32_t t88 = 2;

    t88 = (x605<<((x606>x607)%x608));

    if (t88 != 131070) { NG(); } else { ; }
	
}

void f89(void) {
    	static int32_t x610 = INT32_MIN;
	int16_t x611 = INT16_MAX;
	volatile int32_t t89 = -803276;

    t89 = (x609<<((x610>x611)%x612));

    if (t89 != 676) { NG(); } else { ; }
	
}

void f90(void) {
    	uint64_t x617 = 468043833LLU;
	uint32_t x618 = 369802U;
	int32_t x619 = -18581651;
	static int64_t x620 = INT64_MAX;
	uint64_t t90 = 248743166374009278LLU;

    t90 = (x617<<((x618>x619)%x620));

    if (t90 != 468043833LLU) { NG(); } else { ; }
	
}

void f91(void) {
    	uint32_t x625 = 6061168U;
	uint64_t x626 = UINT64_MAX;
	volatile uint32_t t91 = 563U;

    t91 = (x625<<((x626>x627)%x628));

    if (t91 != 12122336U) { NG(); } else { ; }
	
}

void f92(void) {
    	static uint16_t x630 = 3U;
	int8_t x631 = INT8_MIN;
	static uint64_t x632 = UINT64_MAX;
	static uint64_t t92 = 19363835856493979LLU;

    t92 = (x629<<((x630>x631)%x632));

    if (t92 != 1558740637334370LLU) { NG(); } else { ; }
	
}

void f93(void) {
    	uint8_t x633 = UINT8_MAX;
	uint64_t x634 = 4892LLU;
	static volatile int32_t x635 = -104;
	int32_t x636 = 1;
	static volatile int32_t t93 = 7;

    t93 = (x633<<((x634>x635)%x636));

    if (t93 != 255) { NG(); } else { ; }
	
}

void f94(void) {
    	uint64_t x649 = 355LLU;
	uint64_t x650 = 9273373LLU;
	int8_t x651 = -1;
	int16_t x652 = INT16_MAX;
	uint64_t t94 = 1121707756620LLU;

    t94 = (x649<<((x650>x651)%x652));

    if (t94 != 355LLU) { NG(); } else { ; }
	
}

void f95(void) {
    	uint64_t x658 = 7LLU;
	int64_t x659 = INT64_MIN;
	int64_t t95 = -10063016LL;

    t95 = (x657<<((x658>x659)%x660));

    if (t95 != 3132800691590545335LL) { NG(); } else { ; }
	
}

void f96(void) {
    	uint64_t x665 = UINT64_MAX;
	uint32_t x666 = UINT32_MAX;
	volatile int64_t x667 = INT64_MIN;
	int64_t x668 = -191728681865849LL;
	uint64_t t96 = 11382456713415564LLU;

    t96 = (x665<<((x666>x667)%x668));

    if (t96 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f97(void) {
    	int16_t x669 = INT16_MAX;
	volatile int64_t x670 = -1LL;
	int32_t x671 = INT32_MIN;
	volatile int64_t x672 = 1LL;
	volatile int32_t t97 = 10590949;

    t97 = (x669<<((x670>x671)%x672));

    if (t97 != 32767) { NG(); } else { ; }
	
}

void f98(void) {
    	volatile int32_t x674 = INT32_MAX;
	int8_t x675 = INT8_MIN;
	uint16_t x676 = 124U;
	volatile uint64_t t98 = 1LLU;

    t98 = (x673<<((x674>x675)%x676));

    if (t98 != 379064558061994LLU) { NG(); } else { ; }
	
}

void f99(void) {
    	int32_t x677 = 1;
	uint8_t x678 = UINT8_MAX;
	static int16_t x680 = 1595;
	static int32_t t99 = -2398826;

    t99 = (x677<<((x678>x679)%x680));

    if (t99 != 2) { NG(); } else { ; }
	
}

void f100(void) {
    	int32_t x685 = INT32_MAX;
	static int8_t x687 = 44;
	int32_t x688 = 71;
	static int32_t t100 = INT32_MAX;

    t100 = (x685<<((x686>x687)%x688));

    if (t100 != INT32_MAX) { NG(); } else { ; }
	
}

void f101(void) {
    	int32_t x690 = INT32_MAX;
	int16_t x691 = INT16_MAX;
	uint64_t x692 = 207861372018082952LLU;

    t101 = (x689<<((x690>x691)%x692));

    if (t101 != 131070) { NG(); } else { ; }
	
}

void f102(void) {
    	int16_t x695 = -1;
	int32_t x696 = INT32_MIN;
	static volatile int32_t t102 = -348817;

    t102 = (x693<<((x694>x695)%x696));

    if (t102 != 8) { NG(); } else { ; }
	
}

void f103(void) {
    	static int32_t x701 = INT32_MAX;
	int8_t x702 = INT8_MIN;
	uint8_t x703 = 5U;

    t103 = (x701<<((x702>x703)%x704));

    if (t103 != INT32_MAX) { NG(); } else { ; }
	
}

void f104(void) {
    	int64_t x714 = INT64_MAX;
	volatile uint32_t x715 = 3634U;
	volatile int32_t x716 = INT32_MIN;
	volatile uint32_t t104 = 1041667U;

    t104 = (x713<<((x714>x715)%x716));

    if (t104 != 1067067240U) { NG(); } else { ; }
	
}

void f105(void) {
    	uint64_t x717 = 23266042LLU;
	int16_t x719 = 4;
	int64_t x720 = -25198575326168228LL;
	volatile uint64_t t105 = 5068461561195683LLU;

    t105 = (x717<<((x718>x719)%x720));

    if (t105 != 23266042LLU) { NG(); } else { ; }
	
}

void f106(void) {
    	uint8_t x725 = 7U;
	static int16_t x726 = INT16_MIN;
	int32_t x727 = INT32_MAX;
	int32_t x728 = INT32_MAX;
	static int32_t t106 = -350837810;

    t106 = (x725<<((x726>x727)%x728));

    if (t106 != 7) { NG(); } else { ; }
	
}

void f107(void) {
    	uint32_t x729 = UINT32_MAX;
	uint8_t x730 = 6U;
	int64_t x732 = INT64_MAX;
	uint32_t t107 = 212010U;

    t107 = (x729<<((x730>x731)%x732));

    if (t107 != 4294967294U) { NG(); } else { ; }
	
}

void f108(void) {
    	uint16_t x741 = 17563U;
	static uint32_t x742 = UINT32_MAX;
	uint8_t x743 = 17U;
	volatile int64_t x744 = INT64_MAX;

    t108 = (x741<<((x742>x743)%x744));

    if (t108 != 35126) { NG(); } else { ; }
	
}

void f109(void) {
    	uint32_t x751 = 1357557U;
	int64_t x752 = INT64_MIN;
	int32_t t109 = 2412;

    t109 = (x749<<((x750>x751)%x752));

    if (t109 != 22) { NG(); } else { ; }
	
}

void f110(void) {
    	static uint16_t x765 = UINT16_MAX;
	int16_t x766 = -275;
	int32_t x767 = -1;
	int8_t x768 = 2;
	int32_t t110 = -415298801;

    t110 = (x765<<((x766>x767)%x768));

    if (t110 != 65535) { NG(); } else { ; }
	
}

void f111(void) {
    	volatile uint16_t x777 = 6U;
	uint64_t x778 = UINT64_MAX;
	int16_t x779 = INT16_MIN;
	int64_t x780 = -1LL;
	int32_t t111 = -114187;

    t111 = (x777<<((x778>x779)%x780));

    if (t111 != 6) { NG(); } else { ; }
	
}

void f112(void) {
    	static uint32_t x781 = UINT32_MAX;
	volatile int64_t x783 = -32951469897LL;
	volatile uint32_t t112 = 4491426U;

    t112 = (x781<<((x782>x783)%x784));

    if (t112 != 4294967294U) { NG(); } else { ; }
	
}

void f113(void) {
    	int8_t x785 = 17;
	static int32_t x786 = -1;
	static int8_t x787 = INT8_MIN;
	int16_t x788 = INT16_MIN;
	volatile int32_t t113 = -120;

    t113 = (x785<<((x786>x787)%x788));

    if (t113 != 34) { NG(); } else { ; }
	
}

void f114(void) {
    	int32_t x789 = INT32_MAX;
	uint8_t x790 = UINT8_MAX;
	static int32_t x791 = INT32_MIN;
	volatile int16_t x792 = 1;
	int32_t t114 = INT32_MAX;

    t114 = (x789<<((x790>x791)%x792));

    if (t114 != INT32_MAX) { NG(); } else { ; }
	
}

void f115(void) {
    	uint32_t x797 = UINT32_MAX;
	int32_t x799 = 15966263;
	uint8_t x800 = UINT8_MAX;
	uint32_t t115 = UINT32_MAX;

    t115 = (x797<<((x798>x799)%x800));

    if (t115 != UINT32_MAX) { NG(); } else { ; }
	
}

void f116(void) {
    	uint64_t x801 = 905LLU;
	uint64_t x802 = 76825608922278710LLU;
	int64_t x803 = INT64_MIN;
	static volatile uint16_t x804 = 11U;
	volatile uint64_t t116 = 235268973768LLU;

    t116 = (x801<<((x802>x803)%x804));

    if (t116 != 905LLU) { NG(); } else { ; }
	
}

void f117(void) {
    	int16_t x809 = 12062;
	int64_t x810 = INT64_MIN;
	uint8_t x811 = 0U;
	int32_t t117 = 0;

    t117 = (x809<<((x810>x811)%x812));

    if (t117 != 12062) { NG(); } else { ; }
	
}

void f118(void) {
    	volatile uint64_t x813 = 638511094375LLU;
	static uint8_t x814 = 61U;
	int64_t x815 = -1LL;
	volatile int8_t x816 = INT8_MIN;
	volatile uint64_t t118 = 5611LLU;

    t118 = (x813<<((x814>x815)%x816));

    if (t118 != 1277022188750LLU) { NG(); } else { ; }
	
}

void f119(void) {
    	uint64_t x825 = UINT64_MAX;
	static int64_t x826 = 8574089986379655LL;
	int32_t x828 = INT32_MIN;
	uint64_t t119 = 268331417650535LLU;

    t119 = (x825<<((x826>x827)%x828));

    if (t119 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f120(void) {
    	uint64_t x829 = 16241094LLU;
	static int8_t x830 = INT8_MAX;
	int64_t x832 = INT64_MIN;
	uint64_t t120 = 11058839845675458LLU;

    t120 = (x829<<((x830>x831)%x832));

    if (t120 != 16241094LLU) { NG(); } else { ; }
	
}

void f121(void) {
    	static volatile uint16_t x833 = 53U;
	uint32_t x834 = 0U;
	volatile int16_t x835 = 36;
	int8_t x836 = INT8_MIN;
	int32_t t121 = -61564;

    t121 = (x833<<((x834>x835)%x836));

    if (t121 != 53) { NG(); } else { ; }
	
}

void f122(void) {
    	int64_t x838 = INT64_MAX;
	int8_t x839 = 10;
	volatile uint64_t x840 = 3LLU;
	volatile uint64_t t122 = 101188539565580981LLU;

    t122 = (x837<<((x838>x839)%x840));

    if (t122 != 2205795469866194734LLU) { NG(); } else { ; }
	
}

void f123(void) {
    	int16_t x841 = 342;
	uint8_t x842 = 40U;
	int64_t x843 = -1LL;
	static volatile int32_t t123 = -643606204;

    t123 = (x841<<((x842>x843)%x844));

    if (t123 != 684) { NG(); } else { ; }
	
}

void f124(void) {
    	uint64_t x861 = 7930504718LLU;
	uint32_t x862 = 30U;
	uint64_t x864 = 29LLU;
	uint64_t t124 = 356LLU;

    t124 = (x861<<((x862>x863)%x864));

    if (t124 != 7930504718LLU) { NG(); } else { ; }
	
}

void f125(void) {
    	uint16_t x865 = 8296U;
	uint8_t x867 = 16U;
	static volatile int32_t x868 = -17254728;
	int32_t t125 = -384;

    t125 = (x865<<((x866>x867)%x868));

    if (t125 != 8296) { NG(); } else { ; }
	
}

void f126(void) {
    	uint16_t x874 = 4672U;
	int16_t x875 = INT16_MIN;
	int32_t x876 = -1;
	volatile int32_t t126 = 1312;

    t126 = (x873<<((x874>x875)%x876));

    if (t126 != 65535) { NG(); } else { ; }
	
}

void f127(void) {
    	uint16_t x878 = 26U;
	uint8_t x879 = 18U;
	uint64_t x880 = 15866560419585LLU;
	volatile int32_t t127 = 2012;

    t127 = (x877<<((x878>x879)%x880));

    if (t127 != 6) { NG(); } else { ; }
	
}

void f128(void) {
    	volatile int64_t x888 = INT64_MAX;

    t128 = (x885<<((x886>x887)%x888));

    if (t128 != UINT64_MAX) { NG(); } else { ; }
	
}

void f129(void) {
    	int8_t x889 = 1;
	volatile uint64_t x890 = 263153390847300LLU;
	int64_t x891 = INT64_MIN;
	static int32_t t129 = 58688455;

    t129 = (x889<<((x890>x891)%x892));

    if (t129 != 1) { NG(); } else { ; }
	
}

void f130(void) {
    	volatile uint32_t x893 = 31634145U;
	uint8_t x894 = UINT8_MAX;
	uint16_t x895 = UINT16_MAX;
	uint8_t x896 = 1U;
	volatile uint32_t t130 = 28478276U;

    t130 = (x893<<((x894>x895)%x896));

    if (t130 != 31634145U) { NG(); } else { ; }
	
}

void f131(void) {
    	int64_t x903 = INT64_MAX;
	int64_t x904 = -1LL;
	int32_t t131 = -431341;

    t131 = (x901<<((x902>x903)%x904));

    if (t131 != 335) { NG(); } else { ; }
	
}

void f132(void) {
    	static volatile uint32_t x909 = 9650U;
	static uint8_t x910 = 116U;
	int16_t x911 = 1;
	uint8_t x912 = 41U;
	volatile uint32_t t132 = 305042985U;

    t132 = (x909<<((x910>x911)%x912));

    if (t132 != 19300U) { NG(); } else { ; }
	
}

void f133(void) {
    	int8_t x913 = INT8_MAX;
	static int32_t x914 = INT32_MIN;
	volatile int16_t x915 = -12901;
	int16_t x916 = 2617;
	int32_t t133 = 644;

    t133 = (x913<<((x914>x915)%x916));

    if (t133 != 127) { NG(); } else { ; }
	
}

void f134(void) {
    	uint64_t x917 = UINT64_MAX;
	int8_t x919 = INT8_MIN;
	static uint64_t x920 = 41LLU;
	static uint64_t t134 = 3012900LLU;

    t134 = (x917<<((x918>x919)%x920));

    if (t134 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f135(void) {
    	static uint64_t x929 = 214LLU;
	int64_t x930 = -402271706141LL;
	int32_t x931 = -1;

    t135 = (x929<<((x930>x931)%x932));

    if (t135 != 214LLU) { NG(); } else { ; }
	
}

void f136(void) {
    	static int64_t x938 = -1LL;
	int8_t x939 = INT8_MAX;
	static int16_t x940 = 239;
	volatile int32_t t136 = 59703;

    t136 = (x937<<((x938>x939)%x940));

    if (t136 != 0) { NG(); } else { ; }
	
}

void f137(void) {
    	uint64_t x945 = UINT64_MAX;
	volatile int32_t x946 = INT32_MAX;
	static volatile uint32_t x947 = 65U;
	static volatile int32_t x948 = INT32_MIN;
	volatile uint64_t t137 = 1792LLU;

    t137 = (x945<<((x946>x947)%x948));

    if (t137 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f138(void) {
    	int8_t x949 = 2;
	static int16_t x950 = 1;
	static volatile int8_t x952 = -1;
	int32_t t138 = 2;

    t138 = (x949<<((x950>x951)%x952));

    if (t138 != 2) { NG(); } else { ; }
	
}

void f139(void) {
    	int32_t x953 = 30309;
	volatile uint16_t x954 = UINT16_MAX;
	int16_t x955 = -322;
	volatile int64_t x956 = -76LL;
	int32_t t139 = -4930;

    t139 = (x953<<((x954>x955)%x956));

    if (t139 != 60618) { NG(); } else { ; }
	
}

void f140(void) {
    	volatile uint8_t x957 = 63U;
	volatile int8_t x958 = INT8_MIN;
	int16_t x959 = 0;
	int32_t x960 = INT32_MIN;
	int32_t t140 = -369958853;

    t140 = (x957<<((x958>x959)%x960));

    if (t140 != 63) { NG(); } else { ; }
	
}

void f141(void) {
    	volatile uint64_t x961 = UINT64_MAX;
	static int64_t x963 = -98900024LL;
	uint16_t x964 = UINT16_MAX;
	volatile uint64_t t141 = 19859766631169LLU;

    t141 = (x961<<((x962>x963)%x964));

    if (t141 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f142(void) {
    	static int8_t x971 = -1;
	static int64_t x972 = -1LL;
	int32_t t142 = 0;

    t142 = (x969<<((x970>x971)%x972));

    if (t142 != 24) { NG(); } else { ; }
	
}

void f143(void) {
    	uint8_t x982 = 0U;
	uint64_t x983 = 1253589280LLU;
	uint16_t x984 = 6420U;
	int32_t t143 = 103;

    t143 = (x981<<((x982>x983)%x984));

    if (t143 != 255) { NG(); } else { ; }
	
}

void f144(void) {
    	uint8_t x989 = 1U;
	int64_t x990 = -1LL;
	static int8_t x991 = INT8_MAX;
	uint64_t x992 = UINT64_MAX;

    t144 = (x989<<((x990>x991)%x992));

    if (t144 != 1) { NG(); } else { ; }
	
}

void f145(void) {
    	uint32_t x993 = UINT32_MAX;
	int16_t x994 = INT16_MAX;
	int32_t x995 = INT32_MIN;
	static int8_t x996 = -1;
	volatile uint32_t t145 = UINT32_MAX;

    t145 = (x993<<((x994>x995)%x996));

    if (t145 != UINT32_MAX) { NG(); } else { ; }
	
}

void f146(void) {
    	static uint64_t x997 = UINT64_MAX;
	int16_t x998 = -1;
	volatile int32_t x999 = INT32_MIN;
	int16_t x1000 = INT16_MIN;
	uint64_t t146 = 295LLU;

    t146 = (x997<<((x998>x999)%x1000));

    if (t146 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f147(void) {
    	uint8_t x1002 = UINT8_MAX;
	int64_t x1003 = INT64_MAX;
	int32_t x1004 = INT32_MAX;
	uint64_t t147 = 9050LLU;

    t147 = (x1001<<((x1002>x1003)%x1004));

    if (t147 != 63920666034610414LLU) { NG(); } else { ; }
	
}

void f148(void) {
    	static volatile uint32_t x1013 = 1U;
	int32_t x1015 = INT32_MIN;
	uint32_t x1016 = 5707690U;

    t148 = (x1013<<((x1014>x1015)%x1016));

    if (t148 != 2U) { NG(); } else { ; }
	
}

void f149(void) {
    	volatile uint8_t x1022 = 3U;
	uint32_t x1023 = 13063958U;
	static int64_t x1024 = -1LL;

    t149 = (x1021<<((x1022>x1023)%x1024));

    if (t149 != 0LL) { NG(); } else { ; }
	
}

void f150(void) {
    	int32_t x1025 = INT32_MAX;
	int64_t x1026 = INT64_MIN;
	uint16_t x1027 = UINT16_MAX;
	volatile int32_t t150 = INT32_MAX;

    t150 = (x1025<<((x1026>x1027)%x1028));

    if (t150 != INT32_MAX) { NG(); } else { ; }
	
}

void f151(void) {
    	uint32_t x1029 = 44402U;
	uint32_t x1030 = UINT32_MAX;
	int8_t x1031 = 0;
	volatile int8_t x1032 = INT8_MIN;
	volatile uint32_t t151 = 395U;

    t151 = (x1029<<((x1030>x1031)%x1032));

    if (t151 != 88804U) { NG(); } else { ; }
	
}

void f152(void) {
    	static int32_t x1034 = INT32_MIN;
	int8_t x1035 = -1;
	int32_t t152 = -73199;

    t152 = (x1033<<((x1034>x1035)%x1036));

    if (t152 != 255) { NG(); } else { ; }
	
}

void f153(void) {
    	uint8_t x1037 = UINT8_MAX;
	static volatile uint8_t x1039 = UINT8_MAX;
	int32_t t153 = 1095;

    t153 = (x1037<<((x1038>x1039)%x1040));

    if (t153 != 255) { NG(); } else { ; }
	
}

void f154(void) {
    	volatile int8_t x1049 = INT8_MAX;
	int32_t x1050 = -1;
	int64_t x1051 = INT64_MIN;
	static volatile int32_t t154 = 7643;

    t154 = (x1049<<((x1050>x1051)%x1052));

    if (t154 != 254) { NG(); } else { ; }
	
}

void f155(void) {
    	int32_t x1053 = 916869;
	static int32_t x1054 = 653533;
	int32_t x1055 = -1;
	int32_t x1056 = INT32_MIN;
	int32_t t155 = -8121;

    t155 = (x1053<<((x1054>x1055)%x1056));

    if (t155 != 1833738) { NG(); } else { ; }
	
}

void f156(void) {
    	uint64_t x1070 = 1420961937291633994LLU;
	volatile int64_t x1072 = -2LL;

    t156 = (x1069<<((x1070>x1071)%x1072));

    if (t156 != 1219U) { NG(); } else { ; }
	
}

void f157(void) {
    	volatile int8_t x1077 = 21;
	uint16_t x1078 = 62U;
	uint8_t x1079 = 44U;
	int64_t x1080 = INT64_MAX;
	volatile int32_t t157 = -1204;

    t157 = (x1077<<((x1078>x1079)%x1080));

    if (t157 != 42) { NG(); } else { ; }
	
}

void f158(void) {
    	int8_t x1098 = INT8_MIN;
	uint32_t x1099 = UINT32_MAX;
	static int64_t x1100 = INT64_MIN;
	int32_t t158 = -387845468;

    t158 = (x1097<<((x1098>x1099)%x1100));

    if (t158 != 30) { NG(); } else { ; }
	
}

void f159(void) {
    	static int16_t x1101 = 142;
	uint8_t x1102 = 79U;
	int64_t x1103 = INT64_MIN;
	static volatile int32_t x1104 = -38;
	volatile int32_t t159 = 1001;

    t159 = (x1101<<((x1102>x1103)%x1104));

    if (t159 != 284) { NG(); } else { ; }
	
}

void f160(void) {
    	uint64_t x1105 = 5776786777670906000LLU;
	int8_t x1106 = INT8_MIN;
	static uint16_t x1108 = 4057U;
	uint64_t t160 = 6618LLU;

    t160 = (x1105<<((x1106>x1107)%x1108));

    if (t160 != 11553573555341812000LLU) { NG(); } else { ; }
	
}

void f161(void) {
    	int32_t x1109 = 191623;
	int8_t x1110 = INT8_MAX;
	static int8_t x1112 = INT8_MIN;
	int32_t t161 = -1781;

    t161 = (x1109<<((x1110>x1111)%x1112));

    if (t161 != 383246) { NG(); } else { ; }
	
}

void f162(void) {
    	static int32_t x1115 = INT32_MAX;
	int8_t x1116 = 2;
	int64_t t162 = 132543655LL;

    t162 = (x1113<<((x1114>x1115)%x1116));

    if (t162 != 4804640545LL) { NG(); } else { ; }
	
}

void f163(void) {
    	int16_t x1118 = -3;
	volatile uint16_t x1119 = 9U;
	uint16_t x1120 = 1U;

    t163 = (x1117<<((x1118>x1119)%x1120));

    if (t163 != 255) { NG(); } else { ; }
	
}

void f164(void) {
    	int32_t x1125 = INT32_MAX;
	int32_t x1126 = -93;
	int8_t x1127 = -1;
	int16_t x1128 = INT16_MAX;
	int32_t t164 = INT32_MAX;

    t164 = (x1125<<((x1126>x1127)%x1128));

    if (t164 != INT32_MAX) { NG(); } else { ; }
	
}

void f165(void) {
    	uint16_t x1145 = UINT16_MAX;
	volatile uint16_t x1147 = UINT16_MAX;
	uint64_t x1148 = 275010797LLU;
	static volatile int32_t t165 = 97931;

    t165 = (x1145<<((x1146>x1147)%x1148));

    if (t165 != 65535) { NG(); } else { ; }
	
}

void f166(void) {
    	uint16_t x1149 = UINT16_MAX;
	static uint16_t x1151 = 67U;
	int32_t x1152 = -3;

    t166 = (x1149<<((x1150>x1151)%x1152));

    if (t166 != 65535) { NG(); } else { ; }
	
}

void f167(void) {
    	static uint64_t x1153 = 9470LLU;
	uint64_t x1154 = 78221645474423835LLU;
	int16_t x1155 = 1;
	static uint8_t x1156 = 4U;
	volatile uint64_t t167 = 7271623136892886LLU;

    t167 = (x1153<<((x1154>x1155)%x1156));

    if (t167 != 18940LLU) { NG(); } else { ; }
	
}

void f168(void) {
    	int64_t x1178 = 175132483468606LL;
	volatile int8_t x1180 = -9;

    t168 = (x1177<<((x1178>x1179)%x1180));

    if (t168 != 0LL) { NG(); } else { ; }
	
}

void f169(void) {
    	uint8_t x1185 = UINT8_MAX;
	static int64_t x1186 = INT64_MAX;
	volatile int8_t x1187 = -1;
	static uint8_t x1188 = 1U;
	int32_t t169 = -859475411;

    t169 = (x1185<<((x1186>x1187)%x1188));

    if (t169 != 255) { NG(); } else { ; }
	
}

void f170(void) {
    	volatile int32_t x1197 = INT32_MAX;
	uint64_t x1198 = UINT64_MAX;
	volatile uint64_t x1199 = UINT64_MAX;
	volatile int32_t t170 = INT32_MAX;

    t170 = (x1197<<((x1198>x1199)%x1200));

    if (t170 != INT32_MAX) { NG(); } else { ; }
	
}

void f171(void) {
    	uint64_t x1205 = UINT64_MAX;
	int8_t x1206 = INT8_MIN;
	uint32_t x1207 = 390098893U;
	uint64_t t171 = 8297219035600343074LLU;

    t171 = (x1205<<((x1206>x1207)%x1208));

    if (t171 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f172(void) {
    	int32_t x1209 = INT32_MAX;
	uint16_t x1210 = 14U;
	int32_t x1211 = INT32_MIN;
	int32_t t172 = INT32_MAX;

    t172 = (x1209<<((x1210>x1211)%x1212));

    if (t172 != INT32_MAX) { NG(); } else { ; }
	
}

void f173(void) {
    	static uint64_t x1233 = 1857474832041LLU;
	uint8_t x1234 = 1U;
	int64_t x1235 = -10520343LL;
	int64_t x1236 = 604439279LL;

    t173 = (x1233<<((x1234>x1235)%x1236));

    if (t173 != 3714949664082LLU) { NG(); } else { ; }
	
}

void f174(void) {
    	uint32_t x1237 = 1297988U;
	int32_t x1239 = -252;
	int8_t x1240 = INT8_MIN;

    t174 = (x1237<<((x1238>x1239)%x1240));

    if (t174 != 2595976U) { NG(); } else { ; }
	
}

void f175(void) {
    	volatile int8_t x1243 = -9;

    t175 = (x1241<<((x1242>x1243)%x1244));

    if (t175 != 31) { NG(); } else { ; }
	
}

void f176(void) {
    	uint32_t x1265 = 8U;
	uint32_t x1266 = 191304307U;
	uint64_t x1268 = 279113715185984587LLU;
	static uint32_t t176 = 2395241U;

    t176 = (x1265<<((x1266>x1267)%x1268));

    if (t176 != 16U) { NG(); } else { ; }
	
}

void f177(void) {
    	volatile uint64_t x1278 = 1058986576557LLU;
	int8_t x1279 = INT8_MIN;
	int64_t x1280 = INT64_MAX;
	uint32_t t177 = 119111842U;

    t177 = (x1277<<((x1278>x1279)%x1280));

    if (t177 != 5554U) { NG(); } else { ; }
	
}

void f178(void) {
    	volatile int32_t x1281 = INT32_MAX;
	int16_t x1283 = -360;

    t178 = (x1281<<((x1282>x1283)%x1284));

    if (t178 != INT32_MAX) { NG(); } else { ; }
	
}

void f179(void) {
    	int64_t x1285 = 1273418725LL;
	static volatile uint8_t x1286 = 126U;
	static int8_t x1288 = INT8_MIN;
	int64_t t179 = -67976952799LL;

    t179 = (x1285<<((x1286>x1287)%x1288));

    if (t179 != 1273418725LL) { NG(); } else { ; }
	
}

void f180(void) {
    	int64_t x1289 = 86LL;
	uint16_t x1291 = 215U;
	volatile int64_t t180 = -6LL;

    t180 = (x1289<<((x1290>x1291)%x1292));

    if (t180 != 86LL) { NG(); } else { ; }
	
}

void f181(void) {
    	uint8_t x1293 = 1U;
	static int8_t x1295 = -1;
	int8_t x1296 = 24;
	int32_t t181 = 3481;

    t181 = (x1293<<((x1294>x1295)%x1296));

    if (t181 != 1) { NG(); } else { ; }
	
}

void f182(void) {
    	static uint8_t x1297 = 25U;
	volatile int16_t x1298 = -1;
	int16_t x1299 = -1;
	volatile int32_t t182 = 5;

    t182 = (x1297<<((x1298>x1299)%x1300));

    if (t182 != 25) { NG(); } else { ; }
	
}

void f183(void) {
    	static uint64_t x1305 = UINT64_MAX;
	uint64_t x1306 = UINT64_MAX;
	uint64_t x1307 = 3633561976262811699LLU;
	volatile int32_t x1308 = INT32_MAX;
	static uint64_t t183 = 132044936192LLU;

    t183 = (x1305<<((x1306>x1307)%x1308));

    if (t183 != 18446744073709551614LLU) { NG(); } else { ; }
	
}

void f184(void) {
    	int8_t x1315 = INT8_MIN;
	static uint8_t x1316 = UINT8_MAX;
	volatile int32_t t184 = -2069505;

    t184 = (x1313<<((x1314>x1315)%x1316));

    if (t184 != 38762) { NG(); } else { ; }
	
}

void f185(void) {
    	uint64_t x1325 = 3995904LLU;
	int8_t x1326 = -1;
	int64_t x1327 = INT64_MIN;
	int16_t x1328 = -10;
	uint64_t t185 = 1LLU;

    t185 = (x1325<<((x1326>x1327)%x1328));

    if (t185 != 7991808LLU) { NG(); } else { ; }
	
}

void f186(void) {
    	static uint32_t x1341 = UINT32_MAX;
	static int64_t x1342 = INT64_MIN;
	volatile uint32_t x1343 = 0U;
	int64_t x1344 = 7316979851LL;
	static volatile uint32_t t186 = UINT32_MAX;

    t186 = (x1341<<((x1342>x1343)%x1344));

    if (t186 != UINT32_MAX) { NG(); } else { ; }
	
}

void f187(void) {
    	volatile int8_t x1349 = 16;
	static uint32_t x1351 = UINT32_MAX;
	static uint16_t x1352 = 7U;
	volatile int32_t t187 = 9;

    t187 = (x1349<<((x1350>x1351)%x1352));

    if (t187 != 32) { NG(); } else { ; }
	
}

void f188(void) {
    	static volatile uint32_t x1361 = 69083303U;
	uint8_t x1362 = 26U;
	static int64_t x1363 = 13515999228843LL;
	int32_t x1364 = -3082;

    t188 = (x1361<<((x1362>x1363)%x1364));

    if (t188 != 69083303U) { NG(); } else { ; }
	
}

void f189(void) {
    	volatile uint8_t x1370 = 8U;
	int8_t x1371 = INT8_MIN;
	int64_t x1372 = INT64_MIN;
	uint32_t t189 = 6408845U;

    t189 = (x1369<<((x1370>x1371)%x1372));

    if (t189 != 6U) { NG(); } else { ; }
	
}

void f190(void) {
    	static uint32_t x1381 = 50067U;
	int8_t x1382 = -1;
	uint8_t x1383 = UINT8_MAX;
	static volatile uint32_t t190 = 14U;

    t190 = (x1381<<((x1382>x1383)%x1384));

    if (t190 != 50067U) { NG(); } else { ; }
	
}

void f191(void) {
    	int8_t x1385 = INT8_MAX;
	uint32_t x1386 = 1196U;
	static volatile int64_t x1387 = -1LL;
	uint32_t x1388 = UINT32_MAX;
	volatile int32_t t191 = -4;

    t191 = (x1385<<((x1386>x1387)%x1388));

    if (t191 != 254) { NG(); } else { ; }
	
}

void f192(void) {
    	uint8_t x1389 = 4U;
	int8_t x1391 = -62;
	int8_t x1392 = INT8_MIN;
	static volatile int32_t t192 = 55968;

    t192 = (x1389<<((x1390>x1391)%x1392));

    if (t192 != 8) { NG(); } else { ; }
	
}

void f193(void) {
    	int32_t x1397 = 4914;
	int64_t x1399 = -1LL;
	uint32_t x1400 = UINT32_MAX;
	volatile int32_t t193 = 400;

    t193 = (x1397<<((x1398>x1399)%x1400));

    if (t193 != 4914) { NG(); } else { ; }
	
}

void f194(void) {
    	volatile int64_t x1402 = INT64_MIN;
	uint64_t x1403 = UINT64_MAX;
	uint64_t x1404 = 29772370LLU;

    t194 = (x1401<<((x1402>x1403)%x1404));

    if (t194 != UINT64_MAX) { NG(); } else { ; }
	
}

void f195(void) {
    	static uint8_t x1405 = 2U;
	uint8_t x1407 = 74U;
	uint32_t x1408 = 3U;
	int32_t t195 = -33347995;

    t195 = (x1405<<((x1406>x1407)%x1408));

    if (t195 != 4) { NG(); } else { ; }
	
}

void f196(void) {
    	int32_t x1410 = INT32_MAX;
	volatile uint32_t x1411 = UINT32_MAX;
	int8_t x1412 = INT8_MIN;
	volatile int64_t t196 = INT64_MAX;

    t196 = (x1409<<((x1410>x1411)%x1412));

    if (t196 != INT64_MAX) { NG(); } else { ; }
	
}

void f197(void) {
    	uint32_t x1413 = 25U;
	int64_t x1414 = -71146692620LL;
	int8_t x1415 = 2;

    t197 = (x1413<<((x1414>x1415)%x1416));

    if (t197 != 25U) { NG(); } else { ; }
	
}

void f198(void) {
    	volatile uint8_t x1425 = UINT8_MAX;
	int32_t x1426 = 183014;
	static volatile int64_t x1427 = -1LL;
	static int32_t x1428 = -1;
	volatile int32_t t198 = 0;

    t198 = (x1425<<((x1426>x1427)%x1428));

    if (t198 != 255) { NG(); } else { ; }
	
}

void f199(void) {
    	uint32_t x1429 = UINT32_MAX;
	int8_t x1430 = INT8_MIN;
	static int16_t x1431 = 28;
	int8_t x1432 = INT8_MIN;
	static uint32_t t199 = UINT32_MAX;

    t199 = (x1429<<((x1430>x1431)%x1432));

    if (t199 != UINT32_MAX) { NG(); } else { ; }
	
}

int main(void) {
        f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();
    f100();
    f101();
    f102();
    f103();
    f104();
    f105();
    f106();
    f107();
    f108();
    f109();
    f110();
    f111();
    f112();
    f113();
    f114();
    f115();
    f116();
    f117();
    f118();
    f119();
    f120();
    f121();
    f122();
    f123();
    f124();
    f125();
    f126();
    f127();
    f128();
    f129();
    f130();
    f131();
    f132();
    f133();
    f134();
    f135();
    f136();
    f137();
    f138();
    f139();
    f140();
    f141();
    f142();
    f143();
    f144();
    f145();
    f146();
    f147();
    f148();
    f149();
    f150();
    f151();
    f152();
    f153();
    f154();
    f155();
    f156();
    f157();
    f158();
    f159();
    f160();
    f161();
    f162();
    f163();
    f164();
    f165();
    f166();
    f167();
    f168();
    f169();
    f170();
    f171();
    f172();
    f173();
    f174();
    f175();
    f176();
    f177();
    f178();
    f179();
    f180();
    f181();
    f182();
    f183();
    f184();
    f185();
    f186();
    f187();
    f188();
    f189();
    f190();
    f191();
    f192();
    f193();
    f194();
    f195();
    f196();
    f197();
    f198();
    f199();


    return 0;
}

