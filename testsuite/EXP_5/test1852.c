
/*
CF3

Copyright (c) 2015 ishiura-lab.

Released under the MIT license.  
https://github.com/ishiura-compiler/CF3/MIT-LICENSE.md
*/

#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

int32_t t0 = 691010711;
volatile int64_t x10 = INT64_MIN;
static int8_t x13 = INT8_MAX;
int64_t x15 = INT64_MAX;
volatile uint16_t x17 = UINT16_MAX;
volatile uint8_t x26 = 48U;
int32_t t5 = -283422925;
uint8_t x29 = UINT8_MAX;
volatile int8_t x30 = -26;
volatile int8_t x33 = INT8_MIN;
int32_t x35 = INT32_MIN;
int8_t x36 = INT8_MIN;
volatile int32_t t8 = 3;
static int32_t x42 = -1;
int16_t x43 = -1;
int16_t x45 = -12;
volatile uint8_t x47 = UINT8_MAX;
volatile int32_t t10 = -1588;
volatile int16_t x51 = -1;
volatile int8_t x57 = INT8_MAX;
volatile uint32_t x67 = UINT32_MAX;
static volatile uint8_t x70 = 1U;
static int32_t t17 = INT32_MIN;
static uint16_t x78 = 206U;
uint64_t x79 = 1628645938798585LLU;
static volatile uint64_t t19 = 414566LLU;
static int32_t x100 = 9967;
uint16_t x102 = 27873U;
static int32_t x104 = 3759;
uint32_t x105 = 1547826U;
uint32_t x116 = 3U;
int32_t t27 = 267039741;
static int16_t x119 = INT16_MIN;
uint64_t x124 = UINT64_MAX;
volatile int32_t t30 = -528297;
volatile int32_t t31 = -874106695;
volatile int64_t x137 = 85LL;
uint8_t x147 = 43U;
int32_t t35 = 54235149;
uint32_t x149 = 10353232U;
int8_t x156 = -1;
volatile uint64_t x158 = 1520329LLU;
static int16_t x164 = -1;
static uint16_t x165 = UINT16_MAX;
volatile int64_t t44 = INT64_MAX;
int32_t t47 = -310;
volatile uint64_t t48 = 740449529397200029LLU;
int64_t x207 = -2663546328128822704LL;
volatile int32_t x218 = -369;
uint32_t x222 = 19256904U;
int64_t t53 = INT64_MIN;
int32_t x228 = 11208;
static int8_t x235 = 1;
int32_t x251 = INT32_MIN;
int32_t t60 = 2;
static uint64_t x260 = UINT64_MAX;
uint8_t x261 = 0U;
int64_t x270 = 18801233045049LL;
static uint8_t x275 = 0U;
volatile int32_t t66 = 0;
volatile int32_t t67 = 4712;
uint64_t x287 = 29LLU;
uint8_t x288 = UINT8_MAX;
volatile int32_t t68 = 32345;
int64_t x303 = INT64_MIN;
volatile int32_t t72 = 583152;
uint8_t x305 = 1U;
uint64_t x308 = 10286609801405LLU;
static volatile uint8_t x311 = UINT8_MAX;
volatile int32_t t75 = INT32_MAX;
int16_t x319 = -1;
volatile int32_t x321 = 25607553;
int32_t x333 = -122468;
uint64_t x337 = 143287900452LLU;
volatile int64_t x338 = 1LL;
uint16_t x342 = 4734U;
int64_t x347 = INT64_MIN;
int16_t x356 = INT16_MIN;
static int32_t t85 = -82900;
static int64_t t86 = INT64_MAX;
int8_t x365 = INT8_MAX;
uint64_t x367 = 474434LLU;
static int64_t x376 = -1LL;
int32_t x380 = INT32_MIN;
int16_t x385 = 2;
volatile int32_t x393 = -26865;
int8_t x397 = INT8_MIN;
static int32_t x399 = -1;
int64_t x406 = INT64_MIN;
static int64_t x420 = INT64_MAX;
static volatile int64_t x421 = INT64_MIN;
static int8_t x429 = INT8_MAX;
static int8_t x430 = 0;
int32_t t102 = 8307924;
int8_t x434 = -1;
static int8_t x437 = INT8_MIN;
int64_t x443 = -65191331436LL;
int32_t x444 = 34949;
static volatile int32_t x445 = INT32_MAX;
volatile uint64_t x456 = UINT64_MAX;
int32_t x458 = INT32_MIN;
static int32_t x460 = -1;
int64_t x468 = 7LL;
volatile int16_t x473 = -9;
int32_t x483 = INT32_MIN;
volatile int64_t t115 = 6564LL;
int8_t x485 = INT8_MIN;
int32_t x486 = 531221;
volatile uint16_t x488 = UINT16_MAX;
static volatile int32_t t116 = 0;
static uint8_t x491 = UINT8_MAX;
volatile int32_t t117 = -468974300;
int8_t x495 = INT8_MIN;
static uint64_t x502 = UINT64_MAX;
uint8_t x503 = UINT8_MAX;
int32_t t120 = INT32_MAX;
uint8_t x509 = 7U;
int64_t t123 = -648261658009408LL;
static int8_t x522 = INT8_MIN;
uint8_t x523 = 7U;
uint64_t x529 = UINT64_MAX;
volatile uint32_t x531 = 408955U;
int16_t x540 = INT16_MIN;
int32_t x544 = 4427;
volatile int32_t t130 = 106;
uint16_t x549 = 355U;
volatile uint32_t x550 = 20585U;
static volatile int32_t t133 = 126;
int16_t x559 = INT16_MIN;
static volatile int32_t t134 = -476648189;
static volatile int32_t x561 = -1;
int16_t x562 = 198;
int8_t x563 = INT8_MAX;
volatile int64_t x574 = -33306206380LL;
int32_t x578 = -1;
int64_t x580 = 449201368LL;
static volatile int8_t x589 = INT8_MIN;
volatile int8_t x590 = INT8_MAX;
volatile int32_t x593 = 1742;
int8_t x607 = -5;
volatile int64_t t146 = INT64_MAX;
int64_t x617 = INT64_MIN;
static int64_t t149 = INT64_MIN;
uint8_t x622 = UINT8_MAX;
static uint16_t x628 = 7377U;
int16_t x631 = INT16_MIN;
int32_t t152 = INT32_MAX;
int32_t x639 = INT32_MIN;
static int16_t x644 = -1;
static volatile int32_t x648 = -3362251;
volatile int32_t t157 = 214274;
static int32_t t158 = 174;
static uint64_t t159 = 77062620435395LLU;
uint64_t x668 = 121914758LLU;
int8_t x670 = INT8_MAX;
volatile int32_t x680 = -91937;
static uint8_t x683 = 11U;
uint32_t x686 = UINT32_MAX;
int64_t x688 = -1LL;
int16_t x693 = INT16_MIN;
volatile int32_t t168 = 8752;
volatile uint32_t x699 = UINT32_MAX;
static volatile int32_t t169 = -7;
uint8_t x701 = 6U;
int64_t x702 = -15LL;
static uint16_t x703 = 3151U;
int64_t x706 = -1LL;
int16_t x707 = INT16_MIN;
volatile int32_t t171 = 202;
uint32_t x713 = UINT32_MAX;
uint32_t x714 = 17246U;
int64_t x723 = INT64_MAX;
static int16_t x724 = -1;
static volatile int32_t x733 = -10118391;
int16_t x736 = 6;
int32_t t178 = 0;
int8_t x737 = 1;
uint16_t x739 = 429U;
int32_t x745 = INT32_MAX;
uint64_t x749 = UINT64_MAX;
int64_t x750 = -1960517617532506LL;
static int32_t t183 = 0;
volatile int64_t x762 = INT64_MIN;
uint8_t x772 = 57U;
volatile int16_t x782 = 73;
uint32_t x792 = UINT32_MAX;
static uint32_t x802 = UINT32_MAX;
int64_t x808 = -1LL;
uint16_t x812 = 370U;
volatile uint8_t x814 = 123U;
int64_t x817 = 70827574301830874LL;
uint64_t x818 = UINT64_MAX;


void f0(void) {
    	int8_t x1 = 1;
	int8_t x2 = 31;
	int64_t x3 = -1LL;
	int8_t x4 = INT8_MIN;

    t0 = (x1/((x2<=x3)!=x4));

    if (t0 != 1) { NG(); } else { ; }
	
}

void f1(void) {
    	static uint8_t x9 = UINT8_MAX;
	int64_t x11 = INT64_MIN;
	volatile int32_t x12 = INT32_MIN;
	int32_t t1 = 735632098;

    t1 = (x9/((x10<=x11)!=x12));

    if (t1 != 255) { NG(); } else { ; }
	
}

void f2(void) {
    	int16_t x14 = INT16_MIN;
	uint16_t x16 = UINT16_MAX;
	int32_t t2 = -1;

    t2 = (x13/((x14<=x15)!=x16));

    if (t2 != 127) { NG(); } else { ; }
	
}

void f3(void) {
    	int16_t x18 = 477;
	int8_t x19 = 5;
	int16_t x20 = INT16_MAX;
	static int32_t t3 = 6;

    t3 = (x17/((x18<=x19)!=x20));

    if (t3 != 65535) { NG(); } else { ; }
	
}

void f4(void) {
    	static uint32_t x21 = 92253U;
	volatile int32_t x22 = 737057537;
	volatile int16_t x23 = -1;
	volatile int64_t x24 = 27744LL;
	volatile uint32_t t4 = 211U;

    t4 = (x21/((x22<=x23)!=x24));

    if (t4 != 92253U) { NG(); } else { ; }
	
}

void f5(void) {
    	volatile int16_t x25 = INT16_MIN;
	volatile uint8_t x27 = 2U;
	int32_t x28 = INT32_MIN;

    t5 = (x25/((x26<=x27)!=x28));

    if (t5 != -32768) { NG(); } else { ; }
	
}

void f6(void) {
    	int64_t x31 = 64646812735906LL;
	volatile int32_t x32 = -1;
	volatile int32_t t6 = -929385;

    t6 = (x29/((x30<=x31)!=x32));

    if (t6 != 255) { NG(); } else { ; }
	
}

void f7(void) {
    	static uint32_t x34 = 479U;
	int32_t t7 = -422380069;

    t7 = (x33/((x34<=x35)!=x36));

    if (t7 != -128) { NG(); } else { ; }
	
}

void f8(void) {
    	int8_t x37 = INT8_MAX;
	int16_t x38 = INT16_MAX;
	uint8_t x39 = 120U;
	static int64_t x40 = INT64_MIN;

    t8 = (x37/((x38<=x39)!=x40));

    if (t8 != 127) { NG(); } else { ; }
	
}

void f9(void) {
    	uint64_t x41 = UINT64_MAX;
	static int64_t x44 = INT64_MIN;
	volatile uint64_t t9 = UINT64_MAX;

    t9 = (x41/((x42<=x43)!=x44));

    if (t9 != UINT64_MAX) { NG(); } else { ; }
	
}

void f10(void) {
    	int32_t x46 = INT32_MAX;
	int32_t x48 = -388;

    t10 = (x45/((x46<=x47)!=x48));

    if (t10 != -12) { NG(); } else { ; }
	
}

void f11(void) {
    	int32_t x49 = -54886937;
	int64_t x50 = INT64_MIN;
	int8_t x52 = 0;
	static int32_t t11 = -8536067;

    t11 = (x49/((x50<=x51)!=x52));

    if (t11 != -54886937) { NG(); } else { ; }
	
}

void f12(void) {
    	int16_t x53 = INT16_MIN;
	uint32_t x54 = 510U;
	static int32_t x55 = -31921;
	int8_t x56 = INT8_MAX;
	int32_t t12 = 99;

    t12 = (x53/((x54<=x55)!=x56));

    if (t12 != -32768) { NG(); } else { ; }
	
}

void f13(void) {
    	volatile uint8_t x58 = UINT8_MAX;
	int64_t x59 = 1010438LL;
	int8_t x60 = 0;
	int32_t t13 = 1;

    t13 = (x57/((x58<=x59)!=x60));

    if (t13 != 127) { NG(); } else { ; }
	
}

void f14(void) {
    	int16_t x61 = INT16_MAX;
	uint32_t x62 = 6074760U;
	int64_t x63 = INT64_MAX;
	volatile int64_t x64 = INT64_MAX;
	volatile int32_t t14 = -5;

    t14 = (x61/((x62<=x63)!=x64));

    if (t14 != 32767) { NG(); } else { ; }
	
}

void f15(void) {
    	uint32_t x65 = 74U;
	int8_t x66 = -15;
	static uint32_t x68 = UINT32_MAX;
	uint32_t t15 = 192262000U;

    t15 = (x65/((x66<=x67)!=x68));

    if (t15 != 74U) { NG(); } else { ; }
	
}

void f16(void) {
    	int8_t x69 = INT8_MIN;
	int32_t x71 = INT32_MAX;
	int32_t x72 = -2720173;
	int32_t t16 = 3;

    t16 = (x69/((x70<=x71)!=x72));

    if (t16 != -128) { NG(); } else { ; }
	
}

void f17(void) {
    	static int32_t x73 = INT32_MIN;
	int16_t x74 = INT16_MIN;
	int32_t x75 = INT32_MIN;
	int8_t x76 = -38;

    t17 = (x73/((x74<=x75)!=x76));

    if (t17 != INT32_MIN) { NG(); } else { ; }
	
}

void f18(void) {
    	int8_t x77 = INT8_MIN;
	int8_t x80 = INT8_MIN;
	volatile int32_t t18 = 421;

    t18 = (x77/((x78<=x79)!=x80));

    if (t18 != -128) { NG(); } else { ; }
	
}

void f19(void) {
    	uint64_t x81 = 543366593267691LLU;
	static uint16_t x82 = 18U;
	volatile uint32_t x83 = UINT32_MAX;
	static int32_t x84 = INT32_MIN;

    t19 = (x81/((x82<=x83)!=x84));

    if (t19 != 543366593267691LLU) { NG(); } else { ; }
	
}

void f20(void) {
    	uint32_t x85 = 30374840U;
	uint8_t x86 = UINT8_MAX;
	int64_t x87 = -17926918714LL;
	uint32_t x88 = 401951041U;
	uint32_t t20 = 4007U;

    t20 = (x85/((x86<=x87)!=x88));

    if (t20 != 30374840U) { NG(); } else { ; }
	
}

void f21(void) {
    	uint8_t x89 = 8U;
	int64_t x90 = INT64_MAX;
	uint32_t x91 = 2U;
	int32_t x92 = -9584;
	volatile int32_t t21 = -235819911;

    t21 = (x89/((x90<=x91)!=x92));

    if (t21 != 8) { NG(); } else { ; }
	
}

void f22(void) {
    	static uint16_t x93 = 16U;
	int16_t x94 = INT16_MIN;
	uint8_t x95 = 0U;
	static int16_t x96 = -7;
	int32_t t22 = -240926;

    t22 = (x93/((x94<=x95)!=x96));

    if (t22 != 16) { NG(); } else { ; }
	
}

void f23(void) {
    	static int64_t x97 = INT64_MAX;
	uint8_t x98 = UINT8_MAX;
	uint16_t x99 = UINT16_MAX;
	int64_t t23 = INT64_MAX;

    t23 = (x97/((x98<=x99)!=x100));

    if (t23 != INT64_MAX) { NG(); } else { ; }
	
}

void f24(void) {
    	int8_t x101 = INT8_MAX;
	static uint64_t x103 = 24711628505166505LLU;
	int32_t t24 = -1772151;

    t24 = (x101/((x102<=x103)!=x104));

    if (t24 != 127) { NG(); } else { ; }
	
}

void f25(void) {
    	uint8_t x106 = UINT8_MAX;
	static volatile int32_t x107 = -4;
	static int16_t x108 = 86;
	uint32_t t25 = 18U;

    t25 = (x105/((x106<=x107)!=x108));

    if (t25 != 1547826U) { NG(); } else { ; }
	
}

void f26(void) {
    	int16_t x109 = -16062;
	volatile uint32_t x110 = 312U;
	int32_t x111 = 1;
	static int8_t x112 = -1;
	int32_t t26 = 5849678;

    t26 = (x109/((x110<=x111)!=x112));

    if (t26 != -16062) { NG(); } else { ; }
	
}

void f27(void) {
    	int8_t x113 = INT8_MIN;
	int16_t x114 = -149;
	int32_t x115 = INT32_MIN;

    t27 = (x113/((x114<=x115)!=x116));

    if (t27 != -128) { NG(); } else { ; }
	
}

void f28(void) {
    	int32_t x117 = INT32_MAX;
	uint8_t x118 = UINT8_MAX;
	uint32_t x120 = 13085674U;
	volatile int32_t t28 = INT32_MAX;

    t28 = (x117/((x118<=x119)!=x120));

    if (t28 != INT32_MAX) { NG(); } else { ; }
	
}

void f29(void) {
    	int16_t x121 = -35;
	int16_t x122 = INT16_MIN;
	uint32_t x123 = 12159U;
	volatile int32_t t29 = 994096;

    t29 = (x121/((x122<=x123)!=x124));

    if (t29 != -35) { NG(); } else { ; }
	
}

void f30(void) {
    	int16_t x125 = -1;
	volatile int32_t x126 = 9772779;
	volatile int16_t x127 = -1;
	uint16_t x128 = 333U;

    t30 = (x125/((x126<=x127)!=x128));

    if (t30 != -1) { NG(); } else { ; }
	
}

void f31(void) {
    	static volatile int16_t x129 = -1;
	volatile uint64_t x130 = 340038423222LLU;
	uint64_t x131 = 308031855002439LLU;
	uint8_t x132 = 59U;

    t31 = (x129/((x130<=x131)!=x132));

    if (t31 != -1) { NG(); } else { ; }
	
}

void f32(void) {
    	int8_t x133 = INT8_MAX;
	static int64_t x134 = 512100622394LL;
	uint32_t x135 = 1581427152U;
	uint8_t x136 = UINT8_MAX;
	static int32_t t32 = -6353;

    t32 = (x133/((x134<=x135)!=x136));

    if (t32 != 127) { NG(); } else { ; }
	
}

void f33(void) {
    	static uint32_t x138 = 3U;
	static volatile uint8_t x139 = 5U;
	int8_t x140 = INT8_MIN;
	volatile int64_t t33 = -206934648728004LL;

    t33 = (x137/((x138<=x139)!=x140));

    if (t33 != 85LL) { NG(); } else { ; }
	
}

void f34(void) {
    	int32_t x141 = INT32_MIN;
	static volatile int64_t x142 = 238374814780588LL;
	int8_t x143 = INT8_MAX;
	int64_t x144 = -151928LL;
	static int32_t t34 = INT32_MIN;

    t34 = (x141/((x142<=x143)!=x144));

    if (t34 != INT32_MIN) { NG(); } else { ; }
	
}

void f35(void) {
    	uint16_t x145 = 1462U;
	int32_t x146 = -364647;
	int32_t x148 = INT32_MIN;

    t35 = (x145/((x146<=x147)!=x148));

    if (t35 != 1462) { NG(); } else { ; }
	
}

void f36(void) {
    	int8_t x150 = 1;
	static int8_t x151 = -1;
	int32_t x152 = -1;
	uint32_t t36 = 60U;

    t36 = (x149/((x150<=x151)!=x152));

    if (t36 != 10353232U) { NG(); } else { ; }
	
}

void f37(void) {
    	uint8_t x153 = UINT8_MAX;
	static uint16_t x154 = UINT16_MAX;
	int16_t x155 = INT16_MAX;
	static volatile int32_t t37 = -15;

    t37 = (x153/((x154<=x155)!=x156));

    if (t37 != 255) { NG(); } else { ; }
	
}

void f38(void) {
    	int64_t x157 = INT64_MIN;
	int8_t x159 = 61;
	volatile int64_t x160 = 5LL;
	volatile int64_t t38 = INT64_MIN;

    t38 = (x157/((x158<=x159)!=x160));

    if (t38 != INT64_MIN) { NG(); } else { ; }
	
}

void f39(void) {
    	static int16_t x161 = INT16_MAX;
	volatile int32_t x162 = INT32_MIN;
	static int64_t x163 = INT64_MIN;
	int32_t t39 = -34196192;

    t39 = (x161/((x162<=x163)!=x164));

    if (t39 != 32767) { NG(); } else { ; }
	
}

void f40(void) {
    	uint32_t x166 = 2U;
	static int8_t x167 = 1;
	int32_t x168 = -367;
	int32_t t40 = -5;

    t40 = (x165/((x166<=x167)!=x168));

    if (t40 != 65535) { NG(); } else { ; }
	
}

void f41(void) {
    	static uint8_t x169 = 1U;
	int32_t x170 = INT32_MAX;
	static int64_t x171 = INT64_MIN;
	static int16_t x172 = INT16_MAX;
	int32_t t41 = -1998;

    t41 = (x169/((x170<=x171)!=x172));

    if (t41 != 1) { NG(); } else { ; }
	
}

void f42(void) {
    	int64_t x173 = -1LL;
	volatile int16_t x174 = 342;
	uint32_t x175 = 63336U;
	int64_t x176 = -1LL;
	int64_t t42 = -1043226050LL;

    t42 = (x173/((x174<=x175)!=x176));

    if (t42 != -1LL) { NG(); } else { ; }
	
}

void f43(void) {
    	volatile int64_t x177 = -40342LL;
	volatile int32_t x178 = 115;
	uint16_t x179 = 6034U;
	uint8_t x180 = 3U;
	volatile int64_t t43 = -218554626LL;

    t43 = (x177/((x178<=x179)!=x180));

    if (t43 != -40342LL) { NG(); } else { ; }
	
}

void f44(void) {
    	int64_t x181 = INT64_MAX;
	volatile uint64_t x182 = 81663LLU;
	volatile int64_t x183 = INT64_MIN;
	uint64_t x184 = 5743358234LLU;

    t44 = (x181/((x182<=x183)!=x184));

    if (t44 != INT64_MAX) { NG(); } else { ; }
	
}

void f45(void) {
    	static int16_t x189 = INT16_MIN;
	uint64_t x190 = 10369687570601001LLU;
	uint8_t x191 = 44U;
	int64_t x192 = INT64_MIN;
	int32_t t45 = -12630267;

    t45 = (x189/((x190<=x191)!=x192));

    if (t45 != -32768) { NG(); } else { ; }
	
}

void f46(void) {
    	volatile int8_t x193 = INT8_MIN;
	static int16_t x194 = INT16_MIN;
	static int16_t x195 = INT16_MIN;
	static int64_t x196 = -71LL;
	int32_t t46 = 980590304;

    t46 = (x193/((x194<=x195)!=x196));

    if (t46 != -128) { NG(); } else { ; }
	
}

void f47(void) {
    	volatile int8_t x197 = INT8_MIN;
	int8_t x198 = 15;
	int8_t x199 = -1;
	int64_t x200 = 1430668323786403256LL;

    t47 = (x197/((x198<=x199)!=x200));

    if (t47 != -128) { NG(); } else { ; }
	
}

void f48(void) {
    	volatile uint64_t x201 = 3LLU;
	static int32_t x202 = INT32_MIN;
	static int16_t x203 = -1;
	int64_t x204 = -63128LL;

    t48 = (x201/((x202<=x203)!=x204));

    if (t48 != 3LLU) { NG(); } else { ; }
	
}

void f49(void) {
    	uint16_t x205 = 62U;
	int8_t x206 = INT8_MIN;
	volatile uint8_t x208 = 6U;
	volatile int32_t t49 = -750976;

    t49 = (x205/((x206<=x207)!=x208));

    if (t49 != 62) { NG(); } else { ; }
	
}

void f50(void) {
    	uint64_t x209 = 46LLU;
	static uint64_t x210 = 15703LLU;
	int8_t x211 = 14;
	static uint64_t x212 = 172518954LLU;
	uint64_t t50 = 4463737051584155533LLU;

    t50 = (x209/((x210<=x211)!=x212));

    if (t50 != 46LLU) { NG(); } else { ; }
	
}

void f51(void) {
    	uint32_t x213 = 44214716U;
	static int16_t x214 = -1;
	int8_t x215 = -1;
	volatile int8_t x216 = INT8_MIN;
	uint32_t t51 = 955U;

    t51 = (x213/((x214<=x215)!=x216));

    if (t51 != 44214716U) { NG(); } else { ; }
	
}

void f52(void) {
    	static int64_t x217 = INT64_MAX;
	int64_t x219 = INT64_MIN;
	uint32_t x220 = UINT32_MAX;
	static int64_t t52 = INT64_MAX;

    t52 = (x217/((x218<=x219)!=x220));

    if (t52 != INT64_MAX) { NG(); } else { ; }
	
}

void f53(void) {
    	int64_t x221 = INT64_MIN;
	volatile uint64_t x223 = 5477LLU;
	int64_t x224 = 18484778157173LL;

    t53 = (x221/((x222<=x223)!=x224));

    if (t53 != INT64_MIN) { NG(); } else { ; }
	
}

void f54(void) {
    	static int64_t x225 = INT64_MIN;
	int16_t x226 = -1;
	int64_t x227 = INT64_MIN;
	volatile int64_t t54 = INT64_MIN;

    t54 = (x225/((x226<=x227)!=x228));

    if (t54 != INT64_MIN) { NG(); } else { ; }
	
}

void f55(void) {
    	int8_t x229 = -1;
	int64_t x230 = INT64_MAX;
	int8_t x231 = -46;
	volatile int8_t x232 = INT8_MIN;
	volatile int32_t t55 = 2723715;

    t55 = (x229/((x230<=x231)!=x232));

    if (t55 != -1) { NG(); } else { ; }
	
}

void f56(void) {
    	int16_t x233 = INT16_MAX;
	static uint32_t x234 = 103438786U;
	int64_t x236 = -1674LL;
	int32_t t56 = -8992;

    t56 = (x233/((x234<=x235)!=x236));

    if (t56 != 32767) { NG(); } else { ; }
	
}

void f57(void) {
    	uint64_t x237 = 17339728716601LLU;
	int16_t x238 = INT16_MAX;
	volatile int64_t x239 = -7850379927522896LL;
	int64_t x240 = INT64_MIN;
	volatile uint64_t t57 = 8711455670615LLU;

    t57 = (x237/((x238<=x239)!=x240));

    if (t57 != 17339728716601LLU) { NG(); } else { ; }
	
}

void f58(void) {
    	static int16_t x241 = INT16_MIN;
	uint16_t x242 = UINT16_MAX;
	static uint8_t x243 = UINT8_MAX;
	int8_t x244 = 2;
	volatile int32_t t58 = -421547;

    t58 = (x241/((x242<=x243)!=x244));

    if (t58 != -32768) { NG(); } else { ; }
	
}

void f59(void) {
    	volatile int64_t x245 = 11471LL;
	uint64_t x246 = UINT64_MAX;
	uint16_t x247 = 29982U;
	volatile int64_t x248 = INT64_MIN;
	volatile int64_t t59 = 49497182735LL;

    t59 = (x245/((x246<=x247)!=x248));

    if (t59 != 11471LL) { NG(); } else { ; }
	
}

void f60(void) {
    	int8_t x249 = INT8_MAX;
	volatile int16_t x250 = INT16_MIN;
	volatile uint64_t x252 = 256409LLU;

    t60 = (x249/((x250<=x251)!=x252));

    if (t60 != 127) { NG(); } else { ; }
	
}

void f61(void) {
    	int8_t x253 = INT8_MIN;
	static int16_t x254 = 8266;
	int32_t x255 = -432594;
	volatile int16_t x256 = INT16_MIN;
	int32_t t61 = 945057405;

    t61 = (x253/((x254<=x255)!=x256));

    if (t61 != -128) { NG(); } else { ; }
	
}

void f62(void) {
    	static volatile int32_t x257 = -1;
	int16_t x258 = 203;
	int64_t x259 = INT64_MIN;
	int32_t t62 = 1;

    t62 = (x257/((x258<=x259)!=x260));

    if (t62 != -1) { NG(); } else { ; }
	
}

void f63(void) {
    	int32_t x262 = -339;
	int16_t x263 = INT16_MIN;
	volatile uint64_t x264 = 953522321959116145LLU;
	int32_t t63 = -1;

    t63 = (x261/((x262<=x263)!=x264));

    if (t63 != 0) { NG(); } else { ; }
	
}

void f64(void) {
    	uint32_t x265 = 913U;
	volatile int64_t x266 = -1LL;
	static volatile uint16_t x267 = UINT16_MAX;
	int8_t x268 = -1;
	volatile uint32_t t64 = 134307U;

    t64 = (x265/((x266<=x267)!=x268));

    if (t64 != 913U) { NG(); } else { ; }
	
}

void f65(void) {
    	int16_t x269 = -10;
	static int8_t x271 = INT8_MIN;
	static int16_t x272 = -133;
	volatile int32_t t65 = -7943212;

    t65 = (x269/((x270<=x271)!=x272));

    if (t65 != -10) { NG(); } else { ; }
	
}

void f66(void) {
    	volatile uint16_t x273 = 8139U;
	int16_t x274 = 0;
	static uint64_t x276 = 43042325740707LLU;

    t66 = (x273/((x274<=x275)!=x276));

    if (t66 != 8139) { NG(); } else { ; }
	
}

void f67(void) {
    	int8_t x277 = INT8_MAX;
	volatile int16_t x278 = 7682;
	uint32_t x279 = 1437687808U;
	uint64_t x280 = UINT64_MAX;

    t67 = (x277/((x278<=x279)!=x280));

    if (t67 != 127) { NG(); } else { ; }
	
}

void f68(void) {
    	int8_t x285 = 53;
	static int32_t x286 = INT32_MIN;

    t68 = (x285/((x286<=x287)!=x288));

    if (t68 != 53) { NG(); } else { ; }
	
}

void f69(void) {
    	int16_t x289 = INT16_MAX;
	int64_t x290 = INT64_MIN;
	int64_t x291 = INT64_MAX;
	int32_t x292 = INT32_MIN;
	volatile int32_t t69 = 409829560;

    t69 = (x289/((x290<=x291)!=x292));

    if (t69 != 32767) { NG(); } else { ; }
	
}

void f70(void) {
    	volatile uint16_t x293 = 2286U;
	uint64_t x294 = UINT64_MAX;
	int64_t x295 = 77445508LL;
	static int8_t x296 = INT8_MAX;
	static int32_t t70 = 0;

    t70 = (x293/((x294<=x295)!=x296));

    if (t70 != 2286) { NG(); } else { ; }
	
}

void f71(void) {
    	int8_t x297 = INT8_MIN;
	static uint32_t x298 = UINT32_MAX;
	int64_t x299 = INT64_MAX;
	static uint8_t x300 = UINT8_MAX;
	int32_t t71 = -839;

    t71 = (x297/((x298<=x299)!=x300));

    if (t71 != -128) { NG(); } else { ; }
	
}

void f72(void) {
    	static uint16_t x301 = UINT16_MAX;
	int8_t x302 = -60;
	static uint64_t x304 = UINT64_MAX;

    t72 = (x301/((x302<=x303)!=x304));

    if (t72 != 65535) { NG(); } else { ; }
	
}

void f73(void) {
    	int8_t x306 = INT8_MAX;
	uint64_t x307 = UINT64_MAX;
	int32_t t73 = -1;

    t73 = (x305/((x306<=x307)!=x308));

    if (t73 != 1) { NG(); } else { ; }
	
}

void f74(void) {
    	uint16_t x309 = UINT16_MAX;
	int32_t x310 = INT32_MAX;
	int8_t x312 = -1;
	int32_t t74 = 0;

    t74 = (x309/((x310<=x311)!=x312));

    if (t74 != 65535) { NG(); } else { ; }
	
}

void f75(void) {
    	static int32_t x313 = INT32_MAX;
	volatile uint16_t x314 = UINT16_MAX;
	int16_t x315 = INT16_MIN;
	uint8_t x316 = 14U;

    t75 = (x313/((x314<=x315)!=x316));

    if (t75 != INT32_MAX) { NG(); } else { ; }
	
}

void f76(void) {
    	volatile uint16_t x317 = 6U;
	volatile uint64_t x318 = UINT64_MAX;
	uint32_t x320 = UINT32_MAX;
	volatile int32_t t76 = -785075;

    t76 = (x317/((x318<=x319)!=x320));

    if (t76 != 6) { NG(); } else { ; }
	
}

void f77(void) {
    	int16_t x322 = -1275;
	uint64_t x323 = 1101033018739418LLU;
	int32_t x324 = -1;
	static int32_t t77 = 283141;

    t77 = (x321/((x322<=x323)!=x324));

    if (t77 != 25607553) { NG(); } else { ; }
	
}

void f78(void) {
    	volatile uint32_t x325 = 10098U;
	int8_t x326 = INT8_MIN;
	int64_t x327 = INT64_MIN;
	static uint8_t x328 = UINT8_MAX;
	volatile uint32_t t78 = 112391U;

    t78 = (x325/((x326<=x327)!=x328));

    if (t78 != 10098U) { NG(); } else { ; }
	
}

void f79(void) {
    	int64_t x329 = 28740499LL;
	uint16_t x330 = 0U;
	static int16_t x331 = 37;
	int32_t x332 = 5129;
	int64_t t79 = -2171131918156LL;

    t79 = (x329/((x330<=x331)!=x332));

    if (t79 != 28740499LL) { NG(); } else { ; }
	
}

void f80(void) {
    	int8_t x334 = 33;
	static int8_t x335 = -1;
	int16_t x336 = -1;
	static int32_t t80 = 465360;

    t80 = (x333/((x334<=x335)!=x336));

    if (t80 != -122468) { NG(); } else { ; }
	
}

void f81(void) {
    	int8_t x339 = 4;
	static uint32_t x340 = 501U;
	uint64_t t81 = 13767412969144935LLU;

    t81 = (x337/((x338<=x339)!=x340));

    if (t81 != 143287900452LLU) { NG(); } else { ; }
	
}

void f82(void) {
    	uint32_t x341 = 41846676U;
	uint8_t x343 = UINT8_MAX;
	volatile int8_t x344 = INT8_MIN;
	static volatile uint32_t t82 = 203544U;

    t82 = (x341/((x342<=x343)!=x344));

    if (t82 != 41846676U) { NG(); } else { ; }
	
}

void f83(void) {
    	volatile uint64_t x345 = 1LLU;
	volatile int8_t x346 = INT8_MIN;
	uint16_t x348 = 1U;
	static uint64_t t83 = 80LLU;

    t83 = (x345/((x346<=x347)!=x348));

    if (t83 != 1LLU) { NG(); } else { ; }
	
}

void f84(void) {
    	static int16_t x349 = -1;
	uint8_t x350 = 4U;
	int64_t x351 = INT64_MAX;
	static int32_t x352 = INT32_MIN;
	int32_t t84 = -155;

    t84 = (x349/((x350<=x351)!=x352));

    if (t84 != -1) { NG(); } else { ; }
	
}

void f85(void) {
    	int16_t x353 = INT16_MAX;
	volatile int16_t x354 = INT16_MIN;
	static int64_t x355 = 8575187LL;

    t85 = (x353/((x354<=x355)!=x356));

    if (t85 != 32767) { NG(); } else { ; }
	
}

void f86(void) {
    	volatile int64_t x357 = INT64_MAX;
	static volatile uint16_t x358 = 12203U;
	static int8_t x359 = INT8_MAX;
	static int8_t x360 = -1;

    t86 = (x357/((x358<=x359)!=x360));

    if (t86 != INT64_MAX) { NG(); } else { ; }
	
}

void f87(void) {
    	int16_t x366 = -1;
	static volatile int32_t x368 = 951845597;
	volatile int32_t t87 = -78;

    t87 = (x365/((x366<=x367)!=x368));

    if (t87 != 127) { NG(); } else { ; }
	
}

void f88(void) {
    	static volatile int32_t x369 = 418136;
	uint64_t x370 = UINT64_MAX;
	uint8_t x371 = UINT8_MAX;
	uint16_t x372 = UINT16_MAX;
	volatile int32_t t88 = 492698;

    t88 = (x369/((x370<=x371)!=x372));

    if (t88 != 418136) { NG(); } else { ; }
	
}

void f89(void) {
    	uint8_t x373 = 22U;
	int16_t x374 = INT16_MAX;
	int64_t x375 = INT64_MIN;
	int32_t t89 = 17226;

    t89 = (x373/((x374<=x375)!=x376));

    if (t89 != 22) { NG(); } else { ; }
	
}

void f90(void) {
    	static int16_t x377 = INT16_MIN;
	uint64_t x378 = UINT64_MAX;
	int32_t x379 = INT32_MAX;
	volatile int32_t t90 = 459;

    t90 = (x377/((x378<=x379)!=x380));

    if (t90 != -32768) { NG(); } else { ; }
	
}

void f91(void) {
    	int64_t x381 = -3990036LL;
	int16_t x382 = 1;
	int16_t x383 = INT16_MAX;
	static int32_t x384 = INT32_MIN;
	volatile int64_t t91 = 0LL;

    t91 = (x381/((x382<=x383)!=x384));

    if (t91 != -3990036LL) { NG(); } else { ; }
	
}

void f92(void) {
    	int16_t x386 = INT16_MIN;
	uint32_t x387 = 501280837U;
	int8_t x388 = INT8_MIN;
	static volatile int32_t t92 = -3412;

    t92 = (x385/((x386<=x387)!=x388));

    if (t92 != 2) { NG(); } else { ; }
	
}

void f93(void) {
    	uint32_t x389 = 0U;
	static int8_t x390 = INT8_MIN;
	uint16_t x391 = UINT16_MAX;
	int64_t x392 = INT64_MIN;
	static uint32_t t93 = 0U;

    t93 = (x389/((x390<=x391)!=x392));

    if (t93 != 0U) { NG(); } else { ; }
	
}

void f94(void) {
    	int16_t x394 = INT16_MIN;
	static uint8_t x395 = UINT8_MAX;
	int64_t x396 = INT64_MAX;
	int32_t t94 = 976373578;

    t94 = (x393/((x394<=x395)!=x396));

    if (t94 != -26865) { NG(); } else { ; }
	
}

void f95(void) {
    	volatile int8_t x398 = INT8_MIN;
	volatile uint32_t x400 = 1942578U;
	static int32_t t95 = 7;

    t95 = (x397/((x398<=x399)!=x400));

    if (t95 != -128) { NG(); } else { ; }
	
}

void f96(void) {
    	uint8_t x401 = UINT8_MAX;
	volatile int8_t x402 = -1;
	static volatile int32_t x403 = INT32_MIN;
	uint32_t x404 = 2054106821U;
	volatile int32_t t96 = 5;

    t96 = (x401/((x402<=x403)!=x404));

    if (t96 != 255) { NG(); } else { ; }
	
}

void f97(void) {
    	uint8_t x405 = 18U;
	volatile int32_t x407 = -1;
	int8_t x408 = -1;
	int32_t t97 = -417951;

    t97 = (x405/((x406<=x407)!=x408));

    if (t97 != 18) { NG(); } else { ; }
	
}

void f98(void) {
    	uint32_t x409 = 2U;
	static int8_t x410 = INT8_MIN;
	static volatile int32_t x411 = 6;
	int32_t x412 = INT32_MAX;
	uint32_t t98 = 59U;

    t98 = (x409/((x410<=x411)!=x412));

    if (t98 != 2U) { NG(); } else { ; }
	
}

void f99(void) {
    	uint32_t x417 = 422123U;
	static int16_t x418 = INT16_MIN;
	volatile uint32_t x419 = 192124481U;
	volatile uint32_t t99 = 607U;

    t99 = (x417/((x418<=x419)!=x420));

    if (t99 != 422123U) { NG(); } else { ; }
	
}

void f100(void) {
    	static uint64_t x422 = 10078503845LLU;
	int64_t x423 = -11LL;
	volatile int32_t x424 = -65955646;
	int64_t t100 = INT64_MIN;

    t100 = (x421/((x422<=x423)!=x424));

    if (t100 != INT64_MIN) { NG(); } else { ; }
	
}

void f101(void) {
    	static int32_t x425 = INT32_MAX;
	int16_t x426 = INT16_MIN;
	uint64_t x427 = 138817LLU;
	static int16_t x428 = INT16_MIN;
	int32_t t101 = INT32_MAX;

    t101 = (x425/((x426<=x427)!=x428));

    if (t101 != INT32_MAX) { NG(); } else { ; }
	
}

void f102(void) {
    	int8_t x431 = INT8_MAX;
	uint8_t x432 = 25U;

    t102 = (x429/((x430<=x431)!=x432));

    if (t102 != 127) { NG(); } else { ; }
	
}

void f103(void) {
    	uint16_t x433 = UINT16_MAX;
	int8_t x435 = -1;
	uint32_t x436 = UINT32_MAX;
	int32_t t103 = -3928;

    t103 = (x433/((x434<=x435)!=x436));

    if (t103 != 65535) { NG(); } else { ; }
	
}

void f104(void) {
    	int32_t x438 = 7966;
	static volatile uint8_t x439 = 3U;
	volatile int32_t x440 = 1;
	int32_t t104 = 1;

    t104 = (x437/((x438<=x439)!=x440));

    if (t104 != -128) { NG(); } else { ; }
	
}

void f105(void) {
    	volatile uint64_t x441 = 3951LLU;
	int64_t x442 = INT64_MIN;
	uint64_t t105 = 46430953226LLU;

    t105 = (x441/((x442<=x443)!=x444));

    if (t105 != 3951LLU) { NG(); } else { ; }
	
}

void f106(void) {
    	volatile int64_t x446 = -1062849LL;
	volatile int8_t x447 = -8;
	uint8_t x448 = UINT8_MAX;
	int32_t t106 = INT32_MAX;

    t106 = (x445/((x446<=x447)!=x448));

    if (t106 != INT32_MAX) { NG(); } else { ; }
	
}

void f107(void) {
    	int64_t x449 = -1LL;
	int32_t x450 = INT32_MIN;
	volatile uint32_t x451 = UINT32_MAX;
	volatile uint64_t x452 = 6004280LLU;
	static int64_t t107 = 30094641LL;

    t107 = (x449/((x450<=x451)!=x452));

    if (t107 != -1LL) { NG(); } else { ; }
	
}

void f108(void) {
    	static int64_t x453 = 29416LL;
	static int8_t x454 = INT8_MIN;
	int8_t x455 = -35;
	volatile int64_t t108 = 4185LL;

    t108 = (x453/((x454<=x455)!=x456));

    if (t108 != 29416LL) { NG(); } else { ; }
	
}

void f109(void) {
    	static volatile uint32_t x457 = 11U;
	int64_t x459 = -1LL;
	uint32_t t109 = 0U;

    t109 = (x457/((x458<=x459)!=x460));

    if (t109 != 11U) { NG(); } else { ; }
	
}

void f110(void) {
    	volatile int64_t x461 = -87709272887110603LL;
	uint32_t x462 = UINT32_MAX;
	uint32_t x463 = UINT32_MAX;
	int8_t x464 = -30;
	volatile int64_t t110 = -93364490026878999LL;

    t110 = (x461/((x462<=x463)!=x464));

    if (t110 != -87709272887110603LL) { NG(); } else { ; }
	
}

void f111(void) {
    	uint16_t x465 = 794U;
	volatile int64_t x466 = 60651953896806947LL;
	static int16_t x467 = -1;
	volatile int32_t t111 = 23;

    t111 = (x465/((x466<=x467)!=x468));

    if (t111 != 794) { NG(); } else { ; }
	
}

void f112(void) {
    	uint8_t x469 = 0U;
	static volatile int64_t x470 = INT64_MIN;
	uint64_t x471 = 1289325072489167214LLU;
	uint8_t x472 = 3U;
	static volatile int32_t t112 = 554;

    t112 = (x469/((x470<=x471)!=x472));

    if (t112 != 0) { NG(); } else { ; }
	
}

void f113(void) {
    	static int32_t x474 = -1;
	uint16_t x475 = 7U;
	int64_t x476 = -1LL;
	volatile int32_t t113 = -330685215;

    t113 = (x473/((x474<=x475)!=x476));

    if (t113 != -9) { NG(); } else { ; }
	
}

void f114(void) {
    	int8_t x477 = INT8_MIN;
	uint16_t x478 = 13974U;
	int64_t x479 = INT64_MIN;
	int32_t x480 = INT32_MAX;
	int32_t t114 = 29445110;

    t114 = (x477/((x478<=x479)!=x480));

    if (t114 != -128) { NG(); } else { ; }
	
}

void f115(void) {
    	static int64_t x481 = -1LL;
	int32_t x482 = INT32_MIN;
	int16_t x484 = -1;

    t115 = (x481/((x482<=x483)!=x484));

    if (t115 != -1LL) { NG(); } else { ; }
	
}

void f116(void) {
    	static int16_t x487 = INT16_MIN;

    t116 = (x485/((x486<=x487)!=x488));

    if (t116 != -128) { NG(); } else { ; }
	
}

void f117(void) {
    	int16_t x489 = 429;
	static int8_t x490 = INT8_MAX;
	volatile int32_t x492 = 13;

    t117 = (x489/((x490<=x491)!=x492));

    if (t117 != 429) { NG(); } else { ; }
	
}

void f118(void) {
    	uint32_t x493 = 521103797U;
	static volatile int64_t x494 = -1LL;
	uint16_t x496 = 21U;
	volatile uint32_t t118 = 520696U;

    t118 = (x493/((x494<=x495)!=x496));

    if (t118 != 521103797U) { NG(); } else { ; }
	
}

void f119(void) {
    	static int32_t x497 = -1;
	volatile uint32_t x498 = UINT32_MAX;
	uint8_t x499 = 0U;
	int16_t x500 = INT16_MAX;
	int32_t t119 = -153;

    t119 = (x497/((x498<=x499)!=x500));

    if (t119 != -1) { NG(); } else { ; }
	
}

void f120(void) {
    	static int32_t x501 = INT32_MAX;
	uint8_t x504 = UINT8_MAX;

    t120 = (x501/((x502<=x503)!=x504));

    if (t120 != INT32_MAX) { NG(); } else { ; }
	
}

void f121(void) {
    	static volatile uint64_t x505 = UINT64_MAX;
	int64_t x506 = INT64_MIN;
	int32_t x507 = INT32_MIN;
	uint64_t x508 = 0LLU;
	volatile uint64_t t121 = UINT64_MAX;

    t121 = (x505/((x506<=x507)!=x508));

    if (t121 != UINT64_MAX) { NG(); } else { ; }
	
}

void f122(void) {
    	static volatile uint8_t x510 = 5U;
	int8_t x511 = -7;
	int32_t x512 = INT32_MIN;
	volatile int32_t t122 = 9019025;

    t122 = (x509/((x510<=x511)!=x512));

    if (t122 != 7) { NG(); } else { ; }
	
}

void f123(void) {
    	int64_t x513 = -5988012680869273LL;
	static int8_t x514 = -1;
	int16_t x515 = -1147;
	int8_t x516 = INT8_MIN;

    t123 = (x513/((x514<=x515)!=x516));

    if (t123 != -5988012680869273LL) { NG(); } else { ; }
	
}

void f124(void) {
    	uint8_t x517 = 7U;
	volatile int16_t x518 = -1;
	static int16_t x519 = INT16_MAX;
	static uint32_t x520 = UINT32_MAX;
	static volatile int32_t t124 = -123;

    t124 = (x517/((x518<=x519)!=x520));

    if (t124 != 7) { NG(); } else { ; }
	
}

void f125(void) {
    	static uint32_t x521 = UINT32_MAX;
	uint8_t x524 = 4U;
	volatile uint32_t t125 = UINT32_MAX;

    t125 = (x521/((x522<=x523)!=x524));

    if (t125 != UINT32_MAX) { NG(); } else { ; }
	
}

void f126(void) {
    	uint8_t x525 = 19U;
	int64_t x526 = INT64_MAX;
	int64_t x527 = INT64_MIN;
	static uint16_t x528 = 2356U;
	int32_t t126 = 40331;

    t126 = (x525/((x526<=x527)!=x528));

    if (t126 != 19) { NG(); } else { ; }
	
}

void f127(void) {
    	static int8_t x530 = INT8_MIN;
	volatile uint64_t x532 = 2631983220742100960LLU;
	volatile uint64_t t127 = UINT64_MAX;

    t127 = (x529/((x530<=x531)!=x532));

    if (t127 != UINT64_MAX) { NG(); } else { ; }
	
}

void f128(void) {
    	uint32_t x533 = UINT32_MAX;
	int32_t x534 = INT32_MIN;
	volatile int8_t x535 = INT8_MAX;
	static volatile int32_t x536 = INT32_MIN;
	uint32_t t128 = UINT32_MAX;

    t128 = (x533/((x534<=x535)!=x536));

    if (t128 != UINT32_MAX) { NG(); } else { ; }
	
}

void f129(void) {
    	int32_t x537 = INT32_MIN;
	int8_t x538 = INT8_MIN;
	int32_t x539 = INT32_MIN;
	volatile int32_t t129 = INT32_MIN;

    t129 = (x537/((x538<=x539)!=x540));

    if (t129 != INT32_MIN) { NG(); } else { ; }
	
}

void f130(void) {
    	int8_t x541 = INT8_MIN;
	int32_t x542 = INT32_MIN;
	int32_t x543 = INT32_MIN;

    t130 = (x541/((x542<=x543)!=x544));

    if (t130 != -128) { NG(); } else { ; }
	
}

void f131(void) {
    	volatile uint8_t x545 = 1U;
	volatile int16_t x546 = 178;
	int64_t x547 = -8789301LL;
	static uint64_t x548 = UINT64_MAX;
	int32_t t131 = 27;

    t131 = (x545/((x546<=x547)!=x548));

    if (t131 != 1) { NG(); } else { ; }
	
}

void f132(void) {
    	int8_t x551 = INT8_MIN;
	int64_t x552 = 171962773LL;
	volatile int32_t t132 = -801902;

    t132 = (x549/((x550<=x551)!=x552));

    if (t132 != 355) { NG(); } else { ; }
	
}

void f133(void) {
    	int16_t x553 = 19;
	int8_t x554 = 32;
	int8_t x555 = INT8_MIN;
	static uint32_t x556 = 37U;

    t133 = (x553/((x554<=x555)!=x556));

    if (t133 != 19) { NG(); } else { ; }
	
}

void f134(void) {
    	int16_t x557 = -1;
	int8_t x558 = INT8_MAX;
	int64_t x560 = -12957150021800LL;

    t134 = (x557/((x558<=x559)!=x560));

    if (t134 != -1) { NG(); } else { ; }
	
}

void f135(void) {
    	int32_t x564 = INT32_MAX;
	int32_t t135 = 660736166;

    t135 = (x561/((x562<=x563)!=x564));

    if (t135 != -1) { NG(); } else { ; }
	
}

void f136(void) {
    	int32_t x565 = INT32_MIN;
	static int32_t x566 = -1;
	uint16_t x567 = 426U;
	volatile int32_t x568 = INT32_MAX;
	static volatile int32_t t136 = INT32_MIN;

    t136 = (x565/((x566<=x567)!=x568));

    if (t136 != INT32_MIN) { NG(); } else { ; }
	
}

void f137(void) {
    	uint32_t x569 = UINT32_MAX;
	static int16_t x570 = INT16_MIN;
	uint16_t x571 = 433U;
	static int64_t x572 = INT64_MIN;
	uint32_t t137 = UINT32_MAX;

    t137 = (x569/((x570<=x571)!=x572));

    if (t137 != UINT32_MAX) { NG(); } else { ; }
	
}

void f138(void) {
    	uint64_t x573 = 1126378958983930512LLU;
	volatile int32_t x575 = INT32_MIN;
	int8_t x576 = INT8_MIN;
	uint64_t t138 = 21834135364LLU;

    t138 = (x573/((x574<=x575)!=x576));

    if (t138 != 1126378958983930512LLU) { NG(); } else { ; }
	
}

void f139(void) {
    	volatile int32_t x577 = INT32_MIN;
	int8_t x579 = -1;
	volatile int32_t t139 = INT32_MIN;

    t139 = (x577/((x578<=x579)!=x580));

    if (t139 != INT32_MIN) { NG(); } else { ; }
	
}

void f140(void) {
    	int16_t x581 = INT16_MIN;
	uint16_t x582 = 0U;
	uint64_t x583 = 6007LLU;
	int32_t x584 = -1;
	volatile int32_t t140 = -843;

    t140 = (x581/((x582<=x583)!=x584));

    if (t140 != -32768) { NG(); } else { ; }
	
}

void f141(void) {
    	int64_t x585 = INT64_MIN;
	volatile int16_t x586 = INT16_MIN;
	volatile uint16_t x587 = UINT16_MAX;
	static uint64_t x588 = 52901951297995937LLU;
	volatile int64_t t141 = INT64_MIN;

    t141 = (x585/((x586<=x587)!=x588));

    if (t141 != INT64_MIN) { NG(); } else { ; }
	
}

void f142(void) {
    	uint8_t x591 = 49U;
	int16_t x592 = INT16_MAX;
	volatile int32_t t142 = -290235564;

    t142 = (x589/((x590<=x591)!=x592));

    if (t142 != -128) { NG(); } else { ; }
	
}

void f143(void) {
    	static int64_t x594 = INT64_MAX;
	uint16_t x595 = 12U;
	uint32_t x596 = 1705469U;
	static volatile int32_t t143 = -1;

    t143 = (x593/((x594<=x595)!=x596));

    if (t143 != 1742) { NG(); } else { ; }
	
}

void f144(void) {
    	int16_t x597 = -1;
	uint8_t x598 = UINT8_MAX;
	int64_t x599 = -203405344078LL;
	volatile uint64_t x600 = UINT64_MAX;
	volatile int32_t t144 = -48176952;

    t144 = (x597/((x598<=x599)!=x600));

    if (t144 != -1) { NG(); } else { ; }
	
}

void f145(void) {
    	int64_t x601 = INT64_MAX;
	int32_t x602 = -148179464;
	int16_t x603 = -10451;
	volatile int64_t x604 = INT64_MAX;
	int64_t t145 = INT64_MAX;

    t145 = (x601/((x602<=x603)!=x604));

    if (t145 != INT64_MAX) { NG(); } else { ; }
	
}

void f146(void) {
    	int64_t x605 = INT64_MAX;
	volatile uint32_t x606 = UINT32_MAX;
	int64_t x608 = -1LL;

    t146 = (x605/((x606<=x607)!=x608));

    if (t146 != INT64_MAX) { NG(); } else { ; }
	
}

void f147(void) {
    	static int16_t x609 = 0;
	int32_t x610 = INT32_MIN;
	int16_t x611 = 2403;
	int16_t x612 = 171;
	int32_t t147 = 53;

    t147 = (x609/((x610<=x611)!=x612));

    if (t147 != 0) { NG(); } else { ; }
	
}

void f148(void) {
    	volatile int16_t x613 = -1;
	int16_t x614 = 417;
	int8_t x615 = -1;
	static uint16_t x616 = UINT16_MAX;
	volatile int32_t t148 = -6;

    t148 = (x613/((x614<=x615)!=x616));

    if (t148 != -1) { NG(); } else { ; }
	
}

void f149(void) {
    	int64_t x618 = INT64_MAX;
	uint64_t x619 = 1252599LLU;
	static uint32_t x620 = 2U;

    t149 = (x617/((x618<=x619)!=x620));

    if (t149 != INT64_MIN) { NG(); } else { ; }
	
}

void f150(void) {
    	uint64_t x621 = 27235469893178874LLU;
	int32_t x623 = INT32_MIN;
	static uint16_t x624 = UINT16_MAX;
	volatile uint64_t t150 = 7446094309796164909LLU;

    t150 = (x621/((x622<=x623)!=x624));

    if (t150 != 27235469893178874LLU) { NG(); } else { ; }
	
}

void f151(void) {
    	int32_t x625 = INT32_MAX;
	int64_t x626 = -1LL;
	volatile int8_t x627 = INT8_MIN;
	volatile int32_t t151 = INT32_MAX;

    t151 = (x625/((x626<=x627)!=x628));

    if (t151 != INT32_MAX) { NG(); } else { ; }
	
}

void f152(void) {
    	int32_t x629 = INT32_MAX;
	int8_t x630 = 1;
	int8_t x632 = 3;

    t152 = (x629/((x630<=x631)!=x632));

    if (t152 != INT32_MAX) { NG(); } else { ; }
	
}

void f153(void) {
    	static int32_t x633 = INT32_MAX;
	int64_t x634 = -1LL;
	int8_t x635 = -1;
	static int32_t x636 = INT32_MAX;
	static volatile int32_t t153 = INT32_MAX;

    t153 = (x633/((x634<=x635)!=x636));

    if (t153 != INT32_MAX) { NG(); } else { ; }
	
}

void f154(void) {
    	uint64_t x637 = UINT64_MAX;
	static volatile int16_t x638 = INT16_MIN;
	static uint16_t x640 = 11U;
	uint64_t t154 = UINT64_MAX;

    t154 = (x637/((x638<=x639)!=x640));

    if (t154 != UINT64_MAX) { NG(); } else { ; }
	
}

void f155(void) {
    	static volatile int8_t x641 = -1;
	uint32_t x642 = 17860U;
	uint64_t x643 = 8692107460724871LLU;
	volatile int32_t t155 = 1234;

    t155 = (x641/((x642<=x643)!=x644));

    if (t155 != -1) { NG(); } else { ; }
	
}

void f156(void) {
    	int32_t x645 = INT32_MIN;
	static int16_t x646 = INT16_MIN;
	uint16_t x647 = 472U;
	volatile int32_t t156 = INT32_MIN;

    t156 = (x645/((x646<=x647)!=x648));

    if (t156 != INT32_MIN) { NG(); } else { ; }
	
}

void f157(void) {
    	static volatile uint16_t x649 = 9U;
	int64_t x650 = INT64_MIN;
	volatile int64_t x651 = INT64_MIN;
	int16_t x652 = INT16_MAX;

    t157 = (x649/((x650<=x651)!=x652));

    if (t157 != 9) { NG(); } else { ; }
	
}

void f158(void) {
    	int8_t x653 = 19;
	uint32_t x654 = UINT32_MAX;
	int64_t x655 = 12976123646LL;
	int64_t x656 = 2568240556LL;

    t158 = (x653/((x654<=x655)!=x656));

    if (t158 != 19) { NG(); } else { ; }
	
}

void f159(void) {
    	uint64_t x657 = 9089630954LLU;
	int32_t x658 = 1017;
	uint16_t x659 = UINT16_MAX;
	int64_t x660 = -172696694165LL;

    t159 = (x657/((x658<=x659)!=x660));

    if (t159 != 9089630954LLU) { NG(); } else { ; }
	
}

void f160(void) {
    	uint32_t x661 = 1963949U;
	static uint16_t x662 = 70U;
	int64_t x663 = -214468091775336733LL;
	int64_t x664 = INT64_MIN;
	uint32_t t160 = 66197564U;

    t160 = (x661/((x662<=x663)!=x664));

    if (t160 != 1963949U) { NG(); } else { ; }
	
}

void f161(void) {
    	volatile int16_t x665 = INT16_MAX;
	uint32_t x666 = 179U;
	uint16_t x667 = 3U;
	int32_t t161 = -1;

    t161 = (x665/((x666<=x667)!=x668));

    if (t161 != 32767) { NG(); } else { ; }
	
}

void f162(void) {
    	volatile int32_t x669 = 10272;
	uint8_t x671 = 0U;
	static uint64_t x672 = 1507012855031252646LLU;
	static int32_t t162 = -1052;

    t162 = (x669/((x670<=x671)!=x672));

    if (t162 != 10272) { NG(); } else { ; }
	
}

void f163(void) {
    	static int32_t x673 = -31;
	static int8_t x674 = INT8_MIN;
	int8_t x675 = 1;
	int8_t x676 = -1;
	volatile int32_t t163 = 306979;

    t163 = (x673/((x674<=x675)!=x676));

    if (t163 != -31) { NG(); } else { ; }
	
}

void f164(void) {
    	volatile int32_t x677 = INT32_MAX;
	uint64_t x678 = 651735421409LLU;
	int32_t x679 = -13275;
	static volatile int32_t t164 = INT32_MAX;

    t164 = (x677/((x678<=x679)!=x680));

    if (t164 != INT32_MAX) { NG(); } else { ; }
	
}

void f165(void) {
    	static volatile int32_t x681 = INT32_MAX;
	int8_t x682 = -2;
	int32_t x684 = INT32_MAX;
	int32_t t165 = INT32_MAX;

    t165 = (x681/((x682<=x683)!=x684));

    if (t165 != INT32_MAX) { NG(); } else { ; }
	
}

void f166(void) {
    	int64_t x685 = INT64_MAX;
	int64_t x687 = 10415634832344LL;
	static volatile int64_t t166 = INT64_MAX;

    t166 = (x685/((x686<=x687)!=x688));

    if (t166 != INT64_MAX) { NG(); } else { ; }
	
}

void f167(void) {
    	int64_t x689 = INT64_MAX;
	uint64_t x690 = UINT64_MAX;
	uint8_t x691 = 4U;
	int32_t x692 = INT32_MAX;
	volatile int64_t t167 = INT64_MAX;

    t167 = (x689/((x690<=x691)!=x692));

    if (t167 != INT64_MAX) { NG(); } else { ; }
	
}

void f168(void) {
    	int64_t x694 = 3152810LL;
	int64_t x695 = INT64_MIN;
	static volatile int64_t x696 = -1LL;

    t168 = (x693/((x694<=x695)!=x696));

    if (t168 != -32768) { NG(); } else { ; }
	
}

void f169(void) {
    	uint16_t x697 = 277U;
	volatile int32_t x698 = INT32_MIN;
	int64_t x700 = INT64_MIN;

    t169 = (x697/((x698<=x699)!=x700));

    if (t169 != 277) { NG(); } else { ; }
	
}

void f170(void) {
    	int8_t x704 = INT8_MIN;
	volatile int32_t t170 = 5;

    t170 = (x701/((x702<=x703)!=x704));

    if (t170 != 6) { NG(); } else { ; }
	
}

void f171(void) {
    	int16_t x705 = 53;
	int16_t x708 = INT16_MAX;

    t171 = (x705/((x706<=x707)!=x708));

    if (t171 != 53) { NG(); } else { ; }
	
}

void f172(void) {
    	static volatile int16_t x709 = 2;
	uint16_t x710 = 3U;
	int32_t x711 = INT32_MIN;
	volatile uint8_t x712 = UINT8_MAX;
	volatile int32_t t172 = -17248806;

    t172 = (x709/((x710<=x711)!=x712));

    if (t172 != 2) { NG(); } else { ; }
	
}

void f173(void) {
    	uint64_t x715 = 88746LLU;
	int16_t x716 = INT16_MIN;
	uint32_t t173 = UINT32_MAX;

    t173 = (x713/((x714<=x715)!=x716));

    if (t173 != UINT32_MAX) { NG(); } else { ; }
	
}

void f174(void) {
    	static int16_t x717 = INT16_MIN;
	int16_t x718 = -1;
	int8_t x719 = -24;
	int64_t x720 = INT64_MIN;
	volatile int32_t t174 = 14931;

    t174 = (x717/((x718<=x719)!=x720));

    if (t174 != -32768) { NG(); } else { ; }
	
}

void f175(void) {
    	uint16_t x721 = 6U;
	int16_t x722 = INT16_MIN;
	volatile int32_t t175 = -1060816980;

    t175 = (x721/((x722<=x723)!=x724));

    if (t175 != 6) { NG(); } else { ; }
	
}

void f176(void) {
    	volatile int32_t x725 = 40;
	int64_t x726 = 19307231582001LL;
	volatile uint32_t x727 = 569411U;
	volatile int32_t x728 = -218882;
	volatile int32_t t176 = -2628;

    t176 = (x725/((x726<=x727)!=x728));

    if (t176 != 40) { NG(); } else { ; }
	
}

void f177(void) {
    	int8_t x729 = INT8_MIN;
	uint32_t x730 = 4U;
	volatile int64_t x731 = INT64_MAX;
	int16_t x732 = 0;
	volatile int32_t t177 = -5;

    t177 = (x729/((x730<=x731)!=x732));

    if (t177 != -128) { NG(); } else { ; }
	
}

void f178(void) {
    	int16_t x734 = INT16_MAX;
	int32_t x735 = 49;

    t178 = (x733/((x734<=x735)!=x736));

    if (t178 != -10118391) { NG(); } else { ; }
	
}

void f179(void) {
    	uint16_t x738 = UINT16_MAX;
	volatile int32_t x740 = INT32_MIN;
	int32_t t179 = 13705;

    t179 = (x737/((x738<=x739)!=x740));

    if (t179 != 1) { NG(); } else { ; }
	
}

void f180(void) {
    	static uint8_t x741 = 61U;
	static uint8_t x742 = 27U;
	uint64_t x743 = 169483322LLU;
	int32_t x744 = INT32_MIN;
	volatile int32_t t180 = -3696;

    t180 = (x741/((x742<=x743)!=x744));

    if (t180 != 61) { NG(); } else { ; }
	
}

void f181(void) {
    	static uint8_t x746 = 63U;
	uint32_t x747 = 1040099U;
	uint32_t x748 = 65735U;
	int32_t t181 = INT32_MAX;

    t181 = (x745/((x746<=x747)!=x748));

    if (t181 != INT32_MAX) { NG(); } else { ; }
	
}

void f182(void) {
    	int8_t x751 = INT8_MAX;
	static volatile int8_t x752 = INT8_MIN;
	volatile uint64_t t182 = UINT64_MAX;

    t182 = (x749/((x750<=x751)!=x752));

    if (t182 != UINT64_MAX) { NG(); } else { ; }
	
}

void f183(void) {
    	int8_t x753 = -3;
	uint32_t x754 = UINT32_MAX;
	uint8_t x755 = 3U;
	int8_t x756 = -13;

    t183 = (x753/((x754<=x755)!=x756));

    if (t183 != -3) { NG(); } else { ; }
	
}

void f184(void) {
    	volatile int32_t x757 = -919;
	int32_t x758 = INT32_MIN;
	int16_t x759 = INT16_MIN;
	uint16_t x760 = 496U;
	int32_t t184 = -47;

    t184 = (x757/((x758<=x759)!=x760));

    if (t184 != -919) { NG(); } else { ; }
	
}

void f185(void) {
    	int64_t x761 = INT64_MIN;
	static volatile int16_t x763 = INT16_MIN;
	volatile int64_t x764 = INT64_MAX;
	volatile int64_t t185 = INT64_MIN;

    t185 = (x761/((x762<=x763)!=x764));

    if (t185 != INT64_MIN) { NG(); } else { ; }
	
}

void f186(void) {
    	int16_t x765 = INT16_MIN;
	int8_t x766 = INT8_MIN;
	volatile int8_t x767 = INT8_MAX;
	uint64_t x768 = UINT64_MAX;
	int32_t t186 = 3109146;

    t186 = (x765/((x766<=x767)!=x768));

    if (t186 != -32768) { NG(); } else { ; }
	
}

void f187(void) {
    	int8_t x769 = 1;
	static int16_t x770 = INT16_MIN;
	uint8_t x771 = 1U;
	volatile int32_t t187 = -16205;

    t187 = (x769/((x770<=x771)!=x772));

    if (t187 != 1) { NG(); } else { ; }
	
}

void f188(void) {
    	uint64_t x773 = 2306LLU;
	int16_t x774 = -1;
	uint16_t x775 = UINT16_MAX;
	int8_t x776 = INT8_MAX;
	static uint64_t t188 = 108771718193260572LLU;

    t188 = (x773/((x774<=x775)!=x776));

    if (t188 != 2306LLU) { NG(); } else { ; }
	
}

void f189(void) {
    	static int32_t x777 = INT32_MIN;
	static int16_t x778 = 7281;
	uint32_t x779 = 859270829U;
	int64_t x780 = -1LL;
	int32_t t189 = INT32_MIN;

    t189 = (x777/((x778<=x779)!=x780));

    if (t189 != INT32_MIN) { NG(); } else { ; }
	
}

void f190(void) {
    	uint8_t x781 = 3U;
	static uint32_t x783 = 105142360U;
	uint16_t x784 = 4948U;
	int32_t t190 = 231191740;

    t190 = (x781/((x782<=x783)!=x784));

    if (t190 != 3) { NG(); } else { ; }
	
}

void f191(void) {
    	volatile int8_t x785 = INT8_MIN;
	int64_t x786 = INT64_MIN;
	int64_t x787 = INT64_MIN;
	volatile uint32_t x788 = 1841U;
	int32_t t191 = 90203;

    t191 = (x785/((x786<=x787)!=x788));

    if (t191 != -128) { NG(); } else { ; }
	
}

void f192(void) {
    	uint16_t x789 = UINT16_MAX;
	uint32_t x790 = UINT32_MAX;
	uint16_t x791 = UINT16_MAX;
	volatile int32_t t192 = -1815;

    t192 = (x789/((x790<=x791)!=x792));

    if (t192 != 65535) { NG(); } else { ; }
	
}

void f193(void) {
    	static volatile uint32_t x793 = 241402011U;
	uint32_t x794 = 193803302U;
	int32_t x795 = INT32_MAX;
	uint64_t x796 = 56117495623146LLU;
	uint32_t t193 = 11528U;

    t193 = (x793/((x794<=x795)!=x796));

    if (t193 != 241402011U) { NG(); } else { ; }
	
}

void f194(void) {
    	uint64_t x797 = 5LLU;
	volatile uint8_t x798 = 11U;
	int8_t x799 = INT8_MIN;
	int32_t x800 = INT32_MIN;
	uint64_t t194 = 141008191474309852LLU;

    t194 = (x797/((x798<=x799)!=x800));

    if (t194 != 5LLU) { NG(); } else { ; }
	
}

void f195(void) {
    	int16_t x801 = INT16_MIN;
	int64_t x803 = -1LL;
	volatile uint16_t x804 = 300U;
	volatile int32_t t195 = -28014;

    t195 = (x801/((x802<=x803)!=x804));

    if (t195 != -32768) { NG(); } else { ; }
	
}

void f196(void) {
    	uint32_t x805 = 3599U;
	volatile int32_t x806 = INT32_MAX;
	int16_t x807 = 82;
	static volatile uint32_t t196 = 574768557U;

    t196 = (x805/((x806<=x807)!=x808));

    if (t196 != 3599U) { NG(); } else { ; }
	
}

void f197(void) {
    	int64_t x809 = -1LL;
	uint16_t x810 = 8784U;
	uint64_t x811 = 20523LLU;
	volatile int64_t t197 = -791494732270LL;

    t197 = (x809/((x810<=x811)!=x812));

    if (t197 != -1LL) { NG(); } else { ; }
	
}

void f198(void) {
    	static int32_t x813 = INT32_MIN;
	uint32_t x815 = 24410876U;
	uint32_t x816 = 2U;
	volatile int32_t t198 = INT32_MIN;

    t198 = (x813/((x814<=x815)!=x816));

    if (t198 != INT32_MIN) { NG(); } else { ; }
	
}

void f199(void) {
    	volatile int32_t x819 = 10768114;
	int64_t x820 = -1LL;
	int64_t t199 = -12274LL;

    t199 = (x817/((x818<=x819)!=x820));

    if (t199 != 70827574301830874LL) { NG(); } else { ; }
	
}

int main(void) {
        f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();
    f100();
    f101();
    f102();
    f103();
    f104();
    f105();
    f106();
    f107();
    f108();
    f109();
    f110();
    f111();
    f112();
    f113();
    f114();
    f115();
    f116();
    f117();
    f118();
    f119();
    f120();
    f121();
    f122();
    f123();
    f124();
    f125();
    f126();
    f127();
    f128();
    f129();
    f130();
    f131();
    f132();
    f133();
    f134();
    f135();
    f136();
    f137();
    f138();
    f139();
    f140();
    f141();
    f142();
    f143();
    f144();
    f145();
    f146();
    f147();
    f148();
    f149();
    f150();
    f151();
    f152();
    f153();
    f154();
    f155();
    f156();
    f157();
    f158();
    f159();
    f160();
    f161();
    f162();
    f163();
    f164();
    f165();
    f166();
    f167();
    f168();
    f169();
    f170();
    f171();
    f172();
    f173();
    f174();
    f175();
    f176();
    f177();
    f178();
    f179();
    f180();
    f181();
    f182();
    f183();
    f184();
    f185();
    f186();
    f187();
    f188();
    f189();
    f190();
    f191();
    f192();
    f193();
    f194();
    f195();
    f196();
    f197();
    f198();
    f199();


    return 0;
}

