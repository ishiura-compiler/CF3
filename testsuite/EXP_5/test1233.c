
/*
CF3

Copyright (c) 2015 ishiura-lab.

Released under the MIT license.  
https://github.com/ishiura-compiler/CF3/MIT-LICENSE.md
*/

#include<stdio.h>
#include<stdint.h>
#include<stdlib.h>
#include"test1.h"

uint16_t x3 = 0U;
volatile int32_t t4 = 2078403;
int8_t x27 = -6;
int32_t x31 = INT32_MIN;
int64_t x32 = -1LL;
int64_t t6 = 61783367268783737LL;
static int8_t x35 = INT8_MIN;
int16_t x41 = INT16_MIN;
int8_t x42 = INT8_MIN;
int32_t t9 = 12256;
int32_t x49 = INT32_MIN;
volatile uint16_t x56 = 45U;
static int64_t t14 = -5LL;
int32_t x67 = -1;
uint32_t x71 = 580808006U;
volatile uint8_t x72 = 80U;
int32_t t16 = 228026;
int16_t x74 = INT16_MIN;
int64_t x77 = 10904557014LL;
volatile int32_t t21 = 5;
int64_t x103 = INT64_MAX;
int16_t x110 = -1;
int32_t x111 = INT32_MIN;
volatile int32_t t26 = 5;
int64_t x115 = INT64_MAX;
static int8_t x117 = INT8_MIN;
static int64_t x123 = -308LL;
volatile int64_t t30 = 1688027955780LL;
int64_t t31 = -45LL;
int16_t x133 = INT16_MIN;
volatile int8_t x134 = -1;
volatile int32_t t32 = 24453687;
static int64_t x142 = INT64_MIN;
uint64_t x145 = UINT64_MAX;
int8_t x154 = INT8_MAX;
int32_t x159 = INT32_MIN;
volatile int32_t x160 = INT32_MIN;
int64_t x162 = -1LL;
int8_t x163 = 15;
static volatile int16_t x171 = INT16_MAX;
int32_t t41 = -152;
static int16_t x176 = 3389;
int32_t t42 = 6416;
int16_t x188 = INT16_MIN;
int32_t t45 = 786794;
int8_t x193 = INT8_MIN;
int32_t t46 = -523228968;
static int32_t x199 = INT32_MIN;
int8_t x201 = INT8_MIN;
volatile int32_t x202 = INT32_MIN;
int64_t t49 = 29891LL;
volatile int32_t x210 = INT32_MAX;
int32_t x217 = -118466;
uint64_t x225 = 382LLU;
volatile uint64_t t54 = 1308201541LLU;
int64_t x238 = -13848218410368LL;
volatile uint64_t x244 = UINT64_MAX;
int64_t x250 = -10972LL;
static volatile int32_t x258 = INT32_MAX;
uint16_t x260 = 31U;
int64_t x264 = INT64_MIN;
uint64_t x269 = 21753793899LLU;
static int64_t x270 = 381458504142LL;
volatile uint64_t t64 = 12106LLU;
int32_t x280 = -109531;
uint16_t x282 = 2U;
int64_t x283 = -1LL;
uint32_t x285 = 59U;
uint64_t x286 = UINT64_MAX;
uint32_t t69 = 106694U;
int8_t x295 = INT8_MIN;
static int8_t x297 = -4;
static int32_t x312 = INT32_MIN;
volatile uint8_t x315 = 44U;
static int32_t x318 = -1;
static int64_t x321 = INT64_MIN;
int64_t x330 = INT64_MIN;
int8_t x333 = INT8_MIN;
int32_t x335 = INT32_MAX;
static int16_t x340 = -2;
int8_t x346 = INT8_MIN;
int8_t x347 = INT8_MAX;
int64_t t85 = -41265975LL;
static uint8_t x358 = 1U;
int64_t x360 = -208646993019000LL;
static int16_t x365 = INT16_MIN;
int8_t x368 = -1;
uint8_t x369 = UINT8_MAX;
int8_t x377 = -1;
volatile int64_t x380 = -821873382978LL;
int64_t x391 = -267LL;
static uint64_t t94 = 19LLU;
volatile int8_t x405 = INT8_MIN;
int8_t x424 = -12;
uint16_t x434 = UINT16_MAX;
uint16_t x439 = 1U;
uint64_t t105 = 411052106LLU;
int16_t x443 = -4012;
int16_t x445 = -10462;
static int8_t x447 = INT8_MAX;
int8_t x452 = INT8_MIN;
volatile int32_t t108 = 1;
static uint8_t x457 = UINT8_MAX;
int16_t x460 = INT16_MIN;
int16_t x461 = -1;
volatile int64_t x463 = INT64_MIN;
int16_t x470 = INT16_MAX;
volatile int32_t x474 = INT32_MIN;
int8_t x475 = 11;
volatile int16_t x477 = -1783;
uint64_t x479 = UINT64_MAX;
volatile int32_t x492 = INT32_MAX;
int16_t x507 = -6349;
int16_t x512 = -1;
int64_t t122 = -2122436189990579LL;
static int32_t t123 = 20619;
int64_t x529 = -96111545772LL;
uint32_t x531 = UINT32_MAX;
volatile uint32_t x532 = 191607669U;
int16_t x536 = INT16_MAX;
int16_t x537 = INT16_MAX;
volatile uint8_t x550 = 9U;
static uint16_t x552 = UINT16_MAX;
volatile int16_t x553 = INT16_MAX;
static int8_t x559 = INT8_MIN;
static int32_t x562 = -1;
uint64_t x566 = 321887643873LLU;
int8_t x570 = INT8_MIN;
static uint8_t x571 = 0U;
int64_t x572 = INT64_MIN;
int16_t x578 = INT16_MAX;
int16_t x581 = -1;
volatile int32_t t136 = 259;
uint32_t x593 = UINT32_MAX;
static uint16_t x604 = 2U;
int8_t x606 = INT8_MAX;
volatile uint32_t t142 = 15977281U;
uint16_t x614 = UINT16_MAX;
volatile uint16_t x618 = 31U;
int16_t x626 = INT16_MAX;
int8_t x628 = -5;
static uint8_t x643 = UINT8_MAX;
int64_t x647 = -1694331934786407533LL;
int32_t x649 = -7230799;
int16_t x651 = INT16_MIN;
int32_t t151 = -17;
uint16_t x659 = 179U;
static int8_t x662 = -42;
int64_t x673 = INT64_MAX;
uint8_t x675 = 1U;
volatile int32_t t161 = 468;
volatile int8_t x696 = -1;
int64_t x702 = INT64_MIN;
int16_t x704 = -1;
int16_t x706 = INT16_MIN;
uint16_t x709 = 7925U;
static int32_t x718 = -1;
volatile uint32_t t168 = 14234U;
int16_t x732 = -40;
int64_t x740 = 114060963991LL;
static volatile int64_t t173 = -20LL;
uint32_t x742 = 1668U;
volatile uint32_t x745 = 740U;
int16_t x746 = INT16_MAX;
volatile int64_t t176 = -226531LL;
uint8_t x770 = UINT8_MAX;
uint64_t x775 = UINT64_MAX;
volatile int8_t x776 = -5;
int32_t t180 = -2;
volatile int64_t t181 = -432244747027108LL;
uint16_t x781 = 57U;
int64_t x782 = -1LL;
uint32_t x783 = 29197U;
volatile int16_t x784 = -2;
uint32_t x790 = 2977U;
uint64_t x802 = 1344415336LLU;
volatile uint64_t x804 = 17338806596674237LLU;
uint64_t x806 = 401244LLU;
int8_t x816 = 8;
static int32_t x817 = INT32_MIN;
static volatile int8_t x818 = INT8_MIN;
volatile int32_t x820 = -794316;
int64_t x824 = -1LL;
volatile uint32_t x828 = 48257504U;
int32_t x833 = -1;
uint32_t x844 = UINT32_MAX;
uint32_t t197 = 2U;
volatile int32_t t198 = -59;
int32_t x859 = INT32_MAX;


void f0(void) {
    	int16_t x1 = INT16_MAX;
	volatile int8_t x2 = INT8_MAX;
	int64_t x4 = INT64_MIN;
	static volatile int64_t t0 = 11513782266LL;

    t0 = (x1+((x2!=x3)%x4));

    if (t0 != 32768LL) { NG(); } else { ; }
	
}

void f1(void) {
    	int8_t x5 = INT8_MAX;
	static int8_t x6 = -1;
	volatile uint16_t x7 = UINT16_MAX;
	volatile int32_t x8 = INT32_MAX;
	volatile int32_t t1 = 1927;

    t1 = (x5+((x6!=x7)%x8));

    if (t1 != 128) { NG(); } else { ; }
	
}

void f2(void) {
    	int16_t x9 = -1888;
	uint32_t x10 = 12587U;
	int8_t x11 = -7;
	uint64_t x12 = 5882471720810LLU;
	uint64_t t2 = 56739LLU;

    t2 = (x9+((x10!=x11)%x12));

    if (t2 != 18446744073709549729LLU) { NG(); } else { ; }
	
}

void f3(void) {
    	volatile int32_t x13 = INT32_MAX;
	int8_t x14 = -1;
	int32_t x15 = 19;
	int64_t x16 = INT64_MAX;
	int64_t t3 = -1701575LL;

    t3 = (x13+((x14!=x15)%x16));

    if (t3 != 2147483648LL) { NG(); } else { ; }
	
}

void f4(void) {
    	static uint8_t x17 = UINT8_MAX;
	int64_t x18 = INT64_MIN;
	int32_t x19 = INT32_MAX;
	static int16_t x20 = 3;

    t4 = (x17+((x18!=x19)%x20));

    if (t4 != 256) { NG(); } else { ; }
	
}

void f5(void) {
    	volatile int64_t x25 = -1LL;
	volatile int16_t x26 = INT16_MAX;
	int64_t x28 = INT64_MAX;
	volatile int64_t t5 = -7994572LL;

    t5 = (x25+((x26!=x27)%x28));

    if (t5 != 0LL) { NG(); } else { ; }
	
}

void f6(void) {
    	int8_t x29 = INT8_MIN;
	int64_t x30 = INT64_MIN;

    t6 = (x29+((x30!=x31)%x32));

    if (t6 != -128LL) { NG(); } else { ; }
	
}

void f7(void) {
    	volatile uint32_t x33 = UINT32_MAX;
	int32_t x34 = INT32_MAX;
	volatile int32_t x36 = INT32_MIN;
	static volatile uint32_t t7 = 1U;

    t7 = (x33+((x34!=x35)%x36));

    if (t7 != 0U) { NG(); } else { ; }
	
}

void f8(void) {
    	int8_t x37 = INT8_MIN;
	volatile int16_t x38 = INT16_MIN;
	uint64_t x39 = 1011511LLU;
	int8_t x40 = INT8_MIN;
	int32_t t8 = 0;

    t8 = (x37+((x38!=x39)%x40));

    if (t8 != -127) { NG(); } else { ; }
	
}

void f9(void) {
    	volatile int64_t x43 = 3LL;
	static volatile int32_t x44 = -1;

    t9 = (x41+((x42!=x43)%x44));

    if (t9 != -32768) { NG(); } else { ; }
	
}

void f10(void) {
    	int64_t x45 = -106256LL;
	volatile int64_t x46 = INT64_MAX;
	static uint16_t x47 = 78U;
	volatile int32_t x48 = INT32_MAX;
	volatile int64_t t10 = 5369809990090510LL;

    t10 = (x45+((x46!=x47)%x48));

    if (t10 != -106255LL) { NG(); } else { ; }
	
}

void f11(void) {
    	uint64_t x50 = UINT64_MAX;
	uint16_t x51 = UINT16_MAX;
	int16_t x52 = -1;
	volatile int32_t t11 = INT32_MIN;

    t11 = (x49+((x50!=x51)%x52));

    if (t11 != INT32_MIN) { NG(); } else { ; }
	
}

void f12(void) {
    	uint64_t x53 = 68973682198934372LLU;
	static volatile uint16_t x54 = 13U;
	int8_t x55 = 1;
	volatile uint64_t t12 = 0LLU;

    t12 = (x53+((x54!=x55)%x56));

    if (t12 != 68973682198934373LLU) { NG(); } else { ; }
	
}

void f13(void) {
    	uint16_t x57 = UINT16_MAX;
	int32_t x58 = INT32_MAX;
	int16_t x59 = INT16_MIN;
	static volatile int8_t x60 = INT8_MAX;
	volatile int32_t t13 = -220219;

    t13 = (x57+((x58!=x59)%x60));

    if (t13 != 65536) { NG(); } else { ; }
	
}

void f14(void) {
    	int32_t x61 = INT32_MAX;
	int64_t x62 = INT64_MAX;
	volatile uint8_t x63 = 14U;
	int64_t x64 = -158554220111LL;

    t14 = (x61+((x62!=x63)%x64));

    if (t14 != 2147483648LL) { NG(); } else { ; }
	
}

void f15(void) {
    	int8_t x65 = INT8_MAX;
	static uint8_t x66 = UINT8_MAX;
	int8_t x68 = INT8_MIN;
	static volatile int32_t t15 = -81932;

    t15 = (x65+((x66!=x67)%x68));

    if (t15 != 128) { NG(); } else { ; }
	
}

void f16(void) {
    	uint8_t x69 = 1U;
	int32_t x70 = 158886;

    t16 = (x69+((x70!=x71)%x72));

    if (t16 != 2) { NG(); } else { ; }
	
}

void f17(void) {
    	volatile int8_t x73 = -25;
	static uint16_t x75 = 324U;
	int8_t x76 = INT8_MIN;
	volatile int32_t t17 = 10539866;

    t17 = (x73+((x74!=x75)%x76));

    if (t17 != -24) { NG(); } else { ; }
	
}

void f18(void) {
    	uint8_t x78 = UINT8_MAX;
	int32_t x79 = -59828808;
	int64_t x80 = -1LL;
	int64_t t18 = 16839526635388155LL;

    t18 = (x77+((x78!=x79)%x80));

    if (t18 != 10904557014LL) { NG(); } else { ; }
	
}

void f19(void) {
    	static int8_t x81 = INT8_MIN;
	int16_t x82 = INT16_MIN;
	static volatile uint8_t x83 = UINT8_MAX;
	int16_t x84 = INT16_MAX;
	int32_t t19 = -50;

    t19 = (x81+((x82!=x83)%x84));

    if (t19 != -127) { NG(); } else { ; }
	
}

void f20(void) {
    	int32_t x85 = 50752036;
	int64_t x86 = -132409903430LL;
	uint64_t x87 = 4374513253953557LLU;
	static uint32_t x88 = 81U;
	volatile uint32_t t20 = 3365649U;

    t20 = (x85+((x86!=x87)%x88));

    if (t20 != 50752037U) { NG(); } else { ; }
	
}

void f21(void) {
    	volatile int8_t x89 = 0;
	int32_t x90 = 32943345;
	static uint64_t x91 = 2422948711811500469LLU;
	int32_t x92 = -183769913;

    t21 = (x89+((x90!=x91)%x92));

    if (t21 != 1) { NG(); } else { ; }
	
}

void f22(void) {
    	int8_t x93 = INT8_MIN;
	volatile int8_t x94 = -1;
	volatile uint16_t x95 = 1618U;
	int16_t x96 = INT16_MAX;
	int32_t t22 = -15996;

    t22 = (x93+((x94!=x95)%x96));

    if (t22 != -127) { NG(); } else { ; }
	
}

void f23(void) {
    	volatile int8_t x97 = INT8_MIN;
	uint8_t x98 = 0U;
	int16_t x99 = -1;
	uint64_t x100 = 154134LLU;
	uint64_t t23 = 949879858265963LLU;

    t23 = (x97+((x98!=x99)%x100));

    if (t23 != 18446744073709551489LLU) { NG(); } else { ; }
	
}

void f24(void) {
    	uint8_t x101 = UINT8_MAX;
	int8_t x102 = INT8_MAX;
	int32_t x104 = -1384541;
	int32_t t24 = 1903114;

    t24 = (x101+((x102!=x103)%x104));

    if (t24 != 256) { NG(); } else { ; }
	
}

void f25(void) {
    	int8_t x105 = INT8_MIN;
	int8_t x106 = -1;
	volatile int64_t x107 = INT64_MAX;
	static volatile int64_t x108 = INT64_MIN;
	volatile int64_t t25 = 7694893462043631LL;

    t25 = (x105+((x106!=x107)%x108));

    if (t25 != -127LL) { NG(); } else { ; }
	
}

void f26(void) {
    	uint8_t x109 = 2U;
	uint8_t x112 = UINT8_MAX;

    t26 = (x109+((x110!=x111)%x112));

    if (t26 != 3) { NG(); } else { ; }
	
}

void f27(void) {
    	uint8_t x113 = 42U;
	int16_t x114 = -1;
	volatile int8_t x116 = 1;
	static int32_t t27 = -4091775;

    t27 = (x113+((x114!=x115)%x116));

    if (t27 != 42) { NG(); } else { ; }
	
}

void f28(void) {
    	static int32_t x118 = INT32_MAX;
	volatile int64_t x119 = -12LL;
	int8_t x120 = -2;
	static int32_t t28 = 23015945;

    t28 = (x117+((x118!=x119)%x120));

    if (t28 != -127) { NG(); } else { ; }
	
}

void f29(void) {
    	int16_t x121 = -1;
	volatile int8_t x122 = INT8_MIN;
	int32_t x124 = INT32_MIN;
	volatile int32_t t29 = -558060;

    t29 = (x121+((x122!=x123)%x124));

    if (t29 != 0) { NG(); } else { ; }
	
}

void f30(void) {
    	static int64_t x125 = INT64_MIN;
	uint8_t x126 = 0U;
	int64_t x127 = 1889742415LL;
	volatile int64_t x128 = INT64_MIN;

    t30 = (x125+((x126!=x127)%x128));

    if (t30 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f31(void) {
    	int16_t x129 = -1387;
	int8_t x130 = INT8_MIN;
	int8_t x131 = INT8_MAX;
	static int64_t x132 = 24957939386909849LL;

    t31 = (x129+((x130!=x131)%x132));

    if (t31 != -1386LL) { NG(); } else { ; }
	
}

void f32(void) {
    	int16_t x135 = INT16_MIN;
	int16_t x136 = INT16_MAX;

    t32 = (x133+((x134!=x135)%x136));

    if (t32 != -32767) { NG(); } else { ; }
	
}

void f33(void) {
    	static uint64_t x137 = UINT64_MAX;
	int32_t x138 = 212;
	static uint16_t x139 = UINT16_MAX;
	uint32_t x140 = UINT32_MAX;
	volatile uint64_t t33 = 5697420106701942801LLU;

    t33 = (x137+((x138!=x139)%x140));

    if (t33 != 0LLU) { NG(); } else { ; }
	
}

void f34(void) {
    	int8_t x141 = INT8_MAX;
	uint16_t x143 = 8U;
	int64_t x144 = -1LL;
	volatile int64_t t34 = 1918374926LL;

    t34 = (x141+((x142!=x143)%x144));

    if (t34 != 127LL) { NG(); } else { ; }
	
}

void f35(void) {
    	int16_t x146 = 1818;
	volatile int32_t x147 = INT32_MIN;
	uint32_t x148 = 48676520U;
	uint64_t t35 = 109225323212LLU;

    t35 = (x145+((x146!=x147)%x148));

    if (t35 != 0LLU) { NG(); } else { ; }
	
}

void f36(void) {
    	int16_t x149 = 2940;
	uint8_t x150 = 5U;
	static uint64_t x151 = 2016760185LLU;
	int32_t x152 = -57;
	int32_t t36 = 59;

    t36 = (x149+((x150!=x151)%x152));

    if (t36 != 2941) { NG(); } else { ; }
	
}

void f37(void) {
    	int16_t x153 = -53;
	static uint64_t x155 = 4LLU;
	static int32_t x156 = INT32_MAX;
	int32_t t37 = 844;

    t37 = (x153+((x154!=x155)%x156));

    if (t37 != -52) { NG(); } else { ; }
	
}

void f38(void) {
    	static int16_t x157 = INT16_MAX;
	static volatile int32_t x158 = 38;
	volatile int32_t t38 = -364950;

    t38 = (x157+((x158!=x159)%x160));

    if (t38 != 32768) { NG(); } else { ; }
	
}

void f39(void) {
    	int8_t x161 = INT8_MIN;
	int16_t x164 = INT16_MAX;
	int32_t t39 = -354;

    t39 = (x161+((x162!=x163)%x164));

    if (t39 != -127) { NG(); } else { ; }
	
}

void f40(void) {
    	static uint8_t x165 = UINT8_MAX;
	int8_t x166 = INT8_MIN;
	uint32_t x167 = UINT32_MAX;
	int8_t x168 = -1;
	volatile int32_t t40 = 71893;

    t40 = (x165+((x166!=x167)%x168));

    if (t40 != 255) { NG(); } else { ; }
	
}

void f41(void) {
    	int32_t x169 = 39248348;
	uint64_t x170 = UINT64_MAX;
	int16_t x172 = -1;

    t41 = (x169+((x170!=x171)%x172));

    if (t41 != 39248348) { NG(); } else { ; }
	
}

void f42(void) {
    	volatile int16_t x173 = -8037;
	int8_t x174 = -57;
	uint16_t x175 = 1U;

    t42 = (x173+((x174!=x175)%x176));

    if (t42 != -8036) { NG(); } else { ; }
	
}

void f43(void) {
    	int32_t x177 = INT32_MAX;
	static volatile int16_t x178 = INT16_MIN;
	volatile uint64_t x179 = 382731540924167LLU;
	static int64_t x180 = INT64_MIN;
	volatile int64_t t43 = 51858900699LL;

    t43 = (x177+((x178!=x179)%x180));

    if (t43 != 2147483648LL) { NG(); } else { ; }
	
}

void f44(void) {
    	uint8_t x181 = 1U;
	static uint16_t x182 = UINT16_MAX;
	int32_t x183 = -721812364;
	static int32_t x184 = INT32_MIN;
	int32_t t44 = 3243944;

    t44 = (x181+((x182!=x183)%x184));

    if (t44 != 2) { NG(); } else { ; }
	
}

void f45(void) {
    	int8_t x185 = INT8_MIN;
	volatile int16_t x186 = -2;
	int8_t x187 = -31;

    t45 = (x185+((x186!=x187)%x188));

    if (t45 != -127) { NG(); } else { ; }
	
}

void f46(void) {
    	int64_t x194 = -557948LL;
	int16_t x195 = INT16_MIN;
	static volatile uint8_t x196 = 19U;

    t46 = (x193+((x194!=x195)%x196));

    if (t46 != -127) { NG(); } else { ; }
	
}

void f47(void) {
    	int32_t x197 = INT32_MIN;
	uint16_t x198 = UINT16_MAX;
	uint64_t x200 = 5421217602597LLU;
	volatile uint64_t t47 = 1459LLU;

    t47 = (x197+((x198!=x199)%x200));

    if (t47 != 18446744071562067969LLU) { NG(); } else { ; }
	
}

void f48(void) {
    	volatile uint32_t x203 = 190883U;
	uint8_t x204 = 15U;
	int32_t t48 = 1139583;

    t48 = (x201+((x202!=x203)%x204));

    if (t48 != -127) { NG(); } else { ; }
	
}

void f49(void) {
    	volatile uint16_t x205 = 1527U;
	int32_t x206 = INT32_MIN;
	static int64_t x207 = INT64_MAX;
	int64_t x208 = INT64_MIN;

    t49 = (x205+((x206!=x207)%x208));

    if (t49 != 1528LL) { NG(); } else { ; }
	
}

void f50(void) {
    	uint16_t x209 = 29U;
	uint16_t x211 = 12958U;
	int64_t x212 = INT64_MIN;
	static volatile int64_t t50 = 3991900547817064LL;

    t50 = (x209+((x210!=x211)%x212));

    if (t50 != 30LL) { NG(); } else { ; }
	
}

void f51(void) {
    	uint64_t x213 = 88667252LLU;
	int8_t x214 = 6;
	uint64_t x215 = UINT64_MAX;
	static volatile uint64_t x216 = 36557LLU;
	volatile uint64_t t51 = 487118901132LLU;

    t51 = (x213+((x214!=x215)%x216));

    if (t51 != 88667253LLU) { NG(); } else { ; }
	
}

void f52(void) {
    	uint32_t x218 = 2642071U;
	volatile int64_t x219 = INT64_MIN;
	int64_t x220 = -1283952196653925395LL;
	static volatile int64_t t52 = -637863873184463694LL;

    t52 = (x217+((x218!=x219)%x220));

    if (t52 != -118465LL) { NG(); } else { ; }
	
}

void f53(void) {
    	static uint8_t x221 = 0U;
	int16_t x222 = INT16_MIN;
	int16_t x223 = INT16_MIN;
	int16_t x224 = -1;
	static int32_t t53 = 2;

    t53 = (x221+((x222!=x223)%x224));

    if (t53 != 0) { NG(); } else { ; }
	
}

void f54(void) {
    	int16_t x226 = 3223;
	int8_t x227 = INT8_MIN;
	static int8_t x228 = -1;

    t54 = (x225+((x226!=x227)%x228));

    if (t54 != 382LLU) { NG(); } else { ; }
	
}

void f55(void) {
    	static int32_t x229 = INT32_MIN;
	int8_t x230 = -1;
	int16_t x231 = 1;
	uint64_t x232 = 16353909022LLU;
	uint64_t t55 = 28701033067156451LLU;

    t55 = (x229+((x230!=x231)%x232));

    if (t55 != 18446744071562067969LLU) { NG(); } else { ; }
	
}

void f56(void) {
    	int32_t x233 = INT32_MIN;
	int16_t x234 = -1;
	volatile uint8_t x235 = 2U;
	int16_t x236 = INT16_MAX;
	volatile int32_t t56 = -130621557;

    t56 = (x233+((x234!=x235)%x236));

    if (t56 != -2147483647) { NG(); } else { ; }
	
}

void f57(void) {
    	static int64_t x237 = INT64_MIN;
	static uint64_t x239 = UINT64_MAX;
	int8_t x240 = INT8_MIN;
	int64_t t57 = -4LL;

    t57 = (x237+((x238!=x239)%x240));

    if (t57 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f58(void) {
    	static int64_t x241 = INT64_MIN;
	static volatile int32_t x242 = INT32_MIN;
	uint16_t x243 = 10628U;
	uint64_t t58 = 62264LLU;

    t58 = (x241+((x242!=x243)%x244));

    if (t58 != 9223372036854775809LLU) { NG(); } else { ; }
	
}

void f59(void) {
    	uint32_t x245 = 196U;
	int32_t x246 = -286958;
	uint16_t x247 = 13727U;
	uint16_t x248 = 1U;
	volatile uint32_t t59 = 14U;

    t59 = (x245+((x246!=x247)%x248));

    if (t59 != 196U) { NG(); } else { ; }
	
}

void f60(void) {
    	static int16_t x249 = -1;
	uint32_t x251 = 2U;
	int16_t x252 = 1;
	int32_t t60 = 125;

    t60 = (x249+((x250!=x251)%x252));

    if (t60 != -1) { NG(); } else { ; }
	
}

void f61(void) {
    	int32_t x257 = 0;
	static int16_t x259 = INT16_MIN;
	int32_t t61 = -1;

    t61 = (x257+((x258!=x259)%x260));

    if (t61 != 1) { NG(); } else { ; }
	
}

void f62(void) {
    	static int64_t x261 = INT64_MIN;
	static volatile int8_t x262 = -1;
	uint8_t x263 = 4U;
	int64_t t62 = 58798686LL;

    t62 = (x261+((x262!=x263)%x264));

    if (t62 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f63(void) {
    	int16_t x265 = -6935;
	volatile int64_t x266 = -1LL;
	int8_t x267 = -1;
	static uint8_t x268 = 9U;
	int32_t t63 = -62;

    t63 = (x265+((x266!=x267)%x268));

    if (t63 != -6935) { NG(); } else { ; }
	
}

void f64(void) {
    	int64_t x271 = -1LL;
	volatile int8_t x272 = -2;

    t64 = (x269+((x270!=x271)%x272));

    if (t64 != 21753793900LLU) { NG(); } else { ; }
	
}

void f65(void) {
    	static volatile int32_t x273 = 5064132;
	int32_t x274 = INT32_MIN;
	int16_t x275 = -1;
	int16_t x276 = 3;
	int32_t t65 = 15379;

    t65 = (x273+((x274!=x275)%x276));

    if (t65 != 5064133) { NG(); } else { ; }
	
}

void f66(void) {
    	int32_t x277 = -1;
	uint16_t x278 = UINT16_MAX;
	int8_t x279 = INT8_MIN;
	static int32_t t66 = -6;

    t66 = (x277+((x278!=x279)%x280));

    if (t66 != 0) { NG(); } else { ; }
	
}

void f67(void) {
    	static volatile uint32_t x281 = 433556865U;
	volatile uint64_t x284 = 345627410592838LLU;
	uint64_t t67 = 1426LLU;

    t67 = (x281+((x282!=x283)%x284));

    if (t67 != 433556866LLU) { NG(); } else { ; }
	
}

void f68(void) {
    	uint16_t x287 = UINT16_MAX;
	uint32_t x288 = 4365463U;
	uint32_t t68 = 1507U;

    t68 = (x285+((x286!=x287)%x288));

    if (t68 != 60U) { NG(); } else { ; }
	
}

void f69(void) {
    	static uint32_t x289 = 13355843U;
	volatile int64_t x290 = -20748240839294LL;
	uint64_t x291 = UINT64_MAX;
	int16_t x292 = -63;

    t69 = (x289+((x290!=x291)%x292));

    if (t69 != 13355844U) { NG(); } else { ; }
	
}

void f70(void) {
    	volatile int16_t x293 = -1;
	int64_t x294 = -1LL;
	int16_t x296 = -1;
	volatile int32_t t70 = -1107;

    t70 = (x293+((x294!=x295)%x296));

    if (t70 != -1) { NG(); } else { ; }
	
}

void f71(void) {
    	int32_t x298 = INT32_MIN;
	int16_t x299 = INT16_MAX;
	int64_t x300 = -22154523LL;
	int64_t t71 = -241LL;

    t71 = (x297+((x298!=x299)%x300));

    if (t71 != -3LL) { NG(); } else { ; }
	
}

void f72(void) {
    	uint64_t x301 = 943LLU;
	static uint16_t x302 = 504U;
	int64_t x303 = INT64_MIN;
	uint32_t x304 = 524929007U;
	volatile uint64_t t72 = 6205946217639LLU;

    t72 = (x301+((x302!=x303)%x304));

    if (t72 != 944LLU) { NG(); } else { ; }
	
}

void f73(void) {
    	uint16_t x305 = UINT16_MAX;
	volatile int8_t x306 = INT8_MAX;
	static int8_t x307 = -1;
	static int16_t x308 = 1;
	volatile int32_t t73 = -2058869;

    t73 = (x305+((x306!=x307)%x308));

    if (t73 != 65535) { NG(); } else { ; }
	
}

void f74(void) {
    	int8_t x309 = -1;
	int16_t x310 = INT16_MIN;
	uint16_t x311 = 1251U;
	static volatile int32_t t74 = -1603;

    t74 = (x309+((x310!=x311)%x312));

    if (t74 != 0) { NG(); } else { ; }
	
}

void f75(void) {
    	int64_t x313 = INT64_MIN;
	uint16_t x314 = 2U;
	int8_t x316 = 3;
	static volatile int64_t t75 = -459865715LL;

    t75 = (x313+((x314!=x315)%x316));

    if (t75 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f76(void) {
    	static int32_t x317 = INT32_MIN;
	volatile uint32_t x319 = 428U;
	uint32_t x320 = UINT32_MAX;
	uint32_t t76 = 566701506U;

    t76 = (x317+((x318!=x319)%x320));

    if (t76 != 2147483649U) { NG(); } else { ; }
	
}

void f77(void) {
    	uint16_t x322 = UINT16_MAX;
	uint64_t x323 = 516873LLU;
	volatile int32_t x324 = 433;
	int64_t t77 = 208LL;

    t77 = (x321+((x322!=x323)%x324));

    if (t77 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f78(void) {
    	uint8_t x325 = UINT8_MAX;
	int16_t x326 = INT16_MIN;
	volatile int16_t x327 = -1;
	int64_t x328 = INT64_MIN;
	volatile int64_t t78 = 13154840253239LL;

    t78 = (x325+((x326!=x327)%x328));

    if (t78 != 256LL) { NG(); } else { ; }
	
}

void f79(void) {
    	uint32_t x329 = 799042U;
	volatile uint16_t x331 = UINT16_MAX;
	int64_t x332 = -5933LL;
	int64_t t79 = -2781LL;

    t79 = (x329+((x330!=x331)%x332));

    if (t79 != 799043LL) { NG(); } else { ; }
	
}

void f80(void) {
    	uint8_t x334 = 90U;
	static uint8_t x336 = UINT8_MAX;
	static int32_t t80 = -515;

    t80 = (x333+((x334!=x335)%x336));

    if (t80 != -127) { NG(); } else { ; }
	
}

void f81(void) {
    	static int16_t x337 = INT16_MIN;
	int64_t x338 = 59850664LL;
	uint64_t x339 = UINT64_MAX;
	static volatile int32_t t81 = -1526;

    t81 = (x337+((x338!=x339)%x340));

    if (t81 != -32767) { NG(); } else { ; }
	
}

void f82(void) {
    	volatile int8_t x341 = INT8_MAX;
	static int32_t x342 = 1;
	int64_t x343 = INT64_MIN;
	int16_t x344 = -74;
	int32_t t82 = -18122;

    t82 = (x341+((x342!=x343)%x344));

    if (t82 != 128) { NG(); } else { ; }
	
}

void f83(void) {
    	volatile int8_t x345 = 0;
	int8_t x348 = INT8_MIN;
	int32_t t83 = 4556819;

    t83 = (x345+((x346!=x347)%x348));

    if (t83 != 1) { NG(); } else { ; }
	
}

void f84(void) {
    	uint32_t x349 = 1U;
	volatile int16_t x350 = 6;
	uint16_t x351 = 19473U;
	static int64_t x352 = INT64_MAX;
	volatile int64_t t84 = -1443924LL;

    t84 = (x349+((x350!=x351)%x352));

    if (t84 != 2LL) { NG(); } else { ; }
	
}

void f85(void) {
    	static int64_t x353 = INT64_MIN;
	int32_t x354 = INT32_MIN;
	int64_t x355 = INT64_MAX;
	volatile uint16_t x356 = 11U;

    t85 = (x353+((x354!=x355)%x356));

    if (t85 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f86(void) {
    	int32_t x357 = INT32_MIN;
	static volatile int16_t x359 = -1;
	int64_t t86 = -6060LL;

    t86 = (x357+((x358!=x359)%x360));

    if (t86 != -2147483647LL) { NG(); } else { ; }
	
}

void f87(void) {
    	static int16_t x361 = -1;
	int8_t x362 = INT8_MIN;
	int32_t x363 = INT32_MAX;
	int32_t x364 = INT32_MIN;
	volatile int32_t t87 = 654;

    t87 = (x361+((x362!=x363)%x364));

    if (t87 != 0) { NG(); } else { ; }
	
}

void f88(void) {
    	int8_t x366 = 1;
	uint8_t x367 = 5U;
	int32_t t88 = -1;

    t88 = (x365+((x366!=x367)%x368));

    if (t88 != -32768) { NG(); } else { ; }
	
}

void f89(void) {
    	volatile int32_t x370 = 8581;
	static int64_t x371 = -4LL;
	int8_t x372 = INT8_MIN;
	static int32_t t89 = -14704478;

    t89 = (x369+((x370!=x371)%x372));

    if (t89 != 256) { NG(); } else { ; }
	
}

void f90(void) {
    	volatile uint32_t x373 = 133521753U;
	int16_t x374 = INT16_MAX;
	int16_t x375 = INT16_MIN;
	static uint64_t x376 = UINT64_MAX;
	uint64_t t90 = 177846227918LLU;

    t90 = (x373+((x374!=x375)%x376));

    if (t90 != 133521754LLU) { NG(); } else { ; }
	
}

void f91(void) {
    	int16_t x378 = -13;
	uint32_t x379 = 845U;
	volatile int64_t t91 = -505030875616734LL;

    t91 = (x377+((x378!=x379)%x380));

    if (t91 != 0LL) { NG(); } else { ; }
	
}

void f92(void) {
    	uint8_t x381 = UINT8_MAX;
	static uint8_t x382 = 60U;
	uint8_t x383 = UINT8_MAX;
	int8_t x384 = INT8_MAX;
	volatile int32_t t92 = -1;

    t92 = (x381+((x382!=x383)%x384));

    if (t92 != 256) { NG(); } else { ; }
	
}

void f93(void) {
    	uint32_t x385 = UINT32_MAX;
	int16_t x386 = -1;
	uint32_t x387 = 1U;
	int16_t x388 = -1;
	volatile uint32_t t93 = UINT32_MAX;

    t93 = (x385+((x386!=x387)%x388));

    if (t93 != UINT32_MAX) { NG(); } else { ; }
	
}

void f94(void) {
    	uint64_t x389 = 1650392173575330784LLU;
	int16_t x390 = -1;
	int8_t x392 = -1;

    t94 = (x389+((x390!=x391)%x392));

    if (t94 != 1650392173575330784LLU) { NG(); } else { ; }
	
}

void f95(void) {
    	volatile uint16_t x393 = 94U;
	int16_t x394 = INT16_MAX;
	int16_t x395 = -3;
	int16_t x396 = -2;
	volatile int32_t t95 = 1;

    t95 = (x393+((x394!=x395)%x396));

    if (t95 != 95) { NG(); } else { ; }
	
}

void f96(void) {
    	int32_t x397 = INT32_MIN;
	int16_t x398 = INT16_MIN;
	int8_t x399 = INT8_MAX;
	uint8_t x400 = 19U;
	volatile int32_t t96 = -109138;

    t96 = (x397+((x398!=x399)%x400));

    if (t96 != -2147483647) { NG(); } else { ; }
	
}

void f97(void) {
    	static int32_t x401 = -1;
	int16_t x402 = 13234;
	int8_t x403 = 0;
	int16_t x404 = -1;
	volatile int32_t t97 = -37;

    t97 = (x401+((x402!=x403)%x404));

    if (t97 != -1) { NG(); } else { ; }
	
}

void f98(void) {
    	volatile int32_t x406 = INT32_MAX;
	int32_t x407 = INT32_MIN;
	int8_t x408 = -27;
	int32_t t98 = 7826526;

    t98 = (x405+((x406!=x407)%x408));

    if (t98 != -127) { NG(); } else { ; }
	
}

void f99(void) {
    	volatile uint32_t x409 = 6568202U;
	static volatile int64_t x410 = INT64_MIN;
	volatile int8_t x411 = INT8_MIN;
	static int32_t x412 = -23563279;
	static volatile uint32_t t99 = 260349735U;

    t99 = (x409+((x410!=x411)%x412));

    if (t99 != 6568203U) { NG(); } else { ; }
	
}

void f100(void) {
    	int64_t x413 = -1LL;
	int32_t x414 = 23176401;
	uint64_t x415 = 173335828541578LLU;
	int32_t x416 = -12942286;
	int64_t t100 = 227631598381284LL;

    t100 = (x413+((x414!=x415)%x416));

    if (t100 != 0LL) { NG(); } else { ; }
	
}

void f101(void) {
    	volatile uint16_t x417 = 28287U;
	int16_t x418 = INT16_MAX;
	int64_t x419 = 485LL;
	volatile int64_t x420 = INT64_MIN;
	int64_t t101 = -357LL;

    t101 = (x417+((x418!=x419)%x420));

    if (t101 != 28288LL) { NG(); } else { ; }
	
}

void f102(void) {
    	uint16_t x421 = 2345U;
	int64_t x422 = 8159061196LL;
	int32_t x423 = INT32_MAX;
	static int32_t t102 = -452;

    t102 = (x421+((x422!=x423)%x424));

    if (t102 != 2346) { NG(); } else { ; }
	
}

void f103(void) {
    	volatile uint8_t x429 = 3U;
	volatile int64_t x430 = INT64_MIN;
	volatile int64_t x431 = INT64_MIN;
	int32_t x432 = INT32_MAX;
	int32_t t103 = -881333640;

    t103 = (x429+((x430!=x431)%x432));

    if (t103 != 3) { NG(); } else { ; }
	
}

void f104(void) {
    	uint8_t x433 = 68U;
	int8_t x435 = INT8_MIN;
	uint8_t x436 = 23U;
	int32_t t104 = 16191;

    t104 = (x433+((x434!=x435)%x436));

    if (t104 != 69) { NG(); } else { ; }
	
}

void f105(void) {
    	uint64_t x437 = 3LLU;
	int8_t x438 = -13;
	volatile uint16_t x440 = 9483U;

    t105 = (x437+((x438!=x439)%x440));

    if (t105 != 4LLU) { NG(); } else { ; }
	
}

void f106(void) {
    	static int8_t x441 = INT8_MIN;
	static volatile int64_t x442 = INT64_MIN;
	volatile int8_t x444 = INT8_MAX;
	volatile int32_t t106 = -313;

    t106 = (x441+((x442!=x443)%x444));

    if (t106 != -127) { NG(); } else { ; }
	
}

void f107(void) {
    	uint64_t x446 = UINT64_MAX;
	volatile uint16_t x448 = UINT16_MAX;
	int32_t t107 = -92;

    t107 = (x445+((x446!=x447)%x448));

    if (t107 != -10461) { NG(); } else { ; }
	
}

void f108(void) {
    	static int32_t x449 = -1;
	int64_t x450 = INT64_MIN;
	volatile int16_t x451 = -227;

    t108 = (x449+((x450!=x451)%x452));

    if (t108 != 0) { NG(); } else { ; }
	
}

void f109(void) {
    	volatile uint32_t x458 = 15579U;
	int32_t x459 = -1;
	int32_t t109 = 120;

    t109 = (x457+((x458!=x459)%x460));

    if (t109 != 256) { NG(); } else { ; }
	
}

void f110(void) {
    	int32_t x462 = -1;
	int64_t x464 = 88320671920566836LL;
	int64_t t110 = 8982458604571LL;

    t110 = (x461+((x462!=x463)%x464));

    if (t110 != 0LL) { NG(); } else { ; }
	
}

void f111(void) {
    	int64_t x465 = INT64_MIN;
	static uint16_t x466 = UINT16_MAX;
	int64_t x467 = INT64_MIN;
	int32_t x468 = 8;
	volatile int64_t t111 = -1053947628LL;

    t111 = (x465+((x466!=x467)%x468));

    if (t111 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f112(void) {
    	volatile int16_t x469 = 187;
	int32_t x471 = -1054;
	uint32_t x472 = UINT32_MAX;
	volatile uint32_t t112 = 1U;

    t112 = (x469+((x470!=x471)%x472));

    if (t112 != 188U) { NG(); } else { ; }
	
}

void f113(void) {
    	int64_t x473 = 595342747463909790LL;
	int32_t x476 = -45;
	volatile int64_t t113 = 3930264137443767275LL;

    t113 = (x473+((x474!=x475)%x476));

    if (t113 != 595342747463909791LL) { NG(); } else { ; }
	
}

void f114(void) {
    	static volatile int32_t x478 = 210;
	int64_t x480 = INT64_MIN;
	int64_t t114 = -6LL;

    t114 = (x477+((x478!=x479)%x480));

    if (t114 != -1782LL) { NG(); } else { ; }
	
}

void f115(void) {
    	uint8_t x481 = UINT8_MAX;
	static int8_t x482 = -1;
	static int8_t x483 = INT8_MIN;
	uint32_t x484 = 223825708U;
	static volatile uint32_t t115 = 885520581U;

    t115 = (x481+((x482!=x483)%x484));

    if (t115 != 256U) { NG(); } else { ; }
	
}

void f116(void) {
    	volatile int32_t x489 = 220;
	int64_t x490 = 0LL;
	volatile int16_t x491 = -1;
	int32_t t116 = 155320020;

    t116 = (x489+((x490!=x491)%x492));

    if (t116 != 221) { NG(); } else { ; }
	
}

void f117(void) {
    	int8_t x493 = 5;
	int8_t x494 = INT8_MIN;
	int16_t x495 = INT16_MIN;
	int8_t x496 = INT8_MIN;
	volatile int32_t t117 = 12152814;

    t117 = (x493+((x494!=x495)%x496));

    if (t117 != 6) { NG(); } else { ; }
	
}

void f118(void) {
    	static uint32_t x505 = 12136841U;
	uint16_t x506 = 19U;
	static volatile int32_t x508 = INT32_MAX;
	volatile uint32_t t118 = 451820U;

    t118 = (x505+((x506!=x507)%x508));

    if (t118 != 12136842U) { NG(); } else { ; }
	
}

void f119(void) {
    	uint16_t x509 = 13403U;
	uint32_t x510 = 7U;
	volatile uint16_t x511 = 50U;
	int32_t t119 = 502004337;

    t119 = (x509+((x510!=x511)%x512));

    if (t119 != 13403) { NG(); } else { ; }
	
}

void f120(void) {
    	volatile int16_t x513 = INT16_MIN;
	int64_t x514 = INT64_MIN;
	volatile uint32_t x515 = UINT32_MAX;
	int32_t x516 = 13150491;
	static int32_t t120 = -28755;

    t120 = (x513+((x514!=x515)%x516));

    if (t120 != -32767) { NG(); } else { ; }
	
}

void f121(void) {
    	static int8_t x517 = INT8_MIN;
	uint8_t x518 = 0U;
	uint8_t x519 = 26U;
	static int64_t x520 = INT64_MIN;
	volatile int64_t t121 = -7LL;

    t121 = (x517+((x518!=x519)%x520));

    if (t121 != -127LL) { NG(); } else { ; }
	
}

void f122(void) {
    	int64_t x521 = -1LL;
	int8_t x522 = 16;
	int32_t x523 = -1;
	int64_t x524 = -53020078620464LL;

    t122 = (x521+((x522!=x523)%x524));

    if (t122 != 0LL) { NG(); } else { ; }
	
}

void f123(void) {
    	uint8_t x525 = UINT8_MAX;
	int16_t x526 = 233;
	volatile int8_t x527 = INT8_MIN;
	uint16_t x528 = 2978U;

    t123 = (x525+((x526!=x527)%x528));

    if (t123 != 256) { NG(); } else { ; }
	
}

void f124(void) {
    	int8_t x530 = -1;
	volatile int64_t t124 = -137115719408LL;

    t124 = (x529+((x530!=x531)%x532));

    if (t124 != -96111545772LL) { NG(); } else { ; }
	
}

void f125(void) {
    	int8_t x533 = INT8_MIN;
	uint64_t x534 = 0LLU;
	static uint32_t x535 = 3100054U;
	volatile int32_t t125 = 233;

    t125 = (x533+((x534!=x535)%x536));

    if (t125 != -127) { NG(); } else { ; }
	
}

void f126(void) {
    	int32_t x538 = INT32_MIN;
	int16_t x539 = INT16_MIN;
	int8_t x540 = INT8_MIN;
	int32_t t126 = 146616;

    t126 = (x537+((x538!=x539)%x540));

    if (t126 != 32768) { NG(); } else { ; }
	
}

void f127(void) {
    	uint8_t x541 = 127U;
	int32_t x542 = INT32_MIN;
	static int16_t x543 = 3;
	volatile int16_t x544 = -5;
	volatile int32_t t127 = -3;

    t127 = (x541+((x542!=x543)%x544));

    if (t127 != 128) { NG(); } else { ; }
	
}

void f128(void) {
    	static volatile int8_t x549 = INT8_MIN;
	int16_t x551 = INT16_MIN;
	volatile int32_t t128 = 4;

    t128 = (x549+((x550!=x551)%x552));

    if (t128 != -127) { NG(); } else { ; }
	
}

void f129(void) {
    	uint32_t x554 = UINT32_MAX;
	volatile int8_t x555 = INT8_MIN;
	static int32_t x556 = 142401;
	int32_t t129 = -897827;

    t129 = (x553+((x554!=x555)%x556));

    if (t129 != 32768) { NG(); } else { ; }
	
}

void f130(void) {
    	int16_t x557 = 2;
	uint8_t x558 = UINT8_MAX;
	uint8_t x560 = UINT8_MAX;
	volatile int32_t t130 = -953577;

    t130 = (x557+((x558!=x559)%x560));

    if (t130 != 3) { NG(); } else { ; }
	
}

void f131(void) {
    	volatile int8_t x561 = INT8_MIN;
	static int8_t x563 = INT8_MAX;
	volatile uint32_t x564 = 76U;
	static volatile uint32_t t131 = 115U;

    t131 = (x561+((x562!=x563)%x564));

    if (t131 != 4294967169U) { NG(); } else { ; }
	
}

void f132(void) {
    	uint32_t x565 = 191U;
	static int16_t x567 = 2614;
	int32_t x568 = -31254986;
	static uint32_t t132 = 29U;

    t132 = (x565+((x566!=x567)%x568));

    if (t132 != 192U) { NG(); } else { ; }
	
}

void f133(void) {
    	uint32_t x569 = UINT32_MAX;
	static int64_t t133 = 135588LL;

    t133 = (x569+((x570!=x571)%x572));

    if (t133 != 4294967296LL) { NG(); } else { ; }
	
}

void f134(void) {
    	static volatile int32_t x573 = -1;
	uint8_t x574 = UINT8_MAX;
	int32_t x575 = INT32_MIN;
	int32_t x576 = INT32_MIN;
	volatile int32_t t134 = 172656;

    t134 = (x573+((x574!=x575)%x576));

    if (t134 != 0) { NG(); } else { ; }
	
}

void f135(void) {
    	int8_t x577 = INT8_MAX;
	int32_t x579 = -32630;
	int8_t x580 = 1;
	volatile int32_t t135 = -35;

    t135 = (x577+((x578!=x579)%x580));

    if (t135 != 127) { NG(); } else { ; }
	
}

void f136(void) {
    	int32_t x582 = -1;
	uint32_t x583 = 455U;
	int16_t x584 = INT16_MIN;

    t136 = (x581+((x582!=x583)%x584));

    if (t136 != 0) { NG(); } else { ; }
	
}

void f137(void) {
    	uint16_t x589 = 3U;
	int8_t x590 = 33;
	uint16_t x591 = 252U;
	static volatile int16_t x592 = -1106;
	int32_t t137 = 248658;

    t137 = (x589+((x590!=x591)%x592));

    if (t137 != 4) { NG(); } else { ; }
	
}

void f138(void) {
    	int32_t x594 = INT32_MIN;
	static int64_t x595 = INT64_MIN;
	int64_t x596 = INT64_MIN;
	volatile int64_t t138 = 9LL;

    t138 = (x593+((x594!=x595)%x596));

    if (t138 != 4294967296LL) { NG(); } else { ; }
	
}

void f139(void) {
    	volatile int8_t x597 = INT8_MIN;
	volatile int8_t x598 = INT8_MIN;
	volatile int64_t x599 = INT64_MIN;
	uint8_t x600 = 1U;
	volatile int32_t t139 = -3;

    t139 = (x597+((x598!=x599)%x600));

    if (t139 != -128) { NG(); } else { ; }
	
}

void f140(void) {
    	int16_t x601 = -1;
	int64_t x602 = INT64_MIN;
	uint8_t x603 = 0U;
	int32_t t140 = -8843;

    t140 = (x601+((x602!=x603)%x604));

    if (t140 != 0) { NG(); } else { ; }
	
}

void f141(void) {
    	uint32_t x605 = UINT32_MAX;
	static int64_t x607 = INT64_MAX;
	static uint32_t x608 = UINT32_MAX;
	uint32_t t141 = 2028U;

    t141 = (x605+((x606!=x607)%x608));

    if (t141 != 0U) { NG(); } else { ; }
	
}

void f142(void) {
    	int16_t x609 = -12893;
	int16_t x610 = -1;
	int64_t x611 = INT64_MAX;
	volatile uint32_t x612 = 4524U;

    t142 = (x609+((x610!=x611)%x612));

    if (t142 != 4294954404U) { NG(); } else { ; }
	
}

void f143(void) {
    	static int16_t x613 = INT16_MIN;
	static volatile uint8_t x615 = 38U;
	volatile uint8_t x616 = UINT8_MAX;
	int32_t t143 = -2523;

    t143 = (x613+((x614!=x615)%x616));

    if (t143 != -32767) { NG(); } else { ; }
	
}

void f144(void) {
    	int8_t x617 = INT8_MAX;
	int64_t x619 = -1859923LL;
	uint8_t x620 = 55U;
	static volatile int32_t t144 = -38;

    t144 = (x617+((x618!=x619)%x620));

    if (t144 != 128) { NG(); } else { ; }
	
}

void f145(void) {
    	uint8_t x621 = 0U;
	static int32_t x622 = -1;
	int32_t x623 = -22818;
	static int64_t x624 = INT64_MIN;
	int64_t t145 = -25369440108LL;

    t145 = (x621+((x622!=x623)%x624));

    if (t145 != 1LL) { NG(); } else { ; }
	
}

void f146(void) {
    	volatile int8_t x625 = INT8_MIN;
	int64_t x627 = -439876265LL;
	volatile int32_t t146 = 97002536;

    t146 = (x625+((x626!=x627)%x628));

    if (t146 != -127) { NG(); } else { ; }
	
}

void f147(void) {
    	int64_t x633 = -1LL;
	uint8_t x634 = 0U;
	int16_t x635 = INT16_MAX;
	static uint64_t x636 = 36685348104355LLU;
	volatile uint64_t t147 = 0LLU;

    t147 = (x633+((x634!=x635)%x636));

    if (t147 != 0LLU) { NG(); } else { ; }
	
}

void f148(void) {
    	uint8_t x637 = 65U;
	int8_t x638 = INT8_MIN;
	static int8_t x639 = INT8_MIN;
	static uint64_t x640 = 24685507256447LLU;
	static volatile uint64_t t148 = 27853260348469370LLU;

    t148 = (x637+((x638!=x639)%x640));

    if (t148 != 65LLU) { NG(); } else { ; }
	
}

void f149(void) {
    	int8_t x641 = INT8_MAX;
	static uint64_t x642 = 457852219005362LLU;
	uint64_t x644 = 1227023135LLU;
	uint64_t t149 = 5642108918LLU;

    t149 = (x641+((x642!=x643)%x644));

    if (t149 != 128LLU) { NG(); } else { ; }
	
}

void f150(void) {
    	static volatile uint16_t x645 = 3U;
	int8_t x646 = INT8_MIN;
	int32_t x648 = INT32_MIN;
	int32_t t150 = -1649;

    t150 = (x645+((x646!=x647)%x648));

    if (t150 != 4) { NG(); } else { ; }
	
}

void f151(void) {
    	uint8_t x650 = 7U;
	int16_t x652 = 1;

    t151 = (x649+((x650!=x651)%x652));

    if (t151 != -7230799) { NG(); } else { ; }
	
}

void f152(void) {
    	uint8_t x653 = 1U;
	uint64_t x654 = UINT64_MAX;
	static int8_t x655 = INT8_MIN;
	uint32_t x656 = UINT32_MAX;
	static volatile uint32_t t152 = 439936U;

    t152 = (x653+((x654!=x655)%x656));

    if (t152 != 2U) { NG(); } else { ; }
	
}

void f153(void) {
    	volatile uint8_t x657 = UINT8_MAX;
	static int32_t x658 = INT32_MIN;
	static int32_t x660 = INT32_MAX;
	static volatile int32_t t153 = 0;

    t153 = (x657+((x658!=x659)%x660));

    if (t153 != 256) { NG(); } else { ; }
	
}

void f154(void) {
    	uint32_t x661 = UINT32_MAX;
	volatile int8_t x663 = 1;
	static int32_t x664 = -1;
	uint32_t t154 = UINT32_MAX;

    t154 = (x661+((x662!=x663)%x664));

    if (t154 != UINT32_MAX) { NG(); } else { ; }
	
}

void f155(void) {
    	int64_t x665 = 33501115940LL;
	int8_t x666 = 0;
	uint32_t x667 = 326910488U;
	static int64_t x668 = 7331214466061809LL;
	int64_t t155 = 12103LL;

    t155 = (x665+((x666!=x667)%x668));

    if (t155 != 33501115941LL) { NG(); } else { ; }
	
}

void f156(void) {
    	int64_t x669 = -1LL;
	static int32_t x670 = INT32_MIN;
	static uint32_t x671 = UINT32_MAX;
	volatile uint16_t x672 = UINT16_MAX;
	static volatile int64_t t156 = 213474436959LL;

    t156 = (x669+((x670!=x671)%x672));

    if (t156 != 0LL) { NG(); } else { ; }
	
}

void f157(void) {
    	int64_t x674 = INT64_MAX;
	uint64_t x676 = UINT64_MAX;
	volatile uint64_t t157 = 180148583606LLU;

    t157 = (x673+((x674!=x675)%x676));

    if (t157 != 9223372036854775808LLU) { NG(); } else { ; }
	
}

void f158(void) {
    	int8_t x677 = -18;
	static int16_t x678 = -1;
	static int16_t x679 = INT16_MIN;
	static int64_t x680 = -2273801707180505563LL;
	int64_t t158 = -10LL;

    t158 = (x677+((x678!=x679)%x680));

    if (t158 != -17LL) { NG(); } else { ; }
	
}

void f159(void) {
    	uint64_t x681 = UINT64_MAX;
	volatile int8_t x682 = -1;
	volatile uint8_t x683 = UINT8_MAX;
	int32_t x684 = INT32_MIN;
	volatile uint64_t t159 = 866206731526430278LLU;

    t159 = (x681+((x682!=x683)%x684));

    if (t159 != 0LLU) { NG(); } else { ; }
	
}

void f160(void) {
    	uint16_t x685 = UINT16_MAX;
	static uint8_t x686 = 3U;
	volatile int32_t x687 = INT32_MIN;
	volatile uint64_t x688 = 1369672772508876LLU;
	static volatile uint64_t t160 = 1729310LLU;

    t160 = (x685+((x686!=x687)%x688));

    if (t160 != 65536LLU) { NG(); } else { ; }
	
}

void f161(void) {
    	int32_t x689 = -1;
	int8_t x690 = INT8_MAX;
	uint32_t x691 = 69U;
	int16_t x692 = INT16_MAX;

    t161 = (x689+((x690!=x691)%x692));

    if (t161 != 0) { NG(); } else { ; }
	
}

void f162(void) {
    	static volatile uint8_t x693 = UINT8_MAX;
	uint64_t x694 = 110607801LLU;
	static int64_t x695 = -1LL;
	volatile int32_t t162 = 910703;

    t162 = (x693+((x694!=x695)%x696));

    if (t162 != 255) { NG(); } else { ; }
	
}

void f163(void) {
    	static int64_t x697 = -3116442353LL;
	static int64_t x698 = 441700LL;
	volatile uint16_t x699 = 3U;
	int32_t x700 = -1;
	static volatile int64_t t163 = 4LL;

    t163 = (x697+((x698!=x699)%x700));

    if (t163 != -3116442353LL) { NG(); } else { ; }
	
}

void f164(void) {
    	int32_t x701 = -9951472;
	int16_t x703 = INT16_MIN;
	volatile int32_t t164 = 25672152;

    t164 = (x701+((x702!=x703)%x704));

    if (t164 != -9951472) { NG(); } else { ; }
	
}

void f165(void) {
    	static int8_t x705 = INT8_MIN;
	volatile int8_t x707 = INT8_MIN;
	static int64_t x708 = INT64_MIN;
	volatile int64_t t165 = 53104332819703LL;

    t165 = (x705+((x706!=x707)%x708));

    if (t165 != -127LL) { NG(); } else { ; }
	
}

void f166(void) {
    	int64_t x710 = -1121328765024595LL;
	volatile int8_t x711 = 5;
	uint64_t x712 = 6011004601LLU;
	uint64_t t166 = 22175033LLU;

    t166 = (x709+((x710!=x711)%x712));

    if (t166 != 7926LLU) { NG(); } else { ; }
	
}

void f167(void) {
    	int16_t x713 = 275;
	int16_t x714 = -1;
	volatile int32_t x715 = INT32_MAX;
	int64_t x716 = INT64_MAX;
	volatile int64_t t167 = -1805LL;

    t167 = (x713+((x714!=x715)%x716));

    if (t167 != 276LL) { NG(); } else { ; }
	
}

void f168(void) {
    	uint16_t x717 = 2U;
	int64_t x719 = 263091237057840103LL;
	static uint32_t x720 = 3827U;

    t168 = (x717+((x718!=x719)%x720));

    if (t168 != 3U) { NG(); } else { ; }
	
}

void f169(void) {
    	int16_t x721 = -9301;
	uint64_t x722 = 3463393451176597LLU;
	static int8_t x723 = INT8_MAX;
	int64_t x724 = -551294146092721644LL;
	volatile int64_t t169 = 0LL;

    t169 = (x721+((x722!=x723)%x724));

    if (t169 != -9300LL) { NG(); } else { ; }
	
}

void f170(void) {
    	int64_t x725 = 3971714946LL;
	int64_t x726 = -6080002LL;
	int64_t x727 = INT64_MAX;
	static volatile int8_t x728 = -1;
	static volatile int64_t t170 = 128LL;

    t170 = (x725+((x726!=x727)%x728));

    if (t170 != 3971714946LL) { NG(); } else { ; }
	
}

void f171(void) {
    	int16_t x729 = INT16_MAX;
	int32_t x730 = INT32_MIN;
	uint8_t x731 = 8U;
	int32_t t171 = 87883;

    t171 = (x729+((x730!=x731)%x732));

    if (t171 != 32768) { NG(); } else { ; }
	
}

void f172(void) {
    	int8_t x733 = -1;
	int16_t x734 = 3228;
	int64_t x735 = INT64_MIN;
	int64_t x736 = 13918580031LL;
	volatile int64_t t172 = 14883LL;

    t172 = (x733+((x734!=x735)%x736));

    if (t172 != 0LL) { NG(); } else { ; }
	
}

void f173(void) {
    	int32_t x737 = INT32_MAX;
	static int32_t x738 = INT32_MIN;
	uint64_t x739 = 16333090LLU;

    t173 = (x737+((x738!=x739)%x740));

    if (t173 != 2147483648LL) { NG(); } else { ; }
	
}

void f174(void) {
    	static uint64_t x741 = 7000519531LLU;
	int64_t x743 = INT64_MIN;
	uint32_t x744 = 18558U;
	static uint64_t t174 = 16500882782LLU;

    t174 = (x741+((x742!=x743)%x744));

    if (t174 != 7000519532LLU) { NG(); } else { ; }
	
}

void f175(void) {
    	int16_t x747 = -7;
	uint8_t x748 = 5U;
	static volatile uint32_t t175 = 77813778U;

    t175 = (x745+((x746!=x747)%x748));

    if (t175 != 741U) { NG(); } else { ; }
	
}

void f176(void) {
    	int64_t x749 = INT64_MIN;
	int64_t x750 = INT64_MAX;
	uint32_t x751 = 243U;
	static uint16_t x752 = 3U;

    t176 = (x749+((x750!=x751)%x752));

    if (t176 != -9223372036854775807LL) { NG(); } else { ; }
	
}

void f177(void) {
    	int16_t x761 = INT16_MIN;
	volatile int8_t x762 = INT8_MIN;
	static int32_t x763 = INT32_MAX;
	int16_t x764 = INT16_MIN;
	int32_t t177 = -14305;

    t177 = (x761+((x762!=x763)%x764));

    if (t177 != -32767) { NG(); } else { ; }
	
}

void f178(void) {
    	int32_t x765 = -1;
	int16_t x766 = INT16_MIN;
	int32_t x767 = INT32_MAX;
	int8_t x768 = INT8_MIN;
	int32_t t178 = -218894;

    t178 = (x765+((x766!=x767)%x768));

    if (t178 != 0) { NG(); } else { ; }
	
}

void f179(void) {
    	int8_t x769 = 62;
	static int64_t x771 = INT64_MAX;
	volatile int16_t x772 = -1;
	int32_t t179 = -182710;

    t179 = (x769+((x770!=x771)%x772));

    if (t179 != 62) { NG(); } else { ; }
	
}

void f180(void) {
    	int16_t x773 = INT16_MIN;
	int16_t x774 = -1768;

    t180 = (x773+((x774!=x775)%x776));

    if (t180 != -32767) { NG(); } else { ; }
	
}

void f181(void) {
    	volatile uint32_t x777 = UINT32_MAX;
	static int64_t x778 = INT64_MIN;
	int64_t x779 = INT64_MIN;
	int64_t x780 = INT64_MIN;

    t181 = (x777+((x778!=x779)%x780));

    if (t181 != 4294967295LL) { NG(); } else { ; }
	
}

void f182(void) {
    	int32_t t182 = 2;

    t182 = (x781+((x782!=x783)%x784));

    if (t182 != 58) { NG(); } else { ; }
	
}

void f183(void) {
    	static int16_t x785 = INT16_MIN;
	static uint64_t x786 = UINT64_MAX;
	static int32_t x787 = INT32_MIN;
	static int8_t x788 = -1;
	static volatile int32_t t183 = -2161;

    t183 = (x785+((x786!=x787)%x788));

    if (t183 != -32768) { NG(); } else { ; }
	
}

void f184(void) {
    	int16_t x789 = -1;
	int32_t x791 = INT32_MIN;
	uint16_t x792 = 3539U;
	int32_t t184 = 7;

    t184 = (x789+((x790!=x791)%x792));

    if (t184 != 0) { NG(); } else { ; }
	
}

void f185(void) {
    	uint32_t x793 = 7404U;
	volatile int8_t x794 = -1;
	int32_t x795 = INT32_MIN;
	int8_t x796 = INT8_MAX;
	uint32_t t185 = 235U;

    t185 = (x793+((x794!=x795)%x796));

    if (t185 != 7405U) { NG(); } else { ; }
	
}

void f186(void) {
    	uint32_t x797 = 883769U;
	uint16_t x798 = 0U;
	int64_t x799 = INT64_MAX;
	static int16_t x800 = 565;
	static volatile uint32_t t186 = 229U;

    t186 = (x797+((x798!=x799)%x800));

    if (t186 != 883770U) { NG(); } else { ; }
	
}

void f187(void) {
    	static int64_t x801 = INT64_MIN;
	volatile int16_t x803 = INT16_MAX;
	static volatile uint64_t t187 = 55LLU;

    t187 = (x801+((x802!=x803)%x804));

    if (t187 != 9223372036854775809LLU) { NG(); } else { ; }
	
}

void f188(void) {
    	volatile int32_t x805 = INT32_MIN;
	int8_t x807 = INT8_MIN;
	static int32_t x808 = INT32_MAX;
	int32_t t188 = 859725800;

    t188 = (x805+((x806!=x807)%x808));

    if (t188 != -2147483647) { NG(); } else { ; }
	
}

void f189(void) {
    	uint64_t x809 = 4395220360438724LLU;
	volatile int64_t x810 = -133389461433214898LL;
	static volatile int16_t x811 = 9501;
	int16_t x812 = 5625;
	volatile uint64_t t189 = 49927LLU;

    t189 = (x809+((x810!=x811)%x812));

    if (t189 != 4395220360438725LLU) { NG(); } else { ; }
	
}

void f190(void) {
    	volatile int32_t x813 = INT32_MIN;
	int32_t x814 = 1794187;
	int64_t x815 = -202857LL;
	volatile int32_t t190 = 26;

    t190 = (x813+((x814!=x815)%x816));

    if (t190 != -2147483647) { NG(); } else { ; }
	
}

void f191(void) {
    	uint64_t x819 = 120927LLU;
	static volatile int32_t t191 = -15;

    t191 = (x817+((x818!=x819)%x820));

    if (t191 != -2147483647) { NG(); } else { ; }
	
}

void f192(void) {
    	int64_t x821 = INT64_MIN;
	int8_t x822 = INT8_MIN;
	static volatile uint64_t x823 = UINT64_MAX;
	int64_t t192 = INT64_MIN;

    t192 = (x821+((x822!=x823)%x824));

    if (t192 != INT64_MIN) { NG(); } else { ; }
	
}

void f193(void) {
    	static int64_t x825 = 1996007LL;
	volatile uint8_t x826 = 30U;
	volatile uint16_t x827 = UINT16_MAX;
	int64_t t193 = 101259748062LL;

    t193 = (x825+((x826!=x827)%x828));

    if (t193 != 1996008LL) { NG(); } else { ; }
	
}

void f194(void) {
    	volatile int8_t x829 = -1;
	uint64_t x830 = UINT64_MAX;
	int64_t x831 = INT64_MIN;
	int32_t x832 = 262;
	int32_t t194 = -25062841;

    t194 = (x829+((x830!=x831)%x832));

    if (t194 != 0) { NG(); } else { ; }
	
}

void f195(void) {
    	static volatile int8_t x834 = INT8_MIN;
	int64_t x835 = INT64_MIN;
	static volatile int64_t x836 = INT64_MAX;
	volatile int64_t t195 = 441LL;

    t195 = (x833+((x834!=x835)%x836));

    if (t195 != 0LL) { NG(); } else { ; }
	
}

void f196(void) {
    	int8_t x837 = INT8_MAX;
	int8_t x838 = INT8_MAX;
	uint8_t x839 = UINT8_MAX;
	int32_t x840 = -1;
	static int32_t t196 = -61664;

    t196 = (x837+((x838!=x839)%x840));

    if (t196 != 127) { NG(); } else { ; }
	
}

void f197(void) {
    	int8_t x841 = INT8_MAX;
	volatile int16_t x842 = INT16_MIN;
	int64_t x843 = 349155418615LL;

    t197 = (x841+((x842!=x843)%x844));

    if (t197 != 128U) { NG(); } else { ; }
	
}

void f198(void) {
    	int8_t x849 = INT8_MIN;
	static int8_t x850 = -1;
	int16_t x851 = INT16_MAX;
	static int16_t x852 = -1;

    t198 = (x849+((x850!=x851)%x852));

    if (t198 != -128) { NG(); } else { ; }
	
}

void f199(void) {
    	static int16_t x857 = INT16_MIN;
	uint64_t x858 = 1380852LLU;
	uint8_t x860 = 2U;
	volatile int32_t t199 = -3;

    t199 = (x857+((x858!=x859)%x860));

    if (t199 != -32767) { NG(); } else { ; }
	
}

int main(void) {
        f0();
    f1();
    f2();
    f3();
    f4();
    f5();
    f6();
    f7();
    f8();
    f9();
    f10();
    f11();
    f12();
    f13();
    f14();
    f15();
    f16();
    f17();
    f18();
    f19();
    f20();
    f21();
    f22();
    f23();
    f24();
    f25();
    f26();
    f27();
    f28();
    f29();
    f30();
    f31();
    f32();
    f33();
    f34();
    f35();
    f36();
    f37();
    f38();
    f39();
    f40();
    f41();
    f42();
    f43();
    f44();
    f45();
    f46();
    f47();
    f48();
    f49();
    f50();
    f51();
    f52();
    f53();
    f54();
    f55();
    f56();
    f57();
    f58();
    f59();
    f60();
    f61();
    f62();
    f63();
    f64();
    f65();
    f66();
    f67();
    f68();
    f69();
    f70();
    f71();
    f72();
    f73();
    f74();
    f75();
    f76();
    f77();
    f78();
    f79();
    f80();
    f81();
    f82();
    f83();
    f84();
    f85();
    f86();
    f87();
    f88();
    f89();
    f90();
    f91();
    f92();
    f93();
    f94();
    f95();
    f96();
    f97();
    f98();
    f99();
    f100();
    f101();
    f102();
    f103();
    f104();
    f105();
    f106();
    f107();
    f108();
    f109();
    f110();
    f111();
    f112();
    f113();
    f114();
    f115();
    f116();
    f117();
    f118();
    f119();
    f120();
    f121();
    f122();
    f123();
    f124();
    f125();
    f126();
    f127();
    f128();
    f129();
    f130();
    f131();
    f132();
    f133();
    f134();
    f135();
    f136();
    f137();
    f138();
    f139();
    f140();
    f141();
    f142();
    f143();
    f144();
    f145();
    f146();
    f147();
    f148();
    f149();
    f150();
    f151();
    f152();
    f153();
    f154();
    f155();
    f156();
    f157();
    f158();
    f159();
    f160();
    f161();
    f162();
    f163();
    f164();
    f165();
    f166();
    f167();
    f168();
    f169();
    f170();
    f171();
    f172();
    f173();
    f174();
    f175();
    f176();
    f177();
    f178();
    f179();
    f180();
    f181();
    f182();
    f183();
    f184();
    f185();
    f186();
    f187();
    f188();
    f189();
    f190();
    f191();
    f192();
    f193();
    f194();
    f195();
    f196();
    f197();
    f198();
    f199();


    return 0;
}

